...
1. Computes a perceptual hash from each video
...
> **Note:** Top-Shelf is a read-only client for StashDB, ThePornDB, FansDB, and JAVStash. It never submits fingerprints, scene edits, or new scenes back to any of those databases. Fingerprints computed here are used only for local matching against the databases' `queryFingerprints` endpoints and stored in the local SQLite cache.
...
- **Manual filing** — search databases, pick a scene, file locally with metadata and artwork
...
- **Database:** SQLite
- **Phash:** FFmpeg + a pure-Python port of Stash's `videophash` / `goimagehash.PerceptionHash` (bilinear resize + Lee-1984 DCT + goimagehash quickselect median), producing hex output that matches Stash's Go implementation. Verified against the goimagehash reference test corpus with zero Hamming deviation.
...
- [Stash](https://github.com/stashapp/stash) — the phash pipeline in `app/core/phash.py` is a direct Python port of Stash's `pkg/hash/videophash` and the underlying [`goimagehash`](https://github.com/corona10/goimagehash) / [`nfnt/resize`](https://github.com/nfnt/resize) libraries, so hex output matches on identical inputs
