...
class UploadSearchTests(unittest.TestCase):
    ...
    def test_results_are_not_cut_off_at_25(self):
        ...
        ns=load({'prowlarr_search', '_prowlarr_result_sort_key'},dict(_prowlarr_url=lambda:'http://test',db=SimpleNamespace(get_settings=lambda:{'prowlarr_api_key':'test'}),_get_indexers=lambda:[{"id":1,"name":"Fixture"}],_search_indexer=lambda *args:items))
