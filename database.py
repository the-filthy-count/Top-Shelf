...
def resolve_source_video_path(source_dir: Path, db_filename: str) -> Path | None:
    ...
    to a file under source_dir. Alternate-extension fallback preserves
    the parent-directory prefix so a subdirectory file re-muxed to a
...
def upsert_file(filename: str) -> int:
    ...
    with get_conn() as conn:
        ...
        if row:
            ...
            if stored != filename:
                # Filename changed — invalidate the cached parser blob,
                # the library-match guess, AND the resolver output blob
                # so subsequent reads re-parse / re-match / re-resolve
                # against the new name instead of returning stale results
                # tied to the previous name.
                conn.execute(
                    """
                    UPDATE processed_files
...
def library_match_generation() -> str:
    ...
    inputs (favourite_entities.performer rows + studio_logos).

    Composite of ``MAX(updated_at)`` and ``COUNT(*)`` on both tables —
    any INSERT/UPDATE/DELETE on either shifts at least one component,
    ...
    return "{}:{}:{}:{}".format(
        (fav["m"] if fav else "") or "",
        (fav["n"] if fav else 0) or 0,
