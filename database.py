...
DEFAULTS = {
    ...
    "api_key_fansdb":    "",
    "api_key_javstash":  "",
    #: Dedicated JAV library root (indexed like Series/Features). Optional.
    "jav_dir":           "",
    #: Distinct download inbox for JAV releases. Files landing here go
    #: through the movie pipeline in place; their processed_movies row
    #: is what the /unmatched JAV tab reads. Empty = no JAV pipeline
    #: (falls back to movies_source_dir).
    "jav_download_watch_dir": "",
    ...
    "download_rss_feeds": "[]",
    # Seeded news-feed list — applied on first launch so /news has
    # content immediately. Mirrors `_NEWS_DEFAULT_FEEDS` in main.py.
    # Stored as JSON so users can prune / extend via Settings without
    # reaching for the database.
    "news_rss_feeds":     '["http://www.lukeisback.com/feed/", "https://www.venus-adult-news.com/en/feed", "https://www.xbiz.com/rss/all.xml", "https://www.mypornstarblogs.com/feed/", "https://adultfyi.com/feed/", "https://blog.iwantclips.com/feed/", "https://avn.com/feed/articles.rss", "https://theporndude.com/blog/feed/", "https://mikesouth.com/feed/", "https://www.ladbible.com/adult-industry.rss"]',
    ...
    "dl_torrent_user":    "",
    "dl_torrent_pass":    "",
    # Native priority value for the active client. Interpreted per-
    # client at grab time:
    #   NZBGet  → int (-100/-50/0/50/100/900)
    #   SABnzbd → int (-2/-1/0/1/2)
    #   Transmission → int (-1/0/1)
    #   qBittorrent → "top"/"normal"/"bottom"  (queue position only)
    #   Deluge      → "top"/"normal"/"bottom"  (queue position only)
    "dl_nzb_priority":     "0",
    "dl_torrent_priority": "normal",
...
_conn_thread_local = threading.local()


def get_conn() -> sqlite3.Connection:
    """Return a SQLite connection with per-connection PRAGMAs applied.

    Connections are cached per-thread — every helper used to open a
    fresh sqlite3 connection (and immediately close it when its `with`
    block exited), which under WAL is cheap but not free: sqlite3.connect
    still opens the DB file, reads the header, allocates the connection
    object and reapplies PRAGMAs. Under the load of the queue snapshot
    (hundreds of small helpers back-to-back), that adds up. The cached
    conn is reused across every subsequent `with get_conn() as conn:`
    in the same thread — sqlite3's Connection context manager only
    commits/rolls back on exit, it never closes.
    ...
    conn = getattr(_conn_thread_local, "conn", None)
    if conn is not None:
        return conn
...
def processed_files_find_on_disk_path_loose(
    destination: str | None,
    destination_base: str | None,
    destination_ext: str | None,
) -> str | None:
    """Like ``processed_files_find_on_disk_path`` but, if the strict
    path/extension permutations all miss, falls back to a case-insensitive
    directory scan: any sibling video file whose stem matches (lowercased)
    counts as a re-encoded version of the same logical file. Useful when
    the user re-muxed a file in a tool that swapped case or normalised
    whitespace alongside the extension change."""
    strict = processed_files_find_on_disk_path(destination, destination_base, destination_ext)
    if strict:
        return strict
    base = (destination_base or "").strip()
    dest = (destination or "").strip()
    ...
    if not parent.is_dir():
        return None
    want_stem = Path(base).name.casefold()
    try:
        for child in parent.iterdir():
            if not child.is_file():
                continue
            if child.suffix.lower() not in _VIDEO_SUFFIXES_TRY:
                continue
            if child.with_suffix("").name.casefold() == want_stem:
                try:
...
def processed_files_repair_destination_row_if_found(row: dict, *, loose: bool = False) -> bool:
    """If the video exists (exact path or alternate extension), refresh destination* columns. Returns True if on disk.

    ``loose=True`` also accepts a same-stem (case-insensitive) sibling
    video file in the same directory — used by the manual "Resolve all"
    orphan action so users can clean up re-encoded files whose names
    drifted in case/whitespace as well as extension.
    """
    finder = (
        processed_files_find_on_disk_path_loose if loose else processed_files_find_on_disk_path
    )
    resolved = finder(
        row.get("destination"),
...
def resolve_source_video_path(
    source_dir: Path,
    db_filename: str,
    vice_dl_root: Path | None = None,
    jav_dl_root: Path | None = None,
) -> Path | None:
    """Resolve a processed_files.filename (full basename, stem, or a
    subdirectory-prefixed posix rel path like ``Jane Smith/scene.mp4``)
    to a file on disk. Alternate-extension fallback preserves the
    parent-directory prefix so a subdirectory file re-muxed to a
    different container still resolves to its own folder.

    When ``vice_dl_root`` is passed AND the rel is ``Vices/<Vice>/…``,
    that root is checked first — vice-scope files live in
    ``<vice_download_watch_dir>/<Vice>/…`` and only fall back to
    ``<source_dir>/Vices/<Vice>/…`` for legacy placements. Same for
    ``jav_dl_root`` with the ``JAV/…`` prefix."""
    rel_str = str(db_filename or "").strip()
    # Vice-scope preference: `Vices/<Vice>/<basename>` maps to the vice
    # download inbox, not source_dir.
    if vice_dl_root is not None and rel_str.startswith("Vices/"):
        parts = rel_str.split("/", 2)
        if len(parts) == 3 and parts[1] and parts[2]:
            inner_rel = parts[2]
            cand = vice_dl_root / parts[1] / inner_rel
            if cand.is_file():
                return cand
            inner_path = Path(inner_rel)
            inner_stem = inner_path.stem
            inner_parent = inner_path.parent
            for suf in _VIDEO_SUFFIXES_TRY:
                q = vice_dl_root / parts[1] / inner_parent / (inner_stem + suf)
                if q.is_file():
                    return q
    # JAV-scope preference: `JAV/<inner_rel>` maps to the jav download
    # inbox. No per-category subfolder split (unlike vices), so the
    # first path component is the release folder itself.
    if jav_dl_root is not None and rel_str.startswith("JAV/"):
        inner_rel = rel_str.split("/", 1)[1] if "/" in rel_str else ""
        if inner_rel:
            cand = jav_dl_root / inner_rel
            if cand.is_file():
                return cand
            inner_path = Path(inner_rel)
            inner_stem = inner_path.stem
            inner_parent = inner_path.parent
            for suf in _VIDEO_SUFFIXES_TRY:
                q = jav_dl_root / inner_parent / (inner_stem + suf)
                if q.is_file():
                    return q
    p = source_dir / db_filename
    if p.is_file():
        return p
    rel = Path(db_filename)
    parent_rel = rel.parent
    stem = rel.stem
    for suf in _VIDEO_SUFFIXES_TRY:
        q = source_dir / parent_rel / (stem + suf)
        if q.is_file():
            return q
    return None
...
def init_db() -> None:
    with get_conn() as conn:
        ...
        for col_sql in (
            ...
            "ALTER TABLE processed_files ADD COLUMN media_duration INTEGER",
            # Cached filename-parser output: a JSON blob shaped like
            # ``{title, date, site, performers, file_mtime, parsed_at}``.
            # Populated lazily by the queue snapshot builder so the
            # manual-search modal can render parsed fields without
            # waiting on a per-click /api/parse/filename round-trip.
            "ALTER TABLE processed_files ADD COLUMN parsed_meta_json TEXT",
            # Cached studio + performer guess from
            # ``_library_guess_from_release_name`` — persisted so the
            # queue snapshot rebuild doesn't re-run the token scan
            # against every library performer on every rebuild. Reused
            # while ``guess_lib_gen`` matches ``library_match_generation()``
            # — any INSERT/UPDATE/DELETE on favourite_entities or
            # studio_logos shifts the gen and invalidates the cache.
            "ALTER TABLE processed_files ADD COLUMN guess_studio TEXT",
            "ALTER TABLE processed_files ADD COLUMN guess_performers TEXT",
            "ALTER TABLE processed_files ADD COLUMN guess_lib_gen TEXT",
            # Cached output of the queue-snapshot performer resolver
            # (headshot URLs + in/missing-in-library breakdown + dropped-
            # by-filename list). JSON blob carrying its own generation
            # + perf-list hash so the reader can validate before use;
            # avoids the favourite_list scan + IAFD meta reads on
            # cache-hit snapshot rebuilds.
            "ALTER TABLE processed_files ADD COLUMN perf_resolver_json TEXT",
            # Import scope. NULL / 'scene' behaves as-is (routed to Studio /
            # Performer / Vice). 'vice' means the file originated in the
            # Vice download watch dir — routing must file to a matched
            # vice folder or leave the row unmatched; never fall through
            # to Studio / Performer library dirs.
            "ALTER TABLE processed_files ADD COLUMN scope TEXT",
        ):
            try:
                conn.execute(col_sql)
            except sqlite3.OperationalError:
                pass
        # scope filters the Vices / JAV tile snapshot builder; without an
        # index the tile query full-scans processed_files. Index is
        # created after the ADD COLUMN above so the column exists on
        # fresh install AND after the first-run migration.
        try:
            conn.execute("CREATE INDEX IF NOT EXISTS idx_pf_scope ON processed_files(scope)")
        except sqlite3.OperationalError:
            pass
        ...
        # Session idle timeout migrated to minute precision so users can
        # configure sub-hour lockouts. session_hours kept for back-compat
        # but session_minutes is authoritative.
        try:
            conn.execute(
                "ALTER TABLE auth ADD COLUMN session_minutes INTEGER NOT NULL DEFAULT 1440"
            )
            conn.execute(
                "UPDATE auth SET session_minutes = COALESCE(session_hours, 24) * 60 "
                "WHERE id = 1"
            )
        except sqlite3.OperationalError:
            pass
        ...
        # client_kind / client_job_id capture the stable identifier the
        #   download client returns from append/add/torrent-add (NZBID,
        #   nzo_id, infohash, transmission torrent id, deluge hash). The
        #   ID-based path is the structural source of truth — if the
        #   client tells us "this is job 4711", we don't need to guess
        #   from filenames once it lands on disk.
        # final_name: the canonical top-level on-disk name we asked the
        #   client to use (post-sanitisation / rename). Watch handler
        #   matches the unpacked folder/file against this exactly to
        #   recover the scene_grab_log row when the uid tag was lost.
        for col, decl in (
            ("download_url",  "TEXT"),
            ("kind",          "TEXT NOT NULL DEFAULT 'scene'"),
            ("consumed_at",   "TEXT"),
        ...
        conn.execute("CREATE INDEX IF NOT EXISTS idx_scene_grab_clientjob ON scene_grab_log(client_kind, client_job_id)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_scene_grab_final ON scene_grab_log(final_name COLLATE NOCASE)")
        conn.commit()
        ...
        try:
            conn.execute(
                "ALTER TABLE processed_movies ADD COLUMN javstash_id TEXT"
            )
        except sqlite3.OperationalError:
            pass
        ...
        conn.execute("""
            CREATE TABLE IF NOT EXISTS directories (
                ...
                gender_filters TEXT,
                bucket_by_letter INTEGER NOT NULL DEFAULT 0
            )
        """)
        try:
            ...
            pass
        try:
            conn.execute("ALTER TABLE directories ADD COLUMN bucket_by_letter INTEGER NOT NULL DEFAULT 0")
        except sqlite3.OperationalError:
            pass
        # Legacy settings UI offered type "Series" for this table, but only
        # ``performer`` rows are used for filing and favourites indexing — so
        # mis-tagged roots vanished from Settings and were skipped by scans.
        conn.execute(
            "UPDATE directories SET type = 'performer' "
            "WHERE lower(trim(coalesce(type, ''))) <> 'performer'"
        )
        ...
        conn.execute("""CREATE TABLE IF NOT EXISTS performer_bio_fields (
            row_id INTEGER NOT NULL REFERENCES favourite_entities(id) ON DELETE CASCADE,
            field TEXT NOT NULL, value TEXT NOT NULL DEFAULT '',
        ...
        try:
            ...
            pass
        # Capture the date a row was first added to the library. Unlike
        # scanned_at / updated_at — which are clobbered on every scan via
        # the ON CONFLICT(path) DO UPDATE upsert — added_at is set on
        # INSERT only and never touched again. The spotlight library
        # top-up uses this to surface "most recently added" performers
        # when the StashDB/FansDB fetch underfills.
        try:
            conn.execute(
                "ALTER TABLE favourite_entities ADD COLUMN added_at TEXT"
            )
            # Backfill existing rows with their best-known date — scanned_at
            # is the closest signal we have for legacy entries (it was set
            # on insert, even though it gets overwritten thereafter), and
            # falls back to NOW for rows that never had one.
            conn.execute(
                "UPDATE favourite_entities SET added_at = COALESCE(scanned_at, datetime('now')) "
        ...
        try:
            conn.execute(
                "ALTER TABLE favourite_entities ADD COLUMN match_javstash_id TEXT"
            )
        except sqlite3.OperationalError:
            pass
        try:
            conn.execute(
                "ALTER TABLE favourite_entities ADD COLUMN match_javstash_name TEXT"
            )
        except sqlite3.OperationalError:
            pass

        conn.execute("CREATE TABLE IF NOT EXISTS download_cleanup_pending (name TEXT PRIMARY KEY, created_at TEXT NOT NULL)")
        ...
        for _alter in (
            ...
            "ALTER TABLE library_files ADD COLUMN phash_2_size INTEGER",
            "ALTER TABLE library_files ADD COLUMN phash_2_mtime_ns INTEGER",
        ):
            try:
                conn.execute(_alter)
            except sqlite3.OperationalError:
        ...
        # Scene-side fingerprint cache. Populated when stash-box scene
        # responses include fingerprints (or via on-demand backfill via
        # findScene(id)). Joined against library_files.phash_1/2/3 to
        # answer "is this stash-box scene in my library?" without
        # re-running phash matching.
        conn.execute("""
            CREATE TABLE IF NOT EXISTS scene_fingerprints (
                source       TEXT NOT NULL,    -- 'stashdb' / 'fansdb' / 'tpdb'
                scene_id     TEXT NOT NULL,
                phash        TEXT NOT NULL,    -- normalised lowercase hex
                algorithm    TEXT,             -- 'PHASH' / 'OSHASH' / 'MD5'
                duration     INTEGER,
                fetched_at   TEXT NOT NULL,
                PRIMARY KEY (source, scene_id, phash)
            )
        """)
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_scene_fp_phash "
            "ON scene_fingerprints(phash)"
        )
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_scene_fp_scene "
            "ON scene_fingerprints(source, scene_id)"
        )
        ...
        # Persistent manual-action queue. Each row is one user-initiated
        # filing job (bulk-sort, IAFD bulk, vice, single manual) so the
        # sidebar Sort card + the worker itself survive a restart. Rows
        # are inserted with status='pending' by the enqueue helper,
        # flipped to 'running' when the worker picks them up, and either
        # deleted on success or flipped to 'failed' with an error blob
        # on exception. Interrupted jobs (status='running' at startup)
        # get rehydrated back to 'pending'.
        conn.execute("""
            CREATE TABLE IF NOT EXISTS manual_queue_jobs (
                id            INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        conn.execute(
            ...
            "ON manual_queue_jobs(status, id)"
        )
        ...
        # Persistent raw-performer cache keyed by (source, performer_id).
        # Scraper writes here as it walks recent scenes; rows that aren't
        # re-encountered within `spotlight_ttl_days` get evicted. Composite
        # PK lets the same person live in StashDB+FansDB+TPDB rows without
        # collision — dedup-by-name is a render-time concern.
        conn.execute("""
            CREATE TABLE IF NOT EXISTS spotlight_performers (
                ...
                PRIMARY KEY (source, performer_id)
            )
        """)
        conn.execute("CREATE INDEX IF NOT EXISTS idx_spotlight_perf_last_seen ON spotlight_performers(last_seen)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_spotlight_perf_latest_scene ON spotlight_performers(latest_scene_at)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_spotlight_perf_name ON spotlight_performers(name)")

        # Spotlight v2 migration: collapse any saved `require` mode to
        # `prefer`, EXCEPT `spotlight_gender_mode` which is kept as a real
        # hard gate (so straight users don't get gay results regardless
        # of weighting). Idempotent on re-boot.
        try:
            conn.execute("""
                UPDATE settings
                   SET value = 'prefer'
                 WHERE key LIKE 'spotlight_%_mode'
                   AND key != 'spotlight_gender_mode'
                   AND lower(value) = 'require'
            """)
        except sqlite3.OperationalError:
            pass
        ...
        # Phash submissions log: one row per (source, scene_id, phash)
        # we've pushed via submitFingerprint. Used by /health to badge
        # the per-scene DB icons so a user can see, per file, which
        # source DBs have received our fingerprint.
        conn.execute("""
            CREATE TABLE IF NOT EXISTS phash_submissions (
                id           INTEGER PRIMARY KEY AUTOINCREMENT,
                source       TEXT    NOT NULL,
                scene_id     TEXT    NOT NULL,
                phash        TEXT    NOT NULL,
                ...
                UNIQUE(source, scene_id, phash)
            )
        """)
        conn.execute("CREATE INDEX IF NOT EXISTS idx_phash_sub_phash ON phash_submissions(phash)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_phash_sub_source_scene ON phash_submissions(source, scene_id)")
        ...
        for _col, _sql in [
            ...
            # Cached `[{"name", "headshot_url"}]` per performer, captured
            # from the source DB's scene/movie payload at enrich time.
            # Lets the wanted tile render headshots for every performer
            # — including ones not in the local library.
            ("performers_json", "ALTER TABLE wanted_items ADD COLUMN performers_json TEXT NOT NULL DEFAULT '[]'"),
        ]:
            try:
                conn.execute(_sql)
            except sqlite3.OperationalError:
        ...
        # Name-search suggestions for files that didn't match on phash.
        # Populated by a background pass that runs the cleaned filename
        # through the same StashDB → TPDB → FansDB search the queue
        # search panel uses, then keeps the top-scoring candidates so the
        # user can accept or reject without re-querying.
        # Composite primary key keeps each (file, source, scene) unique
        # — re-running the search just refreshes scores in place.
        conn.execute("""
            CREATE TABLE IF NOT EXISTS queue_suggestions (
                ...
                PRIMARY KEY (filename, source_db, source_id)
            )
        """)
        conn.execute("CREATE INDEX IF NOT EXISTS idx_qsugg_filename ON queue_suggestions(filename)")
        # Per (file, source_id) rejection log — once the user dismisses a
        # candidate, we never resurface it for that file (even on a fresh
        # name-search pass). A different scene from the same source can
        # still surface, which is the user's chosen rejection scope.
        conn.execute("""
            CREATE TABLE IF NOT EXISTS queue_suggestion_rejects (
                filename     TEXT NOT NULL,
                source_db    TEXT NOT NULL,
                source_id    TEXT NOT NULL,
        ...
        # Durable merged /scenes + Latest Movies grids: successive API pulls
        # append into a capped pool (dedupe + sort) so the payload survives
        # process restarts. Cleared when content-filter fingerprint changes
        # or when the user hits refresh (see main.py).
        conn.execute("""
            CREATE TABLE IF NOT EXISTS feed_display_pools (
                kind         TEXT NOT NULL,
                pool_key     TEXT NOT NULL,
                filter_key   TEXT NOT NULL,
                payload_json TEXT NOT NULL,
                updated_at   REAL NOT NULL,
                PRIMARY KEY (kind, pool_key)
            )
        """)

        # Generic JSON snapshots (RSS caches, tag pool, TPDB detail caches).
        conn.execute("""
            CREATE TABLE IF NOT EXISTS app_json_cache (
        ...
        conn.execute("DELETE FROM settings WHERE key = 'library_show_jav'")
        conn.commit()
    ...
    with get_conn() as conn:
        ...
        # Purge legacy stash-box submission settings + log if a prior version
        # of Top-Shelf created them. The whole submission feature was removed
        # after StashDB editors clarified that our pHash isn't verified
        # byte-identical to Stash's Go implementation and that stash-boxes
        # don't permit automated submissions in any case. Guarded by a marker
        # so we only run the drop once. See conversation with StashDB editors
        # on 2026-08-25.
        marker = conn.execute(
            "SELECT value FROM settings WHERE key = 'stashbox_submit_purged_v1'"
        ).fetchone()
        if not marker:
            legacy_setting_keys = (
                "submit_phash_enabled",
                "submit_phash_stashdb",
                "submit_phash_fansdb",
                "submit_phash_javstash",
                "submit_scene_stashdb",
                "submit_scene_fansdb",
                "submit_scene_javstash",
                "manual_submit_stashdb_default",
                "stashdb_submit_rate_per_hour",
                "stashdb_submit_pending_cap",
                "fansdb_submit_rate_per_hour",
                "fansdb_submit_pending_cap",
                "javstash_submit_rate_per_hour",
                "javstash_submit_pending_cap",
                "stashdb_paywalled_hosts",
                "stashdb_retailer_hosts",
                "stashdb_banned_studios",
                "phash_submit_forced_off_v1",
            )
            for k in legacy_setting_keys:
                conn.execute("DELETE FROM settings WHERE key = ?", (k,))
            conn.execute("DROP TABLE IF EXISTS stashdb_submission_log")
            conn.execute(
                "INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)",
                ("stashbox_submit_purged_v1", "true"),
            )
        # (The Stash-compatible pHash wipe migrations v1 and v2 were
        # removed after they'd both fired on the live install — the
        # ``stash_compat_phash_wiped_v{1,2}`` markers remain in the
        # settings table on migrated DBs and act as tombstones so no
        # replacement migration accidentally reuses those slugs. The
        # boot rebuild thread stays put in main.py as a safety net for
        # any DB where ``stash_compat_phash_rehash_pending`` is still
        # 'true' — a rebuild that hadn't completed by the time this
        # code shipped will still resume on next start.)
        conn.commit()
...
# --- /scenes & Latest Movies durable display pools (feed_display_pools) ---


def feed_display_pool_get(kind: str, pool_key: str) -> dict | None:
    """Return ``{"items": [...], "filter_key": str}`` or ``None`` if missing."""
    k = (kind or "").strip()
    ...
    with get_conn() as conn:
        row = conn.execute(
            "SELECT filter_key, payload_json FROM feed_display_pools WHERE kind = ? AND pool_key = ?",
            (k, pk),
        ).fetchone()
    if not row:
        return None
    ...
    if not isinstance(items, list):
        items = []
    return {"items": items, "filter_key": str(row["filter_key"] or "")}


def feed_display_pool_upsert(kind: str, pool_key: str, filter_key: str, items: list) -> None:
    """Replace the stored JSON payload for one pool slot."""
    k = (kind or "").strip()
    pk = (pool_key or "").strip()
    if not k or not pk:
        return
    now = time.time()
    blob = json.dumps(items or [], separators=(",", ":"))
    with get_conn() as conn:
        conn.execute(
            """INSERT INTO feed_display_pools (kind, pool_key, filter_key, payload_json, updated_at)
                   VALUES (?, ?, ?, ?, ?)
                   ON CONFLICT(kind, pool_key) DO UPDATE SET
                       filter_key   = excluded.filter_key,
                       payload_json = excluded.payload_json,
                       updated_at   = excluded.updated_at""",
            (k, pk, str(filter_key), blob, now),
        )
        conn.commit()


def feed_display_pools_clear_kind(kind: str) -> None:
    """Wipe every pool row for ``kind`` (``scenes`` or ``movies``)."""
    k = (kind or "").strip()
    if not k:
        return
    with get_conn() as conn:
        conn.execute("DELETE FROM feed_display_pools WHERE kind = ?", (k,))
        conn.commit()


# --- Generic JSON blob cache (RSS, tag pool, TPDB lookups) ---


def app_json_cache_get(cache_key: str):
    """Return parsed JSON payload or ``None``."""
    ck = (cache_key or "").strip()
    if not ck:
        return None
    with get_conn() as conn:
        row = conn.execute(
            "SELECT payload_json FROM app_json_cache WHERE cache_key = ?",
            (ck,),
        ).fetchone()
    if not row:
        return None
...
def app_json_cache_put(cache_key: str, value) -> None:
    ck = (cache_key or "").strip()
    if not ck:
        return
    now = time.time()
    ...
    with get_conn() as conn:
        conn.execute(
            """INSERT INTO app_json_cache (cache_key, payload_json, updated_at)
                   VALUES (?, ?, ?)
...
def app_json_cache_delete(cache_key: str) -> None:
    ck = (cache_key or "").strip()
    if not ck:
        return
    with get_conn() as conn:
        conn.execute("DELETE FROM app_json_cache WHERE cache_key = ?", (ck,))
        conn.commit()


def app_json_cache_delete_prefix(prefix: str) -> int:
    """Delete every row whose ``cache_key`` starts with ``prefix``. Returns rows removed."""
    p = (prefix or "").strip()
    if not p:
        return 0
    with get_conn() as conn:
        cur = conn.execute("DELETE FROM app_json_cache WHERE cache_key LIKE ?", (p + "%",))
        conn.commit()
        return cur.rowcount or 0


def app_json_cache_delete_tpdb_stale(cutoff_ts: float) -> int:
    """Drop TPDB movie-detail / breakdown rows older than ``cutoff_ts`` (unix)."""
    with get_conn() as conn:
        cur = conn.execute(
            """DELETE FROM app_json_cache
                WHERE (cache_key LIKE 'tpdb_movie_detail:%' OR cache_key LIKE 'tpdb_breakdown:%')
                  AND updated_at < ?""",
            (cutoff_ts,),
        )
        conn.commit()
...
def spotlight_excluded_clear() -> int:
    """Wipe every never-show-again entry. Returns rows deleted."""
    with get_conn() as conn:
        cur = conn.execute("DELETE FROM spotlight_excluded")
        conn.commit()
        return cur.rowcount or 0
...
def spotlight_clear_top_cache() -> None:
    """Wipe the spotlight disk cache. Called on settings save so a filter
    change can't re-pull stale performers via the underfill fallback in
    `_fetch_spotlight_performers`."""
    with get_conn() as conn:
        conn.execute("DELETE FROM spotlight_top_cache")
        conn.commit()


# ── Spotlight raw performer cache (v2) ────────────────────────────────
# The scraper walks recent scenes from each source and upserts every
# performer it sees. `last_seen` is bumped on every re-encounter,
# `times_seen` counts scrape ticks that surfaced the row, and
# `latest_scene_at` records the freshest release_date seen for that
# performer. TTL eviction (`spotlight_evict_stale`) drops rows that
# haven't been re-encountered within the configured window.

def spotlight_perf_upsert_many(rows: list[dict]) -> int:
    """Bulk upsert performer rows. Each row dict needs:
        source, performer_id, name, payload_json, latest_scene_at (int|None)

    On insert: first_seen = last_seen = now, times_seen = 1.
    On conflict: bumps last_seen, increments times_seen, refreshes
    payload_json/name, and pulls latest_scene_at forward when newer.
    ...
    with get_conn() as conn:
        for r in rows:
            src = r.get("source") or ""
            pid = r.get("performer_id") or ""
            if not src or not pid:
                continue
            conn.execute(
                """INSERT INTO spotlight_performers
                       (source, performer_id, name, payload_json,
                        first_seen, last_seen, latest_scene_at, times_seen)
                   VALUES (?, ?, ?, ?, ?, ?, ?, 1)
                   ON CONFLICT(source, performer_id) DO UPDATE SET
                       name            = excluded.name,
                       payload_json    = excluded.payload_json,
                       last_seen       = excluded.last_seen,
                       latest_scene_at = MAX(
                           COALESCE(spotlight_performers.latest_scene_at, 0),
                           COALESCE(excluded.latest_scene_at, 0)
                       ),
                       times_seen      = spotlight_performers.times_seen + 1""",
                (
                    src, pid,
                    r.get("name") or "",
                    r.get("payload_json") or "{}",
                    now, now,
                    r.get("latest_scene_at"),
                ),
            )
            written += 1
        conn.commit()
...
def spotlight_evict_stale(ttl_days: int = 30) -> int:
    """Delete performer rows whose last_seen is older than ttl_days.
    Returns the number of rows evicted."""
    cutoff = int(datetime.now(timezone.utc).timestamp()) - (max(1, ttl_days) * 86400)
    with get_conn() as conn:
        cur = conn.execute(
            "DELETE FROM spotlight_performers WHERE last_seen < ?", (cutoff,),
        )
        conn.commit()
        return cur.rowcount or 0


def spotlight_perf_clear_source(source: str) -> int:
    """Wipe every cached row from a single source. Used by the harvest
    strategy migration so old listing-style rows don't linger after a
    switch to scene-driven harvesting. Returns rows deleted."""
    src = (source or "").strip()
    if not src:
        return 0
    with get_conn() as conn:
        cur = conn.execute(
            "DELETE FROM spotlight_performers WHERE source = ?", (src,),
        )
        conn.commit()
        return cur.rowcount or 0


def spotlight_perf_load_active(min_last_seen_days: int | None = None) -> list[dict]:
    """Return all cached performer rows, optionally filtered to those
    seen within the last N days. Each row is the parsed payload dict
    with `_source`, `_first_seen`, `_last_seen`, `_latest_scene_at`,
    `_times_seen` injected for the scorer."""
    import json as _json
    sql = """SELECT source, performer_id, name, payload_json,
                    first_seen, last_seen, latest_scene_at, times_seen
               FROM spotlight_performers"""
    args: tuple = ()
    if min_last_seen_days:
        cutoff = int(datetime.now(timezone.utc).timestamp()) - (min_last_seen_days * 86400)
        sql += " WHERE last_seen >= ?"
        args = (cutoff,)
    out: list[dict] = []
    with get_conn() as conn:
        for r in conn.execute(sql, args).fetchall():
            try:
                ...
                continue
            p["_source"]          = r["source"]
            p["_first_seen"]      = r["first_seen"]
            p["_last_seen"]       = r["last_seen"]
            p["_latest_scene_at"] = r["latest_scene_at"]
            p["_times_seen"]      = r["times_seen"]
            out.append(p)
    return out


def spotlight_perf_count_by_source() -> dict[str, int]:
    """Return {source: row_count} so the scheduler can detect empty cache /
    newly-enabled sources."""
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT source, COUNT(*) AS n FROM spotlight_performers GROUP BY source"
        ).fetchall()
    return {r["source"]: int(r["n"]) for r in rows}


def spotlight_perf_clear() -> None:
    """Wipe the raw cache. Called on a hard reset (e.g. settings explicitly
    request it). Routine settings changes only clear the scored top cache."""
    with get_conn() as conn:
        conn.execute("DELETE FROM spotlight_performers")
        conn.commit()
...
def wanted_add(
    ...
    `performers_json` (optional): JSON-encoded list of
    ``[{"name", "headshot_url"}]`` from the source DB's scene/movie
    payload, so the wanted-tile renderer can show every performer's
    face — not just the ones present in the local Favourites index."""
    ...
    perf_json = (performers_json or "").strip() or "[]"
        ...
        cur = conn.execute(
            ...
            INSERT INTO wanted_items
              (kind, source, external_id, title, studio, release_date,
               ...
               tags_json, duration, source_url, performers_json,
            ...
            ON CONFLICT(kind, source, external_id) DO UPDATE SET
              ...
              performers_json = excluded.performers_json,
              acquired_at     = NULL
            RETURNING id
            """,
            (k, s, eid, title or "", studio or "", release_date or "",
             performers or "", thumb_url or "", description or "",
             phash or None, tags_json, dur, src_url, perf_json),
        )
        row = cur.fetchone()
        conn.commit()
...
def wanted_mark_acquired_by_id(row_id: int) -> int:
    """Mark a single wanted row acquired by primary key — used by the
    predicted-path probe, which has already pinned the exact row and
    just needs it cleared."""
    with get_conn() as conn:
        cur = conn.execute(
            "UPDATE wanted_items SET acquired_at = CURRENT_TIMESTAMP "
            "WHERE id = ? AND acquired_at IS NULL",
            (int(row_id),),
        )
        conn.commit()
        return cur.rowcount or 0
...
def _normalize_wanted_source(source: str) -> str:
    s = (source or "").strip().lower()
    if s in ("theporndb", "porndb"):
        return "tpdb"
    return s


def library_has_phash(phash: str) -> bool:
    """True when an active library_files row carries this phash."""
    ph = (phash or "").strip().lower()
    if not ph:
        return False
    with get_conn() as conn:
        cur = conn.execute(
            ...
            "  lower(COALESCE(phash_1, '')) = ? "
            "  OR lower(COALESCE(phash_2, '')) = ? "
            "  OR lower(COALESCE(phash_3, '')) = ?"
            ") LIMIT 1",
            (ph, ph, ph),
        )
...
def library_match_movies(items: list) -> dict[str, dict]:
    """Like ``library_match_scenes`` for wanted movies — keyed
    ``"<source>:<external_id>"`` when a filed ``processed_movies`` row
    exists with a live library file."""
    if not items:
        return {}
    out: dict[str, dict] = {}
    with get_conn() as conn:
        for it in items:
            if isinstance(it, dict):
                src = _normalize_wanted_source(it.get("source") or "")
                eid = str(it.get("id") or "").strip()
                title = (it.get("title") or "").strip()
                year = str(it.get("year") or it.get("release_date") or "").strip()[:4]
            else:
                continue
            ...
            if src in ("tpdb",):
                cur = conn.execute(
                    "SELECT pm.destination AS dest, lf.id AS lf_id "
                    "FROM processed_movies pm "
                    "LEFT JOIN library_files lf ON lf.destination = pm.destination "
                    "WHERE pm.status = 'filed' AND trim(COALESCE(pm.tpdb_id, '')) = ? "
                    "  AND COALESCE(lf.is_removed, 0) = 0 "
                    "LIMIT 1",
                    (eid,),
                )
                row = cur.fetchone()
                ...
                cur = conn.execute(
                    ...
                    "FROM processed_movies pm "
                    "LEFT JOIN library_files lf ON lf.destination = pm.destination "
                    "WHERE pm.status = 'filed' AND trim(COALESCE(pm.tmdb_id, '')) = ? "
                    "  AND COALESCE(lf.is_removed, 0) = 0 "
                    "LIMIT 1",
                    (eid,),
                )
                ...
                cur = conn.execute(
                    ...
                    "FROM processed_movies pm "
                    "LEFT JOIN library_files lf ON lf.destination = pm.destination "
                    "WHERE pm.status = 'filed' AND trim(COALESCE(pm.javstash_id, '')) = ? "
                    "  AND COALESCE(lf.is_removed, 0) = 0 "
                    "LIMIT 1",
                    (eid,),
                )
            ...
            if not row and title:
                ...
                if year:
                    cur = conn.execute(
                        ...
                        "FROM processed_movies pm "
                        "LEFT JOIN library_files lf ON lf.destination = pm.destination "
                        "WHERE pm.status = 'filed' "
                        "  AND lower(COALESCE(pm.title, '')) = ? "
                        "  AND substr(COALESCE(pm.year, ''), 1, 4) = ? "
                        "  AND COALESCE(lf.is_removed, 0) = 0 "
                        "LIMIT 1",
                        (tl, year),
                    )
            ...
            if row:
                out[key] = {
                    "row_id": int(row["lf_id"]) if row["lf_id"] is not None else None,
                    "destination": row["dest"] or "",
                    "via": "processed_movies",
                }
    return out


def wanted_reconcile_with_library(
    *,
    ...
    """Mark active wanted rows acquired when the library already has a
    match (same rules as ``/api/library/scenes-in`` + movie ids).

    Optional filters narrow to one row (after add/toggle). Returns
    ``{"checked": N, "cleared": M}``."""
    rows = wanted_list(active_only=True)
    if row_id is not None:
        rows = [r for r in rows if int(r.get("id") or 0) == int(row_id)]
    elif kind and source and external_id:
        k = (kind or "").strip().lower()
        s = _normalize_wanted_source(source)
        eid = (external_id or "").strip()
        rows = [
            r for r in rows
            if (r.get("kind") or "").lower() == k
            and _normalize_wanted_source(r.get("source") or "") == s
            and (r.get("external_id") or "").strip() == eid
        ]
    checked = len(rows)
    if not checked:
    ...
    scene_row_by_key: dict[str, dict] = {}
    movie_items: list[dict] = []
    movie_row_by_key: dict[str, dict] = {}

    def _add_scene_probe(row: dict, src: str, sid: str) -> None:
        src = _normalize_wanted_source(src)
        sid = (sid or "").strip()
        if not src or not sid or src not in ("stashdb", "fansdb", "tpdb", "javstash"):
            return
        key = f"{src}:{sid}"
        if key in scene_row_by_key:
            return
        scene_row_by_key[key] = row
        scene_items.append({
            "source": src,
            "id": sid,
    ...
    for row in rows:
        kind_r = (row.get("kind") or "scene").strip().lower()
        src_r = _normalize_wanted_source(row.get("source") or "")
        eid_r = (row.get("external_id") or "").strip()
        if not eid_r:
            continue
        if kind_r == "movie":
            key = f"{src_r}:{eid_r}"
            if key not in movie_row_by_key:
                movie_row_by_key[key] = row
                year = str(row.get("release_date") or "").strip()
                if len(year) >= 4:
                    year = year[:4]
                movie_items.append({
                    "source": src_r,
            ...
            continue
        _add_scene_probe(row, src_r, eid_r)
        stash_id = (row.get("stashdb_id") or "").strip()
        fans_id = (row.get("fansdb_id") or "").strip()
        if stash_id:
            _add_scene_probe(row, "stashdb", stash_id)
        if fans_id:
            _add_scene_probe(row, "fansdb", fans_id)

    cleared = 0
    seen_rows: set[int] = set()

    def _clear_row(row: dict) -> None:
        nonlocal cleared
        rid = int(row.get("id") or 0)
        if not rid or rid in seen_rows:
            return
        n = wanted_mark_acquired(
            (row.get("kind") or "scene").lower(),
            _normalize_wanted_source(row.get("source") or ""),
            (row.get("external_id") or "").strip(),
        )
        ph = (row.get("phash") or "").strip().lower()
        m = wanted_mark_acquired_by_phash(ph) if ph else 0
        if n or m:
            seen_rows.add(rid)
            cleared += 1

    if scene_items:
        matches = library_match_scenes(scene_items)
        for key, row in scene_row_by_key.items():
            if key in matches:
                _clear_row(row)

    if movie_items:
        mm = library_match_movies(movie_items)
        for key, row in movie_row_by_key.items():
            if key in mm:
                _clear_row(row)
    ...
    for row in rows:
        ...
        if rid in seen_rows:
            continue
        ph = (row.get("phash") or "").strip().lower()
        if ph and library_has_phash(ph):
            _clear_row(row)

    return {"checked": checked, "cleared": cleared}


def queue_suggestions_clear(filename: str) -> int:
    """Drop every cached candidate for one file. Called before a fresh
    search pass and after a successful accept (the file no longer
    needs suggestions once it's been filed)."""
    with get_conn() as conn:
        cur = conn.execute(
            "DELETE FROM queue_suggestions WHERE filename = ?",
            (filename,),
        )
        conn.commit()
        return cur.rowcount or 0
...
def queue_suggestions_add(filename: str, suggestions: list[dict]) -> int:
    """Bulk-insert (or refresh) candidates for one file. Existing rows
    with the same (filename, source_db, source_id) primary key are
    overwritten via INSERT OR REPLACE so re-running the search just
    bumps scores instead of accumulating duplicates."""
    if not suggestions:
        return 0
    rows = []
    for s in suggestions:
        rows.append((
            filename,
            (s.get("source_db") or "").strip(),
            (s.get("source_id") or "").strip(),
            float(s.get("score") or 0.0),
            (s.get("title") or "")[:300],
            (s.get("studio") or "")[:200],
            (s.get("date") or "")[:32],
            (s.get("performers") or "")[:500],
            (s.get("thumb_url") or "")[:1000],
            json.dumps(s.get("payload") or {}),
        ))
    with get_conn() as conn:
        conn.executemany(
            "INSERT OR REPLACE INTO queue_suggestions "
            "(filename, source_db, source_id, score, title, studio, date, performers, thumb_url, payload_json, searched_at) "
...
def queue_suggestions_for_file(filename: str) -> list[dict]:
    """All cached candidates for one file, highest score first."""
    with get_conn() as conn:
        cur = conn.execute(
            "SELECT filename, source_db, source_id, score, title, studio, date, performers, thumb_url, payload_json, searched_at "
            "FROM queue_suggestions WHERE filename = ? "
            "ORDER BY score DESC, source_db ASC",
            (filename,),
        )
        return [dict(r) for r in cur.fetchall()]


def queue_suggestions_all_with_files() -> list[dict]:
    """All unmatched files that have at least one suggestion, with their
    candidates nested. Joins against `processed_files` so the row also
    carries each file's status / phash for the UI to decide whether to
    show it (status must still be 'unmatched')."""
    with get_conn() as conn:
        cur = conn.execute(
            "SELECT pf.filename, pf.status, pf.phash, "
            "       qs.source_db, qs.source_id, qs.score, qs.title, qs.studio, qs.date, "
            "       qs.performers, qs.thumb_url, qs.payload_json, qs.searched_at "
            "FROM processed_files pf "
            "JOIN queue_suggestions qs ON qs.filename = pf.filename "
            "WHERE pf.status = 'unmatched' "
            "ORDER BY pf.filename ASC, qs.score DESC"
        )
        rows = cur.fetchall()
...
def queue_suggestions_remove(filename: str, source_db: str, source_id: str) -> int:
    with get_conn() as conn:
        cur = conn.execute(
            "DELETE FROM queue_suggestions "
            "WHERE filename = ? AND source_db = ? AND source_id = ?",
            (filename, source_db, source_id),
        )
        conn.commit()
        return cur.rowcount or 0


def queue_suggestions_reject(filename: str, source_db: str, source_id: str) -> int:
    """Atomically: persist the rejection and drop the candidate row.
    The rejection survives future name-search passes so the same scene
    never resurfaces for this file."""
    with get_conn() as conn:
        conn.execute(
            "INSERT OR IGNORE INTO queue_suggestion_rejects "
            "(filename, source_db, source_id) VALUES (?, ?, ?)",
            (filename, source_db, source_id),
        )
        cur = conn.execute(
            "DELETE FROM queue_suggestions "
...
def queue_suggestion_rejects_for_file(filename: str) -> set[tuple[str, str]]:
    """Set of (source_db, source_id) tuples the user has dismissed for
    this file — used by the search worker to prune candidates before
    persisting them."""
    with get_conn() as conn:
        cur = conn.execute(
            "SELECT source_db, source_id FROM queue_suggestion_rejects WHERE filename = ?",
            (filename,),
        )
        return {(r["source_db"], r["source_id"]) for r in cur.fetchall()}


def queue_suggestions_count_files() -> int:
    """Number of distinct unmatched files that currently have at least
    one suggestion — drives the queue stats counter."""
    with get_conn() as conn:
        r = conn.execute(
            "SELECT COUNT(DISTINCT pf.filename) AS n "
            "FROM processed_files pf "
            "JOIN queue_suggestions qs ON qs.filename = pf.filename "
            "WHERE pf.status = 'unmatched'"
        ).fetchone()
...
def _directory_row_dict(row: sqlite3.Row) -> dict:
    ...
    if isinstance(raw, str) and raw.strip():
        ...
        d["gender_filters"] = []
    try:
        d["bucket_by_letter"] = bool(int(d.get("bucket_by_letter") or 0))
    except (TypeError, ValueError):
        d["bucket_by_letter"] = False
    return d
        ...
        for d in dirs:
            ...
            if d.get("type") == "performer":
                ...
                if isinstance(raw, list):
                    ...
                    gf_json = raw.strip()
            bucket = 1 if bool(d.get("bucket_by_letter")) else 0
            conn.execute(
                "INSERT INTO directories (type, path, label, rank, gender_filters, bucket_by_letter) VALUES (?, ?, ?, ?, ?, ?)",
                (d["type"], d["path"], d["label"], int(d["rank"]), gf_json, bucket),
            )
        conn.commit()
...
def favourite_list(kind: str | None = None) -> list[dict]:
    """Library index rows (performers, studios, movies): alphabetical by
    folder name within each kind. Optional ``kind`` filter restricts the
    SQL scan to one kind — used by /library's lazy per-tab fetch path
    so switching to a fresh tab only pulls that tab's rows."""
    with get_conn() as conn:
        if kind:
            cur = conn.execute(
                """
        ...
        if kind in (None, 'performer'):
            present = {}
            for bio in conn.execute("SELECT row_id, field FROM performer_bio_fields WHERE TRIM(value) != ''"):
                present.setdefault(bio["row_id"], set()).add(bio["field"])
            for row in rows:
                if row["kind"] == 'performer':
                    row["bio_missing_fields"] = sorted({'gender','birthdate','country'} - present.get(row['id'], set()))
        return rows
...
def favourite_lookup_performer_by_name(name: str) -> dict | None:
    """Find a performer favourite_entities row by exact (case-insensitive)
    folder_name. No alias fallback — single-word aliases like "Luna" can
    appear on multiple performers' alias lists, so an alias hit risks
    routing the popup to the wrong row. Callers that need alias-aware
    lookup should pass an explicit stash_id instead."""
    n = (name or "").strip()
    if not n:
        return None
    with get_conn() as conn:
        row = conn.execute(
            "SELECT * FROM favourite_entities "
            "WHERE kind = 'performer' AND lower(folder_name) = ? LIMIT 1",
            (n.lower(),),
        ).fetchone()
        return dict(row) if row else None


def favourite_lookup_performer_by_alias(name: str) -> dict | None:
    """Find a performer row whose ``aliases_json`` contains an exact
    case-insensitive match. Used when the on-disk folder uses an alias
    but the popup was opened with the canonical StashDB name."""
    ...
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT * FROM favourite_entities "
            "WHERE kind = 'performer' AND aliases_json IS NOT NULL "
            "AND trim(aliases_json) != '' AND aliases_json != '[]'"
        ).fetchall()
        for row in rows:
            try:
...
def favourite_update_path(row_id: int, path: str) -> None:
    """Update the on-disk folder path for a favourites row."""
    p = (path or "").strip()
    if not p:
        return
    now = _iso_now()
    with get_conn() as conn:
        conn.execute(
            "UPDATE favourite_entities SET path = ?, updated_at = ? WHERE id = ?",
            (p, now, int(row_id)),
        )
        conn.commit()
...
def favourite_upsert_folder(
    ...
    `added_at` is populated on INSERT only and intentionally absent from
    the DO UPDATE clause so it survives every subsequent re-scan."""
    now = datetime.now(timezone.utc).isoformat()
    with get_conn() as conn:
        conn.execute(
            """
            INSERT INTO favourite_entities (
                ...
                scanned_at, updated_at, added_at, path_missing
            ) VALUES (?, ?, ?, ?, 0, NULL, ?, ?, ?, ?, 0)
            ON CONFLICT(path) DO UPDATE SET
                ...
                path_missing = 0
            """,
            (kind, folder_name, path, root_label, gender_filters_json, now, now, now),
        )
        conn.commit()
...
def favourite_update_matches(
    ...
    *,
    ...
    match_javstash_id: str | None = None,
    match_javstash_name: str | None = None,
) -> None:
    now = datetime.now(timezone.utc).isoformat()
    ...
    for k, v in [
        ...
        ("match_javstash_id", match_javstash_id),
        ("match_javstash_name", match_javstash_name),
    ]:
        if v is not None:
            fields.append((k, v))
...
def favourite_overwrite_matches(
    ...
    *,
    ...
    match_javstash_id: str | None,
    match_javstash_name: str | None,
...
def _group_ids_load(row_id: int) -> dict:
    ...
    if not row or not row["group_ids_json"]:
        return {"tpdb": [], "stashdb": [], "fansdb": [], "javstash": []}
    try:
    ...
    with get_conn() as conn:
        conn.execute(
            ...
            UPDATE favourite_entities SET
                ...
                match_javstash_id = NULL, match_javstash_name = NULL,
...
def favourite_clear_source_match(row_id: int, source: str) -> None:
    """Clear a single source's match fields. source: TMDB, TPDB, STASHDB, FANSDB, JAVSTASH, IAFD, FREEONES, or BABEPEDIA."""
    ...
    elif u in ("JAVSTASH", "JAV"):
        clause = "match_javstash_id = NULL, match_javstash_name = NULL"
    elif u == "IAFD":
    ...
    elif u in ("JAVDATABASE", "JAV DATABASE"):
        clause = "match_javdatabase_url = NULL"
    else:
        raise ValueError(
            "source must be TMDB, TPDB, StashDB, FansDB, JAVStash, IAFD, Freeones, "
            "Babepedia, Coomer, or JAV Database"
        )
    now = datetime.now(timezone.utc).isoformat()
    with get_conn() as conn:
        conn.execute(
...
def favourite_set_ext_link(row_id: int, site: str, url: str) -> None:
    ...
    """
    import re as _re
    s = _re.sub(r"\s+", "", (site or "").strip().lower())
    if s == "tmdb":
        ...
        col_map = {
            ...
            "javdatabase": "match_javdatabase_url",
        }
        col = col_map.get(s)
        if not col:
            raise ValueError(
                "site must be tmdb, iafd, freeones, babepedia, coomer, or javdatabase"
            )
...
def favourite_find_performer_folder_by_crosswalk(
    ...
    """Resolve library folder via entity index (Favourites DB table) by TPDB / StashDB / FansDB / JAVStash performer id.
    ...
    jid = (javstash_id or "").strip()
    if not (tid or sid or fid or jid):
        return None
    parts: list[str] = []
    args: list[str] = []
    if tid:
    ...
    if fid:
        ...
        args.append(fid)
    if jid:
        parts.append("trim(COALESCE(match_javstash_id,'')) = ?")
        args.append(jid)
    if not parts:
        return None
    where = " OR ".join(parts)
    with get_conn() as conn:
        # Pull the weighting + is_group flag alongside path/folder_name so
        # the scene-filing router can both score and detect group folders
        # without a second query per performer.
        row = conn.execute(
            f"SELECT id, path, folder_name, is_favourite, weight, is_group, group_ids_json "
            f"FROM favourite_entities "
        ...
        for gr in group_rows:
            ...
            jav_list = [str(x).strip() for x in (data.get("javstash") or []) if str(x).strip()]
            if (
                (tid and tid in tpdb_list)
                or (sid and sid in stash_list)
                or (fid and fid in fans_list)
                or (jid and jid in jav_list)
            ):
                return {
                    "id": gr["id"],
                    "path": gr["path"],
...
def favourite_find_studio_folder_by_crosswalk(
    ...
    with get_conn() as conn:
        row = conn.execute(
            f"SELECT id, path, folder_name FROM favourite_entities WHERE kind = 'studio' AND ({where}) LIMIT 1",
            args,
        ).fetchone()
        if row:
            return dict(row)
        ...
        group_rows = conn.execute(
            "SELECT id, path, folder_name, group_ids_json "
            "FROM favourite_entities "
...
def upsert_file(filename: str, scope: str | None = None) -> int:
    ...
    with get_conn() as conn:
        ...
        if row:
            ...
            if stored != filename:
                conn.execute(
                    ...
                    SET filename = ?, filename_stem = ?, filename_ext = ?,
                        parsed_meta_json = NULL,
                        guess_studio = NULL, guess_performers = NULL,
                        guess_lib_gen = NULL,
                        perf_resolver_json = NULL
                    WHERE id = ?
                    """,
            ...
            if scope is not None:
                conn.execute(
                    "UPDATE processed_files SET scope = ? WHERE id = ?",
                    (scope, rid),
                )
            conn.commit()
            return rid
        cur = conn.execute(
            """
            INSERT INTO processed_files (filename, filename_stem, filename_ext, status, scope)
            VALUES (?, ?, ?, 'pending', ?)
            """,
            (filename, fst, fex, scope),
        )
        conn.commit()
        return cur.lastrowid


def get_parsed_meta(filename: str) -> dict | None:
    """Return the cached filename-parser blob for a queue file, or None.

    The queue snapshot builder writes this after running ``parse_filename``
    so subsequent reads (manual-search modal open, scene-search modal
    pre-fill, /api/parse/filename) are zero-cost.
    """
    fst, _ = _filename_stem_ext(filename)
    with get_conn() as conn:
        row = conn.execute(
            "SELECT parsed_meta_json FROM processed_files "
            "WHERE (filename = ? OR filename = ?) "
            "AND parsed_meta_json IS NOT NULL AND parsed_meta_json != '' "
            "LIMIT 1",
            (filename, fst),
        ).fetchone()
        if not row or not row["parsed_meta_json"]:
            return None
        try:
            data = json.loads(row["parsed_meta_json"])
            return data if isinstance(data, dict) else None
        except Exception:
            return None


def set_parsed_meta(filename: str, meta: dict) -> None:
    """Persist the filename-parser output. Upserts a row if needed so
    a fresh file can be parsed before the pipeline runs."""
    if not filename:
        return
    payload = json.dumps(meta) if isinstance(meta, dict) else None
    if payload is None:
        return
    upsert_file(filename)
    with get_conn() as conn:
        conn.execute(
            "UPDATE processed_files SET parsed_meta_json = ? WHERE filename = ?",
            (payload, filename),
        )
        conn.commit()


def library_match_generation() -> str:
    """Return a token describing the current state of the library-match
    inputs (favourite_entities.performer + .studio rows, and studio_logos).

    Composite of ``MAX(updated_at)`` and ``COUNT(*)`` on each source —
    any INSERT/UPDATE/DELETE on any of them shifts at least one component,
    which invalidates cached guesses stored on ``processed_files`` rows.
    Cheaper than instrumenting every write path (there are 20+ favourite
    UPDATE sites) and self-correcting for DELETEs (which don't touch
    ``updated_at`` but do drop the row count).
    """
    with get_conn() as conn:
        fav_p = conn.execute(
            "SELECT MAX(updated_at) AS m, COUNT(*) AS n "
            "FROM favourite_entities WHERE kind = 'performer'"
        ).fetchone()
        fav_s = conn.execute(
            "SELECT MAX(updated_at) AS m, COUNT(*) AS n "
            "FROM favourite_entities WHERE kind = 'studio'"
        ).fetchone()
        stu = conn.execute(
            "SELECT MAX(last_fetched_at) AS m, COUNT(*) AS n "
            "FROM studio_logos WHERE logo_path IS NOT NULL"
        ).fetchone()
    return "{}:{}:{}:{}:{}:{}".format(
        (fav_p["m"] if fav_p else "") or "",
        (fav_p["n"] if fav_p else 0) or 0,
        (fav_s["m"] if fav_s else "") or "",
        (fav_s["n"] if fav_s else 0) or 0,
        (stu["m"] if stu else "") or "",
        (stu["n"] if stu else 0) or 0,
    )


def processed_files_set_guess(
    filename: str,
    studio: str,
    performers: str,
    lib_gen: str,
) -> None:
    """Persist the library-match guess (studio + performers) on the
    processed_files row so the queue snapshot builder can reuse it on
    the next rebuild instead of re-tokenising the filename against
    every library performer.

    Upserts the row (via ``upsert_file``) so files that haven't been
    through the pipeline yet still get a durable cache entry.
    ``lib_gen`` is the value of ``library_match_generation()`` at the
    time the guess was computed — the snapshot builder re-computes the
    guess whenever the stored token no longer matches the current one.
    """
    if not filename:
        return
    upsert_file(filename)
    with get_conn() as conn:
        conn.execute(
            "UPDATE processed_files "
            "SET guess_studio = ?, guess_performers = ?, guess_lib_gen = ? "
            "WHERE filename = ?",
            (studio or "", performers or "", lib_gen or "", filename),
        )
        conn.commit()


def processed_files_set_resolver_json(filename: str, payload: str) -> None:
    """Persist the performer-resolver output blob (headshot URLs +
    dropped-by-filename + in/missing-in-library breakdown) on the row.

    Same shape as ``processed_files_set_guess`` — upserts so pending /
    untracked files still get a durable cache entry. Payload validity
    is a caller responsibility (must be a JSON string carrying its own
    ``gen`` + ``perf_hash`` fields the reader compares against current
    library state before trusting it).
    """
    if not filename:
        return
    upsert_file(filename)
    with get_conn() as conn:
        conn.execute(
            "UPDATE processed_files SET perf_resolver_json = ? WHERE filename = ?",
            (payload or "", filename),
        )
        conn.commit()


def favourite_touch(row_id: int) -> None:
    """Bump ``updated_at`` on a favourite_entities row without changing
    any other columns.

    Used by side-channel writes that mutate a performer's derived state
    (IAFD meta refresh, headshot re-download) but don't touch any of
    the columns that already refresh ``updated_at`` on write. Shifts
    ``library_match_generation()`` so queue-snapshot resolver caches
    invalidate on next rebuild — otherwise a fresh IAFD scrape would
    leave the previous cache-busted URL wired into cached headshots
    with no way to knock it loose short of a real library edit.
    """
    try:
        rid = int(row_id)
    except (TypeError, ValueError):
    ...
    with get_conn() as conn:
        conn.execute(
            "UPDATE favourite_entities SET updated_at = ? WHERE id = ?",
            (datetime.now().isoformat(timespec="seconds"), rid),
        )
        conn.commit()


def processed_files_map_by_filename(limit: int = 10000) -> dict[str, dict]:
    """Return ``{filename: row_dict}`` for all processed_files rows
    regardless of status.

    ``get_history`` filters out ``status = 'pending'`` because /history
    only surfaces post-pipeline rows, but the queue snapshot builder
    also needs pending rows so it can read the cached
    ``guess_studio`` / ``guess_performers`` / ``guess_lib_gen`` for
    files still waiting to be filed.
    """
    with get_conn() as conn:
        cur = conn.execute(
...
def processed_file_set_phash(filename: str, phash: str) -> int:
    """Set ``phash`` on all processed_files rows matching ``filename``.

    Unlike ``update_file`` this touches only the phash column — no status
    change, no ``processed_at`` bump — so the rehash-on-boot job (see
    ``_stash_compat_phash_rebuild`` in main.py) can refresh a row's
    fingerprint without pretending the file was just re-processed.
    ...
    with get_conn() as conn:
        cur = conn.execute(
            "UPDATE processed_files SET phash = ? WHERE filename = ? OR filename = ?",
            (ph, filename, fst),
        )
        conn.commit()
        return cur.rowcount or 0
...
def get_media_duration(filename: str) -> int | None:
    """Return cached duration for a filename, or None."""
    fst, _ = _filename_stem_ext(filename)
    with get_conn() as conn:
        row = conn.execute(
            "SELECT media_duration FROM processed_files WHERE (filename = ? OR filename = ?)",
            (filename, fst),
        ).fetchone()
        return row["media_duration"] if row else None


def get_file_scope(filename: str) -> str | None:
    """Return the import scope for a queued file ('scene' | 'vice'), or None."""
    fst, _ = _filename_stem_ext(filename)
    with get_conn() as conn:
        row = conn.execute(
            "SELECT scope FROM processed_files WHERE (filename = ? OR filename = ?) LIMIT 1",
            (filename, fst),
        ).fetchone()
        return (row["scope"] if row and row["scope"] else None)


def rename_file(old_filename: str, new_filename: str) -> bool:
    """Rekey a processed_files row after the video moves within source_dir
    (e.g. queue root → performer/studio subfolder). Also migrates the
    filename-keyed side tables (queue_suggestions, queue_suggestion_rejects)
    so previously-rejected candidates stay rejected under the new path.
    Returns True if a processed_files row was updated. No-op when old ==
    new, when no row matches the old key, or when a row already exists
    under new_filename."""
    if not old_filename or not new_filename or old_filename == new_filename:
        return False
    old_stem, _ = _filename_stem_ext(old_filename)
    new_stem, new_ext = _filename_stem_ext(new_filename)
    with get_conn() as conn:
        existing = conn.execute(
            "SELECT id FROM processed_files WHERE filename = ? LIMIT 1",
            (new_filename,),
        ).fetchone()
        if existing:
            return False
        row = conn.execute(
            ...
            (old_filename, old_stem),
        ).fetchone()
        if not row:
            return False
        conn.execute(
            "UPDATE processed_files SET filename = ?, filename_stem = ?, filename_ext = ? WHERE id = ?",
            (new_filename, new_stem, new_ext, int(row["id"])),
        )
        # Migrate side tables keyed by filename so per-file state (cached
        # suggestion candidates, rejection log) follows the row. Rejects
        # in particular must survive the rename — otherwise a rejected
        # candidate re-surfaces after the queue's auto-group move.
        for table in ("queue_suggestions", "queue_suggestion_rejects"):
            try:
                conn.execute(
                    f"UPDATE OR IGNORE {table} SET filename = ? WHERE filename = ?",
...
def update_file(filename, status, phash=None, match_source=None, match_title=None,
                ...
                destination=None, error=None, match_external_id=None, media_duration=None):
    fst, _ = _filename_stem_ext(filename)
...
def get_history_by_filename(filename: str) -> dict | None:
    """Direct-index lookup instead of get_history(10000) + linear scan.
    Uses idx_filename on processed_files. Returns the most recent
    non-pending row for the filename or None."""
    with get_conn() as conn:
        row = conn.execute(
...
def rename_movie(old_filename: str, new_filename: str) -> bool:
    """Rekey a processed_movies row after the movie file was renamed
    on disk (e.g. release-junk cleanup at pipeline-fire time). Returns
    True if a row was updated. Silently no-op when old == new, when
    the row doesn't exist, or when the new key already has a row."""
    if not old_filename or not new_filename or old_filename == new_filename:
        return False
    with get_conn() as conn:
        existing = conn.execute(
            "SELECT id FROM processed_movies WHERE filename = ? LIMIT 1",
            (new_filename,),
        ).fetchone()
        if existing:
            return False
        ...
        if not row:
            return False
        conn.execute(
            "UPDATE processed_movies SET filename = ? WHERE id = ?",
            (new_filename, int(row["id"])),
        )
        conn.commit()
        return True
...
def update_movie(filename: str, status: str, tmdb_id: str = None, tpdb_id: str = None,
                 javstash_id: str = None, match_source: str = None, title: str = None,
...
def activity_log_append_batch(lines: list[str]) -> None:
    if not lines:
        return
    now = datetime.now(timezone.utc).isoformat(timespec="seconds")
    with get_conn() as conn:
        conn.executemany(
            "INSERT INTO activity_log (created_at, line) VALUES (?, ?)",
            [(now, str(l)) for l in lines if l and str(l).strip()],
        )
        conn.commit()


def activity_log_fetch(*, limit: int = 5000, offset: int = 0, q: str = "") -> list[dict]:
    lim = max(1, min(int(limit), 50_000))
    off = max(0, int(offset))
    qstr = (q or "").strip()
    with get_conn() as conn:
        if qstr:
            # Case-insensitive substring filter at the SQL layer so the
            # frontend's offset/limit pagination tracks the filtered
            # count, not the unfiltered total. Without this, a filter
            # whose first matches sit on page 4 leaves pages 1-3 looking
            # empty even though the matches exist further down.
            cur = conn.execute(
                """
                SELECT id, created_at, line FROM activity_log
                WHERE line LIKE ? ESCAPE '\\' COLLATE NOCASE
                ORDER BY id DESC
                LIMIT ? OFFSET ?
                """,
                (f"%{_sql_like_escape(qstr)}%", lim, off),
            )
        else:
            cur = conn.execute(
                """
...
def activity_log_iter_all(*, q: str = ""):
    """Yield every activity-log row in chronological order (oldest →
    newest) as ``(created_at, line)`` tuples. Optional substring
    filter mirrors ``activity_log_fetch``. Used by the log-download
    endpoint so a multi-megabyte history can stream without pinning
    memory — sqlite3 cursors already iterate lazily, no manual chunking
    needed."""
    qstr = (q or "").strip()
    with get_conn() as conn:
        if qstr:
            cur = conn.execute(
                """
                SELECT created_at, line FROM activity_log
                WHERE line LIKE ? ESCAPE '\\' COLLATE NOCASE
                ORDER BY id ASC
                """,
                (f"%{_sql_like_escape(qstr)}%",),
            )
        else:
            cur = conn.execute(
                "SELECT created_at, line FROM activity_log ORDER BY id ASC"
            )
        for r in cur:
            yield (r["created_at"] or "", r["line"] or "")


def activity_log_count(*, q: str = "") -> int:
    qstr = (q or "").strip()
    with get_conn() as conn:
        if qstr:
            row = conn.execute(
...
def _sql_like_escape(s: str) -> str:
    """Escape SQL LIKE wildcards so a user-typed `%` or `_` filters
    literally instead of acting as a wildcard. Backslash itself is
    escaped first so the doubling-up doesn't turn into `\\\\`."""
...
def download_failure_observe(identity: str, status: str, message: str) -> bool:
    """Persist dedup separately from dismissible alerts, atomically."""
    with get_conn() as conn:
        conn.execute("CREATE TABLE IF NOT EXISTS download_failure_seen (identity TEXT PRIMARY KEY, status TEXT NOT NULL)")
        conn.execute("CREATE TABLE IF NOT EXISTS download_failure_notification (identity TEXT NOT NULL, notification_id INTEGER NOT NULL, PRIMARY KEY(identity, notification_id))")
        # Attach alerts created before linkage was introduced on their next observation.
        if message:
            conn.execute("INSERT OR IGNORE INTO download_failure_notification SELECT ?, id FROM app_notifications WHERE message=? ORDER BY id DESC LIMIT 1", (identity, message[:2000]))
        if not status:
            conn.execute("DELETE FROM app_notifications WHERE id IN (SELECT notification_id FROM download_failure_notification WHERE identity=?)", (identity,))
            conn.execute("DELETE FROM download_failure_notification WHERE identity=?", (identity,))
        previous = conn.execute("SELECT status FROM download_failure_seen WHERE identity=?", (identity,)).fetchone()
        if (previous is None and not status) or (previous is not None and previous[0] == status):
            return False
        if not status:
            conn.execute("DELETE FROM download_failure_seen WHERE identity=?", (identity,))
            return False
        cur = conn.execute("INSERT INTO download_failure_seen VALUES (?,?) ON CONFLICT(identity) DO UPDATE SET status=excluded.status WHERE status != excluded.status", (identity, status))
        if not cur.rowcount:
            return False
        now = datetime.now(timezone.utc)
        notification = conn.execute("INSERT INTO app_notifications(created_at,kind,message,expires_at) VALUES (?,?,?,?)", (now.isoformat(timespec="seconds"), "error", message[:2000], (now + timedelta(days=1)).isoformat(timespec="seconds")))
        conn.execute("INSERT OR REPLACE INTO download_failure_notification VALUES (?,?)", (identity, notification.lastrowid))
        return True
...
def get_retry_files() -> list[str]:
    """Return filenames of all non-filed, non-removed records.

    Includes ``pending`` so files that crashed mid-pipeline (row created
    at ``upsert_file`` but never advanced to ``processing``) get swept
    on the retry cadence instead of sitting forever with no phash and
    no fingerprint icon on the queue — Watchdog won't re-fire on them
    (no new create event) and the poll-fallback in ``_check_pending_files``
    skips rows that already exist in history.
    """
    with get_conn() as conn:
        cur = conn.execute(
            """SELECT filename FROM processed_files
               WHERE status IN ('unmatched', 'no_dir', 'error', 'pending')
               ORDER BY processed_at ASC"""
        )
        return [r["filename"] for r in cur.fetchall()]


def processed_files_purge_source_orphans(present_filenames: set[str]) -> int:
    """Delete processed_files rows whose source file is no longer
    present in source_dir.

    Targets only **unfiled** rows — `status` in
    ``('unmatched', 'no_dir', 'error', 'removed', 'pending')``. Filed
    rows are preserved because they carry the on-disk destination + a
    permanent history of where each scene landed.

    ``present_filenames`` is the set of basenames currently sitting in
    ``source_dir``. Any unfiled row whose ``filename`` isn't in this
    set is deleted. Returns the row count.

    Called from ``_build_queue_snapshot`` after the filesystem walk so
    orphan rows can't drift the queue counters (Errors / No Dir /
    Unmatched tiles read from SQL when the queue payload hasn't loaded
    yet, and a stale "1 error" with no clickable file is confusing).
    """
    targets = ("unmatched", "no_dir", "error", "removed", "pending")
    placeholders = ", ".join("?" * len(targets))
    with get_conn() as conn:
        # Fetch all candidate orphans first so we can diff against the
        # caller-supplied present-set in Python — SQLite has no native
        # set-difference operator and bundling thousands of filenames
        # into a NOT IN clause runs into the variable-binding cap on
        # very large queues.
        rows = conn.execute(
            f"SELECT id, filename FROM processed_files "
            f"WHERE status IN ({placeholders})",
            targets,
        ).fetchall()
        to_delete = [int(r["id"]) for r in rows
                     if (r["filename"] or "") not in present_filenames]
        if not to_delete:
            return 0
        # Chunk the DELETE so an enormous orphan backlog (e.g. queue
        # used to have 50k files, all removed) doesn't blow past
        # SQLite's parameter limit either.
        deleted = 0
        for i in range(0, len(to_delete), 500):
            chunk = to_delete[i:i + 500]
            ph = ", ".join("?" * len(chunk))
            cur = conn.execute(
                f"DELETE FROM processed_files WHERE id IN ({ph})",
                chunk,
            )
...
def get_stats(scope: str | None = None) -> dict:
    """Aggregate counts over processed_files, optionally restricted to a
    scope ('vice' filters to rows tagged as vice-scope imports)."""
...
def processed_files_delete_by_destination(destination: str) -> int:
    """Delete every processed_files row whose ``destination`` matches
    *destination*. Called when a filed video is deleted from disk (e.g.
    /health duplicates → drop the loser) so the row can't linger as an
    "orphaned processed_files record" the user has to hand-resolve.

    Matches on both the raw path and its resolved form so symlinked
    library roots don't leave a survivor. Returns rows removed.
    """
    dst = (destination or "").strip()
    if not dst:
        return 0
    candidates: list[str] = [dst]
    try:
        rp = str(Path(dst).resolve())
        if rp and rp != dst:
    ...
    with get_conn() as conn:
        cur = conn.execute(
            f"DELETE FROM processed_files WHERE destination IN ({placeholders})",
            candidates,
        )
        conn.commit()
        return int(cur.rowcount or 0)
...
def _build_library_match_indexes() -> tuple[list[tuple[str, str]], list[tuple[tuple[str, ...], str, int]]]:
    ...
    Notes on alias handling: aliases from ``aliases_json`` are ingested
    only as **multi-token** phrases (two or more tokens). Single-token
    aliases ("Daisy", "Violet") are too noisy — every first-name match
    would paint unrelated releases — so they're dropped, mirroring the
    safety rule the frontend highlighter uses in `ts-utils.js`. Without
    this, releases whose only library hit is via an alias (e.g. a
    misspelled stage name) would highlight in the title but fail to
    resolve a headshot for the download progress circle.
    """
    ...
    # Dedup studio index against `(key, display)` — the studio_logos
    # table + every favourite row's folder / canonical / aliases can
    # each contribute the same slug, and we don't want the alnum-prefix
    # search to iterate the same key three times.
    seen_studio_keys: set[tuple[str, str]] = set()

    def _push_studio(name_str: str, display: str) -> None:
        s = (name_str or "").strip()
        if not s:
            return
        try:
            slug = studio_logo_slug(s)
        except Exception:
            slug = s.lower()
        key = alnum.sub("", (slug or "").lower())
        if not key or len(key) < 4:
            return
        d = (display or s).strip() or s
        pair = (key, d)
        if pair in seen_studio_keys:
            return
        seen_studio_keys.add(pair)
        studio_idx.append(pair)
    with get_conn() as conn:
        for r in conn.execute(
            ...
            if key and len(key) >= 4:
                pair = (key, name)
                if pair not in seen_studio_keys:
                    seen_studio_keys.add(pair)
                    studio_idx.append(pair)
        # Studio favourites: fold in folder_name, canonical DB names,
        # and aliases_json so a filename like `brcc.foo.mp4` prefix-
        # matches "Backroom Casting Couch"'s BRCC alias and files
        # under that folder. `display` stays folder_name so the queue
        # UI shows the local folder name (find_studio_dir then resolves
        # it back to the path via _favourites_name_index).
        for r in conn.execute(
            "SELECT folder_name, aliases_json, "
            ...
            if not folder_name:
                continue
            _push_studio(folder_name, folder_name)
            for field in ("match_tpdb_name", "match_stashdb_name", "match_fansdb_name"):
                _push_studio((r[field] or "").strip(), folder_name)
            try:
            ...
            for a in aliases or []:
                if isinstance(a, str):
                    _push_studio(a, folder_name)
        for r in conn.execute(
            "SELECT id, folder_name, aliases_json, "
            "match_tpdb_name, match_stashdb_name, match_fansdb_name "
            "FROM favourite_entities WHERE kind='performer'"
        ).fetchall():
            folder_name = (r["folder_name"] or "").strip()
            ...
            def _push(name_str: str, *, tier: int, alias_safe: bool = False) -> None:
                s = (name_str or "").strip()
                if not s:
                    return
                ...
                if total_chars < 5:
                    return
                # Aliases must be multi-token — see docstring; single-
                # word aliases ("Daisy") would paint every release with
                # that first name.
                if alias_safe and len(tokens) < 2:
                    return
            ...
            # Folder name — primary surface (tier 0). User-curated, so a
            # single-token folder ("Christian") is the user's explicit
            # choice; we trust it even if it risks first-name collisions.
            _push(folder_name, tier=0)
            # Canonical DB primary names (tier 1). These come from
            # TPDB/StashDB/FansDB which can have one-word stage names
            # ("Christian") that collide with arbitrary words in
            # releases — so require multi-token here, same safety rule
            # the aliases use. Without this, a Nicole Doshi release
            # containing "Christian Clay" would token-match a library
            # row whose match_tpdb_name happens to be just "Christian"
            # and pull the wrong performer's headshot/details.
            for field in ("match_tpdb_name", "match_stashdb_name", "match_fansdb_name"):
                _push((r[field] or "").strip(), tier=1, alias_safe=True)
            # Multi-token aliases (tier 2). Catches misspellings + stage
            # names that releases use instead of the canonical primary —
            # the frontend already highlights against these, so the
            # backend matching the same set keeps title highlights and
            # download-circle headshots in sync.
            try:
                aliases_raw = r["aliases_json"] or ""
                if aliases_raw:
            ...
            for a in aliases:
                if not isinstance(a, str):
                    continue
                _push(a, tier=2, alias_safe=True)
...
def library_file_update_destination_row(
    ...
    *,
    ...
    now = _iso_now()
    with get_conn() as conn:
        old = conn.execute("SELECT destination FROM library_files WHERE id=?", (int(row_id),)).fetchone()
        conn.execute(
            """
            UPDATE library_files SET
        ...
        if old and old["destination"] != destination:
            base, ext = _path_base_ext(destination)
            conn.execute("UPDATE processed_files SET destination=?, destination_base=?, destination_ext=? WHERE destination=?", (destination, base, ext, old["destination"]))
            conn.execute("UPDATE processed_movies SET destination=? WHERE destination=?", (destination, old["destination"]))
        conn.commit()


def library_file_set_library_root(row_id: int, library_root: str) -> None:
    """Point a row at a different configured root (re-home after path layout changes)."""
    now = _iso_now()
    lr = (library_root or "").strip()
    with get_conn() as conn:
        conn.execute(
            "UPDATE library_files SET library_root = ?, updated_at = ? WHERE id = ?",
            (lr, now, int(row_id)),
        )
        conn.commit()
...
def library_files_delete_under_folder(folder_path: str) -> int:
    """Delete library_files rows whose destination is the folder or a file inside it."""
    raw = (folder_path or "").strip()
    if not raw:
        return 0
    try:
        fp = str(Path(raw).expanduser().resolve())
    except OSError:
        fp = str(Path(raw).expanduser())
    prefix = fp + os.sep
    with get_conn() as conn:
        cur = conn.execute(
            "DELETE FROM library_files WHERE destination = ? OR destination LIKE ?",
            (fp, prefix + "%"),
        )
        conn.commit()
        return int(cur.rowcount or 0)
...
def library_files_mark_removed_bulk(row_ids: list[int]) -> int:
    """Batch equivalent of ``library_file_mark_removed`` — flip
    ``is_removed`` on a list of rows in a single transaction. The
    library-index scanner walks the whole disk after a pass to prune
    stubs whose files have vanished; per-row commits meant N tiny
    transactions each grabbing the writer lock in turn, blocking
    any user-triggered edit for the duration. Batch keeps the lock
    held once, briefly.

    Chunked to 500 ids/statement so we stay well under the SQLite
    default ``SQLITE_MAX_VARIABLE_NUMBER`` (999).
    """
    ids = [int(x) for x in (row_ids or []) if x]
    if not ids:
        return 0
    now = _iso_now()
    total = 0
    with get_conn() as conn:
        for chunk_start in range(0, len(ids), 500):
            ...
            cur = conn.execute(
                ...
                f"SET is_removed = 1, removed_at = ?, updated_at = ? "
...
def library_files_apply_index_heavy_results(items: list[dict]) -> int:
    """Batch-apply a list of library-index heavy-task results in a
    single transaction. Each item is a dict of the form:

        {"row_id": int,
         "phash_2": Optional[str],
         "probe": Optional[(mtime, created_iso, codec, w, h)]}

    Fields set only when present. Replaces per-task
    ``library_file_set_phash2`` + ``library_file_update_media_probe``
    commits so a bulk decode pass doesn't fire hundreds of tiny
    transactions in a tight loop.

    Returns the count of rows updated (statements executed).
    """
    if not items:
        return 0
    now = _iso_now()
    ...
    with get_conn() as conn:
        for it in items:
            row_id = int(it.get("row_id") or 0)
            if not row_id:
                continue
            phash_2 = it.get("phash_2")
            probe = it.get("probe")
            if phash_2:
                conn.execute(
                    "UPDATE library_files SET phash_2 = ?, updated_at = ? "
                    "WHERE id = ? AND (phash_2 IS NULL "
                    "OR TRIM(COALESCE(phash_2, '')) = '')",
                    (phash_2, now, row_id),
                )
                executed += 1
            if probe:
                mt, created_iso, codec, w, h = probe
                conn.execute(
                    "UPDATE library_files SET "
                    "media_codec = ?, media_width = ?, media_height = ?, "
                    "file_created_iso = ?, media_mtime = ?, updated_at = ? "
                    "WHERE id = ?",
                    (codec, w, h, created_iso, mt, now, row_id),
                )
                executed += 1
        conn.commit()
    return executed
...
def library_file_clear_removed_if_exists(destination: str) -> None:
    ...
    with get_conn() as conn:
        conn.execute(
            ...
            WHERE destination = ? AND (is_removed != 0 OR removed_at IS NOT NULL)
            """,
            (now, dst),
        )
        conn.commit()
...
def _library_destination_under_sql(dir_path: str) -> tuple[str, list]:
    """SQL fragment: row destination is under dir_path (folder or file at exact path)."""
    dp = os.path.normpath((dir_path or "").strip())
    if not dp:
        return "", []
    p = dp.rstrip(os.sep) + os.sep
    return " AND (destination LIKE ? OR destination = ?)", [p + "%", dp]


def _library_exclude_prefixes_sql(
    exclude_destination_prefixes: list[str] | None,
) -> tuple[str, list]:
    """Exclude rows under longer nested library roots (avoid double-counting parent roots)."""
    if not exclude_destination_prefixes:
        return "", []
    parts: list[str] = []
    args: list = []
    for raw in exclude_destination_prefixes:
        ep = os.path.normpath((raw or "").strip())
        if not ep:
            continue
        p = ep.rstrip(os.sep) + os.sep
        parts.append("destination NOT LIKE ?")
        args.append(p + "%")
    if not parts:
        return "", []
...
def library_files_count_under_destination(
    dir_path: str,
    scope_prefix: str | None = None,
    exclude_destination_prefixes: list[str] | None = None,
) -> int:
    """Count indexed videos by on-disk path (destination), not library_root column."""
    dest_sql, dest_args = _library_destination_under_sql(dir_path)
    if not dest_sql:
        return 0
    scope_sql, scope_args = _library_scope_sql(scope_prefix)
    excl_sql, excl_args = _library_exclude_prefixes_sql(exclude_destination_prefixes)
    with get_conn() as conn:
        row = conn.execute(
            f"""
            ...
            {excl_sql}
            """,
            (*dest_args, *scope_args, *excl_args),
        ).fetchone()
        return int(row["c"] or 0)


def library_files_expand_dir_by_destination(
    dir_path: str,
    scope_prefix: str | None = None,
) -> tuple[list[dict], list[tuple[str, int]]]:
    """Immediate children under dir_path using destination paths only (Health tree browse)."""
    dp = os.path.normpath((dir_path or "").strip())
    if not dp:
        return [], []
    p = dp.rstrip(os.sep) + os.sep
    plen = len(p)
    dest_sql, dest_args = _library_destination_under_sql(dp)
    scope_sql, scope_args = _library_scope_sql(scope_prefix)
    ...
    with get_conn() as conn:
        sql_leaves = f"""
            ...
            {scope_sql}
        """
        args_leaves = [*dest_args, plen, *scope_args]
        ...
        sql_seg = f"""
            ...
            {scope_sql}
        """
        args_seg = [plen, plen, *dest_args, plen, *scope_args]
        segs: list[str] = []
    ...
    for seg in segs:
        ...
        c = library_files_count_under_destination(child_path, scope_prefix)
...
def get_session_hours() -> int:
    """Back-compat shim — derive from session_minutes (authoritative)."""
    return max(1, int(round(get_session_minutes() / 60)))
...
def get_session_minutes() -> int:
    with get_conn() as conn:
        cur = conn.execute("SELECT session_minutes FROM auth WHERE id = 1")
        row = cur.fetchone()
        return row["session_minutes"] if row else 1440


def set_session_minutes(minutes: int) -> None:
    with get_conn() as conn:
        conn.execute(
            "UPDATE auth SET session_minutes = ?, session_hours = ? WHERE id = 1",
            (int(minutes), max(1, int(round(minutes / 60)))),
        )
        conn.commit()


def set_session_hours(hours: int) -> None:
    """Back-compat shim; routes through the minutes setter."""
    set_session_minutes(int(hours) * 60)


def create_session(token: str, minutes: int) -> None:
...
def validate_session(token: str) -> bool:
    """Pure read — return True iff the session exists and has not
    idle-expired. Does NOT slide the window. Sliding happens only via
    :func:`touch_session`, called from the heartbeat endpoint triggered
    by real user input (mouse / keyboard / touch). Background polls
    therefore can no longer keep an abandoned tab signed in forever.
    """
    if not token:
        return False
    with get_conn() as conn:
...
def touch_session(token: str) -> int:
    """Slide the session's idle window forward. Returns remaining
    seconds after the slide (0 if the session does not exist)."""
    if not token:
        return 0
    from datetime import timedelta
    with get_conn() as conn:
        cur = conn.execute("SELECT 1 FROM sessions WHERE token = ?", (token,))
        if not cur.fetchone():
            return 0
        mcur = conn.execute("SELECT session_minutes FROM auth WHERE id = 1")
        mrow = mcur.fetchone()
        minutes = mrow["session_minutes"] if mrow else 1440
        now = datetime.now()
        expires = now + timedelta(minutes=minutes)
        conn.execute(
            "UPDATE sessions SET expires_at = ? WHERE token = ?",
            (expires.isoformat(), token),
        )
        conn.commit()
        return int(minutes) * 60


def get_session_remaining_seconds(token: str) -> int:
    """Seconds remaining until idle expiry. 0 if expired or unknown."""
    if not token:
        return 0
    with get_conn() as conn:
        cur = conn.execute(
...
def record_scene_grab(
    ...
    grabbed from on /scenes. Stores layered match keys — strongest
    first:
      1. `(client_kind, client_job_id)` — opaque ID the client returns
         from append/add/torrent-add. Stable across renames.
      2. `final_name` — canonical top-level on-disk name we asked the
         client to use, matched against the watched folder.
      3. `ts_uid` — short tag injected into the title; survives most
         renames as a substring on the eventual filename.
      4. `download_url` / `guid` / `release_title` — soft fallbacks.
    Idempotent: re-grabbing the same release just appends another row;
    the lookup picks the most recent. Returns the inserted row id so
    the caller can stamp client_job_id later if the ID isn't known
    until after the client responds."""
        ...
        cur = conn.execute(
            ...
            INSERT INTO scene_grab_log
                (release_title, guid, download_url, source_db, source_id,
                 ...
                 source_poster_url, source_date,
                 client_kind, client_job_id, final_name,
                 created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                ...
                (client_kind or "").strip().lower(),
                (client_job_id or "").strip(),
                (final_name or "").strip(),
...
def update_scene_grab_client_job(grab_id: int, client_kind: str = "",
                                  client_job_id: str = "", final_name: str = "") -> None:
    """Stamp the client-returned identifiers onto an existing
    scene_grab_log row. Used when the grab is recorded before the
    client call and we learn the job ID from the client's response."""
    if not grab_id:
        return
    sets = []
    args: list[str | int] = []
    if client_kind:
        sets.append("client_kind = ?")
        args.append((client_kind or "").strip().lower())
    if client_job_id:
        sets.append("client_job_id = ?")
        args.append((client_job_id or "").strip())
    if final_name:
        sets.append("final_name = ?")
        args.append((final_name or "").strip())
    if not sets:
        return
    args.append(int(grab_id))
    with get_conn() as conn:
        conn.execute(
            f"UPDATE scene_grab_log SET {', '.join(sets)} WHERE id = ?",
            tuple(args),
        )
        conn.commit()


def lookup_scene_grab_by_client_job(client_kind: str, client_job_id: str) -> dict | None:
    """Find the most recent un-consumed scene_grab_log row for a given
    download-client job. The (kind, id) pair is the strongest match
    we have because the client owns it — survives every rename and
    unpack. Returns None if the row was already consumed (so a
    duplicate grab can't be auto-filed twice)."""
    ck = (client_kind or "").strip().lower()
    cj = (client_job_id or "").strip()
    if not ck or not cj:
        return None
    with get_conn() as conn:
        cur = conn.execute(
...
def lookup_scene_grab_by_final_name(name: str) -> dict | None:
    """Find the most recent un-consumed scene_grab_log row whose
    `final_name` matches the on-disk entry name (case-insensitive
    exact). Used by the download-watch handler to recover a row when
    the [ts-XXXXXXXX] tag has been stripped by the client/unpacker
    but the canonical job name is still on disk."""
    n = (name or "").strip()
    if not n:
        return None
    with get_conn() as conn:
...
# ── Scene fingerprint crosswalk (option-3 phash lookup) ──────────────

def scene_fingerprints_upsert(source: str, scene_id: str, fingerprints: list[dict]) -> int:
    """Persist fingerprints for a stash-box scene. Each entry is a dict
    with at least `hash` and `algorithm`; `duration` is optional. The
    table has a composite primary key (source, scene_id, phash) so
    repeat calls are idempotent. Returns count of rows inserted."""
    src = (source or "").strip().lower()
    sid = (scene_id or "").strip()
    if not src or not sid or not fingerprints:
        return 0
    now = datetime.now(timezone.utc).isoformat()
    inserted = 0
    with get_conn() as conn:
        for fp in fingerprints:
            if not isinstance(fp, dict):
                continue
            phash = (fp.get("hash") or "").strip().lower()
            if not phash:
                continue
            try:
                conn.execute(
                    "INSERT OR REPLACE INTO scene_fingerprints "
                    "(source, scene_id, phash, algorithm, duration, fetched_at) "
                    "VALUES (?, ?, ?, ?, ?, ?)",
                    (
                        src, sid, phash,
                        (fp.get("algorithm") or "").upper() or None,
                        int(fp.get("duration") or 0) or None,
                        now,
                    ),
                )
                inserted += 1
...
def scene_fingerprints_match_phashes(phashes: list[str]) -> dict[str, list[dict]]:
    """For each input phash (case-insensitive), return matching
    `{source, scene_id}` entries from scene_fingerprints. Used by the
    /health tree to surface DB links for library files whose phash is
    already crosswalked. Empty phashes are skipped."""
    ...
    with get_conn() as conn:
        for i in range(0, len(norm), 400):
            chunk = norm[i:i + 400]
            ph_marks = ",".join("?" * len(chunk))
            cur = conn.execute(
                f"SELECT phash, source, scene_id FROM scene_fingerprints "
                f"WHERE phash IN ({ph_marks})",
                chunk,
            )
            for r in cur.fetchall():
                ph = (r["phash"] or "").strip().lower()
                src = (r["source"] or "").strip().lower()
                sid = (r["scene_id"] or "").strip()
                if not ph or not src or not sid:
                    continue
                key = (src, sid)
                bucket = seen_pairs.setdefault(ph, set())
                if key in bucket:
                    continue
                bucket.add(key)
                out.setdefault(ph, []).append({"source": src, "scene_id": sid})
    return out


def phash_submission_record(
    source: str,
    scene_id: str,
    phash: str,
    filename: str | None = None,
) -> bool:
    """Log a successful phash submission. Returns True if a new row was
    inserted (dedup on ``(source, scene_id, phash)`` — same phash for
    the same scene doesn't log twice)."""
    src = (source or "").strip().lower()
    sid = (scene_id or "").strip()
    ph  = (phash or "").strip().lower()
    if not src or not sid or not ph:
        return False
    fn = (filename or "").strip() or None
    with get_conn() as conn:
        cur = conn.execute(
            """
            INSERT OR IGNORE INTO phash_submissions (source, scene_id, phash, filename)
            VALUES (?, ?, ?, ?)
            """,
            (src, sid, ph, fn),
        )
...
def phash_submissions_for_phashes(phashes: list[str]) -> dict[str, set[tuple[str, str]]]:
    """For each input phash (case-insensitive), return the set of
    ``(source, scene_id)`` pairs where we've submitted that phash.
    Used by /health to badge per-scene DB icons."""
    ...
    with get_conn() as conn:
        for i in range(0, len(norm), 400):
            ...
            cur = conn.execute(
                f"SELECT phash, source, scene_id FROM phash_submissions "
                f"WHERE phash IN ({marks})",
                chunk,
            )
            for r in cur.fetchall():
                ph = (r["phash"] or "").strip().lower()
                ...
                if not ph or not src or not sid:
                    continue
                out.setdefault(ph, set()).add((src, sid))
    return out


def scene_fingerprints_known_ids(source: str, scene_ids: list[str]) -> set[str]:
    """Return the subset of `scene_ids` for which we already have at
    least one fingerprint cached. Used by the API to decide which
    ids still need backfilling."""
    src = (source or "").strip().lower()
    ids = [str(s).strip() for s in (scene_ids or []) if str(s).strip()]
    if not src or not ids:
        return set()
    out: set[str] = set()
    with get_conn() as conn:
        # SQLite's variadic IN clause; chunk to keep parameter count sane
        for i in range(0, len(ids), 400):
            chunk = ids[i:i+400]
            placeholders = ",".join("?" * len(chunk))
            cur = conn.execute(
                f"SELECT DISTINCT scene_id FROM scene_fingerprints "
                f"WHERE source = ? AND scene_id IN ({placeholders})",
                [src, *chunk],
            )
            for r in cur.fetchall():
                out.add(str(r["scene_id"]))
    return out


def library_match_scenes(items: list) -> dict[str, dict]:
    """For each item, return a match descriptor when the scene is in
    the library, or omit the key when not.

    Each item can be either:
      • A 2-tuple ``(source, scene_id)`` — id-only match
      • A dict ``{"source", "id", "title"?, "date"?, "studio"?}`` — id
        match with title/date/studio fallbacks for cross-source scenes

    Match strategy, in order of cheapness:
      1. processed_files.match_source / match_external_id direct hit
      2. scene_fingerprints joined against library_files.phash_1/2/3
         (exact phash match across any of the three seek points)
      3. processed_files.match_title (case-insensitive) + match_date —
         catches scenes imported via a different stash-box (so the id
         doesn't align with the source the user is currently browsing)

    Returns: ``{"source:scene_id": {"row_id": int, "destination": str}}``
    """
    if not items:
        return {}
    out: dict[str, dict] = {}

    # Normalise inputs to a uniform list of dicts
    ...
    for it in items:
        if isinstance(it, dict):
            ...
            src = str(it[0] or "").strip().lower()
            sid = str(it[1] or "").strip()
            extra = {"title": "", "date": "", "studio": ""}
        else:
            continue
        if not src or not sid:
            continue
        needed[(src, sid)] = extra
    if not needed:
        return {}

    with get_conn() as conn:
        # Step 1 — processed_files direct match
        # processed_files.match_source is stored case-as-supplied
        # (importer writes 'StashDB' / 'TPDB' / 'FansDB' from
        # query_with_fallback); compare case-insensitively so the
        # caller can pass either case. match_external_id is the scene
        # id. destination is the absolute path of the moved file —
        # join library_files on destination to get the row id.
        for (src, sid) in list(needed.keys()):
            cur = conn.execute(
                "SELECT pf.destination AS dest, lf.id AS lf_id "
                "FROM processed_files pf "
                "LEFT JOIN library_files lf ON lf.destination = pf.destination "
                "WHERE lower(COALESCE(pf.match_source, '')) = ? "
                "  AND pf.match_external_id = ? "
            ...
            if r:
                out[f"{src}:{sid}"] = {
                    "row_id": int(r["lf_id"]) if r["lf_id"] is not None else None,
                    "destination": r["dest"] or "",
                    "via": "processed_files",
                }
                needed.pop((src, sid), None)

        # Step 2 — phash crosswalk for whatever's left
        if needed:
            # Fetch every fingerprint for the remaining scenes in one
            # query, then for each fingerprint try to find a library
            # file whose phash_1/2/3 matches. Hash-equality match only
            # (Hamming distance would need bit-popcount; v1 uses exact).
            wanted_keys = list(needed.keys())
            for i in range(0, len(wanted_keys), 200):
                chunk = wanted_keys[i:i+200]
                # OR-of-(source, scene_id) — kept inline since the
                # composite PK isn't expressible as a single IN list.
                where = " OR ".join("(source = ? AND scene_id = ?)" for _ in chunk)
                args: list[str] = []
                for s, sid in chunk:
                    args.append(s); args.append(sid)
                cur = conn.execute(
                    f"SELECT source, scene_id, phash FROM scene_fingerprints "
                    f"WHERE {where}",
                    args,
                )
                # Group fingerprints by scene
                by_scene: dict[tuple[str, str], list[str]] = {}
                for r in cur.fetchall():
                    key = (r["source"], r["scene_id"])
                    by_scene.setdefault(key, []).append(r["phash"])
                if not by_scene:
                    continue
                # Flatten unique phashes for one bulk library_files lookup.
                # scene_fingerprints stores phashes lowercase (see
                # scene_fingerprints_upsert) but library_files preserves
                # whatever case the importer / FFmpeg wrote — case-fold
                # both sides so the IN match succeeds regardless.
                all_phashes: set[str] = set()
                for plist in by_scene.values():
                    all_phashes.update(p.lower() for p in plist if p)
                phash_list = list(all_phashes)
                lf_phash_to_row: dict[str, int] = {}
                for j in range(0, len(phash_list), 400):
                    pchunk = phash_list[j:j+400]
                    ph = ",".join("?" * len(pchunk))
                    cur2 = conn.execute(
                        f"SELECT id, phash_1, phash_2, phash_3 "
                        f"FROM library_files "
                        f"WHERE is_removed = 0 AND ("
                        f"lower(phash_1) IN ({ph}) "
                        f"OR lower(phash_2) IN ({ph}) "
                        f"OR lower(phash_3) IN ({ph}))",
                        [*pchunk, *pchunk, *pchunk],
                    )
                    for lr in cur2.fetchall():
                        for p in (lr["phash_1"], lr["phash_2"], lr["phash_3"]):
                            if not p:
                                continue
                            pl = p.lower()
                            if pl in all_phashes:
                                # Earliest-wins for each phash
                                lf_phash_to_row.setdefault(pl, int(lr["id"]))
                # Also fold the by_scene phashes lowercase so the lookup
                # below hits the lowercase keys we built above.
                by_scene = {
                    k: [p.lower() for p in v if p]
                    for k, v in by_scene.items()
                }
                for (src, sid), plist in by_scene.items():
                    for p in plist:
                        rid = lf_phash_to_row.get(p)
                        if rid:
                            out.setdefault(f"{src}:{sid}", {
                                "row_id": rid,
                                "destination": "",
                                "via": "phash",
                            })
        ...
        # Step 3 — title + date fallback. Catches scenes imported via a
        # different stash-box than the one the user is currently
        # browsing (e.g. file matched on StashDB but they're browsing
        # the same scene on TPDB — IDs don't align). Match is case-
        # insensitive on match_title; date is matched on the YYYY-MM-DD
        # prefix so "2023-01-01" matches "2023-01-01T00:00:00Z" too.
        remaining = [(s, i) for (s, i) in needed if f"{s}:{i}" not in out]
        for (src, sid) in remaining:
            extra = needed.get((src, sid), {})
            title = (extra.get("title") or "").strip()
            date  = (extra.get("date")  or "").strip()[:10]  # YYYY-MM-DD
            if not title:
                continue
            tl = title.lower()
            if date:
                cur = conn.execute(
                    ...
                    "FROM processed_files pf "
                    "LEFT JOIN library_files lf ON lf.destination = pf.destination "
                    "WHERE lower(COALESCE(pf.match_title, '')) = ? "
                    "  AND substr(COALESCE(pf.match_date, ''), 1, 10) = ? "
                    "  AND pf.status = 'success' "
                    "  AND COALESCE(lf.is_removed, 0) = 0 "
                    "LIMIT 1",
                    (tl, date),
                ...
                if r:
                    ...
                    continue
            # Title alone — last-ditch (looser; collisions possible).
            cur = conn.execute(
                "SELECT pf.destination AS dest, lf.id AS lf_id "
                "FROM processed_files pf "
                "LEFT JOIN library_files lf ON lf.destination = pf.destination "
...
# ── Manual queue jobs (persistent sort list) ───────────────────
#
# Backing store for `_manual_action_queue`. Every user-initiated
# filing goes into `manual_queue_jobs` before touching the in-memory
# queue so a restart mid-batch can rehydrate whatever was pending
# (or running mid-move) and pick up right where the worker left off.
# Rows are deleted on success and flipped to `failed` on exception.

def manual_job_create(job_type: str, label: str, display_name: str, payload: dict) -> int:
    """Insert a pending job row and return its id."""
    with get_conn() as conn:
        cur = conn.execute(
            "INSERT INTO manual_queue_jobs "
            "(job_type, label, display_name, payload_json, status, created_at) "
            "VALUES (?, ?, ?, ?, ?, ?)",
            (
                job_type,
                label or "",
...
def manual_job_mark_started(job_id: int) -> None:
    with get_conn() as conn:
        conn.execute(
            "UPDATE manual_queue_jobs SET status = ?, started_at = ?, error = NULL "
            "WHERE id = ?",
            ("running", datetime.utcnow().isoformat(), int(job_id)),
        )
        conn.commit()


def manual_job_mark_pending(job_id: int) -> None:
    """Flip an interrupted (status=running at startup) job back to
    pending so the rehydrator re-enqueues it cleanly."""
    with get_conn() as conn:
        conn.execute(
            "UPDATE manual_queue_jobs SET status = ?, started_at = NULL WHERE id = ?",
            ("pending", int(job_id)),
        )
        conn.commit()


def manual_job_delete(job_id: int) -> None:
    """Success path — the worker filed the item, no reason to keep
    the row around."""
    with get_conn() as conn:
        conn.execute("DELETE FROM manual_queue_jobs WHERE id = ?", (int(job_id),))
        conn.commit()


def manual_job_mark_failed(job_id: int, error: str) -> None:
    with get_conn() as conn:
        conn.execute(
            "UPDATE manual_queue_jobs "
            "SET status = ?, error = ?, completed_at = ? WHERE id = ?",
            ("failed", (error or "")[:1000], datetime.utcnow().isoformat(), int(job_id)),
        )
        conn.commit()


def manual_job_list_active() -> list[dict]:
    """Every pending + running row in insertion order — used at
    startup to rehydrate the in-memory queue."""
    with get_conn() as conn:
        cur = conn.execute(
            "SELECT * FROM manual_queue_jobs "
...
def manual_job_list_failed(limit: int = 50) -> list[dict]:
    with get_conn() as conn:
        cur = conn.execute(
            "SELECT * FROM manual_queue_jobs WHERE status = ? "
...
def manual_job_clear_failed() -> int:
    with get_conn() as conn:
        cur = conn.execute("DELETE FROM manual_queue_jobs WHERE status = ?", ("failed",))
        conn.commit()
        return cur.rowcount or 0


PERFORMER_BIO_FIELDS = frozenset({
    "gender", "birthdate", "death_date", "country", "ethnicity", "hair_color",
    "eye_color", "height", "measurements", "career_start_year", "career_end_year",
    "tattoos", "piercings", "biography",
})


def performer_bio_merge(row_id: int, source: dict) -> dict:
    """Persist individual source fields; manual edits (including blanks) win."""
    with get_conn() as conn:
        for field in PERFORMER_BIO_FIELDS:
            value = str(source.get(field) or "")
            conn.execute("""INSERT INTO performer_bio_fields(row_id, field, value, edited)
                VALUES (?, ?, ?, 0) ON CONFLICT(row_id, field) DO UPDATE SET value=excluded.value
                WHERE performer_bio_fields.edited=0 AND performer_bio_fields.value != excluded.value""",
                (row_id, field, value))
        values = conn.execute("SELECT field, value FROM performer_bio_fields WHERE row_id=?", (row_id,)).fetchall()
    return {**source, **{r["field"]: r["value"] for r in values}}


def _validate_performer_bio_field(field: str, value: str) -> None:
    if field not in PERFORMER_BIO_FIELDS or not isinstance(value, str) or len(value) > 20000:
        raise ValueError("Invalid bio field or value")
    if field in {"career_start_year", "career_end_year"} and value.strip():
        from datetime import date
        import re
        raw = value.strip()
        if not re.fullmatch(r"[0-9]{4}", raw) or not 1900 <= int(raw) <= date.today().year:
            raise ValueError(f"Enter a four-digit year between 1900 and {date.today().year}")
    if field in {"birthdate", "death_date"} and value.strip():
        from datetime import date
        import re
        raw = value.strip()
        ...
        if field == "birthdate" and age < 18:
            raise ValueError("Performer must be at least 18 years old")

def performer_bio_set_many(row_id: int, fields: dict) -> dict:
    if not isinstance(fields, dict) or len(fields) > len(PERFORMER_BIO_FIELDS):
        raise ValueError("Invalid bio fields")
    for field, value in fields.items():
        _validate_performer_bio_field(field, value)
    cleaned = {field: value.strip() for field, value in fields.items()}
    with get_conn() as conn:
        row = conn.execute("SELECT kind FROM favourite_entities WHERE id=?", (row_id,)).fetchone()
        if not row or row["kind"] != "performer":
            raise ValueError("Performer not found")
        conn.executemany("""INSERT INTO performer_bio_fields(row_id, field, value, edited)
            VALUES (?, ?, ?, 1) ON CONFLICT(row_id, field) DO UPDATE SET value=excluded.value, edited=1""",
            [(row_id, field, value) for field, value in cleaned.items()])
    return cleaned


def performer_bio_set(row_id: int, field: str, value: str) -> None:
    performer_bio_set_many(row_id, {field: value})


def favourite_merge_commit(plan: dict) -> None:
    """One commit for group promotion, secondary retirement and replay marker."""
    with get_conn() as conn:
        conn.execute("PRAGMA synchronous=FULL")
        conn.execute("CREATE TABLE IF NOT EXISTS performer_merge_commits (operation_id TEXT PRIMARY KEY)")
        conn.execute("BEGIN IMMEDIATE")
        if conn.execute("SELECT 1 FROM performer_merge_commits WHERE operation_id=?", (plan['operation_id'],)).fetchone():
            return
        for row in plan['rows']:
            current = conn.execute("SELECT path FROM favourite_entities WHERE id=?", (row['id'],)).fetchone()
            if not current or current['path'] != row['path']:
                raise ValueError("A merge source changed in the library; refusing to commit")
        conn.execute("""UPDATE favourite_entities SET folder_name=?,path=?,is_group=1,
                     group_ids_json=?,path_missing=0,updated_at=? WHERE id=?""",
                     (plan['group_name'], plan['destination'], json.dumps(plan['group_ids']),
                      _iso_now(), plan['primary_id']))
        for item in plan['files']:
            source, target = item['source'], item['destination']
            if source != target:
                conn.execute("""UPDATE library_files SET destination=?,current_filename=?,
                             filename_stem=?,library_root=?,updated_at=? WHERE destination=?""",
                             (target, Path(target).name, Path(target).stem,
                              plan['library_root'], _iso_now(), source))
        for row in plan['rows'][1:]:
            conn.execute("DELETE FROM favourite_entities WHERE id=?", (row['id'],))
        conn.execute("INSERT INTO performer_merge_commits VALUES (?)", (plan['operation_id'],))
        conn.commit()


def download_cleanup_enqueue(name):
    with get_conn() as conn:
        conn.execute("INSERT OR IGNORE INTO download_cleanup_pending(name,created_at) VALUES(?,?)", (name, _iso_now()))
        conn.commit()

def download_cleanup_names():
    with get_conn() as conn:
        return [r[0] for r in conn.execute("SELECT name FROM download_cleanup_pending ORDER BY created_at LIMIT 200")]

def download_cleanup_complete(name):
    with get_conn() as conn:
        conn.execute("DELETE FROM download_cleanup_pending WHERE name=?", (name,))
        conn.commit()
