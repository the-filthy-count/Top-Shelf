...
def favourite_find_performer_folder_by_crosswalk(
    ...
    with get_conn() as conn:
        ...
        group_rows = conn.execute(
            "SELECT id, path, folder_name, group_ids_json FROM favourite_entities "
        ...
        for gr in group_rows:
            ...
            if (tid and tid in tpdb_list) or (sid and sid in stash_list) or (fid and fid in fans_list):
                return {"path": gr["path"], "folder_name": gr["folder_name"]}
        return None
...
def library_files_duplicate_groups() -> list[dict]:
    ...
    for ph, members in sorted(buckets.items(), key=lambda x: x[0]):
        if len(members) < 2:
            continue
        out.append({"phash": ph, "files": members})
    return out
...
def validate_session(token: str) -> bool:
    """Validate + slide the session's idle-expiry window.

    `session_hours` now acts as an idle timeout — every authenticated
    request that hits a valid session bumps its `expires_at` forward to
    `now + session_hours`. An abandoned tab stops refreshing, so the
    session naturally dies after the configured idle period; an active
    session keeps renewing itself indefinitely.
    """
    if not token:
        return False
    with get_conn() as conn:
        ...
        if datetime.fromisoformat(row["expires_at"]) < now:
            ...
            conn.commit()
            return False
        # Slide the idle window on every valid hit.
        from datetime import timedelta
        hours = get_session_hours()
        new_expires = (now + timedelta(hours=hours)).isoformat()
        conn.execute(
