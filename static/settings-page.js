/** Top-Shelf settings page logic. */
  ...
  function moveScheduleControls() {
    ...
    tpdb.innerHTML = '<div class="settings-section-title">TPDB favourites sync schedule</div>';
  ...
  async function loadScheduleFields() {
    ...
    try {
      ...
      for (const field of data.fields) {
        const row = document.createElement('label');row.className='field-row';
        ...
        input.type=field.unit==='boolean'?'checkbox':'number';input.className='field-input';
        if (input.type==='checkbox') {input.checked=field.value==='true';input.style.width='auto';}
        else {input.value=field.value;input.min=field.min;input.max=field.max;input.step='1';input.required=true;}
        const hint=document.createElement('span');hint.className='field-hint';
        hint.textContent=(field.unit==='boolean'?'Enabled':field.unit==='hour'?'Hour (0–23)':'Every '+field.unit)+' · Default: '+field.default;
        row.append(label,input,hint);
        if (field.next_run) {const next=document.createElement('span');next.className='field-hint';next.textContent='Next: '+new Date(field.next_run).toLocaleString();row.appendChild(next);}
  ...
  async function loadSettingsPage() {
    ...
    const _phashSubmitDefaults = { stashdb: 'false', fansdb: 'true', tpdb: 'true', javstash: 'true' };
    document.getElementById('cfgPhashSubmitStashdb').checked  = (s.phash_submit_disabled_stashdb  || _phashSubmitDefaults.stashdb)  !== 'true';
  ...
  async function saveSettings() {
    ...
    const payload = {
      settings: {
        ...
        phash_submit_disabled_stashdb:  document.getElementById('cfgPhashSubmitStashdb').checked  ? 'false' : 'true',
