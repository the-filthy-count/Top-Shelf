/** Top-Shelf settings page logic. */
  ...
  async function spotlightReload() {
    ...
    status.textContent = 'Fetching new stars in the background…';
    try {
      await fetch('/api/spotlight/reload', {method: 'POST'});
      status.textContent = 'Done — new stars will appear shortly.';
    } catch(e) {
      status.textContent = 'Error — check logs.';
    } finally {
      btn.disabled = false;
      btn.textContent = 'Load New Stars';
  ...
  async function clearSpotlightPerformers() {
    ...
    const status = document.getElementById('clearCacheStatus');
    if (!confirm('Wipe every harvested star from the spotlight cache? The next harvest tick will re-seed them under the current settings.')) return;
    btn.disabled = true;
    ...
    try {
      ...
      status.textContent = d.ok
        ? `Wiped ${d.cleared || 0} stars — re-harvest started.`
        : ('Error: ' + (d.error || 'unknown'));
    } catch (e) {
      status.textContent = 'Error wiping stars.';
    } finally {
      btn.disabled = false;
      btn.textContent = 'Clear Stars';
      setTimeout(() => { status.textContent = ''; }, 8000);
    }
  }
  ...
  async function clearSpotlightExcluded() {
    ...
    const status = document.getElementById('clearCacheStatus');
    if (!confirm('Reset the never-show-again list? Every star you previously excluded with the red ✕ will become eligible for the spotlight again.')) return;
    btn.disabled = true;
