/** Top-Shelf settings page logic. */
  ...
  function moveScheduleControls() {
    const target = document.getElementById('scheduleExisting');
    if (!target || target.dataset.ready) return;
    target.dataset.ready = '1';
    for (const id of ['cfgRetryEnabled', 'cfgHealthPhash3Recurring']) {
      target.appendChild(document.getElementById(id).closest('.settings-section'));
    }
    const tpdb = document.createElement('div');
    tpdb.className = 'settings-section';
    tpdb.innerHTML = '<div class="settings-section-title">Database favourites sync schedule</div>';
    for (const id of ['cfgTpdbSyncEnabled','cfgTpdbSyncHour','cfgTpdbSyncFreq']) {
      tpdb.appendChild(document.getElementById(id).closest('.field-row'));
    }
    target.appendChild(tpdb);
    const freq = document.getElementById('cfgRetryFreq');
    const numeric = document.createElement('input');
    numeric.type='number';numeric.id=freq.id;numeric.className='field-input';numeric.min='1';numeric.max='8760';numeric.step='1';
    freq.replaceWith(numeric);
    document.getElementById('cfgTpdbSyncFreq').max='8760';
    const hint = document.createElement('p');hint.className='field-hint';
    hint.textContent='The hash check runs on its own schedule below. The rescan interval here is the minimum age of a video’s existing hash before it is updated.';
    document.getElementById('cfgHealthPhash3IntervalDays').closest('.settings-section').appendChild(hint);
  }

  async function loadScheduleFields() {
    const host = document.getElementById('scheduleFields');
    try {
      const response = await fetch('/api/schedules');
      if (!response.ok) throw new Error('Could not load schedules');
      const data = await response.json();
      document.getElementById('scheduleTimezone').textContent = 'Server timezone: ' + data.timezone;
      host.replaceChildren();
      const groups = [
        ['Files & downloads', ['download_cleanup','library_reconcile','pending_check','movie_pending_check','download_check','wanted_library_reconcile','suggestions_trickle']],
        ['Feeds & artwork', ['rss_cache_refresh','news_rss_cache_refresh','studio_logos_fetch','content_cache_refresh','tag_scene_pools_refresh']],
        ['Library maintenance', ['favourites_scan','iafd_trickle','library_phash3_rescan']],
        ['Housekeeping', ['session_purge','activity_log_prune','net_error_flush','scene_grab_purge']],
      ];
      const jobs = new Map();
      for (const field of data.fields) {
        const job = field.job_id || field.key;
        if (!jobs.has(job)) jobs.set(job, []);
        jobs.get(job).push(field);
      }
      const assigned = new Set(groups.flatMap(([, ids]) => ids));
      const other = [...jobs.keys()].filter(id => !assigned.has(id));
      if (other.length) groups.push(['Other schedules', other]);
      const formatTime = value => new Date(value).toLocaleString(undefined, {day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'});
      for (const [heading, ids] of groups) {
        if (!ids.some(id => jobs.has(id))) continue;
        const section = document.createElement('section'); section.className = 'schedule-group';
        const title = document.createElement('h3'); title.textContent = heading; section.appendChild(title);
        const grid = document.createElement('div'); grid.className = 'schedule-job-grid'; section.appendChild(grid);
        for (const id of ids) {
          const fields = jobs.get(id); if (!fields) continue;
          const card = document.createElement('div'); card.className = 'schedule-job';
          const name = document.createElement('h4');
          name.textContent = ({favourites_scan:'Library folder scan',iafd_trickle:'IAFD enrichment',library_phash3_rescan:'Perceptual hash checks'})[id] || fields[0].label;
          card.appendChild(name);
          for (const field of fields) {
            const row = document.createElement('label'); row.className = 'schedule-control';
            const label = document.createElement('span');
            label.textContent = field.unit === 'boolean' ? 'Enabled' : field.unit === 'hour' ? 'Start hour' : 'Run every';
            const input = document.createElement('input'); input.dataset.scheduleKey = field.key;
            input.setAttribute('aria-label',field.label);
            input.type = field.unit === 'boolean' ? 'checkbox' : 'number';
            if (input.type === 'checkbox') {
              ...
              toggle.append(input,slider); row.append(label,toggle);
            } else {
              input.className = 'field-input'; input.value = field.value; input.min = field.min; input.max = field.max; input.step = '1'; input.required = true;
              const unit = document.createElement('span'); unit.className = 'schedule-unit'; unit.textContent = field.unit === 'hour' ? '(0–23)' : field.unit;
              row.append(label,input,unit);
            }
            row.title = 'Default: ' + field.default + (field.unit === 'boolean' ? '' : ' ' + field.unit);
            card.appendChild(row);
          }
          const status = document.createElement('div'); status.className = 'schedule-run-status';
          const first = fields[0];
          const next = document.createElement('div'); next.textContent = first.next_run ? 'Next · ' + formatTime(first.next_run) : 'Not scheduled'; status.appendChild(next);
          const last = document.createElement('div');
          last.textContent = first.last_run?.at ? `${first.last_run.status} · ${formatTime(first.last_run.at)}` : 'No runs since server started';
          if (first.last_run?.error) { last.classList.add('schedule-run-error'); last.title = first.last_run.error; }
          status.appendChild(last); card.appendChild(status); grid.appendChild(card);
        }
  ...
  async function loadSettingsPage() {
    ...
    dirs = rawList.map(x => {
      ...
      return Object.assign({}, x, {
        type: 'performer',
        gender_filters: gf,
        bucket_by_letter: !!x.bucket_by_letter,
      });
    });
    ...
    document.getElementById('cfgViceDownloadWatchDir').value  = s.vice_download_watch_dir  || '';
    document.getElementById('cfgJavDownloadWatchDir').value   = s.jav_download_watch_dir   || '';
    document.getElementById('cfgJavDownloadStageDir').value   = s.jav_download_stage_dir   || '';
    document.getElementById('cfgViceDownloadStageDir').value  = s.vice_download_stage_dir  || '';
    ...
    document.getElementById('cfgDiscGenderFemale').checked      = (s.disc_gender_female   || 'true')  === 'true';
    document.getElementById('cfgDiscGenderMale').checked        = (s.disc_gender_male     || 'false') === 'true';
    document.getElementById('cfgDiscGenderTrans').checked       = (s.disc_gender_trans    || 'true')  === 'true';
    document.getElementById('cfgDiscGenderIntersex').checked    = (s.disc_gender_intersex || 'true')  === 'true';
    document.getElementById('cfgDiscAgeMin').value              = s.disc_age_min || '';
    document.getElementById('cfgDiscAgeMax').value              = s.disc_age_max || '';
    ...
    document.getElementById('cfgProwlarrCategoryJav').value    = s.prowlarr_category_jav    || '';
    document.getElementById('cfgProwlarrCategoryVices').value  = s.prowlarr_category_vices  || '';
    toggleNzbFields();
    ...
    // Phash submission — checkbox represents ENABLED (submitting to
    // that DB); stored value is `phash_submit_disabled_<key>` so a
    // fresh install can ship with StashDB on and the rest off without
    // having to seed a row in the DB.
    const _phashSubmitDefaults = { stashdb: 'true', fansdb: 'true', tpdb: 'true', javstash: 'true' };
    document.getElementById('cfgPhashSubmitStashdb').checked = false;
    document.getElementById('cfgPhashSubmitStashdb').disabled = true;
    document.getElementById('cfgPhashSubmitFansdb').checked   = (s.phash_submit_disabled_fansdb   || _phashSubmitDefaults.fansdb)   !== 'true';
    document.getElementById('cfgPhashSubmitTpdb').checked     = (s.phash_submit_disabled_tpdb     || _phashSubmitDefaults.tpdb)     !== 'true';
    document.getElementById('cfgPhashSubmitJavstash').checked = (s.phash_submit_disabled_javstash || _phashSubmitDefaults.javstash) !== 'true';
    ...
    document.getElementById('cfgFavouritesSyncStashdb').checked = s.favourites_sync_stashdb === 'true';
    document.getElementById('cfgFavouritesSyncFansdb').checked = s.favourites_sync_fansdb === 'true';
    document.getElementById('cfgFavouritesSyncJavstash').checked = s.favourites_sync_javstash === 'true';
  ...
  function dirGenderRowHtml(d, i) {
    ...
    const bucketOn = !!d.bucket_by_letter;
    ...
    return `<label class="dir-gender-chip${on ? ' dir-gender-chip--on' : ''}">
      ...
      <label class="dir-gender-chip dir-bucket-chip${bucketOn ? ' dir-gender-chip--on' : ''}" style="margin-top:8px;align-self:flex-start">
        <input type="checkbox" ${bucketOn ? 'checked' : ''} onchange="setDirBucketByLetter(${i},this.checked)">
        <span class="dir-gender-chip-inner"><i class="fa-solid fa-folder-tree" aria-hidden="true"></i> Bucket folders by first letter (A/, B/, …, #/)</span>
      </label>
      <p class="dir-gender-hint">On save, existing folders here are reshelved automatically to match.</p>
    </div>`;
  }
  function setDirBucketByLetter(displayIdx, checked) {
    const sorted = _performerDirsSortedForDisplay();
    const row = sorted[displayIdx];
    if (!row) return;
    const j = dirs.indexOf(row);
    if (j < 0) return;
    dirs[j].bucket_by_letter = !!checked;
    renderDirList();
  }
  window.setDirBucketByLetter = setDirBucketByLetter;
  ...
  function addDir() {
    ...
    dirs.push({ type: 'performer', path, label, rank: dirs.length + 1, gender_filters: ['female', 'male', 'trans', 'other'], bucket_by_letter: false });
  ...
  async function saveSettings() {
    for (const input of document.querySelectorAll('[data-settings-category="schedule"] input[type="number"]')) {
      if (!input.checkValidity()) {selectSettingsCategory('schedule');input.reportValidity();return;}
    }
    ...
    const payload = {
      settings: {
        ...
        vice_download_watch_dir: document.getElementById('cfgViceDownloadWatchDir').value.trim(),
        jav_download_watch_dir:  document.getElementById('cfgJavDownloadWatchDir').value.trim(),
        jav_download_stage_dir:  document.getElementById('cfgJavDownloadStageDir').value.trim(),
        vice_download_stage_dir: document.getElementById('cfgViceDownloadStageDir').value.trim(),
        ...
        disc_gender_female:            document.getElementById('cfgDiscGenderFemale').checked ? 'true' : 'false',
        disc_gender_male:              document.getElementById('cfgDiscGenderMale').checked ? 'true' : 'false',
        disc_gender_trans:             document.getElementById('cfgDiscGenderTrans').checked ? 'true' : 'false',
        disc_gender_intersex:          document.getElementById('cfgDiscGenderIntersex').checked ? 'true' : 'false',
        disc_age_min:                  document.getElementById('cfgDiscAgeMin').value.trim(),
        disc_age_max:                  document.getElementById('cfgDiscAgeMax').value.trim(),
        ...
        prowlarr_category_jav:    document.getElementById('cfgProwlarrCategoryJav').value.trim(),
        prowlarr_category_vices:  document.getElementById('cfgProwlarrCategoryVices').value.trim(),
        ...
        phash_submit_disabled_stashdb: 'true',
        phash_submit_disabled_fansdb:   document.getElementById('cfgPhashSubmitFansdb').checked   ? 'false' : 'true',
        phash_submit_disabled_tpdb:     document.getElementById('cfgPhashSubmitTpdb').checked     ? 'false' : 'true',
        phash_submit_disabled_javstash: document.getElementById('cfgPhashSubmitJavstash').checked ? 'false' : 'true',
        ...
        favourites_sync_stashdb: document.getElementById('cfgFavouritesSyncStashdb').checked ? 'true' : 'false',
        favourites_sync_fansdb: document.getElementById('cfgFavouritesSyncFansdb').checked ? 'true' : 'false',
        favourites_sync_javstash: document.getElementById('cfgFavouritesSyncJavstash').checked ? 'true' : 'false',
      ...
      directories: dirs.map(d => ({
        ...
        bucket_by_letter: !!d.bucket_by_letter,
      })),
    };
    document.querySelectorAll('[data-schedule-key]').forEach(input => {
      payload.settings[input.dataset.scheduleKey] = input.type==='checkbox' ? String(input.checked) : input.value;
    });
    ...
    if (r.ok) {
      ...
      showSettingsSavedToast(error.error || 'Save failed', { kind: 'error' });
  ...
  async function runTpdbSync() {
    ...
    try {
      ...
      const d = await r.json();
      if (d.started) { el.textContent = 'Sync running in background — follow progress and results in notifications.'; }
      else if (d.skipped) { el.textContent = 'Sync disabled'; el.style.color = 'var(--dim)'; }
      else if (d.error) { el.textContent = d.error; el.style.color = 'var(--red)'; }
      else {
        let failed = false;
        el.textContent = Object.entries(d.databases || {}).map(([source, result]) => {
          if (result.error) { failed = true; return `${source.toUpperCase()}: ${result.error}`; }
          const p = result.performers || {}, s = result.studios || {};
          const errors = (p.failed || 0) + (s.failed || 0);
          failed ||= errors > 0;
          return `${source.toUpperCase()}: ${p.added || 0} performers, ${s.added || 0} studios sent · ${(p.already || 0) + (s.already || 0)} already favourited · ${(p.unlinked || 0) + (s.unlinked || 0)} unlinked skipped · ${errors} failed${Object.keys(result.errors || {}).length ? " (" + Object.entries(result.errors).map(([reason, count]) => `${reason}: ${count}`).join("; ") + ")" : ""}`;
        }).join(' | ');
        el.style.color = failed ? 'var(--red)' : 'var(--green)';
      }
    } catch(e) {
...
// Validate paths using the server's container view, without creating files.
document.addEventListener('focusin', event => {
  const input = event.target;
  if (!(input instanceof HTMLInputElement) || !input.placeholder?.startsWith('/path/to/') || input.dataset.pathCheck) return;
  input.dataset.pathCheck = '1';
  const button = document.createElement('button'); button.type = 'button'; button.className = 'btn-secondary'; button.textContent = 'Check path';
  const result = document.createElement('span'); result.className = 'field-hint'; result.setAttribute('role', 'status');
  input.insertAdjacentElement('afterend', button); button.insertAdjacentElement('afterend', result);
  button.onclick = async () => {
    button.disabled = true;
    try {
      const r = await fetch('/api/settings/check-path', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({path:input.value})});
      const d = await r.json(); if (!r.ok) throw new Error(d.detail || 'Path check failed');
      result.textContent = !d.exists ? 'Not visible inside Top-Shelf' : !d.directory ? 'Exists, but is not a directory' : `Directory · ${d.readable ? 'readable' : 'not readable'} · ${d.writable ? 'writable' : 'not writable'}`;
