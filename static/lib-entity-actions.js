...
(function () {
  ...
  async function _submitMerge() {
    ...
    try {
      ...
      // Nudge /library (if open) to refetch its grid so the merged-in
      // secondary rows disappear and the primary flips to a group tile.
      // Without this the merge modal closes but /library keeps its
      // pre-merge cards until the next full reload.
      try {
        document.dispatchEvent(new CustomEvent('lib-entity-changed', {
          detail: { action: _mergeState.mode || 'merge', id: ctx.id, kind: 'performer' },
        }));
  ...
  async function ungroupPerformer(ctx) {
    ...
    try {
      ...
      try {
        document.dispatchEvent(new CustomEvent('lib-entity-changed', {
          detail: { action: 'ungroup', id: ctx.id, kind: 'performer' },
        }));
      } catch (_) { /* older browsers */ }
      return true;
    } catch (e) {
