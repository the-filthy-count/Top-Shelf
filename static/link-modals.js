...
(function () {
  ...
  let _dbGenders = null;
  let _dbEntityType = 'performer';  // 'performer' or 'studio' — drives type= on /api/metadata/search
  let _extRowId = null;
  let _extSite = null;
  ...
  async function openDb(rowId, site, name, opts) {
    ...
    _dbEntityType = (opts && opts.entityType === 'studio') ? 'studio' : 'performer';
    const noun = _dbEntityType === 'studio' ? 'studio' : 'profile';
    document.getElementById('tsLinkSearchTitle').textContent = `Link ${site} ${noun}`;
    const input = document.getElementById('tsLinkSearchInput');
    input.placeholder = _dbEntityType === 'studio'
      ? 'Search studios on TPDB, StashDB, FansDB, JAVStash…'
      : 'Search TPDB, StashDB, FansDB, JAVStash…';
    input.value = name || '';
  ...
  async function runDbSearch() {
    ...
    // linking against.
    let url = '/api/metadata/search?q=' + encodeURIComponent(q) + '&type=' + encodeURIComponent(_dbEntityType) + '&strict=0';
    if (_dbEntityType !== 'studio' && _dbGenders && _dbGenders.length) url += '&genders=' + encodeURIComponent(_dbGenders.join(','));
    try {
  ...
  async function pickDb(el) {
    ...
    el.querySelector('.ts-link-modal-pick').innerHTML = '<span class="loader loader--btn" role="status" aria-label="Saving"></span> Saving…';
    try {
      ...
      // Capture BEFORE close() — close() nulls _dbRowId, so passing the
      // module var directly to afterChange gave it null and the popup
      // refresh (which requires window._performerPopupActiveId === rowId)
      // silently skipped, leaving the pill in its pre-save missing state.
      const rid = _dbRowId;
      _flagLibraryInvalidation(rid);
  ...
  function afterChange(rowId) {
    if (window._studioPopupActiveId === rowId && typeof window.refreshStudioPopup === 'function') {
      window.refreshStudioPopup();
    } else if (window._performerPopupActiveId === rowId && typeof window.refreshPerformerPopup === 'function') {
      window.refreshPerformerPopup();
    } else if (typeof window.load === 'function') {
      try { window.load(); } catch (_) { /* host page may not expose load() */ }
    }
    // Refresh the underlying grid tile so its pills reflect the new
    // link/image state without a full sub-library reload. Handled by
    // favourites.js's document listener; harmless on pages that don't
    // register one.
    if (rowId) {
      try {
  ...
  // When these save flows run on the entity shell (/performer/{id},
  // /studio/{id}), the sub-library page that history.back() restores
  // isn't in the same document — afterChange() above can't reach it.
  // Drop a sessionStorage flag so favourites.js sees it on return
  // (its pageshow / initial-load listener will force-refresh the
  // affected kind). Kind is inferred from whichever popup is open.
  function _flagLibraryInvalidation(rowId) {
    let kind = 'performer';
    if (window._studioPopupActiveId === rowId) kind = 'studio';
    else if (window._performerPopupActiveId === rowId) kind = 'performer';
    try {
  ...
  // Studio variant — same DB search modal, just with type=studio so the
  // backend hits search_studios() instead of search_performers().
  window.openStudioLinkSearch = function (opts) {
    if (!opts || !opts.rowId || !opts.site) return;
    openDb(opts.rowId, opts.site, opts.name || '', { ...opts, entityType: 'studio' });
  };
