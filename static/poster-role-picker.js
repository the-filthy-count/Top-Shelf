...
<div class="modal-overlay" id="cropModal" onclick="if(event.target===this)closeCropModal()">
  ...
  async function confirmCrop() {
    if (!_cropSrc || !_posterRoleRowId) return;
    ...
    try {
      ...
      // Refresh the headshot preview from the authoritative source.
      const rowId = _posterRoleRowId;
      const refreshed = await fetch('/api/performers/poster-roles?row_id=' + encodeURIComponent(rowId), { credentials: 'same-origin' });
      const rd = await refreshed.json().catch(() => ({}));
      _setPosterRolePreview('headshot', rd.headshot_url || null);
      ...
      window.toast(e.message || 'Crop failed');
    } finally {
      btn.disabled = false;
    }
  }
