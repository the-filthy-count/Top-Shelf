...
(function () {
  ...
  function ensurePosterModals() {
    ...
    host.innerHTML = `<div class="modal-overlay" id="posterRoleModal" data-esc-close="closePosterRolePicker">
  <div class="modal-box fav-roles-modal-box" style="position:relative">
    <button type="button" class="fav-roles-modal-close" onclick="event.stopPropagation();closePosterRolePicker()" title="Close" aria-label="Close"><i class="fa-solid fa-xmark"></i></button>
    <div class="fav-roles-modal-header">
      <div class="fav-roles-header-row">
        ...
        <div class="fav-roles-header-center">
          <button type="button" class="fav-icon-btn" onclick="posterRoleUploadImport()" title="Upload images"><i class="fa-solid fa-upload"></i></button>
      ...
      <input type="file" id="posterRoleImportFile" accept="image/*" multiple style="display:none" onchange="posterRoleHandleImportFile(this.files)">
    </div>
    <div class="modal-body">
      <div class="fav-roles-layout">
        ...
        <div class="fav-roles-left-col">
          <div class="fav-roles-role-group fav-roles-role-group--headshot">
          ...
          <div class="fav-roles-role-group fav-roles-role-group--primary">
            ...
            <div class="fav-roles-apply-row">
              <button type="button" class="fav-icon-btn fav-roles-apply-btn" id="posterRoleSetPrimaryBtn" onclick="applyPosterRole('primary')" disabled title="Crop &amp; set as primary"><i class="fa-solid fa-angles-up"></i></button>
            </div>
          </div>

          <div class="fav-roles-role-group fav-roles-role-group--secondary">
            ...
            <div class="fav-roles-apply-row">
              <button type="button" class="fav-icon-btn fav-roles-apply-btn" id="posterRoleSetSecondaryBtn" onclick="applyPosterRole('secondary')" disabled title="Crop &amp; set as secondary"><i class="fa-solid fa-angles-up"></i></button>
            </div>
          </div>
        </div>
...
<div class="modal-overlay" id="cropModal" onclick="if(event.target===this)closeCropModal()">
  <div class="modal-box crop-modal-box" onclick="event.stopPropagation()">
    <h3 style="display:flex;align-items:center;gap:8px;margin:0 0 4px 0"><i class="fa-solid fa-crop-simple"></i> <span id="cropModalTitle">Crop headshot</span></h3>
  ...
  async function applyPosterRole(role) {
    ...
    if (role !== 'headshot' && role !== 'primary' && role !== 'secondary') return;
    // All three roles now go through the crop dialog. Headshot uses 3:4;
    // primary + secondary use 2:3 (poster ratio). The confirmCrop handler
    // routes to the correct backend endpoint based on `_cropRole`.
    openCropModal(_posterRoleSelectedUrl, { role: role });
  }
  ...
  function posterRoleHandleImportFile(files) {
    ...
    for (const f of Array.from(files)) {
      ...
      if (f.size > 20 * 1024 * 1024) { rejected.push(`${f.name}: exceeds the 20 MiB limit`); continue; }
      if (!f.size) { rejected.push(`${f.name}: empty file`); continue; }
      const url = URL.createObjectURL(f);
      _posterRoleTempUrls.push(url);
      posterRoleAddImportCandidate(url, { source: 'Upload', name: f.name || 'Upload' });
    }
    if (rejected.length && window.toast) window.toast(rejected.join("\n"));
  ...
  let _cropRole = 'headshot';   // 'headshot' | 'primary' | 'secondary'
  ...
  function openCropModal(src, opts = {}) {
    ...
    _cropRole = (opts.role === 'primary' || opts.role === 'secondary') ? opts.role : 'headshot';
    const modal = document.getElementById('cropModal');
    const img = document.getElementById('cropImg');
    const vp = document.getElementById('cropViewport');
    // Aspect + title per role. Headshot = 3:4; primary/secondary = 2:3.
    const aspectByRole = { headshot: '3 / 4', primary: '2 / 3', secondary: '2 / 3' };
    const titleByRole = { headshot: 'Crop headshot', primary: 'Crop primary poster', secondary: 'Crop secondary poster' };
    vp.style.aspectRatio = aspectByRole[_cropRole];
    const titleEl = document.getElementById('cropModalTitle');
    if (titleEl) titleEl.textContent = titleByRole[_cropRole];
  ...
  async function confirmCrop() {
    ...
    try {
      ...
      const endpoint = _cropRole === 'headshot'
        ? '/api/performers/set-headshot-cropped'
        : '/api/performers/set-poster-role-cropped';
      if (_cropRole !== 'headshot') form.append('role', _cropRole);
      try { console.log('[crop] POST', endpoint, { row_id: _posterRoleRowId, role: _cropRole, sx, sy, sw, sh, is_url: /^https?:\/\//i.test(_cropSrc) }); } catch (_) {}
      ...
      // Close the modal as soon as the save returns OK.
      const rowId = _posterRoleRowId;
      const role = _cropRole;
      closeCropModal();
      const roleLabel = role === 'headshot' ? 'Headshot' : role === 'primary' ? 'Primary poster' : 'Secondary poster';
      showMsg(roleLabel + ' updated');
      ...
      if (role === 'headshot') {
        ...
        // The server returns a cache-busted poster_url; fall back to
        // constructing the same shape if the response shape shifts.
        const posterUrl = (d && d.poster_url) || `/api/performers/poster-role-image?row_id=${encodeURIComponent(rowId)}&role=${role}&v=${Date.now()}`;
        _setPosterRolePreview(role, posterUrl);
      }
      ...
      // Cross-document handoff: when the image was changed on
      // /performer/{id} (or /studio/{id}), the `library-row-updated`
      // event above never reaches /library because the event lives in
      // this document only. Drop a session-storage flag so /library's
      // pageshow / DOMContentLoaded consumer refreshes exactly the
      // affected row (which preserves the user's filter chips and
      // pagination — a full `_refreshLibraryKind` reload wiped them).
      try {
