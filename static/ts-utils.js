...
(function () {
  ...
  // Caller-pinned kind for grab-category routing (jav / vice). The
  // visible Scene/Movie toggle is unchanged — when set, this overrides
  // _activeKind for the grab body so the backend can pick the right
  // Prowlarr category tag. Toggling Scene/Movie clears the pin so a
  // user-initiated switch always wins.
  let _pinnedKind = null;
    ...
    try {
      ...
      const r = await fetch('/api/prowlarr/grab', {
        ...
        body: JSON.stringify({
          ...
          kind: _pinnedKind || (_activeKind === 'movie' ? 'movie' : 'scene'),
        }),
      });
      const d = await r.json();
      if (d && d.ok) {
  ...
  window.mountEmbeddedProwlarrSearch = async function (hostEl, opts) {
    ...
    try {
      ...
      if (Array.isArray(o.preloadedResults) && o.preloadedResults.length) {
        ...
        _composeProwlarrQueries(o, title).forEach((q) => params.append('q', q));
        if (o.requirePerformer && (o.performer || '').trim()) {
          params.append('require_performer', o.performer.trim());
        }
        if (o.strictMatch !== false) params.append('strict_match', '1');
        _appendStrictSceneParams(params, o, title);
  ...
  window.openProwlarrSearchPopup = async function (opts) {
    ...
    const explicitKind = (o.kind === 'movie' || o.kind === 'scene' || o.kind === 'jav' || o.kind === 'vice') ? o.kind : null;
    // JAV / Vice aren't in the visible toggle — keep the toggle at
    // Scene/Movie and store the caller's request as a "pinned" kind
    // for the grab body. Explicit 'scene'/'movie' just seeds the toggle
    // like before.
    if (explicitKind === 'jav' || explicitKind === 'vice') {
      _pinnedKind = explicitKind;
      _activeKind = _readStoredKind() || 'scene';
    } else {
      _pinnedKind = null;
...
(() => {
  ...
  for (const type of ['library-row-added', 'library-row-updated', 'lib-entity-changed']) {
    document.addEventListener(type, event => {
      ...
      const change = {kind: detail.kind, rowId: detail.rowId || detail.id,
        ts: Date.now(), type};
