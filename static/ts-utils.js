...
(function initSkyCounterAutopad() {
  ...
  function fitSkyCounterNow(el) {
    ...
    /* Cache hit: same parent width + same text = nothing to re-measure.
     * Re-apply the cached font-size in case something cleared it
     * (className reassignment, manual style flush, etc.). */
    const cached = fitState.get(el);
    if (cached && cached.limit === limit && cached.content === content) {
      if (cached.fontSize) el.style.fontSize = cached.fontSize;
      return;
    }
    // Measure at the reference size — one style write, one scrollWidth
    // read. Subsequent target = refSize × (limit / refWidth).
    ...
    // quirks don't push the last glyph off the edge.
    const target = Math.max(10, Math.min(160, (limit / refWidth) * REF_FONT_SIZE * 0.98));
    const finalSize = target.toFixed(1) + 'px';
    el.style.fontSize = finalSize;
    fitState.set(el, { limit, content, fontSize: finalSize });
  }
