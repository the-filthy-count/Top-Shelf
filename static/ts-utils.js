...
window.performerLinkAllowed = (function () {
  ...
  load();
  function _bucketsFromCached(c) {
    // Map the cached per-gender booleans to the backend
    // ALLOWED_GENDER_BUCKETS the /api/metadata/search endpoint expects:
    // {"female", "male", "trans", "other"}. "other" rides on cat_trans
    // since that's the catch-all bucket the existing UI gate uses.
    const out = [];
    if (c.female) out.push('female');
    if (c.male)   out.push('male');
    if (c.trans)  out.push('trans', 'other');
    return out;
  }
  // Promise-returning lookup — guarantees the caller sees the user's
  // actual filter, not the optimistic "all allowed" default that the
  // pre-cache fast-path returns.
  window.performerGenderAllowlist = async function () {
    const c = cached || await load();
    return _bucketsFromCached(c);
  };
...
function _refreshLibraryTokens(force) {
  ...
  _libraryTokensInflight = (async () => {
    try {
      // Only the studio/performer/vice ingesters run below, so skip
      // fetching movies+jav rows — halves the payload for pages that
      // just need the highlight-token cache (/news, /downloads, /scenes).
      const r = await fetch('/api/favourites?kinds=studio,performer,vice', { credentials: 'same-origin' });
      const d = await r.json();
...
(function () {
  ...
  const DOWNLOAD_KINDS = ['scene', 'movie', 'jav', 'vice'];
  ...
  function _readStoredKind() {
    try {
      ...
      return DOWNLOAD_KINDS.includes(v) ? v : null;
    } catch (_) { return null; }
  }
  function _writeStoredKind(kind) {
    try { localStorage.setItem(KIND_STORAGE_KEY, DOWNLOAD_KINDS.includes(kind) ? kind : 'scene'); } catch (_) {}
  }
  ...
  /** Token set for queue/release dedupe. Splits on non-alphanumerics so
   *  bracket-mashed titles ("[OF]WHAT") yield clean tokens. Drops
   *  single-letter noise but keeps 2-char tokens like "of" and "vr". */
  ...
  /** Match a Prowlarr release against the active queue/download set.
   *  Exact normalised match wins outright. Otherwise we require a high
   *  Jaccard token overlap (≥0.8) AND at least 4 shared tokens — naked
   *  substring containment lit up every megapack when a studio folder
   *  named after the studio was sitting in the source queue. */
  function _isQueuedTitle(norm) {
    if (!norm) return false;
    if (_queuedTitlesNorm.has(norm)) return true;
    const relTokens = _titleTokens(norm);
    if (relTokens.size < 4) return false;
    for (const q of _queuedTitlesNorm) {
      if (!q) continue;
      const qTokens = _titleTokens(q);
      if (qTokens.size < 4) continue;
      let inter = 0;
      const smaller = qTokens.size <= relTokens.size ? qTokens : relTokens;
      const larger = smaller === qTokens ? relTokens : qTokens;
      for (const t of smaller) if (larger.has(t)) inter++;
      if (inter < 4) continue;
      const union = qTokens.size + relTokens.size - inter;
      if (union && inter / union >= 0.8) return true;
    }
    return false;
  }
  ...
  const PROWLARR_QUALITY_PATTERNS = [
    { rx: /\bvr\b/i,                            cls: 'ts-q-vr',     label: 'VR',     img: '/static/logos/vr.webp'        },
    { rx: /\b3d\b/i,                            cls: 'ts-q-3d',     label: '3D',     img: '/static/logos/3d.webp'        },
    { rx: /\buncen(sored)?\b/i,                 cls: 'ts-q-unc',    label: 'UNCENSORED', img: '/static/logos/uncensored.webp' },
    { rx: /\b(bluray|blu[-_ ]?ray|bdrip|brrip|bd[-_ ]?rip)\b/i, cls: 'ts-q-bluray', label: 'BLU-RAY', img: '/static/logos/bluray.webp' },
    { rx: /\b(dvd|dvdrip|dvd[-_ ]?rip)\b/i,     cls: 'ts-q-dvd',    label: 'DVD',    img: '/static/logos/dvd.webp'       },
    { rx: /\b(4320p|8k)\b/i,                    cls: 'ts-q-8k',     label: '8K',     img: '/static/logos/8k.webp'        },
    { rx: /\b(2160p|4k|uhd)\b/i,                cls: 'ts-q-4k',     label: '4K',     img: '/static/logos/4k.webp'        },
    { rx: /\b1080p\b/i,                         cls: 'ts-q-1080',   label: '1080P',  img: '/static/logos/1080.webp'      },
    { rx: /\b720p\b/i,                          cls: 'ts-q-720',    label: '720P',   img: '/static/logos/720.webp'       },
    { rx: /\b480p\b/i,                          cls: 'ts-q-480',    label: '480P',   img: '/static/logos/480.webp'       },
    { rx: /\b(sd|xvid)\b/i,                     cls: 'ts-q-sd',     label: 'SD',     img: '/static/logos/sd.webp'        },
    { rx: /\b(imageset|image[-_ ]?set|imgset|jpg|jpeg|pics?|photos?)\b/i, cls: 'ts-q-img', label: 'IMG', img: '/static/logos/img.webp' },
    { rx: /\b(par2?|rar|zip|7z|tar|tgz|gz|bz2|tbz)\b/i, cls: 'ts-q-archive', label: 'ARCHIVE', img: '/static/logos/archive.webp' },
  ];
  ...
  function _repaintQueuedBadgesIn(resultsEl, activeList) {
    ...
    const isInQueue = (relTitle) => _isQueuedTitle(_normReleaseTitle(relTitle));
  ...
  function ensureMarkup() {
    ...
    // Force a rebuild when the popup pre-dates the split-pane (info pane)
    // layout. The popup never persists in-flight state across opens, so
    // tearing it down and re-injecting is safe.
    if (div && !document.getElementById('tsProwlarrPopupInfo')) {
      div.remove();
      div = null;
    }
    if (div) {
    ...
    div.innerHTML = `
      <div class="ts-prowlarr-popup-shell" role="dialog" aria-modal="true" aria-label="Prowlarr search">
        ...
        <div class="ts-prowlarr-popup-body">
          <aside class="ts-prowlarr-popup-info" id="tsProwlarrPopupInfo" aria-label="Scene info" hidden></aside>
          <div class="ts-prowlarr-popup-right">
  ...
  function _embedCtx() {
    return {
      get all() {
        const keyword = (_embedHost?.dataset.viceKeyword || '').trim().toLocaleLowerCase();
        return keyword ? _embedAllReleases.filter(rel => String(rel.title || '').toLocaleLowerCase().includes(keyword)) : _embedAllReleases;
      },
    ...
    try {
      ...
      const r = await fetch('/api/prowlarr/grab', {
        ...
        body: JSON.stringify({
          ...
          download_url: downloadUrl,
          title: result.title || result.clean_title || result.name || '',
          kind: (['movie', 'jav', 'vice'].includes(_embedKind) ? _embedKind : 'scene'),
        }),
      });
      const d = await r.json();
      if (d && d.ok) {
  ...
  /** Compose the list of ``q=`` strings to fan out to Prowlarr for a
   * given caller. Callers with a known performer name pass it as
   * ``o.performer`` — for multi-word names we push a phrase-quoted
   * variant ("Sarah Jane") so indexers that honour phrase search return
   * only releases where those tokens sit together, not "Sarah Smith"
   * false-positives. An unquoted title query always accompanies the
   * quoted one so indexers that reject or strip quotes still get a
   * shot at matching. Backend dedupes by guid across queries.
   */
  function _composeProwlarrQueries(o, title) {
    const queries = [];
    const perf = (o.performer || '').trim();
    const perfStr = (o.performers || '').trim();
    const studio = (o.studio || '').trim();
    const perfHasSpace = perf && /\s/.test(perf);
    const titleLower = title.toLowerCase();
    const perfLower = perf.toLowerCase();
    if (perf && perfHasSpace) {
      const quoted = `"${perf}"`;
      if (titleLower === perfLower) {
        queries.push(quoted);
      } else {
        queries.push(`${quoted} ${title}`);
      }
    }
    queries.push(title);
    if (o.dotVariant && /\s/.test(title)) {
      queries.push(title.replace(/\s+/g, '.'));
    }
    if (perfStr && studio) {
      const perfSQuoted = /\s/.test(perfStr) ? `"${perfStr}"` : perfStr;
      const studioQ = `${perfSQuoted} ${studio}`.trim();
      if (studioQ.toLowerCase() !== title.toLowerCase()) queries.push(studioQ);
    }
    return queries;
  }

  /** Split an opts.performers CSV / opts.performer single into a
   * de-duped list of trimmed names. */
  function _prowlarrPerformerList(o) {
    const raw = [];
    if (o && typeof o.performers === 'string' && o.performers.trim()) {
      o.performers.split(',').forEach((n) => raw.push(n));
    }
    if (o && Array.isArray(o.performerList)) {
      o.performerList.forEach((n) => raw.push(n));
    }
    if (o && typeof o.performer === 'string' && o.performer.trim()) {
      raw.push(o.performer);
    }
    const out = [];
    const seen = new Set();
    raw.forEach((n) => {
  ...
  /** Append the strict-scene params to a URLSearchParams when the
   * caller has enough scene context (title + at least one performer +
   * either studio or the title itself). The backend then runs a
   * whole-phrase filter: keeps a release only when (title-in-order
   * AND some performer) OR (some performer AND studio), so releases
   * that only share a common word with the performer stop leaking
   * through (``SCARLET.SKIES`` no longer passes a ``Scarlet`` search
   * for scene ``SCARLET 03``). Callers can opt out explicitly with
   * ``strictScene: false``. */
  function _appendStrictSceneParams(params, o, title) {
    if (!o || o.strictScene === false) return;
    if (o.kind && o.kind !== 'scene') return;
    const perfs = _prowlarrPerformerList(o);
    if (!perfs.length) return;
    const studio = (o.studio || '').trim();
    const sceneTitle = (title || '').trim();
    if (!sceneTitle && !studio) return;
    if (sceneTitle) params.append('strict_scene_title', sceneTitle);
    if (studio) params.append('strict_studio', studio);
    perfs.forEach((p) => params.append('strict_performer', p));
  }
  ...
  function _mergeSearchPages(previous, current) {
    const seen = new Set();
    return [...(previous || []), ...(current || [])].filter(item => {
      const key = item.guid || item.download_url || item.magnet || item.title;
      if (seen.has(key)) return false;
      seen.add(key); return true;
    }).sort((a,b) => (a.type === 'torrent') - (b.type === 'torrent')
      || (Number(b.published_ts) || 0) - (Number(a.published_ts) || 0));
  }
  function _showSearchContinuation(status, data, next) {
    if (!data.partial) return;
    const note = document.createElement('span');
    note.textContent = ' — Partial results. ';
    note.title = (data.warnings || []).join('\n');
    status.appendChild(note);
    if (data.continuation) {
      const button = document.createElement('button');
      button.type = 'button'; button.className = 'btn-secondary';
      button.textContent = 'Continue search';
      button.onclick = () => { button.disabled = true; next(); };
      status.appendChild(button);
    }
  }

  window.filterEmbeddedProwlarrKeyword = function (hostEl, keyword) {
    if (!hostEl) return;
    hostEl.dataset.viceKeyword = String(keyword || '').trim();
    if (_embedHost !== hostEl) return;
    _prowlarrPaintResultTable(hostEl.querySelector('.ts-prowlarr-embed-status'),
      hostEl.querySelector('.ts-prowlarr-embed-filters'), hostEl.querySelector('.ts-prowlarr-embed-results'), _embedCtx(), _embedToken);
  };
  ...
  window.mountEmbeddedProwlarrSearch = async function (hostEl, opts) {
    ...
    _embedHost = hostEl;
    _embedKind = (['movie', 'scene', 'jav', 'vice'].includes(o.kind)) ? o.kind : 'scene';
    _embedFilterSpec = null;
    _embedHighlightedIdx = -1;
    ...
    try {
      ...
      if (Array.isArray(o.preloadedResults) && o.preloadedResults.length) {
        ...
        const params = new URLSearchParams(o.searchUrl ? new URL(o.searchUrl, location.href).search : '');
        if (!o.searchUrl) _composeProwlarrQueries(o, title).forEach((q) => params.append('q', q));
        if (!o.searchUrl && o.requirePerformer && (o.performer || '').trim()) {
          params.append('require_performer', o.performer.trim());
        }
        if (!o.searchUrl && o.strictMatch !== false) params.append('strict_match', '1');
        if (!o.searchUrl) _appendStrictSceneParams(params, o, title);
        if (o.continuation) params.set('cursor', JSON.stringify(o.continuation));
      ...
      if (d && !d.error) d.results = _mergeSearchPages(o.previousResults, d.results);
      if (d && d.error) {
      ...
      let releases = ((d && d.results) || []).slice();
      ...
      if (!_embedAllReleases.length) {
        ...
        _showSearchContinuation(statusEl, d, () => window.mountEmbeddedProwlarrSearch(hostEl, {...o, preloadedResults:null, continuation:d.continuation, previousResults:d.results}));
        return;
      }
      _prowlarrPaintResultTable(statusEl, filtersEl, resultsEl, _embedCtx(), myToken);
      if (totalRaw > releases.length && o.dedupe !== false && !hostEl.dataset.viceKeyword) {
  ...
  // ─── Scene-info left pane ──────────────────────────────────────────
  // Renders a /scenes-style poster + plot/meta/perfs/tags using the
  // shared `.scene-card` / `.movie-card` CSS already in app-shell.css.
  // Loads in parallel with the Prowlarr search — never blocks results.
  let _infoToken = 0;
  ...
  function _paintInfoPaneSkeleton(host, opts) {
    if (!host) return;
    host.hidden = false;
    const portrait = (opts.kind === 'movie' || opts.kind === 'jav' || opts.source === 'javstash' || opts.source === 'tmdb');
    host.innerHTML = `
      <div class="ts-prowlarr-info">
        <div class="ts-prowlarr-info-skeleton ts-prowlarr-info-skeleton-poster${portrait ? ' is-portrait' : ''}"></div>
        <div class="ts-prowlarr-info-skeleton ts-prowlarr-info-skeleton-line" style="width:78%"></div>
        <div class="ts-prowlarr-info-skeleton ts-prowlarr-info-skeleton-line" style="width:55%"></div>
        <div class="ts-prowlarr-info-skeleton ts-prowlarr-info-skeleton-line" style="width:92%;height:48px"></div>
      </div>`;
  }

  function _paintInfoPaneData(host, info) {
    if (!host) return;
    if (!info || !info.ok) {
      host.innerHTML = `<div class="ts-prowlarr-info"><div class="ts-prowlarr-info-error">Scene info unavailable.</div></div>`;
      return;
    }
    const kind = info.kind || 'scene';
    const isJav = kind === 'jav';
    const isMovie = kind === 'movie';
    const poster = info.poster || '';
    const backdrop = info.backdrop || '';

    // Poster — mirror the /scenes card markup so the existing
    // VHS/sleeve overlays + cover-split logic apply automatically.
    let posterHtml;
    if (isJav && backdrop && backdrop !== poster) {
      posterHtml = `
        <div class="movie-card">
          <div class="img-load img-load--jav-duo" data-jav-slot-ready="0">
            <div class="jav-duo-slot jav-duo-slot--back">
              <img class="movie-poster" src="${_esc(backdrop)}" alt="" loading="lazy" onload="this.closest('.img-load')?.classList.add('ready');typeof tsApplyMovieCoverSplit==='function'&&tsApplyMovieCoverSplit(this,'back')">
            </div>
            <div class="jav-duo-slot jav-duo-slot--front">
              <img class="movie-poster" src="${_esc(poster)}" alt="" loading="lazy" onload="this.closest('.img-load')?.classList.add('ready');typeof tsApplyMovieCoverSplit==='function'&&tsApplyMovieCoverSplit(this,'front')">
            </div>
            <div class="duo-tint" aria-hidden="true"></div>
          </div>
        </div>`;
    } else if (isMovie || isJav) {
      posterHtml = `
        <div class="movie-card">
          <div class="img-load">
            <img class="movie-poster" src="${_esc(poster || '/static/img/poster.webp')}" alt="" loading="lazy" onload="this.closest('.img-load')?.classList.add('ready');typeof tsApplyMovieCoverSplit==='function'&&tsApplyMovieCoverSplit(this,'solo')" onerror="this.src='/static/img/poster.webp'">
            <div class="duo-tint" aria-hidden="true"></div>
          </div>
        </div>`;
    } else {
      posterHtml = `
        <div class="scene-card">
          <div class="img-load">
            <img class="scene-thumb" src="${_esc(poster || '/static/img/missing.webp')}" alt="" loading="lazy" onload="this.closest('.img-load')?.classList.add('ready')" onerror="this.src='/static/img/missing.webp';this.closest('.img-load')?.classList.add('ready')">
            <div class="duo-tint" aria-hidden="true"></div>
          </div>
        </div>`;
    }
    ...
    const studioLogo = studio.logo_url || '';
    const studioName = studio.name || '';
    const studioHtml = (studioLogo || studioName)
      ? `<div class="ts-prowlarr-info-studio">${
          studioLogo
            ? `<img src="${_esc(studioLogo)}" alt="${_esc(studioName)}" onerror="this.style.display='none'">`
            : ''
        }${studioName ? `<span class="ts-prowlarr-info-studio-name">${_esc(studioName)}</span>` : ''}</div>`
      : '';

    const dateBit = (info.release_date || '').slice(0, 10);
    const durBit = _humanDuration(info.duration);
    const metaBits = [dateBit, durBit].filter(Boolean);
    const metaHtml = metaBits.length
      ? `<div class="ts-prowlarr-info-meta">${metaBits.map(t => `<span>${_esc(t)}</span>`).join('')}</div>`
      : '';

    const plot = (info.plot || '').trim();
    const plotHtml = plot ? `<p class="ts-prowlarr-info-plot">${_esc(plot)}</p>` : '';

    const perfs = Array.isArray(info.performers) ? info.performers : [];
    const perfHtml = perfs.length
      ? `<div class="ts-prowlarr-info-perfs">${perfs.map(p => {
          const nm = (p && p.name) || '';
          if (!nm) return '';
          const initial = nm.charAt(0).toUpperCase() || '?';
          const headHtml = (p && p.image)
            ? `<img src="${_esc(p.image)}" alt="" loading="lazy" referrerpolicy="no-referrer" onerror="this.replaceWith(Object.assign(document.createElement('span'),{className:'ts-prowlarr-info-perf-initial',textContent:'${_esc(initial)}'}))">`
            : `<span class="ts-prowlarr-info-perf-initial">${_esc(initial)}</span>`;
          return `<div class="ts-prowlarr-info-perf">${headHtml}<span class="ts-prowlarr-info-perf-name">${_esc(nm)}</span></div>`;
        }).filter(Boolean).join('')}</div>`
      : '';

    const tags = Array.isArray(info.tags) ? info.tags : [];
    const tagHtml = tags.length
      ? `<div class="ts-prowlarr-info-tags">${tags.map(t => `<span class="ts-prowlarr-info-tag">${_esc(t)}</span>`).join('')}</div>`
      : '';

    host.innerHTML = `
      <div class="ts-prowlarr-info ts-prowlarr-info--${_esc(kind)}">
        <div class="ts-prowlarr-info-poster">${posterHtml}</div>
        ${info.title ? `<h3 class="ts-prowlarr-info-title">${_esc(info.title)}</h3>` : ''}
        ${studioHtml}
        ${metaHtml}
        ${plotHtml}
        ${perfHtml}
  ...
  async function _loadInfoPane(host, opts) {
    const myToken = ++_infoToken;
    try {
      ...
      const r = await fetch('/api/scene-info?' + params.toString(), { credentials: 'same-origin' });
      const d = await r.json();
      if (myToken !== _infoToken) return;
      _paintInfoPaneData(host, d);
    } catch (e) {
      if (myToken !== _infoToken) return;
      _paintInfoPaneData(host, { ok: false, error: String(e && e.message || e) });
    }
  }
  ...
  window.openProwlarrSearchPopup = async function (opts) {
    ...
    const explicitKind = DOWNLOAD_KINDS.includes(o.kind) ? o.kind : null;
    ...
    headerEl.innerHTML = `
      ...
      <div class="ts-prowlarr-kind-toggle" role="group" aria-label="Download category">
        ...
        <button type="button" data-kind="jav" class="${_activeKind === 'jav' ? 'is-active' : ''}">JAV</button>
        <button type="button" data-kind="vice" class="${_activeKind === 'vice' ? 'is-active' : ''}">Vice</button>
      </div>`;
    headerEl.querySelectorAll('.ts-prowlarr-kind-toggle button').forEach((btn) => {
      btn.setAttribute('aria-pressed', String(btn.dataset.kind === _activeKind));
      btn.addEventListener('click', () => {
        ...
        headerEl.querySelectorAll('.ts-prowlarr-kind-toggle button').forEach((b) => {
          ...
          b.setAttribute('aria-pressed', String(b.dataset.kind === _activeKind));
        });
      });
    });
    statusEl.textContent = 'Searching Prowlarr…';
    ...
    // Scene-info left pane — fire-and-forget. Only shown when the caller
    // passed a `{source, ext_id}` pair (the caller side has scene
    // identity). For typed-search / vice / name-only invocations the
    // pane stays hidden and the popup keeps its single-column layout.
    const infoEl = document.getElementById('tsProwlarrPopupInfo');
    if (infoEl) {
      const infoSource = (o.source || '').toLowerCase().trim();
      const infoExtId = (o.ext_id || '').trim();
      if (infoSource && infoExtId) {
        _paintInfoPaneSkeleton(infoEl, { kind: o.kind, source: infoSource });
        _loadInfoPane(infoEl, { source: infoSource, ext_id: infoExtId, kind: o.kind || 'scene' });
      } else {
        infoEl.hidden = true;
        infoEl.innerHTML = '';
      }
    ...
    try {
      ...
      if (Array.isArray(o.preloadedResults) && o.preloadedResults.length) {
        ...
        // Query composition (unquoted-title fallback, dot-variant,
        // quoted-performer, perf+studio combo) lives in the shared
        // `_composeProwlarrQueries` helper so this call and the embedded
        // popup path stay in sync.
        const params = new URLSearchParams(o.searchUrl ? new URL(o.searchUrl, location.href).search : '');
        if (!o.searchUrl) _composeProwlarrQueries(o, title).forEach((q) => params.append('q', q));
        if (!o.searchUrl && o.requirePerformer && (o.performer || '').trim()) {
          params.append('require_performer', o.performer.trim());
      ...
      if (!_prowlarrAllReleases.length) {
        ...
        statusEl.innerHTML = '<strong>0</strong> releases found';
        resultsEl.innerHTML = '<div class="ts-prowlarr-popup-empty">No releases matched this title on your configured indexers.</div>';
        _showSearchContinuation(statusEl, d, () => window.openProwlarrSearchPopup({...o, preloadedResults:null, continuation:d.continuation, previousResults:d.results}));
        return;
      }
...
// ──────────────────────────────────────────────────────────────────
// Entity permalink helpers (Radarr-style URLs)
//
//   window.tsEntityHref({kind, libraryRowId, stashId, tpdbId,
//     fansdbId, javstashId, name, source})   → "/performer/42" etc.
//   window.tsGotoEntity(opts)                → navigate to that URL
//
// One helper per entity keeps every callsite terse and consistent —
// no hand-rolled URL strings scattered through the app.
// ──────────────────────────────────────────────────────────────────
(function () {
  'use strict';
  function tsEntityHref(opts) {
    if (!opts || typeof opts !== 'object') return '';
    const kind = String(opts.kind || '').toLowerCase();
    if (!kind) return '';
    if (kind === 'movie') {
      const id = opts.tpdbId || opts.tmdbId || opts.stashId || opts.id;
      return id ? '/movie/' + encodeURIComponent(String(id)) : '';
    }
    if (kind !== 'performer' && kind !== 'studio') return '';
    const base = '/' + kind;
    if (opts.libraryRowId != null && opts.libraryRowId !== '' && !isNaN(Number(opts.libraryRowId))) {
      return base + '/' + Number(opts.libraryRowId);
    }
    const qs = new URLSearchParams();
    if (opts.stashId)    qs.set('stash', String(opts.stashId));
    if (opts.tpdbId)     qs.set('tpdb', String(opts.tpdbId));
    if (opts.fansdbId)   qs.set('fansdb', String(opts.fansdbId));
    if (opts.javstashId) qs.set('javstash', String(opts.javstashId));
    if (opts.source)     qs.set('source', String(opts.source));
    if (opts.name)       qs.set('name', String(opts.name));
    const s = qs.toString();
    return s ? (base + '?' + s) : '';
  }
  function tsGotoEntity(opts) {
    const href = tsEntityHref(opts);
    if (href) window.location.href = href;
    return !!href;
  }
  window.tsEntityHref = tsEntityHref;
  window.tsGotoEntity = tsGotoEntity;
})();

// ── Shared phash popover ────────────────────────────────────────────
// Used by /queue and /health to reveal the 16-char hex phash of a
// file with a Copy button. Triggered from a button whose
// ``data-phash`` attribute carries the value. Positioned next to the
// button; closes on click-outside, Esc, scroll, or resize. Falls
// back to selecting the text when navigator.clipboard is unavailable
// (insecure context / old browser).
(function () {
  let _el = null;
  let _closer = null;
  function _close() {
    if (_el && _el.parentElement) _el.parentElement.removeChild(_el);
    _el = null;
    if (_closer) {
      document.removeEventListener('mousedown', _closer, true);
      document.removeEventListener('keydown', _closer, true);
      window.removeEventListener('scroll', _closer, true);
      window.removeEventListener('resize', _closer, true);
      _closer = null;
    }
  }
  function tsShowPhashPopup(ev, btn) {
    if (ev && typeof ev.stopPropagation === 'function') ev.stopPropagation();
    if (ev && typeof ev.preventDefault === 'function') ev.preventDefault();
    _close();
    if (!btn) return;
    const phash = (btn.getAttribute('data-phash') || '').trim();
    if (!phash) return;
    const label = (btn.getAttribute('data-phash-label') || 'phash').trim();
    const pop = document.createElement('div');
    pop.className = 'ts-phash-pop';
    pop.setAttribute('role', 'dialog');
    pop.setAttribute('aria-label', 'Perceptual hash');
    pop.innerHTML =
      '<div class="ts-phash-pop-label">' + esc(label) + '</div>' +
      '<div class="ts-phash-pop-value">' + esc(phash) + '</div>' +
      '<div class="ts-phash-pop-actions">' +
        '<button type="button" class="ts-phash-pop-copy" title="Copy to clipboard"><i class="fa-solid fa-copy"></i> Copy</button>' +
        '<button type="button" class="ts-phash-pop-close" title="Close" aria-label="Close">×</button>' +
      '</div>';
    ...
    let top = r.bottom + 6;
    if (top + popH > window.innerHeight - 6) top = Math.max(6, r.top - popH - 6);
    let left = r.left + (r.width / 2) - (popW / 2);
    left = Math.max(6, Math.min(left, window.innerWidth - popW - 6));
    pop.style.top  = Math.round(top + window.scrollY) + 'px';
    pop.style.left = Math.round(left + window.scrollX) + 'px';
    const copyBtn = pop.querySelector('.ts-phash-pop-copy');
    if (copyBtn) copyBtn.addEventListener('click', async () => {
      try {
        await navigator.clipboard.writeText(phash);
        copyBtn.innerHTML = '<i class="fa-solid fa-check"></i> Copied';
        setTimeout(() => { if (copyBtn.isConnected) copyBtn.innerHTML = '<i class="fa-solid fa-copy"></i> Copy'; }, 1400);
      } catch {
        const range = document.createRange();
        const val = pop.querySelector('.ts-phash-pop-value');
        if (val) { range.selectNodeContents(val); window.getSelection().removeAllRanges(); window.getSelection().addRange(range); }
      }
    });
    const closeBtn = pop.querySelector('.ts-phash-pop-close');
    if (closeBtn) closeBtn.addEventListener('click', _close);
    _closer = (e) => {
      if (e && e.type === 'keydown' && e.key !== 'Escape') return;
      if (e && e.type === 'mousedown' && pop.contains(e.target)) return;
      _close();
    };
    setTimeout(() => {
      document.addEventListener('mousedown', _closer, true);
      document.addEventListener('keydown', _closer, true);
      window.addEventListener('scroll', _closer, true);
      window.addEventListener('resize', _closer, true);
    }, 0);
  }
  window.tsShowPhashPopup = tsShowPhashPopup;
})();

// Carry successful library edits across entity pages, tabs and back navigation.
(() => {
  const key = 'ts:library-change';
  for (const type of ['library-row-added', 'library-row-updated', 'lib-entity-changed']) {
    document.addEventListener(type, event => {
      ...
      const change = {kind: detail.kind, action:detail.action,
        rowId: ['merge','group-add','ungroup'].includes(detail.action) ? null : (detail.rowId || detail.id),
        ts: Date.now(), type, changeId: Date.now().toString(36) + Math.random().toString(36).slice(2)};
      let previous = [];
      try {
        const saved = JSON.parse(localStorage.getItem(key) || "null");
        previous = saved ? (saved.changes || [saved]) : [];
      } catch (_) {}
      change.changes = [...previous.slice(-63), {...change}];
      try { sessionStorage.setItem('ts:library-invalidated', JSON.stringify(change)); } catch (_) {}
      try { localStorage.setItem(key, JSON.stringify(change)); } catch (_) {}
    });
  }
  window.addEventListener('storage', event => {
    ...
    try {
      ...
      document.dispatchEvent(new CustomEvent(change.type, {
        detail: {...change, id: change.rowId, remote: true}
      }));
    } catch (_) {}
  });
})();
...
// Notify cached consumers once per library revision, including BFCache return.
window.tsOnLibraryChange = function (callback) {
  let marker = null, timer = null;
  try { marker = localStorage.getItem('ts:library-change'); } catch (_) {}
  const schedule = () => {
    clearTimeout(timer);
    timer = setTimeout(() => Promise.resolve(callback()).catch(console.error), 50);
  };
  const check = () => {
    let next;
    try { next = localStorage.getItem('ts:library-change'); } catch (_) { return; }
    if (next && next !== marker) { marker = next; schedule(); }
  };
  for (const type of ['library-row-added','library-row-updated','lib-entity-changed']) {
...
// Legacy search views expose partial results through the shared continuation UI.
window.tsOfferSearchContinuation = function (data, url) {
  if (!data || !data.partial || !data.continuation) return;
  document.getElementById('tsPartialSearchNotice')?.remove();
  const note = document.createElement('div');
  note.id = 'tsPartialSearchNotice'; note.setAttribute('role','status');
  note.style.cssText = 'position:fixed;bottom:20px;right:20px;z-index:3000;background:var(--panel,#202426);padding:16px;border-radius:10px;box-shadow:0 4px 20px #0008';
  note.append('Search returned partial results. ');
  const button = document.createElement('button');
  button.type='button';button.className='btn-secondary';button.textContent='Continue search';
  button.onclick=() => {
    note.remove();
    window.openProwlarrSearchPopup({title:new URL(url,location.href).searchParams.get('q') || 'Search',
      searchUrl:url, continuation:data.continuation, previousResults:data.results || []});
  };
  note.appendChild(button);
  const close=document.createElement('button');close.type='button';close.className='btn-secondary';
  close.textContent='×';close.setAttribute('aria-label','Dismiss partial search notice');
  close.onclick=()=>note.remove();note.appendChild(close);document.body.appendChild(note);
};

// Shared profile filters apply only to the titles already loaded in that profile.
window.tsProfileOwnershipFilters = async function(host, scenes) {
  const grid = host.querySelector('.pp-scenes-grid'); if (!grid) return;
  const cards = [...grid.querySelectorAll('.scene-card')].filter(c => !c.classList.contains('scene-card--static'));
  const items = scenes.slice(0, cards.length).map(s => ({source:String(s.source || 'tpdb').toLowerCase().replace(/^search_/, ''),id:String(s.id || s._id || ''),title:s.title || '',date:s.date || s.release_date || '',studio:s.studio || ''}));
  const stateKey = 'ts:profile-titles:' + location.pathname + location.search + ':' + (host.id || 'titles');
  let saved = {}; try { saved = JSON.parse(sessionStorage.getItem(stateKey) || '{}'); } catch (_) {}
  const bar = document.createElement('div'); bar.style.cssText = 'display:flex;gap:8px;align-items:center;padding:8px 0;flex-wrap:wrap';
  const select = document.createElement('select'); select.className = 'field-input'; select.style.width = 'auto'; select.setAttribute('aria-label','Filter loaded profile titles');
  for (const text of ['All loaded titles','Owned','Wanted','Available']) { const option = document.createElement('option'); option.value=text; option.textContent=text; select.appendChild(option); }
  const status = document.createElement('span'); status.style.color='var(--dim)'; status.textContent='Checking library…';
  bar.append(select,status);grid.before(bar);select.disabled=true;
  try {
    const r=await fetch('/api/library/scenes-in',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({items})});
    if(!r.ok) throw new Error('Library check unavailable');
    const data=await r.json(); if(!host.contains(grid)) return;
    select.disabled=false;
    const apply=()=>{
      let count=0;
      cards.forEach((card,i)=>{
        const owned=!!data.matches?.[`${items[i].source}:${items[i].id}`];
        const wanted=card.querySelector('.scene-wanted-btn')?.classList.contains('is-wanted');
        const show=select.value==='All loaded titles'||(select.value==='Owned'&&owned)||(select.value==='Wanted'&&wanted&&!owned)||(select.value==='Available'&&!owned&&!wanted);
        card.style.display=show?'':'none';if(show)count++;
      });
      grid.querySelectorAll('.scene-card--static').forEach(card=>card.style.display=select.value==='All loaded titles'?'':'none');
      status.textContent=`${count} of ${cards.length} loaded titles`;
    };
    if (['All loaded titles','Owned','Wanted','Available'].includes(saved.filter)) select.value=saved.filter;
    const save=()=>{try{sessionStorage.setItem(stateKey,JSON.stringify({filter:select.value,scroll:grid.scrollTop}));}catch(_){}};
    select.onchange=()=>{apply();save();};apply();
    grid.scrollTop=Number(saved.scroll)||0;grid.addEventListener('scroll',save,{passive:true});
    grid.addEventListener('click',()=>setTimeout(apply,250));
