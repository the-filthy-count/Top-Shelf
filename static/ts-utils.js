...
(function () {
  ...
  /** Bar + filter matching: quality rows plus uncensored (row pills stay on ``PROWLARR_QUALITY_PATTERNS`` only). */
  const PROWLARR_BAR_RX_PATTERNS = PROWLARR_QUALITY_PATTERNS.concat([
    { rx: /\buncen(sored)?\b/i, cls: 'ts-q-unc', label: 'UNCENSORED' },
  ]);
  ...
  function _prowlarrHtmlFilterBar(chips, total, spec) {
    ...
    for (const ch of chips) {
      ...
      if (ch.label === 'UNCENSORED') {
        parts.push(
          `<button type="button" class="ts-prowlarr-filter-pill ts-prowlarr-filter-pill--unc${active}" data-filter-qual="${_esc(ch.label)}" title="Show uncensored releases only"><img src="/static/logos/uncensored.png" alt="" class="ts-prowlarr-uncensored-mark" style="height:14px;width:auto;object-fit:contain;vertical-align:middle;display:block" onerror="this.style.display='none'"></button>`,
        );
      } else {
  ...
  function _prowlarrPaintResultTable(statusEl, filtersEl, resultsEl, o, myToken) {
    const fe = filtersEl || document.getElementById('tsProwlarrPopupFilters');
    const all = _prowlarrAllReleases;
    if (_prowlarrFilterSpec && _prowlarrFilterSpec.kind === 'kw') _prowlarrFilterSpec = null;
    const chips = _prowlarrBuildQualFilterChips(all);
    const filtered = all.filter((rel) => _prowlarrRowMatchesFilter(rel, _prowlarrFilterSpec));
    _activeReleases = filtered;

    const n = filtered.length;
    const nAll = all.length;
    if (_prowlarrFilterSpec) {
    ...
    if (!chips.length) {
      ...
      fe.hidden = false;
      fe.innerHTML = _prowlarrHtmlFilterBar(chips, nAll, _prowlarrFilterSpec);
    }
    ...
    resultsEl.innerHTML = filtered.map((rel, i) => {
      ...
      const uncBadge = /\buncen(sored)?\b/i.test(rel.title || '')
        ? `<img src="/static/logos/uncensored.png" alt="" class="ts-prowlarr-uncensored-mark" title="Uncensored" style="height:16px;width:auto;vertical-align:middle;margin-left:6px;object-fit:contain" onerror="this.style.display='none'">`
        : '';
      ...
      return `<div class="ts-prowlarr-row${queuedClass}" data-row-idx="${i}">
          ...
          <div class="ts-prowlarr-cell-name">
            ...
            <div class="ts-prowlarr-meta">${qualityPills}${uncBadge}<span>${_esc(meta)}</span>${queuedPill}</div>
          </div>
    ...
    if (firstFreeIdx >= 0) _setHighlightedIdx(firstFreeIdx, false);
    else _highlightedIdx = -1;
    ...
    if (slots.length && _earlyNames.size) {
      _headshotPromise.then((d) => {
        if (myToken !== _searchToken || pg !== _prowlarrPaintGen || !d) return;
        const lookup = {};
  ...
  function ensureMarkup() {
    ...
    if (div) {
      ...
      if (!div.dataset.tsProwlarrFilterNavV2) {
        ...
        div.addEventListener('click', (ev) => {
          ...
          _prowlarrPaintResultTable(statusEl, filtersEl, resultsEl, _prowlarrChipOpts, _prowlarrResultToken);
        });
      }
      return;
    }
    ...
    div.querySelector('.ts-prowlarr-popup-results').addEventListener('mouseover', (ev) => {
      ...
      if (Number.isFinite(idx)) _setHighlightedIdx(idx, false);
    });
  }
  ...
  function _onPopupKeydown(ev) {
    ...
    if (!_activeReleases.length) return;
    if (ev.key === 'ArrowDown') {
      ev.preventDefault();
      ev.stopPropagation();
      ev.stopImmediatePropagation();
      _setHighlightedIdx(Math.min(_activeReleases.length - 1, (_highlightedIdx < 0 ? -1 : _highlightedIdx) + 1), true);
    } else if (ev.key === 'ArrowUp') {
      ev.preventDefault();
      ev.stopPropagation();
      ev.stopImmediatePropagation();
      _setHighlightedIdx(Math.max(0, (_highlightedIdx <= 0 ? 1 : _highlightedIdx) - 1), true);
    } else if (ev.key === 'Enter' && _highlightedIdx >= 0) {
      const row = document.querySelector(`.ts-prowlarr-row[data-row-idx="${_highlightedIdx}"]`);
      if (!row) return;
  ...
  function _setHighlightedIdx(idx, scroll) {
    _highlightedIdx = idx;
    const rows = document.querySelectorAll('.ts-prowlarr-row');
    rows.forEach((r) => r.classList.remove('is-highlighted'));
    const target = document.querySelector(`.ts-prowlarr-row[data-row-idx="${idx}"]`);
    if (target) {
  ...
  async function _refreshQueuedTitles() {
    ...
    // If results have already painted, repaint queued state without
    // refetching Prowlarr — preserves headshots and selection.
    if (_activeReleases.length) _repaintQueuedBadges();
  }

  function _repaintQueuedBadges() {
    const isInQueue = (relTitle) => {
      const norm = _normReleaseTitle(relTitle);
      if (!norm) return false;
      if (_queuedTitlesNorm.has(norm)) return true;
  ...
  window.openProwlarrSearchPopup = async function (opts) {
    ...
    try {
      ...
      _prowlarrPaintResultTable(statusEl, filtersEl, resultsEl, _prowlarrChipOpts, myToken);
    } catch (e) {
      if (myToken !== _searchToken) return;
