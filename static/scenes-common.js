...
// JavaScript for /scenes (scene-feed grid + movie/JAV detail overlays).
  ...
  function openSceneOverlayProwlarrPopup() {
    ...
    // Strip the `search_<src>` prefix the /scenes feed prepends to
    // source keys; backend /api/scene-info expects the bare DB name.
    const rawSrc = String(_scene.source || '').toLowerCase();
    const cleanSrc = rawSrc.startsWith('search_') ? rawSrc.slice(7) : rawSrc;
    window.openProwlarrSearchPopup({
      ...
      kind:       'scene',
      source:     cleanSrc,
      ext_id:     String(_scene.id || ''),
    });
  }
  window.openSceneOverlayProwlarrPopup = openSceneOverlayProwlarrPopup;
  // Home page reuses this exact overlay — expose the opener so callers
  // outside /scenes can drive it without duplicating markup / logic.
  window.openSceneOverlay = openSceneOverlay;
  ...
  let _discoveryHidden = new Set(), _discoveryHideOwned = false;
  try { _discoveryHidden = new Set(JSON.parse(localStorage.getItem('ts:discover:hidden') || '[]')); _discoveryHideOwned = localStorage.getItem('ts:discover:hide-owned') === '1'; } catch (_) {}
  function _applyDiscoveryVisibility() {
    const restore = document.getElementById('discoveryRestoreHidden');
    if (restore) restore.style.display = _discoveryHidden.size ? '' : 'none';
    document.querySelectorAll('[data-discovery-key]').forEach(card => {
      card.hidden = _discoveryHidden.has(card.dataset.discoveryKey) || (_discoveryHideOwned && card.classList.contains('is-in-library'));
      card.style.display = card.hidden ? 'none' : '';
    });
  }
  function _mountDiscoveryControls(items, opts) {
    if (!Array.isArray(items) || !['/discover', '/scenes'].includes(location.pathname)) return;
    const grid = document.getElementById('scenesGrid');
    if (!grid) return;
    if (!document.getElementById('discoveryPreferences')) {
      const bar = document.createElement('span'); bar.id = 'discoveryPreferences'; bar.style.cssText = 'display:inline-flex;gap:8px;align-items:center';
      const toggle = document.createElement('button'); toggle.type = 'button'; toggle.className = 'scenes-palette-btn';
      const paintToggle = () => {
        toggle.innerHTML = `<i class="fa-solid ${_discoveryHideOwned ? 'fa-eye-slash' : 'fa-eye'}" aria-hidden="true"></i>`;
        toggle.title = _discoveryHideOwned ? 'Show already owned' : 'Hide already owned';
        toggle.setAttribute('aria-label', toggle.title);
        toggle.setAttribute('aria-pressed', String(_discoveryHideOwned));
        toggle.classList.toggle('is-active', _discoveryHideOwned);
      };
      toggle.onclick = () => { _discoveryHideOwned = !_discoveryHideOwned; try { localStorage.setItem('ts:discover:hide-owned', _discoveryHideOwned ? '1' : '0'); } catch (_) {} paintToggle(); _applyDiscoveryVisibility(); };
      paintToggle();
      const restore = document.createElement('button'); restore.type = 'button'; restore.className = 'scenes-palette-btn'; restore.id = 'discoveryRestoreHidden'; restore.title = 'Restore hidden'; restore.setAttribute('aria-label', 'Restore hidden'); restore.innerHTML = '<i class="fa-solid fa-rotate-left" aria-hidden="true"></i>';
      restore.onclick = () => { _discoveryHidden.clear(); try { localStorage.removeItem('ts:discover:hidden'); } catch (_) {} _applyDiscoveryVisibility(); };
      bar.append(restore, toggle);
      const refresh = document.getElementById('feedRefreshBtn');
      if (refresh) refresh.before(bar); else grid.before(bar);
    }
    const selector = opts?.containerSelector || '.scene-card', attr = opts?.idAttr || 'data-scene-i';
    items.forEach((item, i) => {
      const id = item.id || item._id;
      if (!id) return;
      const source = String(opts?.sourceMap?.[i] || item.source || '').toLowerCase().replace(/^search_/, '');
      const card = document.querySelector(`${selector}[${attr}="${i}"]`);
      if (!card || !grid.contains(card)) return;
      card.dataset.discoveryKey = `${source}:${id}`;
      if (!card.querySelector('.discovery-dismiss')) {
        const button = document.createElement('button'); button.type = 'button'; button.className = 'discovery-dismiss btn-secondary'; button.title = 'Not interested'; button.setAttribute('aria-label', 'Hide ' + (item.title || 'record')); button.textContent = '×';
        button.style.cssText = 'position:absolute;top:8px;left:8px;z-index:8;border-radius:50%;width:28px;height:28px;padding:0';
        button.onclick = event => { event.preventDefault(); event.stopPropagation(); _discoveryHidden.add(card.dataset.discoveryKey); try { localStorage.setItem('ts:discover:hidden', JSON.stringify([..._discoveryHidden])); } catch (_) {} _applyDiscoveryVisibility(); };
        card.style.position = 'relative'; card.appendChild(button);
      }
    });
    _applyDiscoveryVisibility();
  }

  async function decorateLibraryMatches(items, opts) {
    _mountDiscoveryControls(items, opts);
  ...
  document.getElementById('scenesGrid')?.addEventListener('click', function (e) {
    ...
    _sceneCardActivate(_sceneGridItems[i]);
  });
  ...
  /** In-page scene click stays in-page (overlay opens over the grid),
   *  but the URL is pushed to /scenes?scene=db/id so the address bar
   *  is shareable and browser back closes the overlay. Off-page uses
   *  the /scene/{db}/{id} permalink route which lands here anyway. */
  function _sceneCardActivate(scene) {
    if (!scene) return;
    let pushed = null;
    const pairs = [
      ['stashdb', scene.stash_id || scene.stashdb_id || ''],
      ['tpdb',    scene.tpdb_id || ''],
      ['fansdb',  scene.fansdb_id || ''],
    ];
    for (const [db, id] of pairs) {
      if (id) { pushed = db + '/' + String(id); break; }
    }
    if (pushed) {
      try {
        const u = new URL(window.location.href);
        u.searchParams.set('scene', pushed);
        history.pushState({ scene: pushed }, '', u.pathname + '?' + u.searchParams.toString());
      } catch (_) {}
    }
  ...
  document.getElementById('scenesGrid')?.addEventListener('click', function(e) {
    ...
    if (movieCard) {
      const mid = movieCard.getAttribute('data-movie-id') || '';
      // In-page detail — push the /movie/{id} URL so the address
      // bar is shareable and browser back closes the overlay, but
      // don't reload the /scenes page.
      if (mid) {
        try {
          history.pushState({ movie: mid }, '', '/movie/' + encodeURIComponent(mid));
        } catch (_) {}
      }
      showMovieDetail(mid);
      return;
  ...
  if (document.getElementById('scenesGrid')) {
    ...
    _bootFeed();
    // Deep-link opener: /scene/{db}/{id} redirects to
    // /scenes?scene=db/id — read that here, fetch the scene via
    // /api/scene-info, and open the existing overlay. Best-effort:
    // if the fetch fails the user still lands on /scenes.
    (function () {
      try {
        const q = new URLSearchParams(window.location.search || '');
        const raw = (q.get('scene') || '').trim();
        if (!raw || !raw.includes('/')) return;
        const [srcRaw, ...restParts] = raw.split('/');
        const sid = restParts.join('/');
        const src = (srcRaw || '').toLowerCase();
        if (!src || !sid) return;
        // Deep-link entry point → tag the body so the Path B-lite CSS
        // reflows the scene overlay as inline page content (hides the
        // grid behind, strips the modal backdrop, respects the sidebar).
        document.body.classList.add('ts-entity-page', 'ts-scene-deeplink');
        fetch('/api/scene-info?source=' + encodeURIComponent(src) + '&id=' + encodeURIComponent(sid))
          .then(function (r) { return r.json(); })
          .then(function (info) {
            if (!info || !info.ok) return;
            // /api/scene-info returns a slightly different shape than
            // the feed items openSceneOverlay was written against —
            // adapt the fields the overlay actually reads. Anything
            // it doesn't find falls back gracefully.
            const st = info.studio || {};
            const perfList = (info.performers || []).map(function (p) { return p && p.name; }).filter(Boolean);
            const sceneLike = {
              title: info.title || 'Scene',
              performer: perfList.join(', '),
              hover_performers: perfList,
              tags: (info.tags || []).map(function (t) { return t && t.name || t; }).filter(Boolean),
              description: info.plot || '',
              tpdb_id: src === 'tpdb' ? info.id : '',
              stash_id: src === 'stashdb' ? info.id : '',
              stashdb_id: src === 'stashdb' ? info.id : '',
              fansdb_id: src === 'fansdb' ? info.id : '',
              image: info.image_url || info.poster_url || '',
              studio: (st.name || ''),
              studio_logo_url: (st.logo_url || ''),
              release_date: info.release_date || '',
            };
            openSceneOverlay(sceneLike);
          })
          .catch(function () {});
      } catch (_) {}
    })();
