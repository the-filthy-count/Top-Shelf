...
// ---------------------------------------------------------------------------
// The server-side session expires after N minutes of inactivity. "Activity"
// is real human input on the page (mouse / keyboard / touch), NOT background
// polling — otherwise a tab left open with queue/downloads pollers would
// keep an abandoned session alive indefinitely.
//
// Throttled to one POST per HEARTBEAT_THROTTLE_MS regardless of how many
// events fire. /login and /loading skip the heartbeat (no session yet).
// Open-access mode (no password set) still posts but the endpoint no-ops.
(function () {
  var path = window.location.pathname || "";
  if (path === "/login" || path === "/loading") return;

  var HEARTBEAT_THROTTLE_MS = 60 * 1000;
  var lastSent = 0;
  var pending = false;
  ...
  function sendHeartbeat() {
    ...
    fetch("/api/auth/heartbeat", {
      ...
      // 401 = session already expired server-side. Bounce to /login so
      // the user isn't stuck on a stale page that fails every request.
      if (r && r.status === 401 && window.location.pathname !== "/login") {
        var next = encodeURIComponent(window.location.pathname + window.location.search);
        window.location.replace("/login?next=" + next);
      }
    }).catch(function () {
      // Network blip — let the next event retry.
    }).finally(function () {
      pending = false;
    });
  }
  ...
  // Page navigation counts as activity — fire once after load so a
  // session that's been idle but the user just opened a new tab gets
  // slid forward immediately rather than waiting for the first input.
  if (document.readyState === "loading") {
...
(function () {
  function normalizePath(path) {
    if (!path || path === "/") return "/";
    if (path === "/tv") return "/unmatched";
    if (path === "/queue") return "/unmatched";
    if (path === "/index") return "/releases";
  ...
  function getNavTarget(path) {
    ...
    if (p === "/stars") return "/stars";
    if (p === "/movies") return "/movies";
    if (p === "/jav") return "/jav";
    if (p === "/studios") return "/studios";
    if (p === "/vices") return "/vices";
    if (p === "/library") return "/stars";
    if (p === "/unmatched") return "/unmatched";
    if (p === "/releases") return "/releases";
   ...
   * SVG by replacing /static/icons/<name>.svg, no code change. */
  function dlNavBtn(href, title, icon, attr) {
    ...
    a.innerHTML =
      '<button type="button" class="btn-secondary ts-nav-btn--labeled" title="' + title + '" aria-label="' + title + '">' +
  ...
  function ensureDlNavButtons(nav) {
    ...
    if (!nav.querySelector("[data-ts-nav-queue]")) {
      parent.appendChild(dlNavBtn("/unmatched", "Unmatched", "queue", "data-ts-nav-queue"));
    }
    if (!nav.querySelector("[data-ts-nav-index]")) {
      parent.appendChild(dlNavBtn("/releases", "New Releases", "index", "data-ts-nav-index"));
    }
  }
  ...
  function patchHeaderNavIcons() {
    ...
    const index = nav.querySelector("[data-ts-nav-index] .btn-secondary i")
      || nav.querySelector('a[href="/releases"] .btn-secondary i')
  ...
  function patchHeaderNavTitles() {
    const nav = getHeaderNav();
    if (!nav) return;
    // /downloads button keeps its HTML title ("Downloads"). The
    // /releases button gets normalised to "New Releases" so tooltip
    // and visible label read the same across pages.
    const index = nav.querySelector("[data-ts-nav-index] .btn-secondary")
      || nav.querySelector('a[href="/releases"] .btn-secondary')
      || nav.querySelector('a[href="/index"] .btn-secondary');
    if (index) index.setAttribute("title", "New Releases");
  }

  /** Icon-only nav buttons ship title="…" but historically no aria-label,
   * so screen readers get nothing (title is a hover advisory, not an
   * accessible name). Copy title → aria-label on every button that
   * doesn't already carry one; runs once per init and covers the
   * duplicated header nav markup across every page. */
  function patchHeaderNavAria() {
    const nav = getHeaderNav();
    if (!nav) return;
    nav.querySelectorAll("button[title]:not([aria-label])").forEach(function (btn) {
  ...
  /** Keep queue → downloads → index first; LCD pill just before logout.
   * HOME is not part of the sidebar — the TOP-SHELF logo up top is
   * the Home affordance instead. */
  ...
  /** Download-stats LCD placement.
   *
   * Preferred home: the sidebar footer `.ts-sidebar__dlstats-slot`,
   * directly under the LOGOUT menu item (Radarr-style redesign moved
   * everything into the sidebar). Falls back to the legacy header
   * nav strip when a page still ships the old top-bar chrome. */
  function relocateDlStatsPill(pill) {
    if (!pill) return;
    const sideSlot = document.querySelector(".ts-sidebar__dlstats-slot");
    if (sideSlot) {
      if (pill.parentNode !== sideSlot) sideSlot.appendChild(pill);
      pill.classList.add("ts-dl-stats--sidebar");
      return;
    }
    const nav = getHeaderNav();
    if (!nav) return;
    ...
    pill.classList.remove("ts-dl-stats--sidebar");
    if (logout) {
  ...
  window.__tsInitHeaderNav = function () {
    ...
    // Sidebar must exist before we inject the DL-stats pill —
    // relocateDlStatsPill prefers .ts-sidebar__dlstats-slot when
    // present, but on sidebar-only pages that slot only exists
    // after buildSidebarFromHeaderNav has run.
    buildSidebarFromHeaderNav();
  ...
  // ──────────────────────────────────────────────────────────────────
  // Radarr-style fixed left sidebar
  //
  // Reads the header nav that ``__tsInitHeaderNav`` already assembled
  // and mirrors each entry as a labeled row in a fixed left sidebar.
  // Page templates are untouched — the header keeps its nav in the DOM
  // (hidden via CSS) so per-page code that queries ``.nav-links`` /
  // ``markActiveNav`` still works.
  //
  // Layout breakpoints are driven entirely by the CSS media queries:
  //   ≥1101px : full sidebar with labels
  //   901–1100px : icon-only rail
  //   ≤900px : off-canvas drawer with hamburger toggle
  // ──────────────────────────────────────────────────────────────────

  // Return the icon element's own class list so the sidebar renders
  // the same custom `ts-nav-icon--*` SVG mask (or FA glyph, if the
  // source used one) that the header nav rendered. No mapping —
  // whatever the page-authored icon was, the sidebar mirrors it.
  function tsSidebarIconClass(iEl) {
    if (!iEl) return "ts-nav-icon";
    return iEl.className || "ts-nav-icon";
  }

  function collectSidebarEntries() {
    const nav = getHeaderNav();
    if (!nav) return [];
    // The header nav can be in one of two shapes at this point:
    //   • segmented strip (`.ts-header-nav-seg > a[href] > button` …)
    //   • compact overflow grid (`.ts-header-nav-dropdown-grid a[href]`)
    // Grab every `<a href>` under either — dedupe by href so the sidebar
    // never doubles up when both shapes coexist mid-transition.
    ...
    links.forEach(function (a) {
      ...
      seen.add(href);
      const btn = a.querySelector("button, .btn-secondary");
      const title = (btn && btn.getAttribute("title")) || a.getAttribute("title") || href;
      const iEl = a.querySelector("i, .ts-nav-icon");
      entries.push({
        href: href,
        label: navShortLabel(title),
        title: title,
        iconClass: tsSidebarIconClass(iEl),
      });
    });
    // Also mirror the standalone Logout button that lives inline in the
    // header (no anchor wrapper). Use its own icon class (usually
    // `ts-nav-icon ts-nav-icon--logout`) so the sidebar renders the
    // same custom SVG the header did.
    const logoutBtn = nav.querySelector('button.btn-secondary[title="Logout"]');
    if (logoutBtn && !entries.some(function (e) { return e.title === "Logout"; })) {
      const logoutIcon = logoutBtn.querySelector("i, .ts-nav-icon");
      entries.push({
        href: "#logout",
        label: "Logout",
        title: "Logout",
        iconClass: tsSidebarIconClass(logoutIcon),
        onClick: function () {
  ...
  function sidebarActiveHref(entries) {
    const target = getNavTarget(window.location.pathname);
    if (!target) return "";
    for (var i = 0; i < entries.length; i++) {
      if (entries[i].href === target) return target;
    }
    return "";
  }

  function ensureSidebarShell() {
    let sidebar = document.getElementById("tsSidebar");
    if (sidebar) return sidebar;
    sidebar = document.createElement("aside");
    sidebar.id = "tsSidebar";
    sidebar.className = "ts-sidebar";
    sidebar.setAttribute("role", "navigation");
    sidebar.setAttribute("aria-label", "Site navigation");
    document.body.insertBefore(sidebar, document.body.firstChild);

    const backdrop = document.createElement("div");
    backdrop.id = "tsSidebarBackdrop";
    backdrop.className = "ts-sidebar-backdrop";
    backdrop.addEventListener("click", closeSidebarDrawer);
    document.body.insertBefore(backdrop, sidebar.nextSibling);

    // Hamburger visible only via CSS on narrow screens — placing it in
    // the DOM unconditionally lets the CSS media query take over.
    let toggle = document.getElementById("tsSidebarToggle");
    if (!toggle) {
      toggle = document.createElement("button");
      toggle.type = "button";
      toggle.id = "tsSidebarToggle";
      toggle.className = "ts-sidebar-toggle";
      toggle.setAttribute("aria-label", "Toggle navigation");
      toggle.setAttribute("aria-controls", "tsSidebar");
    ...
    document.addEventListener("keydown", function (ev) {
      if (ev.key === "Escape") closeSidebarDrawer();
    });
  ...
  function closeSidebarDrawer() {
    ...
    const backdrop = document.getElementById("tsSidebarBackdrop");
    const toggle = document.getElementById("tsSidebarToggle");
    if (sidebar) sidebar.classList.remove("is-open");
    if (backdrop) backdrop.classList.remove("is-open");
    if (toggle) toggle.setAttribute("aria-expanded", "false");
  }

  function renderSidebar(sidebar, entries) {
    const activeHref = sidebarActiveHref(entries);
    const version = (document.getElementById("appVersion") || {}).textContent || "";
    // Group the browse-content section (Movies → Vices) between two
    // dividers so it visually separates from the Downloads / Discover
    // cluster above and News/Health/Logout below. Discover (/scenes)
    // sits under New Releases but above the top divider.
    const DIVIDER_BEFORE = new Set(["/movies"]);
    const DIVIDER_AFTER = new Set(["/vices"]);
    const divider = '<div class="ts-sidebar__divider" aria-hidden="true"></div>';
    const items = entries.map(function (e) {
      const isActive = e.href === activeHref;
      const cls = "ts-sidebar__item" + (isActive ? " is-active" : "");
      const tag = e.onClick ? "button" : "a";
      const hrefAttr = e.onClick ? "" : ' href="' + e.href + '"';
      const typeAttr = e.onClick ? ' type="button"' : "";
      const dataOnClick = e.onClick ? ' data-ts-sidebar-logout="1"' : "";
      const before = DIVIDER_BEFORE.has(e.href) ? divider : "";
      const after = DIVIDER_AFTER.has(e.href) ? divider : "";
      return (
        before +
        "<" + tag + hrefAttr + typeAttr + dataOnClick +
        ' class="' + cls + '" title="' + e.title.replace(/"/g, "&quot;") + '"' +
        ' aria-current="' + (isActive ? "page" : "false") + '">' +
        '<i class="' + e.iconClass + '" aria-hidden="true"></i>' +
        '<span class="ts-sidebar__label">' + e.label + '</span>' +
        "</" + tag + ">" +
        after
      );
    }).join("");
    sidebar.innerHTML =
      '<a href="/" class="ts-sidebar__brand" title="Home">' +
      '<img src="/static/img/logo.webp" alt="Top-Shelf"' +
      ' onerror="this.outerHTML=\'<span class=&quot;ts-sidebar__brand-text&quot;>TOP-SHELF</span>\'">' +
      '</a>' +
      '<nav class="ts-sidebar__nav">' + items + '</nav>' +
      '<div class="ts-sidebar__footer">' +
      // Activity banner + notification icons live here (relocated from
      // the deleted top header). Populated by activity-banner.js — this
      // is just the mount point; CSS stacks its progress circles /
      // notification dots bottom-up via column-reverse.
      '<div class="ts-sidebar__activity-slot"></div>' +
      // Download stats counter — sits above the version pill.
      // Populated by the __tsInjectDlStats loop which re-parents the
      // shared `.ts-dl-stats` pill into this slot on every page. CSS
      // shapes the pill into a horizontal 3-tile strip.
      '<div class="ts-sidebar__dlstats-slot"></div>' +
      // Version pill — pinned to the very bottom of the sidebar,
      // rendered as its own edge-to-edge section at ~half the
      // counter strip's height. Emitted unconditionally (may be
      // empty at render time) so the MutationObserver below has a
      // mount point to mirror `#appVersion` updates into.
      '<div class="ts-sidebar__version">' + version.replace(/[<>&"]/g, "") + '</div>' +
      '</div>';

    // Mirror `#appVersion` (populated asynchronously by each page's
    // /api/status poller) into the sidebar version pill. At sidebar
    // render time #appVersion is typically empty — observe it for
    // future writes and copy them across.
    const verSlot = sidebar.querySelector(".ts-sidebar__version");
    const verSource = document.getElementById("appVersion");
    if (verSlot && verSource) {
      const syncVer = function () {
        const t = (verSource.textContent || "").replace(/[<>&"]/g, "");
        if (verSlot.textContent !== t) verSlot.textContent = t;
      };
      syncVer();
      try {
        const obs = new MutationObserver(syncVer);
        obs.observe(verSource, { childList: true, characterData: true, subtree: true });
      } catch (_) { /* MutationObserver unavailable — one-shot copy is fine */ }
    }

    // Move the pre-existing #tsActivityBanner into the sidebar footer.
    // The banner element already lives in each page's HTML (populated
    // by activity-banner.js's tick loop). We just re-parent it into
    // the sidebar so its output renders inside the new chrome.
    const banner = document.getElementById("tsActivityBanner");
    const slot = sidebar.querySelector(".ts-sidebar__activity-slot");
    if (banner && slot && banner.parentNode !== slot) {
      slot.appendChild(banner);
    }

    // Wire the synthetic logout entry.
    sidebar.querySelectorAll("[data-ts-sidebar-logout]").forEach(function (btn) {
      const entry = entries.find(function (e) { return e.onClick && e.title === (btn.getAttribute("title") || ""); });
    ...
    // Clicking a nav item on mobile: close the drawer so the user lands
    // on the new page without an overlay ghost still open.
    sidebar.querySelectorAll("a.ts-sidebar__item").forEach(function (a) {
      a.addEventListener("click", closeSidebarDrawer);
    });
  }

  function buildSidebarFromHeaderNav() {
    const entries = collectSidebarEntries();
    if (!entries.length) return;
    const sidebar = ensureSidebarShell();
    renderSidebar(sidebar, entries);
    document.body.classList.add("ts-has-sidebar");
    // The status bar's own IIFE may have run before the sidebar existed
    // and parked itself under the (now-hidden) header. Re-nudge its
    // mount so it lands on the sidebar's right edge.
    try {
      const existing = document.getElementById("tsStatusBar");
      if (existing && typeof window.__tsSetStatusBar === "function") {
        // The mount function is module-scoped; re-invoke via the
        // exposed setter which internally calls ensureStatusBar
        // → _mountStatusBarUnderHeader, now finding the sidebar.
        const state = existing.getAttribute("data-state") || "idle";
        window.__tsSetStatusBar(state);
      }
    } catch (_) {}
    // Re-render when the download-stats or other injections mutate the
    // header nav after initial paint (rare, but the LCD stats pill is a
    // late arrival on some pages).
    if (!buildSidebarFromHeaderNav._observed) {
      buildSidebarFromHeaderNav._observed = true;
      const nav = getHeaderNav();
      if (nav) {
        const obs = new MutationObserver(function () {
          const fresh = collectSidebarEntries();
          if (!fresh.length) return;
          const cur = sidebar.querySelectorAll(".ts-sidebar__item").length;
          if (cur !== fresh.length) renderSidebar(sidebar, fresh);
        });
        obs.observe(nav, { childList: true, subtree: true });
      }
      // Path-based active state refresh — the sidebar lives across
      // SPA-style navigations on some pages that use history.pushState.
      window.addEventListener("popstate", function () {
        const fresh = collectSidebarEntries();
        if (fresh.length) renderSidebar(sidebar, fresh);
      });
    }
  }

  window.__tsBuildSidebar = buildSidebarFromHeaderNav;
...
(function () {
  ...
  //: Prefer the sidebar's right edge (Radarr-style vertical strip)
  //: when the sidebar is present. Fall back to the header on legacy
  //: pages that still render one, then to document.body as a last
  //: resort (boot-splash route / login).
  function _mountStatusBarUnderHeader(el) {
    var sidebar = document.getElementById('tsSidebar');
    if (sidebar && document.body.classList.contains('ts-has-sidebar')) {
      el.classList.add('ts-status-bar--sidebar-edge');
      el.classList.remove('ts-status-bar--in-header');
      if (el.parentElement !== document.body) document.body.appendChild(el);
      return;
    }
    el.classList.remove('ts-status-bar--sidebar-edge');
...
(function () {
  ...
  function injectDlStats() {
    // Sidebar-only pages (Radarr redesign) have no .nav-links /
    // .header-right — the pill lives in .ts-sidebar__dlstats-slot
    // instead. Bail only when NEITHER anchor exists on the page.
    const nav = document.querySelector(".nav-links") || document.querySelector(".header-right");
    const sideSlot = document.querySelector(".ts-sidebar__dlstats-slot");
    if (!nav && !sideSlot) return null;
    let el = document.querySelector(".ts-dl-stats");
    if (!el) {
      el = document.createElement('a');
      el.className = 'ts-dl-stats';
    el.href = '/downloads';
    el.setAttribute('aria-label', 'Download counts — active, pending, queue');
    el.title = 'Active · Pending · Queue';
    // Complete tile omitted intentionally — completed downloads
    // already leave the queue via the /downloads watch flow, so
    // surfacing a running total added no actionable value.
    el.innerHTML =
...
(function () {
  ...
  var SHORTCUTS = [
    ...
    { keys: ["g", "i"],  label: "Jump to /releases (indexer)" },
  ];
