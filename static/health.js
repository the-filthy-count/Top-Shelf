...
/* Block 1/3: main /health page logic. */
  ...
  function treeLeafRow(leaf, libraryRootKey) {
    ...
    //: Clickable when we have a value to reveal; otherwise stays a
    //: static icon (matches the queue-tab convention).
    const phVal   = (leaf.phash_value || '').trim();
    const phLabel = (leaf.phash_source || 'phash').trim();
    let fp;
    if (unmatched) {
      const t = leaf.phash_any
        ? 'Perceptual hash stored — no sidecar NFO (unmatched). Click to view.'
        : 'No phash cached — no sidecar NFO (unmatched)';
      if (phVal) {
        fp = `<button type="button" class="tree-leaf-fp tree-leaf-fp-unmatched tree-leaf-fp-btn" data-phash="${esc(phVal)}" data-phash-label="${esc(phLabel)}" title="${esc(t)}" onclick="tsShowPhashPopup(event, this)"><i class="fa-solid fa-fingerprint" aria-hidden="true"></i></button>`;
      } else {
        fp = `<i class="tree-leaf-fp tree-leaf-fp-unmatched fa-solid fa-fingerprint" title="${esc(t)}" aria-hidden="true"></i>`;
      }
    } else if (leaf.phash_any && phVal) {
      fp = `<button type="button" class="tree-leaf-fp tree-leaf-fp-btn fa-solid" data-phash="${esc(phVal)}" data-phash-label="${esc(phLabel)}" title="Click to view phash" onclick="tsShowPhashPopup(event, this)"><i class="fa-solid fa-fingerprint" aria-hidden="true"></i></button>`;
    ...
    const matchHtml = matchLinks.map(m => {
      const cls = 'tree-leaf-match-link' + (m.submitted ? ' tree-leaf-match-link--submitted' : '');
      const tip = m.submitted
        ? `Open on ${m.label} — phash submitted`
        : `Open on ${m.label}`;
      const badge = m.submitted
        ? `<span class="tree-leaf-match-badge" aria-label="Phash submitted" title="Phash submitted"><i class="fa-solid fa-check" aria-hidden="true"></i></span>`
        : '';
      return `<a class="${cls}" href="${esc(m.url)}" target="_blank" rel="noopener noreferrer" title="${esc(tip)}" onclick="event.stopPropagation()"><img src="${esc(m.src)}" alt="${esc(m.label)}" loading="lazy">${badge}</a>`;
    }).join('');
  ...
  async function fixAllTvshowNfo(paths, btn) {
    if (btn.dataset.step === '1') document.getElementById('healthRepairPreview')?.remove();
    ...
    if (btn.dataset.step === '1') {
      ...
      let preview = document.getElementById('healthRepairPreview');
      if (!preview) { preview = document.createElement('div'); preview.id = 'healthRepairPreview'; btn.parentElement.insertAdjacentElement('afterend', preview); }
      preview.replaceChildren();
      const summary = document.createElement('p'); summary.textContent = `Write missing series metadata for ${paths.length} folders. Videos are unchanged. Review these paths, then press the confirmation button again:`; preview.appendChild(summary);
      const list = document.createElement('pre'); list.style.cssText = 'max-height:200px;overflow:auto;white-space:pre-wrap'; list.textContent = paths.join('\n'); preview.appendChild(list);
      btn.dataset.step = '1';
      btn.innerHTML = '<span class="btn-confirm-mark">?</span>';
    }
  }
  ...
  async function resolveAllOrphansScoped() {
    //: Snapshot the currently-visible orphans first — after the
    //: repair pass and reload, `_orphanList` will only carry rows
    //: whose files are still genuinely missing (server already
    //: excluded any that repaired). We use the snapshot to compute
    //: how many were repaired for the final toast.
    const beforeScoped = (_orphanList || []).filter(o => pathInScope(o.destination));
    const beforeCount = beforeScoped.length;
    if (!beforeCount) {
      window.toast('No orphaned records in this scope');
      return;
    }
    let r, d = {};
    try {
    ...
    await loadOrphans();
    const remainingScoped = (_orphanList || []).filter(o => pathInScope(o.destination));
    const repaired = Math.max(0, beforeCount - remainingScoped.length);
    //: Anything still in scope after the repair pass points at a file
    //: the server just re-checked and found missing. Delete the
    //: history rows in the same click — that IS the resolution. No
    //: confirm dialog (it can get swallowed after the async chain and
    //: the whole button looked like a dead click); user opted in by
    //: pressing Resolve all. A per-row trash icon is still available
    //: for finer-grained control.
    let deleted = 0;
    if (remainingScoped.length) {
      const ids = remainingScoped.map(o => o.id);
      try {
        const r2 = await fetch('/api/health/orphaned/clear', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (r2.ok) {
          deleted = Number(d2.deleted || 0);
        } else {
          window.toast(d2.error || 'Remove failed');
        }
      } catch (e) {
        window.toast('Remove failed');
      }
      await loadOrphans();
    }
    refreshStatNumbers();
    ...
    window.toast(parts.length ? parts.join(' · ') : 'No orphans changed');
  }
  ...
  let _indexStatusPending = false;
  function pollIndexStatus() {
    if (_indexPoll) clearInterval(_indexPoll);
    _indexPoll = setInterval(() => {
      if (document.visibilityState !== 'visible' || _indexStatusPending) return;
      _indexStatusPending = true;
      void (async () => {
        let d;
        try {
          const r = await fetch('/api/health/index-status', { cache: 'no-store', signal: AbortSignal.timeout(10000) });
          if (!r.ok) return;
          d = await r.json();
          if (!d.index || !d.phash3) return;
        } catch (e) { return; }
        finally { _indexStatusPending = false; }
        const ix = d.index || {};
        const p3 = d.phash3 || {};
        if (window.TsActivity && window.TsActivity.refresh) window.TsActivity.refresh();
  ...
  // Toggle for the "only show path_missing rows" filter, driven by
  // the yellow chevron in the toolbar. Bulk-purge only fires when
  // this is true.
  let _movPathMissingFilter = false;
  ...
  function movApplyFilter() {
    ...
    const rows = _movAllMovies.filter(m => {
      ...
      if (_movPathMissingFilter && !m.path_missing) return false;
  ...
  function movRenderRow(m) {
    ...
    const missingBadge = m.path_missing
      ? '<span class="emb-name-badge is-missing" title="Folder missing on disk" style="margin-left:6px;font-size:9.5px;letter-spacing:0.1em;color:#ef4444;border:1px solid rgba(239,68,68,0.5);padding:1px 6px;border-radius:4px;text-transform:uppercase">Missing</span>'
      : '';
    ...
    const jsPill = jsId
      ...
      : `<span class="emb-link-pill missing">—</span>`;
    return `<tr${m.path_missing ? ' class="is-path-missing"' : ''}>
      <td style="font-size:13px;color:var(--text);font-weight:500"><div style="display:flex;align-items:center;min-width:0">${kindIco}<span style="min-width:0;flex:1;overflow:hidden;text-overflow:ellipsis">${folder}${missingBadge}${alt}</span></div></td>
      <td>${tpdbPill}</td>
      <td>${tmdbPill}</td>
      <td>${jsPill}</td>
      <td style="white-space:nowrap">
  ...
  function _movRefreshMissingUi() {
    const missing = _movAllMovies.filter(m => !!m.path_missing);
    const count = missing.length;
    const chip = document.getElementById('movFtMissing');
    const chipCount = document.getElementById('movMissingCount');
    const purge = document.getElementById('movBulkPurgeBtn');
    const purgeCount = document.getElementById('movBulkPurgeCount');
    if (chip) {
      chip.classList.toggle('filter-missing', _movPathMissingFilter);
      // Hide the chip entirely when there's nothing to clean up so the
      // toolbar stays quiet in the happy state.
      chip.style.display = count ? '' : 'none';
    }
    if (chipCount) chipCount.textContent = String(count);
    if (purge) purge.style.display = (_movPathMissingFilter && count) ? '' : 'none';
    if (purgeCount) purgeCount.textContent = String(count);
  }

  function movTogglePathMissingFilter() {
    _movPathMissingFilter = !_movPathMissingFilter;
    movApplyFilter();
  }

  // Purge every movie / JAV row whose folder is missing on disk. One
  // POST per row — the backend has no bulk-delete endpoint, so we
  // fan out sequentially and toast the outcome. Confirmation up
  // front since this is destructive (rows only, not files on disk).
  async function movBulkPurgeMissing() {
    const missing = _movAllMovies.filter(m => !!m.path_missing);
    if (!missing.length) return;
    const kinds = { movie: 0, jav: 0 };
    missing.forEach(m => { kinds[m.kind === 'jav' ? 'jav' : 'movie'] += 1; });
    const label = missing.length + ' row' + (missing.length === 1 ? '' : 's')
      + (kinds.jav && kinds.movie ? ` (${kinds.movie} movie, ${kinds.jav} JAV)` : '');
    if (!confirm(`Delete ${label} whose folder is missing on disk?\n\nRows are removed from the library database. Files on disk (if any surface later) can be re-added by rescanning.`)) return;
    const btn = document.getElementById('movBulkPurgeBtn');
    if (btn) btn.disabled = true;
    let ok = 0, fail = 0;
    ...
    _movAllMovies = _movAllMovies.filter(m => !m.path_missing);
    refreshMoviesJavStatCounter();
    movApplyFilter();
    if (btn) btn.disabled = false;
    if (typeof window.toast === 'function') {
      window.toast(fail
        ? `Purged ${ok}, ${fail} failed.`
        : `Purged ${ok} missing row${ok === 1 ? '' : 's'}.`,
        { kind: fail ? 'error' : 'success' });
    }
  }
  window.movTogglePathMissingFilter = movTogglePathMissingFilter;
  window.movBulkPurgeMissing = movBulkPurgeMissing;
    ...
    try {
      ...
      if (!d.rows.length) {
        ...
        tbody.innerHTML = d.rows.map(row => {
          ...
          return `<tr>
          <td class="h-td-dim" title="${esc(row.filename)}" style="color:var(--text)"><button type="button" class="btn-secondary" data-history-details="${esc(row.filename)}">${_h(row.display_name || row.filename)}</button></td>
...
// Resource diagnostics are separate from index completion polling: filmstrips
// may be running even when no library index has been started on this page.
(() => {
  let pending = false;
  async function updateMediaWork() {
    if (document.hidden || pending) return;
    const anchor = document.getElementById('btnIndexLib');
    if (!anchor) return;
    pending = true;
    try {
      const response = await fetch('/api/health/index-status', {cache: 'no-store', signal: AbortSignal.timeout(8000)});
      if (!response.ok) return;
      const data = await response.json();
      if (!data.media) return;
      let panel = document.getElementById('mediaWorkStatus');
      if (!panel) {
        panel = document.createElement('div');
        panel.id = 'mediaWorkStatus';
        panel.className = 'media-work-pill';
        panel.setAttribute('role', 'status');
        panel.innerHTML = '<span class="media-work-state"><span class="media-work-light" aria-hidden="true"></span><span data-work-label></span></span><span class="media-work-counts"></span>';
        document.getElementById('statsBar').insertAdjacentElement('beforebegin', panel);
      }
      const jobs = data.media.jobs || [];
      const total = key => jobs.reduce((sum, job) => sum + (Number(job[key]) || 0), 0);
      const active = total('active'), waiting = total('waiting');
      const working = jobs.filter(job => job.active || job.waiting).map(job => job.name === 'Hash frames' ? 'Fingerprinting' : job.name === 'Filmstrip frames' ? 'Filmstrips' : job.name);
      panel.classList.toggle('is-working', !!(active || waiting));
      panel.querySelector('[data-work-label]').textContent = working.length ? working.join(' + ') : 'Video processing · Idle';
      const counts = panel.querySelector('.media-work-counts');
      const metrics = [[`${active}/${data.media.limit}`, 'active'], [waiting, 'queued'], [total('completed'), 'finished']];
      if (total('errors')) metrics.push([total('errors'), 'failed']);
      counts.replaceChildren(...metrics.map(([value, label]) => {
        const item = document.createElement('span');
        item.className = 'media-work-metric' + (label === 'failed' ? ' is-error' : '');
      ...
      panel.title = 'Totals and average rates since server start.\n' + jobs.map(job => `${job.name}: current frame ${job.elapsed_seconds}s; ${job.frames_per_minute} frames/min`).join('\n');
    } catch (_) { /* Keep the last status during connection interruptions. */ }
    finally { pending = false; }
  }
  setInterval(updateMediaWork, 5000);
  updateMediaWork();
})();

document.addEventListener('click', async event => {
  const button = event.target.closest('[data-history-details]'); if (!button) return;
  let dialog = document.getElementById('historyFileDetails');
  if (!dialog) { dialog = document.createElement('dialog'); dialog.id='historyFileDetails'; dialog.style.cssText='background:var(--panel,#202223);color:var(--text);border:1px solid var(--accent);border-radius:10px;padding:24px;max-width:min(800px,90vw)'; document.body.appendChild(dialog); }
  dialog.replaceChildren();
  const close=document.createElement('button');close.type='button';close.className='btn-secondary';close.textContent='Close';close.onclick=()=>dialog.close();dialog.appendChild(close);
  const body=document.createElement('div');body.textContent='Loading…';dialog.appendChild(body);dialog.showModal();
  try {
    const r=await fetch('/api/history/file-details?filename='+encodeURIComponent(button.dataset.historyDetails));const d=await r.json();if(!r.ok)throw new Error(d.detail || 'Could not load history');
    body.replaceChildren();
    const labels={filename:'Original file',status:'Pipeline status',match_source:'Matched using',matched_title:'Matched title',filed_at:'Recorded at',destination:'Current destination',error:'Error',fingerprint_original:'Original fingerprint',fingerprint_post_process:'Post-processing fingerprint',post_process_size:'Post-processing size (bytes)',codec:'Codec',width:'Width',height:'Height',note:'History coverage'};
    const list=document.createElement('dl');
    Object.entries(labels).forEach(([key,label])=>{if(d[key]==null||d[key]==='')return;const term=document.createElement('dt');term.textContent=label;term.style.color='var(--dim)';const value=document.createElement('dd');value.textContent=d[key];value.style.cssText='margin:0 0 12px;overflow-wrap:anywhere';list.append(term,value);});body.appendChild(list);
    const logs=document.createElement('button');logs.className='btn-secondary';logs.textContent='Find related log messages';logs.onclick=()=>{dialog.close();document.querySelector('[data-metric="log"]')?.click();const input=document.getElementById('logFilterInput');if(input){input.value=d.filename;input.dispatchEvent(new Event('input',{bubbles:true}));}};body.appendChild(logs);
