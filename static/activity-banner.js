...
(function () {
  ...
  var _dismissedServerIds = new Set();
  function dismissServerNotification(id) {
    var nid = Number(id);
    if (!nid || _dismissedServerIds.has(nid)) return Promise.resolve();
    clearDismissTimer(nid);
    _dismissedServerIds.add(nid);
    render(_lastBannerData || {});
    return fetch('/api/activity/notifications/dismiss', {
      ...
      if (_lastBannerData) _lastBannerData.notifications = (_lastBannerData.notifications || []).filter(function (n) {return Number(n.id) !== nid;});
      while (_dismissedServerIds.size > 512) _dismissedServerIds.delete(_dismissedServerIds.values().next().value);
      tick();
    }).catch(function () {
      _dismissedServerIds.delete(nid);
      render(_lastBannerData || {});
    });
  }
  ...
  function scheduleAutoDismiss(id) {
    var nid = Number(id);
    if (!nid || _dismissedServerIds.has(nid) || _dismissTimers[String(nid)]) return;
    _dismissTimers[String(nid)] = setTimeout(function () {
      clearDismissTimer(nid);
      dismissServerNotification(nid);
    }, AUTO_DISMISS_MS);
  }
  ...
  function circlePipeline(data) {
    ...
    var tip = "Matching · " + count + (cur ? " · " + cur : "");
    return circleHtml({
      slot: "pipeline",
  ...
  // Sidebar-slot label. The activity slot is shared by every action
  // that touches the favourites index (re-match all, refresh images,
  // scan folders, search missing…), so the raw slot name "Favourites"
  // reads as unrelated when the user is on /jav or /studios and just
  // hit "Re-match all" or "Refresh images". Reflect the phase in the
  // label so it visibly matches the button that started the job.
  function favouritesSlotLabel(p) {
    if (!p || !p.running) return "Favourites";
    if (p.phase === "refresh_all") return "Re-match all";
    if (p.phase === "refresh_images_all") return "Refresh images";
    if (p.phase === "scan_missing") return "Search missing";
    return "Favourites";
  }
   ...
   * The banner also lives in the sidebar footer (moved there by
   * app-shell.js). The default chip-row markup was designed for a
   * horizontal header strip and looks cramped when squeezed into a
   * ~200px-wide vertical slot. When we detect the banner is nested
   * inside `.ts-sidebar__activity-slot`, emit a stacked-card layout
   * instead: one card per active task, each with icon + one-word
   * label + segmented horizontal progress bar. */
  function _bannerIsInSidebar(el) {
    return !!(el && el.closest && el.closest(".ts-sidebar__activity-slot"));
  }

  function _sidebarNotifCardHtml(opts) {
    opts = opts || {};
    var state = opts.indeterminate ? "indeterminate" : (opts.state || "loading");
    var pct = Math.max(0, Math.min(100, Number(opts.pct) || 0));
    var label = esc(opts.label || "");
    var icon = opts.iconHtml || "";
    var tip = esc(opts.tooltip || opts.label || "");
    var dismiss = opts.dismissSlot != null
      ? ' data-finished-slot="' + esc(opts.dismissSlot) + '"'
      : opts.dismissId != null ? ' data-finished-id="' + esc(String(opts.dismissId)) + '" data-is-client="' + (opts.dismissClient ? '1' : '0') + '"' : '';
    var dismissHtml = dismiss ? '<button type="button" class="ts-notif-dismiss ts-bevel-off" aria-label="Dismiss alert" title="Dismiss"' + dismiss + '>×</button>' : '';
    // Card layout: icon column (spans both rows) + right column with
    // label on top and bar below. Grid keeps the icon centered
    // vertically across the full card height.
    return (
      '<div class="ts-sidebar-notif" data-slot="' + esc(opts.slot || "") + '"' +
      ' data-state="' + esc(state) + '" title="' + tip + '">' +
        '<div class="ts-sidebar-notif__icon' + (dismiss ? ' ts-sidebar-notif__icon--dismissible' : '') + '"><span class="ts-sidebar-notif__symbol">' + icon + '</span>' + dismissHtml + '</div>' +
        '<div class="ts-sidebar-notif__label">' + label + '</div>' +
        '<div class="ts-sidebar-notif__bar">' +
          '<span class="ts-sidebar-notif__bar-track" aria-hidden="true"></span>' +
          '<span class="ts-sidebar-notif__bar-fill"' +
            (state === "indeterminate"
              ? ''
  ...
  /* Persistence layer for finished tasks — the /health page's scan
   * buttons queue on the server, so tasks rarely run concurrently.
   * When a slot transitions from running → idle we hold its card as
   * "complete" (bar filled at 100%, still coloured green) for a few
   * seconds so the user visually sees each scan finish and stack up
   * next to earlier completions. */
  var _SIDEBAR_STICKY_MS = 30000;
  var _sidebarSticky = {};  // slot -> { until: ts, card: {...} }
  function _sidebarNoteRunning(slot, cardOpts) {
    _sidebarSticky[slot] = { running: true, opts: cardOpts, since: Date.now() };
  }
  function _sidebarNoteIdle(slot) {
    var s = _sidebarSticky[slot];
    if (!s) return;
    if (s.running) {
      // Just completed — clone the last-seen opts as a filled-green card.
      var doneOpts = Object.assign({}, s.opts || {}, {
        pct: 100,
        indeterminate: false,
        dismissSlot: slot,
        state: "loading",  // keep the loading (green) colour scheme
        tooltip: (s.opts && s.opts.tooltip ? s.opts.tooltip : (s.opts && s.opts.label) || "Complete") + " · done",
      });
      _sidebarSticky[slot] = {
        running: false,
        opts: doneOpts,
        until: Date.now() + _SIDEBAR_STICKY_MS,
      };
    } else if (s.until && s.until < Date.now()) {
      delete _sidebarSticky[slot];
    }
  }
  function _sidebarFlushExpired() {
    var now = Date.now();
    for (var k in _sidebarSticky) {
      var s = _sidebarSticky[k];
      if (s && !s.running && s.until && s.until < now) delete _sidebarSticky[k];
    }
  }

  function _renderSidebarStack(data) {
    var ix = (data && data.library_index) || {};
    var p3 = (data && data.phash3) || {};
    var fav = (data && data.favourites_index) || {};
    var dl = (data && data.downloads) || {};
    _sidebarFlushExpired();
    var cards = [];

    function _emit(slot, opts) {
      _sidebarNoteRunning(slot, opts);
      cards.push(_sidebarNotifCardHtml(opts));
    }
    if (data && data.running) {
      ...
      _emit("pipeline", {
        ...
        tooltip: "Matching · " + (total > 0 ? fmtCount(done) + " / " + fmtCount(total) : fmtCount(done)),
      });
    } else {
      _sidebarNoteIdle("pipeline");
    }
    // Manual-action queue (bulk sort, manual file, vice fling, IAFD
    // bulk — anything routed through _manual_action_enqueue). The
    // icon slot renders the live remaining count instead of the usual
    // FontAwesome glyph, so the user sees the batch counting down
    // from the peak as the worker drains it. The label shows the
    // currently-filing performer/studio name (falls back to "Sort"
    // if the row didn't set one).
    var mq = (data && data.manual_queue) || {};
    var mqRemaining = Math.max(0, parseInt(mq.remaining, 10) || 0);
    var mqPeak      = Math.max(mqRemaining, parseInt(mq.peak, 10) || 0);
    if (mqRemaining > 0 && mqPeak > 0) {
      var mqDone = Math.max(0, mqPeak - mqRemaining);
      var mqCur  = healthBasename(mq.current || "");
      var mqDisplay = String(mq.current_display || "").trim();
      var mqLabel = mqDisplay || "Sort";
      _emit("sort_queue", {
        slot: "sort_queue",
        label: mqLabel,
        // The count IS the icon — square badge that shrinks each
        // time an item finishes and clears the queue.
        iconHtml: '<span class="ts-sidebar-notif__count" aria-hidden="true">' + esc(String(mqRemaining)) + '</span>',
        pct: (mqDone / mqPeak) * 100,
        indeterminate: false,
        tooltip: (mqDisplay ? mqDisplay + " · " : "Sort queue · ") + fmtCount(mqDone) + " / " + fmtCount(mqPeak) + (mqCur ? " · " + mqCur : ""),
      });
    } else {
      _sidebarNoteIdle("sort_queue");
    }
    if (ix.running) {
      var ixTotal = Math.max(0, ix.total || 0);
      var ixDone = Math.max(0, ix.done || 0);
      _emit("library_index", {
        slot: "library_index",
        label: ix.phase === "stageWalk" ? "Scanning" : "Indexing",
        iconHtml: '<i class="fa-solid fa-ticket"></i>',
        pct: !ix.indeterminate && ixTotal > 0 ? (ixDone / ixTotal) * 100 : 0,
        indeterminate: !!ix.indeterminate,
        tooltip: (ix.phase === "stageWalk" ? "Scanning" : "Indexing") + " · " +
                 (ixTotal > 0 ? fmtCount(ixDone) + " / " + fmtCount(ixTotal) : fmtCount(ixDone)),
      });
    } else {
      _sidebarNoteIdle("library_index");
    }
    if (p3.running) {
      var p3Total = Math.max(0, p3.total || 0);
      var p3Done = Math.max(0, p3.done || 0);
      _emit("phash3", {
        slot: "phash3",
        label: "Phash",
        iconHtml: '<i class="fa-solid fa-fingerprint"></i>',
        pct: p3Total > 0 ? (p3Done / p3Total) * 100 : 0,
        indeterminate: p3Total <= 0,
        tooltip: "Phash · " + (p3Total > 0 ? fmtCount(p3Done) + " / " + fmtCount(p3Total) : fmtCount(p3Done)),
      });
    } else {
      _sidebarNoteIdle("phash3");
    }
    if (fav.running) {
      var fT = Math.max(0, parseInt(fav.total, 10) || 0);
      var fD = Math.max(0, parseInt(fav.done, 10) || 0);
      var favLabel = favouritesSlotLabel(fav);
      _emit("favourites", {
        slot: "favourites",
        label: favLabel,
        iconHtml: '<span class="fav-lips"></span>',
        pct: fT > 0 ? (fD / fT) * 100 : 0,
        indeterminate: fT <= 0,
        tooltip: favLabel + " · " + favouritesCenterLabel(fav) + " · " +
                 (fT > 0 ? fmtCount(fD) + " / " + fmtCount(fT) : fmtCount(fD)),
      });
    } else {
      _sidebarNoteIdle("favourites");
    }
    if (typeof window !== "undefined" && window.__tsHealthScanning) {
      _emit("health_scan", {
        slot: "health_scan",
        label: "Health",
        iconHtml: '<i class="fa-solid fa-heart-pulse"></i>',
      ...
      _sidebarNoteIdle("health_scan");
    }

    var sync = (data && data.favourites_sync) || {};
    if (sync.running) {
      cards.push(_sidebarNotifCardHtml({
        slot: "favourites_sync", label: "Favourites sync",
        iconHtml: '<i class="fa-solid fa-arrows-rotate"></i>',
        indeterminate: true,
        tooltip: "Syncing " + (sync.source || "databases") + " · " + (sync.done || 0) + " / " + (sync.total || 0) + " databases complete",
      }));
    }

    // Emit still-visible "completed" cards from slots that finished in
    // the last few seconds. Renders alongside currently-running cards
    // so the user sees the whole batch stack up as scans finish.
    for (var stickyKey in _sidebarSticky) {
      var st = _sidebarSticky[stickyKey];
      if (st && !st.running) cards.push(_sidebarNotifCardHtml(st.opts));
    }
    if (dl && dl.count && Array.isArray(dl.items)) {
      for (var di = 0; di < dl.items.length; di++) {
        var it = dl.items[di] || {};
        var itPct = Math.max(0, Math.min(100, Number(it.pct) || 0));
        var hs = it.headshot_url || "";
        var icon = hs
          ? '<img class="ts-sidebar-notif__headshot" src="' + esc(hs) + '" alt="">'
          : '<i class="fa-solid fa-download"></i>';
        cards.push(_sidebarNotifCardHtml({
          slot: "download",
          label: String(it.label || "Download") || "Download",
          iconHtml: icon,
          pct: itPct,
          indeterminate: itPct <= 0,
          tooltip: it.label || "Download",
        }));
      }
    }

    // Persistent notifications from the server + toast alerts pushed
    // by the client. In chip-row mode these render as coloured dots
    // beside the progress circles; in sidebar mode they get their own
    // full card each — one-word "kind" label, kind-coloured bar
    // sitting at 100% (informational, not a progress meter).
    pruneClientAlerts();
    var serverNotes = (data && data.notifications) || [];
    var clientNotes = _clientAlerts.map(function (a) {
      return { id: a.id, message: a.message, kind: a.kind, _client: true };
    });
    var allNotes = serverNotes.concat(clientNotes);
    for (var ni = 0; ni < allNotes.length; ni++) {
      var n = allNotes[ni];
      var kind = String(n.kind || "info").toLowerCase();
      var state = kind === "error" ? "error" : "loading";
      var kindLabel;
      if (kind === "error") kindLabel = "Error";
      else if (kind === "success") kindLabel = "Done";
      else if (kind === "library") kindLabel = "Library";
      else if (kind === "pipeline") kindLabel = "Pipeline";
      else kindLabel = "Info";
      cards.push(_sidebarNotifCardHtml({
        slot: "notification",
        label: kindLabel,
        iconHtml: kindIconHtml(kind),
        pct: 100,
        indeterminate: false,
        state: state,
        tooltip: n.message || kindLabel,
        dismissId: n.id,
        dismissClient: !!n._client,
      }));
    }
    return cards.join("");
  }
  ...
  function render(data) {
    ...
    if (!el) return;
    data = Object.assign({}, data || {}, {notifications: ((data && data.notifications) || []).filter(function (n) {return !_dismissedServerIds.has(Number(n.id));})});
    var activeIds = new Set(data.notifications.map(function (n) {return String(n.id);}));
    Object.keys(_dismissTimers).forEach(function (id) {if (!activeIds.has(id)) clearDismissTimer(id);});
    data.notifications.forEach(function (n) {scheduleAutoDismiss(n.id);});
    ...
    // Sidebar host: emit stacked cards with a horizontal progress bar
    // each, no chip row / +N overflow. Applies whenever the banner is
    // rendered inside `.ts-sidebar__activity-slot`.
    if (_bannerIsInSidebar(el)) {
      // Demo mode active — leave whatever tsShowSidebarDemo() painted
      // in place so the user can eyeball the stack without the poll
      // wiping it every 2s. Cleared by tsHideSidebarDemo().
      if (window.__tsSidebarDemoActive) return;
      if (!document.getElementById('ts-notif-dismiss-style')) {
        var style = document.createElement('style');
        style.id = 'ts-notif-dismiss-style';
        style.textContent = '.ts-sidebar-notif__icon--dismissible{position:relative}.ts-sidebar-notif__symbol{display:inline-flex;align-items:center;justify-content:center}.ts-notif-dismiss{position:absolute;inset:0;z-index:2;border:0;padding:0;background:transparent;color:inherit;width:100%;height:100%;font:30px/1 sans-serif;cursor:pointer;opacity:0}.ts-sidebar-notif:hover .ts-notif-dismiss,.ts-sidebar-notif:focus-within .ts-notif-dismiss{opacity:1}.ts-sidebar-notif:hover .ts-sidebar-notif__icon--dismissible .ts-sidebar-notif__symbol,.ts-sidebar-notif:focus-within .ts-sidebar-notif__icon--dismissible .ts-sidebar-notif__symbol{opacity:0}.ts-notif-dismiss:focus-visible{outline:2px solid currentColor;border-radius:4px}@media(hover:none){.ts-notif-dismiss{opacity:1}.ts-sidebar-notif__icon--dismissible .ts-sidebar-notif__symbol{opacity:0}}';
        document.head.appendChild(style);
      }
      var stackHtml = _renderSidebarStack(data);
      el.innerHTML = stackHtml
        ? '<div class="ts-sidebar-notif-stack">' + stackHtml + '</div>'
        : "";
      el.querySelectorAll('.ts-notif-dismiss').forEach(function (btn) {
        btn.addEventListener('click', function (event) {
          event.stopPropagation();
          var slot = btn.getAttribute('data-finished-slot');
          if (slot != null) {
            if (_sidebarSticky[slot] && !_sidebarSticky[slot].running) delete _sidebarSticky[slot];
            render(data);
          } else if (btn.getAttribute('data-is-client') === '1') {
            dismissClientAlert(btn.getAttribute('data-finished-id'));
            tick();
          } else {
            dismissServerNotification(btn.getAttribute('data-finished-id'));
          }
        });
      });
    ...
    el.querySelectorAll("[data-dismiss-id]").forEach(function (btn) {
      btn.addEventListener("click", function () {
        dismissServerNotification(btn.getAttribute("data-dismiss-id"));
      });
    });
   ...
   *
   * Skipped when the banner is mounted in the sidebar footer — the
   * sidebar stacks chips vertically with its own scroll container, so
   * width-based collapse doesn't apply and would just hide chips
   * unnecessarily. */
  function applyDynamicOverflow(rootEl) {
    if (rootEl && rootEl.closest && rootEl.closest(".ts-sidebar__activity-slot")) {
  ...
  function _bannerBusy(d) {
    ...
    if (d.manual_queue && (d.manual_queue.remaining || 0) > 0) return true;
    return false;
  }
  ...
  /* Diagnostic: paint N fake sidebar cards + block the poll from
   * overwriting them until tsHideSidebarDemo() clears the flag.
   * Call in DevTools:
   *    tsShowSidebarDemo()   // 4 cards, sticky
   *    tsShowSidebarDemo(6)  // 6 cards, sticky
   *    tsHideSidebarDemo()   // resume normal polling
   */
  window.tsShowSidebarDemo = function (n) {
    n = Math.max(1, Math.min(20, parseInt(n, 10) || 4));
    var el = document.getElementById("tsActivityBanner");
    if (!el) { console.warn("tsShowSidebarDemo: no #tsActivityBanner"); return; }
    if (!_bannerIsInSidebar(el)) {
      console.warn("tsShowSidebarDemo: banner is NOT in sidebar slot; move it there first");
    }
    var demos = [
      { slot: "demo1", label: "Sorting",  iconHtml: '<i class="fa-solid fa-list"></i>',        pct: 35 },
      { slot: "demo2", label: "Indexing", iconHtml: '<i class="fa-solid fa-ticket"></i>',      pct: 72 },
      { slot: "demo3", label: "Phash",    iconHtml: '<i class="fa-solid fa-fingerprint"></i>', pct: 12 },
      { slot: "demo4", label: "Scanning", iconHtml: '<i class="fa-solid fa-heart-pulse"></i>', indeterminate: true },
      { slot: "demo5", label: "Download", iconHtml: '<i class="fa-solid fa-download"></i>',    pct: 88 },
      { slot: "demo6", label: "Error",    iconHtml: '<i class="fa-solid fa-triangle-exclamation"></i>', pct: 100, state: "error" },
    ];
    var html = "";
    for (var i = 0; i < n; i++) {
      var d = demos[i % demos.length];
      html += _sidebarNotifCardHtml({
        slot: d.slot + "_" + i,
        label: d.label,
        iconHtml: d.iconHtml,
        pct: d.pct,
    ...
    el.innerHTML = '<div class="ts-sidebar-notif-stack">' + html + '</div>';
    // Latch the flag so the poll's next render() bails out and leaves
    // our demo DOM alone. Cleared by tsHideSidebarDemo().
    window.__tsSidebarDemoActive = true;
    console.log("tsShowSidebarDemo: painted " + n + " cards; call tsHideSidebarDemo() to release");
    return n;
  };
  window.tsHideSidebarDemo = function () {
    window.__tsSidebarDemoActive = false;
    tick();
    console.log("tsHideSidebarDemo: demo cleared, polling resumed");
  };
