  ...
  function _sidebarNotifCardHtml(opts) {
    ...
    return (
      ...
      ' data-state="' + esc(state) + '" title="' + tip + '">' +
        '<div class="ts-sidebar-notif__head">' +
          '<span class="ts-sidebar-notif__icon">' + icon + '</span>' +
          '<span class="ts-sidebar-notif__label">' + label + '</span>' +
        '</div>' +
  ...
  function _renderSidebarStack(data) {
    ...
    if (data && data.running) {
      ...
      _emit("pipeline", {
        ...
        tooltip: "Sorting · " + (total > 0 ? fmtCount(done) + " / " + fmtCount(total) : fmtCount(done)),
      });
    } else {
    ...
    if (dl && dl.count && Array.isArray(dl.items)) {
      for (var di = 0; di < dl.items.length; di++) {
        ...
        cards.push(_sidebarNotifCardHtml({
          slot: "download",
          label: (String(it.label || "Download").split(/\s+/)[0]) || "Download",
          iconHtml: icon,
          pct: itPct,
