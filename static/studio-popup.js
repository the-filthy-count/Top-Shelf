...
(function () {
  ...
  function ensureModal() {
      ...
      <div class="studio-popup-shell" role="dialog" aria-modal="true" aria-label="Studio details">
        <header class="studio-popup-header performer-popup-header" id="studioPopupHeader">
          <h2 class="pp-name">
            ...
            <button type="button" class="pp-name-action pp-name-action--add" id="studioPopupAddBtn" aria-label="Add to library" title="Add to library" style="display:none">
              <i class="fa-regular fa-bookmark"></i>
            </button>
            <button type="button" class="pp-name-action" id="studioPopupFavBtn" aria-label="Favourite" title="Favourite">
              <i class="fa-solid fa-heart"></i>
            </button>
            <button type="button" class="pp-name-action" id="studioPopupRefreshBtn" aria-label="Refresh from linked DBs" title="Refresh from linked DBs">
              <i class="fa-solid fa-arrows-rotate"></i>
            </button>
            <button type="button" class="pp-name-action" id="studioPopupLockBtn" aria-label="Lock matches" title="Lock matches">
        ...
        <div class="studio-popup-grid">
          <section class="studio-popup-cell studio-popup-info-cell">
            ...
            <div class="studio-popup-desc-box">
              ...
              <div class="studio-popup-aliases" id="studioPopupAliases" hidden></div>
            ...
            <div class="studio-popup-linked-row" id="studioPopupLinkedRow" hidden>
              <span class="studio-popup-linked-label" aria-hidden="true">Linked</span>
              <div class="studio-popup-linked-logos" id="studioPopupLinkedLogos"></div>
            </div>
          </section>
    ...
    const addBtn = div.querySelector('#studioPopupAddBtn');
    if (addBtn) addBtn.addEventListener('click', openStudioAddModal);
    document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape' && !document.querySelector('dialog[open]') && div.classList.contains('open')) closeStudioPopup();
    });
  }
  ...
  function ensureLogoPickModal() {
    ...
    div.innerHTML = `
      <div class="modal-box ts-link-modal-box" style="max-width:1400px;width:min(1400px,calc(100vw - 60px));height:calc(100vh - 80px);max-height:none;display:flex;flex-direction:column">
        ...
        <div id="studioLogoPickError" role="alert" hidden style="color:var(--red,#ff5555);padding:8px 0;white-space:pre-wrap"></div>
    ...
    document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape' && !document.querySelector('dialog[open]') && div.classList.contains('open')) closeLogoPickModal();
    });
  }
  ...
  async function openLogoPickModal() {
    ...
    document.getElementById('studioLogoPickError').hidden = true;
    _logoPickSelectedIdx = -1;
  ...
  async function applyPickedLogo() {
    ...
    const errorBox = document.getElementById('studioLogoPickError');
    ...
    try {
      ...
      const d = await r.json().catch(() => ({}));
      if (!r.ok) {
        errorBox.textContent = (d && (d.error || d.detail)) || ('Could not apply image (HTTP ' + r.status + ')');
        errorBox.hidden = false;
        toast(errorBox.textContent);
        if (apply) { apply.disabled = false; apply.textContent = prev; }
        return;
      }
      closeLogoPickModal();
      toast(saveLocal ? 'Logo saved' : 'Logo applied');
      document.dispatchEvent(new CustomEvent('library-row-updated', {detail:{rowId:rid, kind:'studio'}}));
      if (window._studioPopupActiveId === rid) window.refreshStudioPopup();
    } catch (e) {
      console.error('apply logo failed:', e);
      errorBox.textContent = (e && e.message) || 'Apply failed';
      errorBox.hidden = false;
      toast(errorBox.textContent);
      if (apply) { apply.disabled = false; apply.textContent = prev; }
    }
  ...
  async function openStudioPopup(opts) {
    ...
    const aliasesEl = document.getElementById('studioPopupAliases');
    if (aliasesEl) { aliasesEl.innerHTML = ''; aliasesEl.hidden = true; }
    ...
    if (linkedHost) linkedHost.innerHTML = '';
    const linkedWrap = document.getElementById('studioPopupLinkedRow');
    if (linkedWrap) linkedWrap.hidden = true;
    ...
    try {
      ...
      fetch('/api/favourites/studio-detail?row_id=' + encodeURIComponent(_activeRowId), {
        ...
        .then((dd) => {
          ...
          renderAliasesRow((dd && dd.aliases) || []);
  ...
  /** Toggle which toolbar buttons are visible based on whether the row
   * is in the library (real `row.id`) or an external preview (`id===0`).
   * In external mode the favourite/refresh/lock buttons all no-op (their
   * handlers early-return on `!_activeRowId`), so showing them is just
   * misleading affordance. The bookmark button is shown instead. */
  function syncToolbarMode(row) {
    const isExternal = !row || !row.id;
    const addBtn = document.getElementById('studioPopupAddBtn');
    const favBtn = document.getElementById('studioPopupFavBtn');
    const refreshBtn = document.getElementById('studioPopupRefreshBtn');
    const lockBtn = document.getElementById('studioPopupLockBtn');
    if (favBtn)     favBtn.style.display     = isExternal ? 'none' : '';
    if (refreshBtn) refreshBtn.style.display = isExternal ? 'none' : '';
    if (lockBtn)    lockBtn.style.display    = isExternal ? 'none' : '';
    if (addBtn)     addBtn.style.display     = isExternal ? '' : 'none';
  }
  ...
  function renderLinkedLogos(row) {
    ...
    const wrap = document.getElementById('studioPopupLinkedRow');
    if (!host) return;
    ...
    host.innerHTML = items.map((it) => {
      ...
      const label = (it.name || it.id || '').trim();
      const src = it.source.toUpperCase();
      const tile = `<div class="studio-popup-linked-tile" data-source="${ATTR(it.source)}" data-id="${ATTR(it.id)}" title="${ATTR(label + ' · ' + src)}">
        <div class="studio-popup-linked-img-wrap">
          ${imgSrc
            ? `<img class="studio-popup-linked-img" src="${ATTR(imgSrc)}" alt="${ATTR(label)}" loading="lazy" referrerpolicy="no-referrer" onerror="this.replaceWith(Object.assign(document.createElement('span'),{className:'studio-popup-linked-initial',textContent:'${ESC(initial)}'}))">`
            : `<span class="studio-popup-linked-initial">${ESC(initial)}</span>`}
        </div>
        <span class="studio-popup-linked-name">${ESC(label)}</span>
        <span class="studio-popup-linked-src" aria-hidden="true">${ESC(src)}</span>
        <button type="button" class="studio-popup-linked-remove" data-source="${ATTR(it.source)}" data-id="${ATTR(it.id)}" title="Unlink ${ATTR(label)}" aria-label="Unlink"><i class="fa-solid fa-xmark"></i></button>
      </div>`;
      return url
  ...
  async function toggleMatchesLock() {
    ...
    try {
      ...
      document.dispatchEvent(new CustomEvent('library-row-updated', {detail:{rowId:_activeRowId, kind:'studio'}}));
    } catch (e) {
  ...
  /* ── Add-to-library modal (external-studio mode) ─────────────── */

  function ensureStudioAddModal() {
    if (document.getElementById('studioPopupAddModal')) return;
    const div = document.createElement('div');
    div.id = 'studioPopupAddModal';
    div.className = 'pp-add-overlay';
    div.style.setProperty('z-index', '1800', 'important');
    div.innerHTML = `
      <div class="pp-add-shell" role="dialog" aria-modal="true" aria-label="Add studio to library">
        <div class="pp-add-header">
          <span class="pp-add-title">Add studio to library</span>
        ...
        <div class="pp-add-body">
          <div class="pp-add-row">
            <label class="pp-add-label" for="studioAddName">Name</label>
            <input type="text" class="pp-add-custom" id="studioAddName" style="display:block">
          </div>
          <div class="pp-add-row">
            <label class="pp-add-label" for="studioAddDest">Directory</label>
            <select class="pp-add-select" id="studioAddDest">
              <option value="">Loading…</option>
            </select>
          </div>
          <input type="text" class="pp-add-custom" id="studioAddCustom"
                 placeholder="Custom path…" style="display:none">
          <div class="pp-add-msg" id="studioAddMsg"></div>
        </div>
        <div class="pp-add-footer">
          <button type="button" class="pp-add-btn pp-add-btn--cancel" id="studioAddCancel">Cancel</button>
          <button type="button" class="pp-add-btn pp-add-btn--primary" id="studioAddSubmit">
            <i class="fa-solid fa-plus"></i> Add
          </button>
        </div>
      </div>`;
    document.body.appendChild(div);
    div.addEventListener('click', (e) => { if (e.target === div) closeStudioAddModal(); });
    div.querySelector('.pp-add-close').addEventListener('click', closeStudioAddModal);
    div.querySelector('#studioAddCancel').addEventListener('click', closeStudioAddModal);
    div.querySelector('#studioAddDest').addEventListener('change', () => {
      const sel = div.querySelector('#studioAddDest');
      const custom = div.querySelector('#studioAddCustom');
      custom.style.display = (sel.value === '__custom__') ? 'block' : 'none';
    });
    div.querySelector('#studioAddSubmit').addEventListener('click', submitStudioAdd);
    document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape' && !document.querySelector('dialog[open]') && div.classList.contains('open')) closeStudioAddModal();
    });
  }

  async function openStudioAddModal() {
    ensureStudioAddModal();
    const div = document.getElementById('studioPopupAddModal');
    const opts = _activeOpts || {};
    const source = String(opts.source || '').trim();
    const sid    = String(opts.stashId || opts.tpdbId || '').trim();
    const name   = String(_activeName || opts.name || '').trim();
    if (!source || !sid) {
      toast('No external source id available for this studio.');
      return;
    }
    div._submitCtx = { source, id: sid, name };
    const nameEl = div.querySelector('#studioAddName');
    if (nameEl) nameEl.value = name;
    const msg = div.querySelector('#studioAddMsg');
    msg.textContent = '';
    msg.className = 'pp-add-msg';
    const submitBtn = div.querySelector('#studioAddSubmit');
    submitBtn.disabled = false;
    submitBtn.innerHTML = '<i class="fa-solid fa-plus"></i> Add';
    div.classList.add('open');
    try {
      const r = await fetch('/api/metadata/dirs?kind=studio', { credentials: 'same-origin' });
      const d = await r.json();
      const sel = div.querySelector('#studioAddDest');
      const raw = (d.dirs || []).slice();
      const optsHtml = ['<option value="">Choose directory…</option>']
        .concat(raw.map((dir) => `<option value="${ATTR(dir.path)}">${ESC(dir.label)}</option>`))
        .concat(['<option value="__custom__">Custom path…</option>']);
      sel.innerHTML = optsHtml.join('');
      if (raw.length === 1) sel.value = raw[0].path;
    } catch (e) {
      div.querySelector('#studioAddDest').innerHTML = '<option value="">(failed to load)</option>';
    }
  }

  function closeStudioAddModal() {
    const div = document.getElementById('studioPopupAddModal');
    if (div) div.classList.remove('open');
  }
  window.closeStudioAddModal = closeStudioAddModal;

  async function submitStudioAdd() {
    const div = document.getElementById('studioPopupAddModal');
    if (!div) return;
    const ctx = div._submitCtx || {};
    const sel = div.querySelector('#studioAddDest');
    const customEl = div.querySelector('#studioAddCustom');
    const nameEl = div.querySelector('#studioAddName');
    ...
    const folderName = (nameEl && nameEl.value || ctx.name || '').trim();
    if (!folderName) {
      msg.textContent = 'Studio name is required.';
      msg.className = 'pp-add-msg pp-add-msg--err';
      return;
    }
    if (!ctx.id) {
      msg.textContent = 'No external database id available for this studio.';
      msg.className = 'pp-add-msg pp-add-msg--err';
      return;
    }
    const submitBtn = div.querySelector('#studioAddSubmit');
    submitBtn.disabled = true;
    submitBtn.innerHTML = '<span class="loader loader--btn" role="status" aria-label="Loading"></span> Adding…';
    try {
      ...
      const d = await r.json();
      // 409 duplicate: source id already linked to a library studio row
      // (primary match_*_id or via group_ids_json). Reopen the existing
      // row instead of an error.
      if (r.status === 409 && d && d.duplicate) {
        toast(d.error || 'Already in the library.');
        closeStudioAddModal();
        if (d.row_id) {
          openStudioPopup({ libraryRowId: d.row_id, name: d.folder_name || folderName });
        }
        return;
      }
      ...
      try {
        document.dispatchEvent(new CustomEvent('library-row-added', {
          detail: { kind: 'studio', rowId: d.row_id || null, name: d.name || folderName },
        }));
      } catch (_) { /* noop */ }
      // Cross-document handoff: when the add happens on the entity
      // shell (/studio/{id}), the sub-library page that history.back()
      // restores isn't in the same document. Drop a sessionStorage
      // flag so favourites.js sees it on next page load / pageshow.
      try {
      ...
      closeStudioAddModal();
      // Re-open by row id once indexed so the popup flips into in-library mode.
      if (d.row_id) {
        openStudioPopup({ libraryRowId: d.row_id, name: d.name || folderName });
      } else {
        closeStudioPopup();
        toast('Studio added.');
      }
  ...
  function renderAliasesRow(aliases) {
    const el = document.getElementById('studioPopupAliases');
    if (!el) return;
    const arr = Array.isArray(aliases) ? aliases.filter((a) => a && String(a).trim()) : [];
    if (!arr.length) {
      el.innerHTML = '';
      el.hidden = true;
      return;
    }
    el.innerHTML = '<span class="studio-popup-aliases-label">Also known as</span>'
      + arr.map((a) => `<span class="studio-popup-alias-chip">${ESC(String(a).trim())}</span>`).join('');
    el.hidden = false;
  }
    ...
    if (parentLink) {
      ...
      if (url) {
        ...
        parentLink.onclick = event => {
          ...
          if(typeof window.tsGotoEntity==='function') {
            event.preventDefault();
            window.tsGotoEntity({kind:'studio',stashId:String(parent.id||''),source:String(parent.source||'').toLowerCase(),name});
          }
        };
  ...
  function renderScenesGrid(scenes, opts) {
    ...
    el.innerHTML = '<div class="pp-scenes-grid studio-popup-pp-scenes">'
      + scenes.slice(0, 12).map((s) => {
        ...
        const sidJson    = ESC(JSON.stringify(String((s && (s.id || s.scene_id || s.tpdb_id || s.stash_id)) || '')));
        const sourceJson = ESC(JSON.stringify(srcKey || ''));
        // Pass performers so the backend's strict scene filter can
        // enforce (title AND performer) or (performer AND studio) —
        // without at least one performer the filter is a no-op.
        const perfNames = Array.isArray(s && s.performers)
          ? s.performers.map((p) => (p && p.name) || '').filter(Boolean)
          : [];
        const perfJson = ESC(JSON.stringify(perfNames.join(', ')));
        const sourceLink = url
          ...
          : '';
        const prowlarrBtn = `<button type="button" class="pp-scene-prowlarr-center" onclick="event.stopPropagation(); window.openProwlarrSearchPopup&&window.openProwlarrSearchPopup({title:${titleJson},studio:${studioJson},performers:${perfJson},source:${sourceJson},ext_id:${sidJson},kind:'scene'})" title="Search Prowlarr" aria-label="Search Prowlarr">
      ...
      + '</div>';
    window.tsProfileOwnershipFilters?.(el, scenes.slice(0,12));
  }
  ...
  const _delegatedClick = (e) => {
    ...
    e.preventDefault();
    e.stopPropagation();
    // Radarr-style: navigate to /studio/{id} permalink. Falls back to
    // the popup when the URL builder can't produce a href.
    const opts = {
      kind: 'studio',
