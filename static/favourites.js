/* Externalized from favourites.html. */
  ...
  // Split /library into 4 sub-libraries (/stars, /movies, /studios,
  // /vices) — each page's inline boot script sets
  // `window.__FAV_INITIAL_VIEW` synchronously before this deferred
  // script runs so the correct grid loads on first paint without a
  // double-render. Falls back to 'performer' for the legacy path.
  let _view = (typeof window !== 'undefined' && typeof window.__FAV_INITIAL_VIEW === 'string')
    ? window.__FAV_INITIAL_VIEW
    : 'performer';
  let _rowsPerf = [];
  let _rowsStudio = [];
  let _rowsMovie = [];
  // Rows lookup keyed on the active view. Movies + JAV share
  // `_rowsMovie` (the /api/favourites payload keys them separately
  // but a single array simplified the shared filter/sort code).
  // Now they're separate pages, so filter by row.kind at read
  // time — /movies gets kind='movie' only, /jav gets kind='jav'.
  function _kindListFor(view) {
    if (view === 'studio') return _rowsStudio;
    if (view === 'movie')  return (_rowsMovie || []).filter(r => r.kind === 'movie');
    if (view === 'jav')    return (_rowsMovie || []).filter(r => r.kind === 'jav');
    if (view === 'vice')   return _rowsVice;
    return _rowsPerf;
  }
  let _rowsVice = [];
  ...
  function showMsg(t) {
    // Shared alerts live in the sidebar; never insert status text into the toolbar.
    if (t && typeof window.toast === 'function') window.toast(t, { kind: 'info' });
  }
  ...
  function rowMatchesFilter(row, q) {
    ...
    if (_view === 'jav') {
      const code = value => String(value).toLowerCase().replace(/[^a-z0-9]/g, '');
      if (/[a-z]/i.test(n) && /\d/.test(n) && code(hay).includes(code(n))) return true;
    }
  ...
  function setAlphaFilter(letter, doRender = true) {
    ...
    if (doRender) { _favPage = 1; _persistFilterState(); render(); }
  }
  ...
  function _renderResetPage() { _favPage = 1; _persistFilterState(); render(); }

  // Per-view filter persistence: /library reloads (including the
  // full-page reload triggered by closing /performer/{id} on the entity
  // shell) used to reset every filter chip to "all", wiping the user's
  // "unlocked only" / "no external links" / letter selection whenever
  // they navigated in and out. State is keyed by _view so /stars,
  // /studios, /movies, /vices each keep their own filter memory.
  function _favFilterStorageKey() { return `ts:favFilters:${_view || 'performer'}`; }

  function _persistFilterState() {
    try {
      const state = {
        q:      document.getElementById('favFilter')?.value || '',
        star:   document.getElementById('favFilterStar')?.value || 'all',
        lock:   document.getElementById('favFilterLock')?.value || 'all',
        path:   document.getElementById('favFilterPath')?.value || 'all',
        unlink: document.getElementById('favFilterUnlinked')?.value || 'all',
        root:   document.getElementById('favFilterRoot')?.value || 'all',
        letter: _favLetterFilter || '',
      };
      sessionStorage.setItem(_favFilterStorageKey(), JSON.stringify(state));
    } catch (_) { /* private-window or quota — non-fatal */ }
  }

  function _restoreFilterState() {
    let raw = '';
    try { raw = sessionStorage.getItem(_favFilterStorageKey()) || ''; } catch (_) { return; }
    if (!raw) return;
    let s = null;
    ...
    const root   = document.getElementById('favFilterRoot');
    if (q)      q.value      = s.q || '';
    if (star)   star.value   = s.star || 'all';
    if (lock)   lock.value   = s.lock || 'all';
    if (path)   path.value   = s.path || 'all';
    if (unlink) unlink.value = s.unlink || 'all';
    if (root && s.root != null) root.value = s.root || 'all';
    _favLetterFilter = s.letter || '';
  }

  // Per-page view + search-query persistence. The four sub-library pages
  // (/stars, /movies, /studios, /vices) each present two tabs — the
  // primary grid and Add (`_view === 'search'`). Opening a result from
  // the Add tab navigates to /performer/{id} or /studio/{id}, which is a
  // full-document nav — on the way back the sub-library page boots
  // fresh from its inline `window.__FAV_INITIAL_VIEW` and lands on the
  // grid, wiping the user's search. Persist the active view + query per
  // pathname so the Add tab and its results survive that round-trip.
  function _favPageStateKey() {
    let path = '/';
    try { path = window.location.pathname || '/'; } catch (_) { /* noop */ }
    return `ts:favPageState:${path}`;
  }

  function _persistPageState() {
    try {
      const state = {
        view: _view || '',
        searchQ: _favSearchLastQ || '',
        ts: Date.now(),
      };
      sessionStorage.setItem(_favPageStateKey(), JSON.stringify(state));
    } catch (_) { /* private-window or quota — non-fatal */ }
  }

  function _readPageState() {
    let raw = '';
    try { raw = sessionStorage.getItem(_favPageStateKey()) || ''; } catch (_) { return null; }
    if (!raw) return null;
    try {
  ...
  let _libraryIssue = '';
  function _issueMatches(row) {
    if (!_libraryIssue) return true;
    if (_libraryIssue === 'image') return !(row.image_url || row.headshot_url || row.poster_url || row.logo_url);
    if (_libraryIssue === 'bio') return row.kind === 'performer' && Array.isArray(row.bio_missing_fields) && row.bio_missing_fields.length > 0;
    if (_libraryIssue === 'links') return !rowHasAnyDatabaseLink(row);
    if (_libraryIssue === 'path') return !!row.path_missing;
    if (_libraryIssue === 'duplicates') {
      const name = String(row.folder_name || '').toLowerCase().replace(/[^a-z0-9]/g, '');
      return !!name && _kindListFor(_view).filter(other => String(other.folder_name || '').toLowerCase().replace(/[^a-z0-9]/g, '') === name).length > 1;
    }
    return true;
  }
  function _mountIssueFilter() {
    if (document.getElementById('libraryFilterButton')) return;
    const actions = document.querySelector('.fav-sticky-actions');
    if (!actions) return;
    const button = document.createElement('button');
    button.id = 'libraryFilterButton'; button.type = 'button';
    button.className = 'fav-icon-btn fav-icon-btn--primary';
    button.title = 'Filter library'; button.setAttribute('aria-label', 'Filter library');
    button.setAttribute('aria-haspopup', 'dialog');
    button.innerHTML = '<i class="fa-solid fa-filter" aria-hidden="true"></i>';
    actions.appendChild(button);
    button.onclick = () => {
      const dialog = document.createElement('dialog');
      dialog.id = 'libraryIssueFilter';
      dialog.setAttribute('aria-label', 'Filter library');
      dialog.style.cssText = 'background:var(--panel,#202223);color:var(--text);border:1px solid var(--accent,#087f83);border-radius:12px;padding:24px;width:min(380px,90vw);max-height:85vh;overflow:auto';
      const title = document.createElement('h3'); title.textContent = 'Filter library'; dialog.appendChild(title);
      const options = document.createElement('div'); options.style.cssText = 'display:grid;gap:8px;margin:16px 0';
      for (const [value, text] of [['','All records'],['image','Missing artwork'],['bio','Missing bio fields (stars)'],['links','No database links'],['path','Missing folder'],['duplicates','Possible duplicate names']]) {
        const option = document.createElement('button'); option.type = 'button';
        option.className = 'btn-secondary'; option.textContent = text;
        option.setAttribute('aria-pressed', String(_libraryIssue === value));
        if (_libraryIssue === value) option.style.borderColor = 'var(--accent,#087f83)';
        option.onclick = () => {
          _libraryIssue = value; _favPage = 1;
          button.classList.toggle('is-active', !!value);
          button.title = value ? 'Filter library: ' + text : 'Filter library';
      ...
      const review = document.createElement('button'); review.type = 'button'; review.className = 'btn-secondary'; review.textContent = 'Review records';
      review.onclick = () => { dialog.close(); _reviewLibraryRecords(); }; dialog.appendChild(review);
      const close = document.createElement('button'); close.type = 'button'; close.className = 'btn-secondary'; close.textContent = 'Close'; close.style.marginLeft = '8px'; close.onclick = () => dialog.close(); dialog.appendChild(close);
      dialog.addEventListener('keydown', ev => { if (ev.key === 'Escape') ev.stopPropagation(); });
      dialog.addEventListener('close', () => { dialog.remove(); button.focus(); });
      document.body.appendChild(dialog); dialog.showModal();
    };
  }
  function _reviewLibraryRecords() {
    let dialog = document.getElementById('libraryRecordReview');
    if (!dialog) { dialog=document.createElement('dialog');dialog.id='libraryRecordReview';dialog.style.cssText='background:var(--panel,#202223);color:var(--text);border:1px solid var(--accent);border-radius:10px;padding:24px;width:min(850px,90vw);max-height:85vh;overflow:auto';document.body.appendChild(dialog); }
    dialog.replaceChildren();
    const close=document.createElement('button');close.className='btn-secondary';close.textContent='Close';close.onclick=()=>dialog.close();dialog.appendChild(close);
    const title=document.createElement('h3');title.textContent=_view==='vice'?'Overlapping vice tags':_view==='movie'||_view==='jav'?'Numbered title groups':'Possible duplicate records';dialog.appendChild(title);
    const note=document.createElement('p');note.style.color='var(--dim)';note.textContent=_view==='movie'||_view==='jav'?'Groups inferred from numbered names, not a verified collection catalogue. Gaps are suggestions to investigate.':'Shared names or tags are review hints, not proof that records should be merged. Open a record to inspect its details; no records are changed here.';dialog.appendChild(note);
    const groups=new Map();
    for(const row of _kindListFor(_view)) {
      let keys=[];
      if(_view==='vice')keys=(row.tags||[]).map(tag=>String(tag.name||tag).toLowerCase().trim());
      else if(_view==='movie'||_view==='jav') { const match=String(row.folder_name||'').match(/^(.*?)(?:\s+(?:part|vol(?:ume)?\.?))?\s+(\d{1,2})(?:\s*\(\d{4}\))?$/i); if(match)keys=[match[1].toLowerCase()]; }
      else keys=[row.folder_name,...(row.aliases||[])].map(name=>String(name||'').toLowerCase().trim());
      for(const key of new Set(keys.filter(Boolean))) {if(!groups.has(key))groups.set(key,[]);groups.get(key).push(row);}
    }
    let count=0;
    for(const [key,rows] of groups) {
      if(rows.length<2)continue; count++;
      const heading=document.createElement('h4');heading.textContent=key;dialog.appendChild(heading);
      if(_view==='movie'||_view==='jav') {
        const nums=rows.map(row=>Number(String(row.folder_name).match(/(\d{1,2})(?:\s*\(\d{4}\))?$/)?.[1])).filter(n=>n>0);
        const missing=[];for(let n=Math.min(...nums)+1;n<Math.max(...nums);n++)if(!nums.includes(n))missing.push(n);
        if(missing.length){const gaps=document.createElement('p');gaps.textContent='Numbering gaps to check: '+missing.join(', ');dialog.appendChild(gaps);}
      }
      for(const row of rows) {const button=document.createElement('button');button.type='button';button.className='btn-secondary';button.style.margin='4px';button.textContent=row.folder_name;button.title=row.path||'';button.onclick=()=>{dialog.close();if(_view==='vice')openViceDetail(row.id);else if(_view==='performer'||_view==='studio')window.tsGotoEntity({kind:_view,libraryRowId:row.id,name:row.folder_name});else openEntityDetail(row.id);};dialog.appendChild(button);}
    }
    if(!count){const empty=document.createElement('p');empty.textContent='No overlaps or numbered groups found in this library.';dialog.appendChild(empty);}
    dialog.showModal();
  }

  function rowPassesFilters(row) {
    if (!_issueMatches(row)) return false;
    normalizeUnlinkFilterValue();
  ...
  function filtersActive() {
    ...
    const pathSel = document.getElementById('favFilterPath')?.value || 'all';
    const unlinkSel = document.getElementById('favFilterUnlinked')?.value || 'all';
    return !!(_libraryIssue || q || fav !== 'all' || lock !== 'all' || root !== 'all' || pathSel !== 'all' || unlinkSel !== 'all' || _favLetterFilter);
  }
  ...
  function setView(kind) {
    ...
    // Optional chaining on every seg lookup — the split /library pages
    // (/stars, /movies, /studios, /vices) each keep only their own
    // tab + segSearch, so the other three IDs return null. Without
    // `?.` the .classList call throws and setView() bails before the
    // search host toggle runs — the Add tab flips active but the
    // panel stays on the grid.
    ...
    if (kind === 'movie' || kind === 'jav') grid.classList.add('fav-grid--movie');
  ...
  function normPerformerSearchText(s) {
    return String(s || '').normalize('NFKD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/[^\p{L}\p{N}]+/gu, ' ').trim().replace(/\s+/g, ' ');
  }
  ...
  function renderCard(row) {
    ...
    // newBadge intentionally rendered OUTSIDE the studio/perf placeholder
    // so it paints over the rounded `border-radius` + `overflow:hidden`
    // clip on the placeholder. Anchored to `.fav-cell-visual`
    // (position:relative) so top/left:0 hits the literal outer corner of
    // the tile, not the inside-of-corner-radius edge.
    const folderName = displayFolderName(row.folder_name);
    return `<div class="fav-cell" data-id="${id}" tabindex="0">
      <div class="fav-cell-visual">${visual}${newBadge}
  ...
  function openViceDetail(rowId) {
    ...
    body.innerHTML = `
      <div class="vice-panel-logo-wrap" style="position:relative">
        ...
        <button type="button" class="fav-icon-btn vice-panel-logo-btn" onclick="viceLogoUploadPick('${nameAttr}')" title="Change logo (upload PNG/JPG)" style="position:absolute;top:10px;right:10px;background:rgba(0,0,0,0.55);border:1px solid rgba(255,255,255,0.15)">
          <i class="fa-solid fa-upload"></i>
        </button>
        <input type="file" id="vicePanelLogoFile" accept="image/*" style="display:none" onchange="viceLogoUploadHandle(this)">
      </div>
      <div class="vice-panel-2col">
        <div class="vice-panel-left">
          ...
          <div class="bio-links-block" style="margin-top:auto">
            <button type="button" class="pp-bio-search-btn vice-panel-prowlarr-btn" onclick="window.openProwlarrSearchPopup({title:${nameJson},kind:'vice',thumb_url:${posterJson}})" title="Search Prowlarr for ${esc(name)}" aria-label="Search Prowlarr">
  ...
  // Kick off a file-picker for a new vice logo. Saves to the vice's
  // library folder as logo.png, refreshes the cached copy, and
  // re-renders the tile from the fresh image_url.
  let _pendingViceLogoName = '';
  function viceLogoUploadPick(name) {
    _pendingViceLogoName = String(name || '');
    const inp = document.getElementById('vicePanelLogoFile');
    if (inp) { inp.value = ''; inp.click(); }
  }
  window.viceLogoUploadPick = viceLogoUploadPick;

  async function viceLogoUploadHandle(inp) {
    const f = (inp && inp.files && inp.files[0]) || null;
    if (!f || !_pendingViceLogoName) return;
    const reader = new FileReader();
    reader.onload = async () => {
      try {
        const r = await fetch('/api/vice-logo/upload', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          credentials: 'same-origin',
          body: JSON.stringify({ name: _pendingViceLogoName, data_url: reader.result }),
        });
        const d = await r.json();
        if (!r.ok || d.error) throw new Error(d.error || ('HTTP ' + r.status));
        // Bust the cached image everywhere the row is rendered.
        const bust = '?v=' + Date.now();
        const url = (d.image_url || '') + bust;
        const row = _rowsVice.find(x => (x.folder_name || '').toLowerCase() === _pendingViceLogoName.toLowerCase()
                                     || (x.display_name || '').toLowerCase() === _pendingViceLogoName.toLowerCase());
        if (row) row.image_url = url;
        // Refresh the popup image directly.
        const panelImg = document.querySelector('.vice-panel-logo-wrap img');
        if (panelImg) panelImg.src = url;
        // Refresh the grid tile for this vice.
        if (row && typeof _refreshOneRow === 'function') { try { _refreshOneRow(row.id); } catch (_) {} }
        if (typeof window.toast === 'function') window.toast('Vice logo updated', { kind: 'success' });
      } catch (e) {
        if (typeof window.toast === 'function') window.toast('Logo upload failed: ' + (e.message || e), { kind: 'error' });
      }
    };
    reader.readAsDataURL(f);
  }
  window.viceLogoUploadHandle = viceLogoUploadHandle;
  ...
  function renderEntityPanelContent(d) {
    ...
    if (kind === 'movie' || kind === 'jav') {
      ...
      fetch('/api/library/record-quality?row_id='+encodeURIComponent(row.id)).then(r=>{if(!r.ok)throw new Error();return r.json();}).then(data=>{
        if (_entityPanelRowId !== row.id) return;
        const el=document.getElementById('ownedMovieQuality'); if(!el)return;
        el.textContent=data.files.length?data.files.map(file=>`${file.current_filename}: ${file.media_width&&file.media_height?file.media_width+'×'+file.media_height:'resolution unknown'} · ${file.media_codec||'codec unknown'}`).join(' · '):'No indexed files in this folder yet';
      }).catch(()=>{if(_entityPanelRowId===row.id){const el=document.getElementById('ownedMovieQuality');if(el)el.textContent='File quality unavailable';}});
      ...
      panelBody.innerHTML = `
        ...
        <div class="movie-panel-layout">
          ...
          <div class="movie-panel-info-col">
            <div class="movie-panel-section">Owned file quality</div><div id="ownedMovieQuality">Checking index…</div>
    ...
    // Only performer entities pass a `performer` hint into the Prowlarr
    // popup — studios don't have a single-performer identity to
    // phrase-match on.
    const entityPerfName = kind === 'performer' ? title : '';
    const entityPerfJson = escAttr(JSON.stringify(entityPerfName));
    const scenesBlock = scenes.length
      ? `<div class="discover-info-scenes discover-info-scenes--studio">${scenes.map(s => {
          ...
          const sidJson    = escAttr(JSON.stringify(String(s.id || '')));
          const sourceJson = escAttr(JSON.stringify(String(s.source || scenesSource || '').toLowerCase()));
          ...
          return `<div class="scene-card discover-info-scene-card">
            ...
            <div class="entity-scene-prowlarr-overlay">
              <button type="button" class="fav-icon-btn entity-scene-prowlarr" onclick="window.openProwlarrSearchPopup({title:${titleJson},studio:${studioJson},performer:${entityPerfJson},source:${sourceJson},ext_id:${sidJson},kind:'scene'})" title="Search Prowlarr"><img src="/static/logos/prowlarr.webp" alt="Prowlarr" onerror="this.replaceWith(Object.assign(document.createElement('i'),{className:'fa-solid fa-magnifying-glass'}))"><i class="fa-solid fa-magnifying-glass"></i></button>
            </div>
          </div>`;
        }).join('')}</div>`
  ...
  async function runEntityPanelProwlarrSearch() {
    ...
    try {
      ...
      const d = await r.json();
      if (window.tsOfferSearchContinuation) window.tsOfferSearchContinuation(d, r.url);
      if (d.error) {
  ...
  document.getElementById('favGrid').addEventListener('click', (e) => {
    ...
    if (bioPop) {
      e.preventDefault(); e.stopPropagation();
      window.tsGotoEntity({ kind: 'performer', libraryRowId: parseInt(bioPop.getAttribute('data-id'), 10) });
      return;
    }
    ...
    if (rowData?.kind === 'performer') {
      window.tsGotoEntity({ kind: 'performer', libraryRowId: id });
    }
  ...
  for (const type of ['library-row-added', 'library-row-updated', 'lib-entity-changed']) {
    document.addEventListener(type, () => { _libraryRevision += 1; });
  }
  const _rowRefreshVersions = new Map();
  async function _refreshOneRow(rowId) {
    const version = (_rowRefreshVersions.get(Number(rowId)) || 0) + 1;
    _rowRefreshVersions.set(Number(rowId), version);
    try {
      const r = await fetch(`/api/favourites/row?id=${encodeURIComponent(rowId)}`, { credentials: 'same-origin', cache: 'no-store' });
      if (!r.ok) throw new Error('row fetch failed');
      const d = await r.json();
      if (_rowRefreshVersions.get(Number(rowId)) !== version) return true;
      if (!d.row) throw new Error('no row');
      if (!_replaceRowInLists(d.row)) return false;
      if (_view === 'search') _paintFavSearchHits();
      else if (filtersActive()) render();
      else _replaceTileInDom(Number(rowId));
      return true;
    } catch (e) {
      return false;
    }
  ...
  const _LIB_TAB_KINDS = {
    performer: ['performer'],
    studio:    ['studio'],
    // Movies and JAV are their own top-level pages now; keep them
    // as separate kind-slots so /movies doesn't inhale JAV rows and
    // vice versa. The Movies grid used to fold ['movie', 'jav']
    // together — that behaviour moved to /jav's own page.
    movie:     ['movie'],
    jav:       ['jav'],
    vice:      ['vice'],
  };
  ...
  // Same pattern as `_view` — cloned sub-library pages
  // (/movies, /studios) preset the Add-tab type via
  // `window.__FAV_SEARCH_TYPE` so the type segments render locked to
  // the page's kind.
  let _favSearchType = (typeof window !== 'undefined' && typeof window.__FAV_SEARCH_TYPE === 'string')
    ? window.__FAV_SEARCH_TYPE
    : 'performer';   // 'performer' | 'studio' | 'movie'
  let _favSearchMounted = false;
  let _favSearchLastQ = '';
  let _favSearchSeq = 0;
  let _favSearchHits = null;
  const _favSourceSelections = new Map();
  const _favSourceKey = value => String(value || '').toLowerCase().replace('theporndb', 'tpdb');
  function _favSourcesForType(type) {
    return type === 'movie' ? ['TPDB'] : ['TPDB', 'StashDB', 'FansDB', 'JAVStash'];
  }
  function _mountFavSourceFilters() {
    const toolbar = document.querySelector('#favSearchHost .fav-search-toolbar');
    if (!toolbar) return;
    let group = toolbar.querySelector('.fav-search-sources');
    if (!group) {
      group = document.createElement('div');
      group.className = 'fav-search-sources';
      group.setAttribute('role', 'group');
      group.setAttribute('aria-label', 'Source databases');
      toolbar.appendChild(group);
    }
    const sources = _favSourcesForType(_favSearchType);
    if (!_favSourceSelections.has(_favSearchType)) {
      let saved;
      try { saved = JSON.parse(localStorage.getItem('ts:library-search-sources:' + _favSearchType)); } catch (_) {}
      const valid = Array.isArray(saved) ? saved.filter(k => sources.some(s => _favSourceKey(s) === k)) : [];
      _favSourceSelections.set(_favSearchType, new Set(valid.length ? valid : sources.map(_favSourceKey)));
    }
    const selected = _favSourceSelections.get(_favSearchType);
    group.innerHTML = sources.map(source => {
      const key = _favSourceKey(source);
      return `<button type="button" class="fav-search-source" data-source="${key}" aria-label="${source}" aria-pressed="${selected.has(key)}" title="${source}"><img src="/static/logos/${key}.webp" alt="${source}"></button>`;
    }).join('');
    group.querySelectorAll('button').forEach(button => button.addEventListener('click', () => {
      const key = button.dataset.source;
      if (selected.has(key)) {
        if (selected.size === 1) return; // Keep at least one database selected.
        selected.delete(key);
      } else selected.add(key);
      try { localStorage.setItem('ts:library-search-sources:' + _favSearchType, JSON.stringify([...selected])); } catch (_) {}
      _mountFavSourceFilters();
      _paintFavSearchHits();
    }));
  }
  function _paintFavSearchHits() {
    if (!_favSearchHits || _favSearchHits.type !== _favSearchType) return;
    const selected = _favSourceSelections.get(_favSearchType);
    const hits = _favSearchHits.hits.filter(hit => !selected ||
      selected.has(_favSourceKey(hit.source || hit._source || (_favSearchType === 'movie' ? 'TPDB' : ''))));
    const status = document.getElementById('favSearchStatus');
    if (status) {
      status.hidden = false;
      status.textContent = hits.length ? `${hits.length} result${hits.length === 1 ? '' : 's'}.` : 'No matches from the selected databases.';
    }
    if (status && _favSearchHits.warnings?.length) status.textContent += ' ' + _favSearchHits.warnings.join(' · ');
    _renderFavSearchResults(hits, _favSearchType);
  }
  ...
  // Find a loaded library row linked to this exact source profile ID. Used to mark "already in library"
  ...
  function _favSearchMatchLibraryRow(hit, type) {
    ...
    const hitSrc  = String(hit.source || hit._source || '').trim().toLowerCase();
    // Source → the primary match_* column AND the group_ids bucket to
    // consult. Scoping by source stops cross-DB false positives (a
    // StashDB UUID coincidentally matching a TPDB numeric id) and,
    // more importantly, lets us also check group_ids_json — which
    // holds additional linked profiles from the popup's "Link DB
    // profile" flow. Without this, a StashDB hit whose id is linked
    // as a secondary crosswalk on an existing library performer
    // renders as "not in library" and clicking + would create a
    // duplicate folder.
    const srcMap = {
      tpdb:     { col: 'match_tpdb_id',     grp: 'tpdb' },
      theporndb:{ col: 'match_tpdb_id',     grp: 'tpdb' },
      stash:    { col: 'match_stashdb_id', grp: 'stashdb' },
      stashdb:  { col: 'match_stashdb_id',  grp: 'stashdb' },
      fansdb:   { col: 'match_fansdb_id',   grp: 'fansdb' },
      javstash: { col: 'match_javstash_id', grp: 'javstash' },
      tmdb:     { col: 'match_tmdb_id',     grp: null },
    };
    const cfg = srcMap[hitSrc] || null;
    if (!list.length) return null;
    for (const r of list) {
      if (!r) continue;
      if (!hitId) continue;
      if (cfg) {
        // Source-scoped primary match.
        if (String(r[cfg.col] || '').trim() === hitId) return r;
        // Source-scoped group_ids match. Entries may be plain
        // strings or {id, name, image} dicts (studio popup shape).
        if (cfg.grp && r.group_ids && Array.isArray(r.group_ids[cfg.grp])) {
          for (const g of r.group_ids[cfg.grp]) {
            const gid = (g && typeof g === 'object') ? String(g.id || '').trim() : String(g || '').trim();
            if (gid && gid === hitId) return r;
          }
        }
      }
    }
  ...
  async function runFavSearch() {
    ...
    try {
      if (type === 'movie') {
        ...
        // Performer/studio search inherits the Settings → Content Filters
        // gender gate (cat_solo_female / cat_gay / cat_trans). Studios
        // don't have a gender, so the param is harmless there — the
        // endpoint only consults it on the performer path.
        let gendersParam = '';
        if (type === 'performer' && typeof window.performerGenderAllowlist === 'function') {
          try {
            const allow = await window.performerGenderAllowlist();
            if (Array.isArray(allow) && allow.length && allow.length < 4) {
              gendersParam = '&genders=' + encodeURIComponent(allow.join(','));
            }
          } catch (_) { /* fall through unfiltered if the helper errors */ }
        }
        if (mySeq !== _favSearchSeq) return;
        const r = await fetch('/api/metadata/search?q=' + encodeURIComponent(q) + '&type=' + encodeURIComponent(type) + '&strict=0' + gendersParam, { credentials: 'same-origin', signal: AbortSignal.timeout(20000) });
        const d = await r.json();
        if (mySeq !== _favSearchSeq) return;
        if (d && d.error) throw new Error(d.error);
        warnings = d.warnings || [];
    ...
    if (mySeq !== _favSearchSeq) return;
    if (type === 'performer') hits = sortPerformerSearchResults(hits, q);
    _favSearchHits = {hits, type, warnings};
    _paintFavSearchHits();
  }
  ...
  function _renderFavSearchResults(hits, type) {
    ...
    // Focus-triggered library checks must not recreate identical image nodes.
    // Replacing them makes cached results flash/reload as though search restarted.
    const previous = results._searchRender;
    if (previous && previous.tiles === tiles && previous.type === type &&
        previous.first === results.firstChild) return;
    results.innerHTML = tiles;
    results._searchRender = { tiles, type, first: results.firstChild };
    results.classList.remove('fav-grid--perf', 'fav-grid--studio', 'fav-grid--movie');
    if (studioLike) {
      results.classList.add('fav-grid--studio');
    } else if (movieLike) {
  ...
  document.addEventListener('click', (ev) => {
    ...
    if (type === 'studio') {
      if (inLib && libId) {
        window.tsGotoEntity({ kind: 'studio', libraryRowId: libId, name });
      } else if (stashId) {
        // External studio: pass the source so the studio-page popup
        // can fetch the source-DB's logo, description, parent and
        // recent scenes via /api/studios/external-panel.
        window.tsGotoEntity({ kind: 'studio', stashId, source: src, name });
      } else if (name) {
        window.tsGotoEntity({ kind: 'studio', name });
      }
    } else if (type === 'movie') {
      ...
      if (stashId) {
        window.tsGotoEntity({ kind: 'movie', tpdbId: stashId });
      } else if (typeof window.openProwlarrSearchPopup === 'function') {
        window.openProwlarrSearchPopup({ title: name, kind: 'movie' });
      }
    } else {
      if (inLib && libId) {
        window.tsGotoEntity({ kind: 'performer', libraryRowId: libId, name });
      } else if (stashId) {
        // Route the id to the param matching its source so the entity
        // page's popup backend hits the right crosswalk column. Omit
        // `name` so an unmatched id can't alias-fallback onto a
        // similarly-named library row.
        if (src === 'tpdb' || src === 'theporndb') {
          window.tsGotoEntity({ kind: 'performer', tpdbId: stashId });
        } else if (src === 'fansdb') {
          window.tsGotoEntity({ kind: 'performer', fansdbId: stashId });
        } else if (src === 'javstash') {
          window.tsGotoEntity({ kind: 'performer', javstashId: stashId });
        } else {
          window.tsGotoEntity({ kind: 'performer', stashId });
        }
      } else if (name) {
        window.tsGotoEntity({ kind: 'performer', name });
      }
    }
  });

  const _kindLoadGenerations = new Map();
  async function _loadKind(kind) {
    const generation = (_kindLoadGenerations.get(kind) || 0) + 1;
    _kindLoadGenerations.set(kind, generation);
    const revision = _libraryRevision;
    const url = '/api/favourites?kind=' + encodeURIComponent(kind);
    const r = await fetch(url, { credentials: 'same-origin', cache: 'no-store' });
    const d = await r.json();
    if (!r.ok) throw new Error(d.error || 'Failed to load');
    if (_kindLoadGenerations.get(kind) !== generation) return;
    if (revision !== _libraryRevision) return _loadKind(kind);
  ...
  let _searchMembershipRefresh = null;
  function _refreshSearchMembership() {
    if (_searchMembershipRefresh) return _searchMembershipRefresh;
    _searchMembershipRefresh = (async () => {
      const kinds = _LIB_TAB_KINDS[_favSearchType] || [_favSearchType];
      try {
        await Promise.all(kinds.filter(kind => !_loadedKinds.has(kind)).map(_loadKind));
        if (_view === 'search') _paintFavSearchHits();
      } catch (_) { /* Keep existing results if membership refresh fails. */ }
    })().finally(() => { _searchMembershipRefresh = null; });
    return _searchMembershipRefresh;
  }

  async function _refreshLibraryKind(kind) {
    // Includes changes received on history return, not just DOM events.
    _libraryRevision += 1;
    if (!kind) {
      _loadedKinds.clear();
      if (_view === 'search') return _refreshSearchMembership();
    ...
    if (_view === 'search') {
      await _refreshSearchMembership();
    } else {
      loadActiveView();
    }
  }
  document.addEventListener('library-row-added', async (ev) => {
    ...
    await _refreshLibraryKind(kind);
  });
  // Targeted single-tile refresh — dispatched after popup-side mutations
  // (link add/remove, apply-image, set-poster-role) so the corresponding
  // grid tile picks up the new link pills / image URL without a full
  // sub-library reload. Falls back to a no-op when the tile isn't in
  // the current DOM (different tab or filtered out).
  document.addEventListener('library-row-updated', async (ev) => {
    const rid = ev && ev.detail && ev.detail.rowId;
    if (!rid) { await _refreshLibraryKind(ev.detail && ev.detail.kind); return; }
    if (!await _refreshOneRow(rid)) await _refreshLibraryKind(ev.detail.kind);
  });

  // Cross-document handoff: performer-popup / studio-popup drop a
  // sessionStorage flag when the add happens on the entity shell
  // (/performer/{id}, /studio/{id}) so the sub-library page
  // (history.back destination) refreshes on return. Fires on the
  // initial DOMContentLoaded and on pageshow (bfcache restore) —
  // otherwise coming back via the browser's back button would
  // restore the pre-add cached grid.
  const _seenLibraryChanges = new Set();
  function _consumeLibraryInvalidation() {
    let payload;
    try {
      const raw = sessionStorage.getItem('ts:library-invalidated');
      if (!raw) return;
      payload = JSON.parse(raw);
      sessionStorage.removeItem('ts:library-invalidated');
    } catch (_) { return; }
    if (!payload || typeof payload !== 'object') return;
    const changes = payload.changes || [payload];
    const pending = new Map();
    for (const change of changes) {
      const token = change.changeId || JSON.stringify(change);
      if (_seenLibraryChanges.has(token)) continue;
      _seenLibraryChanges.add(token);
      pending.set(change.rowId || ('kind:' + change.kind), change);
    }
    while (_seenLibraryChanges.size > 256) _seenLibraryChanges.delete(_seenLibraryChanges.values().next().value);
    if (pending.size) _libraryRevision += 1;
    for (const change of pending.values()) {
      const rid = Number(change.rowId);
      if (change.type !== 'library-row-added' && !['merge', 'group-add', 'ungroup'].includes(change.action) && rid && _findRowAcrossLists(rid)) {
        _refreshOneRow(rid).then(ok => { if (!ok) _refreshLibraryKind(change.kind); });
      } else {
        _refreshLibraryKind(change.kind);
      }
    }
    return pending.size > 0;
  }
  _consumeLibraryInvalidation();
  // Storage events can arrive while a background/BFCache page is suspended.
  // Re-check the persistent change marker when the grid becomes visible.
  let _lastLibraryChange = null;
  function _checkLibraryChangesOnReturn(event) {
    let raw;
    try { raw = localStorage.getItem('ts:library-change'); } catch (_) {}
    if (raw && raw !== _lastLibraryChange) {
      _lastLibraryChange = raw;
      try { sessionStorage.setItem('ts:library-invalidated', raw); } catch (_) {}
    }
    const consumed = _consumeLibraryInvalidation();
    // Server-side jobs and older pages may not publish a browser event.
    // Revalidate on an actual return; keep the initial pageshow cheap.
    if (!consumed && event && (event.type === 'focus' ||
        event.type === 'visibilitychange' || event.persisted)) {
      _refreshLibraryKind();
    }
  }
  window.addEventListener('focus', _checkLibraryChangesOnReturn);
  document.addEventListener('visibilitychange', (event) => {
    if (!document.hidden) _checkLibraryChangesOnReturn(event);
  });
  // bfcache handshake — closing the index-progress EventSource on
  // pagehide lets Chrome/Firefox actually store this page in the
  // back/forward cache. Without it, opening a performer/studio via
  // /performer/{id} and hitting back re-runs the full page init and
  // wipes the user's current tab / search text / filter chips. On
  // restore we resume progress polling only if the indexer is still
  // busy (checkIndexProgressOnce() handles the probe + reconnect).
  window.addEventListener('pagehide', function () {
    try { stopIndexPolling(); } catch (_) {}
  });
  window.addEventListener('pageshow', function (ev) {
    // Fires on every navigation TO this page — including bfcache
    // restores where the flag was set by a script in a different
    // document context. Cheap when nothing's queued.
    _checkLibraryChangesOnReturn(ev);
    if (ev && ev.persisted) {
      // bfcache restore — the SSE we closed on pagehide is gone;
      // probe once to decide whether to reopen it.
      try { checkIndexProgressOnce(); } catch (_) {}
    }
  });
  ...
  async function loadActiveView() {
    ...
    try {
      // Sync the grid's kind-specific class before render() runs —
      // setView() does this when the user clicks a tab, but the initial
      // paint takes the loadActiveView path (skipping setView). The
      // page HTML ships with `.fav-grid--perf` hardcoded; on the
      // /studios and /vices pages that leaves the studio-shaped tiles
      // rendering inside a 10-column performer grid (way too small).
      const grid = document.getElementById('favGrid');
      if (grid) {
        grid.classList.remove('fav-grid--perf', 'fav-grid--studio', 'fav-grid--movie', 'fav-grid--vice');
        const studioLike = _view === 'studio' || _view === 'vice';
        grid.classList.add(studioLike ? 'fav-grid--studio' : 'fav-grid--perf');
        if (_view === 'movie' || _view === 'jav') grid.classList.add('fav-grid--movie');
        if (_view === 'vice')  grid.classList.add('fav-grid--vice');
      }
      const missing = kinds.filter(k => !_loadedKinds.has(k));
      if (missing.length) await Promise.all(missing.map(_loadKind));
      populateRootFilter();
      // Filter chips ship as "all" in the template; restore whatever
      // the user last had for this view before painting so a full
      // reload (page nav, back-from-/performer/X) doesn't wipe the
      // active filter set.
      _restoreFilterState();
      // Populate the alpha strip's visual selection to match the
      // restored letter.
      try {
        document.querySelectorAll('#favAlphaBar .fav-alpha-btn').forEach((b) => {
          b.classList.toggle('is-active', (b.dataset.letter || '') === _favLetterFilter);
        });
      } catch (_) { /* alpha strip not present */ }
      updateFavFilterIconBtns();
      render();
  ...
  // rows across kinds.
  let _fullLoadGeneration = 0;
  async function load() {
    const generation = ++_fullLoadGeneration;
    const revision = _libraryRevision;
    try {
      const r = await fetch('/api/favourites', { credentials: 'same-origin', cache: 'no-store' });
      const d = await r.json();
      if (generation !== _fullLoadGeneration) return;
      if (revision !== _libraryRevision) return load();
      if (!r.ok) throw new Error(d.error || 'Failed to load');
  ...
  async function toggleStar(id, on) {
    ...
    try {
      ...
      document.dispatchEvent(new CustomEvent('library-row-updated', {detail:{rowId:id, kind:row && row.kind}}));
    } catch (e) {
  ...
  async function lockAllMatches() {
    ...
    try {
      ...
      document.dispatchEvent(new CustomEvent('library-row-updated', {detail:{}}));
      await load();
  ...
  async function applyPickedImage() {
    ...
    const item = _imagePickItems.find(x => x.image === _imagePickSelectedUrl);
    if (!item) { window.toast('Select an image again'); return; }
    ...
    try {
      const r = await fetch('/api/favourites/apply-image', {
        ...
        body: JSON.stringify({
          row_id: targetRowId,
          ...(item.local_name ? {local_name: item.local_name} : {image_url: item.image}),
      ...
      // Also re-render the tile from the fresh DB row so image_url + any
      // other row fields land in _rowsPerf. Without this, refreshing the
      // page (or switching tabs) would drop back to the pre-save URL
      // because _rowsPerf still holds the stale snapshot.
  ...
  document.addEventListener('lib-entity-changed', async (e) => {
    ...
    if (['merge','group-add','ungroup'].includes(detail.action)) {
      await _refreshLibraryKind(detail.kind || 'performer');
      return;
    }
  ...
  async function _focusBoot() {
    ...
    // performer rows; studio rows route to openStudioPopup. Movie /
    // JAV rows open the inline entity-panel modal directly so a
    // deep-link from home lands on the row's detail view instead of
    // just switching tabs and leaving the user hunting through the
    // grid.
    if (_manage === 'images' && row.kind === 'performer' && typeof openPosterRolePicker === 'function') {
      ...
      window.tsGotoEntity({ kind: 'performer', libraryRowId: _focusId });
    } else if (row.kind === 'studio') {
      window.tsGotoEntity({ kind: 'studio', libraryRowId: _focusId, name: row.display_name || row.folder_name });
    } else if ((row.kind === 'movie' || row.kind === 'jav') && typeof openEntityDetail === 'function') {
      openEntityDetail(_focusId);
    }
  }
  // Restore the Add tab (view === 'search') across full-document navs so
  // that returning from /performer/{id} or /studio/{id} — which was
  // opened by clicking a search result — lands back on the Add tab with
  // the previous query re-run, not on the default grid. Only fires when
  // there's no ?focus= deep link and the stored state actually says the
  // user was on the search view.
  function _bootRestoreSearchView() {
    if (_focusId) return false;
    const s = _readPageState();
    if (!s || s.view !== 'search') return false;
    if (typeof setView !== 'function') return false;
    try { setView('search'); } catch (_) { return false; }
    const q = String(s.searchQ || '').trim();
    if (q) {
      const inp = document.getElementById('favSearchQ');
      if (inp) inp.value = q;
      _favSearchLastQ = q;
      try { runFavSearch(); } catch (_) { /* noop */ }
    }
    return true;
  }
  if (_focusId) {
    _focusBoot();
  } else if (!_bootRestoreSearchView()) {
    loadActiveView();
  }
