/* Externalized from favourites.html. */
  ...
  function _favSearchMatchLibraryRow(hit, type) {
    ...
    const hitSrc  = String(hit.source || '').trim().toLowerCase();
    ...
    for (const r of list) {
      ...
      if (cfg) {
        ...
        // Unknown source (fallback): keep the old broad check.
  ...
  async function _refreshLibraryKind(kind) {
    ...
    if (!kind) {
      _loadedKinds.clear();
      if (_view === 'search') {
        if (_favSearchLastQ) return runFavSearch();
        return;
      }
