/* Externalized from favourites.html. */
  ...
  async function _refreshOneRow(rowId) {
    try {
      ...
      _replaceTileInDom(Number(rowId));
      return true;
    } catch (e) {
      return false;
    }
  ...
  async function runFavSearch() {
    ...
    try {
      if (type === 'movie') {
        ...
        if (mySeq !== _favSearchSeq) return;
        const r = await fetch('/api/metadata/search?q=' + encodeURIComponent(q) + '&type=' + encodeURIComponent(type) + '&strict=0' + gendersParam, { credentials: 'same-origin' });
        const d = await r.json();
        if (mySeq !== _favSearchSeq) return;
        if (d && d.error) throw new Error(d.error);
  ...
  function _consumeLibraryInvalidation() {
    let raw = null;
    try { raw = sessionStorage.getItem('ts:library-invalidated'); } catch (_) { return; }
    if (!raw) return;
    try { sessionStorage.removeItem('ts:library-invalidated'); } catch (_) { /* noop */ }
    let payload;
    try { payload = JSON.parse(raw); } catch (_) { return; }
    if (!payload || typeof payload !== 'object') return;
    // When the flag names a row that already exists in the in-memory
    // list, refresh just that tile so the user's filter chips +
    // pagination + scroll position survive. If the row is new (add-to-
    // library) or the flag omits `rowId`, fall back to the full-kind
    // reload — that's the only way to get the new row rendered.
    const rid = Number(payload.rowId);
    if (rid != null && typeof _refreshOneRow === 'function' && _findRowAcrossLists(rid)) {
      _refreshOneRow(rid).then(ok => {
        if (!ok) _refreshLibraryKind(payload.kind || 'performer');
      });
      return;
    }
    _refreshLibraryKind(payload.kind || 'performer');
  }
  ...
  function _checkLibraryChangesOnReturn() {
    let raw;
  ...
  document.addEventListener('visibilitychange', () => {
    if (!document.hidden) _checkLibraryChangesOnReturn();
  });
