...
(function () {
  ...
  async function openMoviePopup(tpdbId, opts) {
    if (!tpdbId) return;
    _ensureCss();
    ...
    // Optional fallback poster + background from the caller (e.g. the
    // home / library tile the click came from), used when TPDB's own
    // record has an empty image field so the popup falls back to a
    // real cover instead of the "Image not available" placeholder.
    const fallbackPoster = String((opts && opts.fallbackPoster) || '').trim();
    const fallbackBg     = String((opts && opts.fallbackBg)     || '').trim();
    try {
      ...
      const bgSrc = (m.background && String(m.background).trim()) || fallbackBg;
      const bg = bgSrc ? `<img class="movie-detail-bg" src="${ESC(bgSrc)}" onerror="this.remove()" loading="lazy">` : '';
      const posterFallback = '/static/img/poster.webp';
      const detailPosterUrl = (m.poster && String(m.poster).trim()) || fallbackPoster || posterFallback;
      const spreadDup = !!(bgSrc && detailPosterUrl === bgSrc);
