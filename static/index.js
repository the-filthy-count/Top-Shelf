...
const DL_SEARCH_MODE_KEY = 'ts_downloads_search_mode';
  ...
  function renderRssTiles(results, mode) {
    ...
    const rendered = [...el.querySelectorAll('[data-tile-idx]')];
    const batch = list.slice(0,200).map(res => ({title:res.clean_title || '',studio:res.studio || (res.matched_rows||[]).find(row=>row.kind==='studio')?.folder_name || '',releases:res.releases || [res]}));
    if(batch.some(item=>item.title&&item.studio)) fetch('/api/releases/quality-comparison',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({items:batch})}).then(r=>r.ok?r.json():{}).then(data=>{
      for(const [index,match] of Object.entries(data.matches||{})){
        const tile=rendered[Number(index)];if(!tile?.isConnected)continue;
        const badge=document.createElement('div');badge.style.cssText='font-size:11px;color:var(--text);padding:6px 10px;background:rgba(0,0,0,.65)';
        badge.textContent=`Owned: ${match.owned_height}p${match.higher_resolution?' · Higher-resolution release available':''}`;
        badge.title='Exact title and studio match. Resolution alone does not establish overall encoding quality.';tile.appendChild(badge);
      }
    }).catch(()=>{});
  }
  ...
  async function loadWantedPanel(opts) {
    ...
    try {
      ...
      const renderRow = (w) => {
        const kindColor = w.kind === 'movie' ? '#2563eb' : '#a78bfa';
        ...
        const _wTitleHtml = (typeof _libraryAndTokenHighlight === 'function')
          ...
          : _escH(_wTitle);
        // Performer list — prefer the structured performers_json so we
        // get canonical names (studio-column data can be dirty; some
        // rows have a performer name stuffed into `studio` and the
        // subtitle then reads as one performer when the scene actually
        // has several). Fall back to the flat `performers` string and
        // finally an empty list.
        let _wPerformerNames = [];
        try {
          if (Array.isArray(w.performers_json)) {
            _wPerformerNames = w.performers_json
              .map(p => (p && typeof p === 'object' ? (p.name || '') : String(p || '')).trim())
              .filter(Boolean);
          } else if (typeof w.performers_json === 'string' && w.performers_json.trim()) {
            const parsed = JSON.parse(w.performers_json);
            if (Array.isArray(parsed)) {
              _wPerformerNames = parsed
                .map(p => (p && typeof p === 'object' ? (p.name || '') : String(p || '')).trim())
                .filter(Boolean);
            }
          }
        } catch (_) { /* fall through to `performers` */ }
        if (!_wPerformerNames.length && Array.isArray(w.performers)) {
          _wPerformerNames = w.performers.map(p => String(p || '').trim()).filter(Boolean);
        } else if (!_wPerformerNames.length && typeof w.performers === 'string') {
          _wPerformerNames = w.performers.split(',').map(p => p.trim()).filter(Boolean);
        }
        // Dedupe against the studio label — if a wanted row has the
        // studio field populated with a performer name (Row 2 in the
        // reported screenshot: 'INDIGO SIN' as studio while the real
        // studio is empty), don't double up in the subtitle.
        const _wStudioLc = String(w.studio || '').trim().toLowerCase();
        const _wPerformerNamesDedup = _wStudioLc
          ? _wPerformerNames.filter(n => n.toLowerCase() !== _wStudioLc)
          : _wPerformerNames;
        const performersHtml = _wPerformerNamesDedup.length
          ? `<div class="wanted-performers-line" title="${_escH(_wPerformerNamesDedup.join(', '))}">${_escH(_wPerformerNamesDedup.join(', '))}</div>`
          : '';
  ...
  async function searchWantedItem(wantedId) {
    ...
    const kindPillColor = w.kind === 'movie' ? '#2563eb' : '#a78bfa';
    header.innerHTML = `
  ...
  if (window.tsOnLibraryChange) window.tsOnLibraryChange(async () => {
    ...
    await _hydrateProwlarrHeadshots();
  });
