 ...
 * Block 1: main queue + Prowlarr search overlay + IAFD picker. */
  ...
  function _renderSceneQueueItem(f, i) {
    ...
    return `<div class="queue-item" id="qi-${i}" data-filename="${esc(f.filename)}">
      ...
      <div class="qi-cell qi-actions">
        ...
        <button type="button" class="btn-icon qi-filing-btn ${_viceMatchesFilename(f.filename) ? 'is-vice-match' : ''}" onclick="_qCall('openManualSearch', ${i})" title="Search / manual (includes suggested matches + Vice)"><span style="display:inline-block;width:1.1em;height:1.1em;background-color:currentColor;-webkit-mask:url(/static/icons/search.svg) center/contain no-repeat;mask:url(/static/icons/search.svg) center/contain no-repeat;vertical-align:middle"></span></button>
  ...
  function _recomputeQueueFilter() {
    ...
    const scope = _queueCurrentFolder;
    ...
    files.forEach((f, i) => {
      const fn = f.filename || '';
      if (scope) {
        if (fn !== scope && !fn.startsWith(scopePrefix)) return;
      }
      ...
      // Nested somewhere under scope — bucket into its top-level
      // subfolder (relative to scope).
      const rem = scope ? parent.substring(scopePrefix.length) : parent;
      const seg = rem.split('/')[0];
      if (!seg) return;
      const path = scope ? scope + '/' + seg : seg;
    ...
    if (scope && !_queueFilteredIndexes.length) {
      _queueCurrentFolder = '';
      _recomputeQueueFilter();
    }
  }
  ...
  function _renderQueueFolderRow(info) {
    ...
    const dotOrder = ['filed', 'error', 'no_dir', 'unmatched', 'pending'];
    const dots = [];
    for (const k of dotOrder) {
      const n = statuses[k];
      if (!n) continue;
      dots.push(`<span class="qi-folder-dot qi-folder-dot-${k}" title="${esc(k)}: ${n}"></span><span class="qi-folder-dot-count">${n}</span>`);
    }
    for (const k of Object.keys(statuses)) {
      if (dotOrder.includes(k)) continue;
      const n = statuses[k];
      if (!n) continue;
      dots.push(`<span class="qi-folder-dot qi-folder-dot-other" title="${esc(k)}: ${n}"></span><span class="qi-folder-dot-count">${n}</span>`);
    }
    const dotsHtml = dots.length ? `<span class="qi-folder-dots">${dots.join('')}</span>` : '';
    const label = info.count === 1 ? '1 file' : `${info.count} files`;
    ...
    // `dir_studio.svg` once the favourite lookup resolves.
    const folderSlot = folderName ? `
        <img class="qi-folder-svg" data-folder-kind-lookup="${nameAttr}" alt="" aria-hidden="true">` : `
        <img class="qi-folder-svg qi-folder-svg-neutral" src="/static/icons/dir_studio.svg" alt="" aria-hidden="true">`;
    ...
    return `<div class="queue-item queue-item-folder" data-folder-path="${pathAttr}" onclick="queueEnterFolder('${pathAttr.replace(/'/g, "\\'")}')" role="button" tabindex="0" title="Open ${pathAttr}">
      <div class="qi-cell qi-cell-folder-icon">
        ${folderSlot}
      </div>
      <div class="qi-cell"></div>
      <div class="qi-cell-name qi-cell-folder-name-row">
        <span class="qi-title qi-folder-name">${esc(info.name)}</span>
      </div>
  ...
  function _renderQueueBreadcrumb() {
    ...
    if (!_queueTreeModeActive()) return '';
    const segs = _queueCurrentFolder ? _queueCurrentFolder.split('/') : [];
    const crumbs = [`<button type="button" class="qi-crumb${segs.length ? '' : ' is-active'}" onclick="queueBreadcrumbGoto('')"><i class="fa-solid fa-house"></i> Root</button>`];
    let acc = '';
  ...
  async function _hydrateQueueFolderKinds() {
    ...
    // Fire off lookups in parallel (up to a handful — the endpoint is
    // cheap and folder counts stay small per page).
    const jobs = [];
    for (const [key, ent] of byName.entries()) {
      ...
      jobs.push((async () => {
        try {
          const r = await fetch('/api/library/favourite/lookup?name=' + encodeURIComponent(ent.display));
          if (mountGen !== _queueLoadGen) return;
          if (!r.ok) {
            _folderKindCache.set(key, { kind: 'unknown', links: {} });
            ent.slots.forEach(s => apply('unknown', {}, s));
            return;
          }
          const d = await r.json();
          const kind = (d && d.kind === 'studio') ? 'studio'
                     : (d && d.kind === 'performer') ? 'performer'
                     : 'unknown';
  ...
  async function openBulkFolderSort(folderPath, folderName) {
    ...
    const files = window._queueFiles || [];
    const scopePrefix = path ? path + '/' : path;
    ...
    const items = files.filter(f => {
      ...
      if (!fn) return false;
      if (path && !(fn === path || fn.startsWith(scopePrefix))) return false;
      const parent = _queueFileParent(fn);
      return parent === path;
    });
