 ...
 * Block 1: main queue + Prowlarr search overlay + IAFD picker. */
  ...
  function _renderMovieQueueItem(f, i) {
    ...
    return `<div class="queue-item" id="qi-${i}" data-filename="${esc(f.filename)}">
      <div class="qi-cell">${statusEl}</div>
      <div class="qi-cell"><img class="qi-row-dir-svg" src="/static/icons/dir_movie.svg" alt="" aria-hidden="true"></div>
      <div class="qi-cell-name" title="${esc(f.filename)}">
  ...
  function _recomputeQueueFilter() {
    ...
    files.forEach((f, i) => {
      ...
      if (parent === effectiveScope) {
        ...
        // guesser found the studio (match_guess === true); real
        // scraped matches leave the queue entirely on filing.
        if (virtualStudioBucketing && f.match_guess) {
          const studioKey = String(f.match_studio || '').trim();
          if (studioKey) {
