 ...
 * Block 1: main queue + Prowlarr search overlay + IAFD picker. */
  ...
  function _renderQueueFolderRow(info) {
    ...
    // The queue snapshot creates these subfolders only when the folder
    // name resolves to a library favourite (performer OR studio), so
    // fire both a headshot lookup and a studio-logo lookup — whichever
    // matches renders, the other 404s and hides via onerror.
    const folderName = (info.name || '').trim();
    const nameAttr = esc(folderName);
    const headshotSlot = folderName
      ? `<span class="qi-headshots" data-perf-names="${nameAttr}"></span>`
      : '';
    const logoSlot = folderName
      ? `<img class="qi-studio-logo" loading="lazy" decoding="async" src="/api/studio-logo?name=${encodeURIComponent(folderName)}" alt="${nameAttr}" title="${nameAttr}" onerror="this.style.display='none'">`
      : '';
    return `<div class="queue-item queue-item-folder" data-folder-path="${pathAttr}" onclick="queueEnterFolder('${pathAttr.replace(/'/g, "\\'")}')" role="button" tabindex="0" title="Open ${pathAttr}">
      <div class="qi-cell"><i class="fa-solid fa-folder qi-folder-icon"></i></div>
      <div class="qi-cell">${logoSlot}</div>
      <div class="qi-cell-name">
        <div class="qi-title-row"><span class="qi-title qi-folder-name">${esc(info.name)}</span></div>
        <div class="qi-meta qi-folder-meta">${label}${dotsHtml}</div>
      </div>
      <div class="qi-cell qi-cell-headshots">${headshotSlot}</div>
      <div class="qi-cell"></div>
      <div class="qi-cell"></div>
