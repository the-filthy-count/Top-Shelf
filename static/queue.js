 ...
 * Block 1: main queue + Prowlarr search overlay + IAFD picker. */
  ...
  // Share complete strips and in-flight requests between both popups.
  const _filmstripCache = new Map();
  const _filmstripRequests = new Map();
  async function _fetchQueueFilmstrip(filename) {
    const cached = _filmstripCache.get(filename);
    if (cached && Date.now() - cached.at < 60000) return cached.response;
    if (_filmstripRequests.has(filename)) return _filmstripRequests.get(filename);
    const pending = (async () => {
      const response = await fetch('/api/queue/thumbs?data=1&filename=' + encodeURIComponent(filename), {credentials:'same-origin'});
      const body = await response.json();
      const result = {ok:response.ok, status:response.status, json:async () => body};
      if (response.ok && body.ready) {
        _filmstripCache.delete(filename);
        _filmstripCache.set(filename, {at:Date.now(), response:result});
        while (_filmstripCache.size > 40) _filmstripCache.delete(_filmstripCache.keys().next().value);
      }
      return result;
    })();
    _filmstripRequests.set(filename, pending);
    try {return await pending;} finally {_filmstripRequests.delete(filename);}
  }
  ...
  async function openQueueFilmstrip(filename) {
    ...
    const renderState = (d) => {
      const thumbs = Array.isArray(d.thumbs) ? d.thumbs.map(t => ({...t, url:t.data_url || t.url})) : [];
      _qfsItems = thumbs;
    ...
    try {
      while (true) {
        const r = await _fetchQueueFilmstrip(filename);
  ...
  // Queue fingerprint icon → shared phash popover (ts-utils.js).
  // Legacy name kept because the icon's inline onclick calls it.
  window.showQueuePhashPopup = function (ev, btn) {
    if (typeof window.tsShowPhashPopup === 'function') {
      window.tsShowPhashPopup(ev, btn);
    }
  };
  ...
  let _headshotRevision = 0;
  async function _fetchHeadshotsByName(nameSet) {
    const revision = _headshotRevision;
    const lookup = {};
    ...
    if (missing.length) {
      try {
        ...
        if (!r.ok) throw new Error("Headshot lookup failed");
        const d = await r.json();
        if (revision !== _headshotRevision) return _fetchHeadshotsByName(nameSet);
  ...
  //: Post-filter items to paginate + render. Each element is either
  //: `{kind:'file', fi:<index into _queueFiles>}` or
  //: `{kind:'folder', info:{name, path, count, statuses}}`.
  //: Folder entries only appear in scenes tree mode (no filters + no
  //: search) so the queue can be browsed as a directory tree instead
  //: of a flat list.
  ...
  //: Current folder being browsed in scenes tree mode; '' == root.
  //: A file's parent path decides which folder it belongs to; nested
  //: subfolders collapse into a single tile at their parent level.
  let _queueCurrentFolder = '';
   ...
   * `window._queueFiles` between render and click, the index points at
   * a different file. Every row carries its filename in `data-filename`,
   * so route through this helper and read from the DOM node — the row
   * the user actually clicked — instead of trusting the baked index.
   * The index stays a fallback for callers that pass a number without a
   * DOM element (older code paths / programmatic invocation).
   */
  function _qCall(fnName, iOrEl, extra) {
    let filename = '';
    //: Preferred path: the button/event target lets us walk to the
    //: actual row that was clicked and read its data-filename. Immune
    //: to poll-triggered reorders of window._queueFiles.
    if (iOrEl && typeof iOrEl === 'object') {
      const row = (iOrEl.closest && iOrEl.closest('.queue-item')) || null;
      if (row) filename = row.getAttribute('data-filename') || '';
    }
    //: Legacy numeric-index path. Kept for callers that pass an int;
    //: reads the current _queueFiles so it at least uses the freshest
    //: array (still vulnerable to reorders — DOM-element callers are
    //: preferred).
    if (!filename && typeof iOrEl === 'number') {
      const f = (window._queueFiles || [])[iOrEl];
    ...
    if (arguments.length > 2) fn(filename, extra);
    else fn(filename);
  }
  window._qCall = _qCall;
  window.queueTileSearch = function(event, tile, search) {
    if (event.target.closest('button,a,input,select,textarea')) return;
    if (event.type === 'keydown' && (event.target !== tile || !['Enter',' '].includes(event.key))) return;
    event.preventDefault();
    event.stopPropagation();
    _qCall(search, tile);
  };
  window.queuePerformerProfile = function(event, button) {
    event.stopPropagation();
    const opts = {kind:'performer', name:button.dataset.name,
      libraryRowId:Number(button.dataset.libraryRowId) || null};
    if (typeof window.tsGotoEntity === 'function' && window.tsGotoEntity(opts)) return;
    if (typeof window.openPerformerPopup === 'function') window.openPerformerPopup(opts);
  };
  ...
  function setQueueMode(mode, preserveFilter) {
    ...
    // Tree-mode folder state only applies to scenes; drop it on any
    // mode switch so a stale path can't survive a scenes→movies→scenes
    // round-trip.
    _queueCurrentFolder = '';
    if (mode === 'movies') queueMode = 'movies';
    else if (mode === 'vices') queueMode = 'vices';
    else if (mode === 'jav')   queueMode = 'jav';
    else queueMode = 'scenes';
    ...
    const btnVices  = document.getElementById('btnQueueVices');
    const btnJav    = document.getElementById('btnQueueJav');
    ...
    if (btnVices)  btnVices.classList.toggle('active',  queueMode === 'vices');
    if (btnJav)    btnJav.classList.toggle('active',    queueMode === 'jav');
    // Body class drives CSS-only gating of vice-only UI (see queue.html
    // #qSearchViceBtn and .is-vice-match rules).
    document.body.classList.remove('queue-mode-scenes', 'queue-mode-movies', 'queue-mode-vices', 'queue-mode-jav');
    document.body.classList.add('queue-mode-' + queueMode);
    const qpt = document.getElementById('queuePanelTitle');
    if (qpt && typeof tsSetPageTitleText === 'function') {
      const _title = queueMode === 'movies' ? 'Movie queue'
                   : queueMode === 'vices'  ? 'Vice queue'
                   : queueMode === 'jav'    ? 'JAV queue'
                   : 'Scene queue';
      tsSetPageTitleText(qpt, _title, 'queue');
    }
    ...
    if (queueMode === 'movies') {
      if (statFiled) statFiled.setAttribute('href', '/history?status=filed&kind=movie');
      if (statErr)   statErr.setAttribute('href', '/history?status=error&kind=movie');
    } else {
      if (statFiled) statFiled.setAttribute('href', '/history?status=filed');
      if (statErr)   statErr.setAttribute('href', '/history?status=error');
    }
    // Highlight the active tab tile.
    ['Jav','Movies','Scenes','Vices'].forEach(k => {
      const card = document.getElementById('statCard' + k);
      if (card) card.classList.toggle('is-active-tab', queueMode === k.toLowerCase());
    });
    // Vice UI belongs only to the Vices tab now: the vices-only filter
    // chip is meaningless on Scenes/Movies. Keep the flame filter
    // element hidden outside the Vices tab.
    const _btnVicesOnly = document.getElementById('btnQueueVicesOnly');
    if (_btnVicesOnly) _btnVicesOnly.style.display = (queueMode === 'vices') ? '' : 'none';
  ...
  //: Shared pixel-LED status indicator for file rows — mirrors the
  //: `.qi-folder-dot` style used on folder tiles so both surfaces
  //: read as the same retro instrument. `filed` / `no_dir` collapse
  //: into red like the folder LEDs (single "problem" bucket); the
  //: caller supplies the rich tooltip.
  function _renderQueueStatusLed(status, tip) {
    const map = {
      error: 'error', no_dir: 'error',
      unmatched: 'unmatched', processing: 'processing',
      pending: 'pending', filed: 'processing',
    };
    const key = map[status] || 'none';
    return `<span class="qi-status-led qi-status-led-${key}" title="${esc(tip || status || '')}"></span>`;
  }
  ...
  function _renderSceneQueueItem(f, i) {
    ...
    if (f.prev_status === 'unmatched') {
      ...
      const _ndBits = [];
      const _ndScope = (f.scope || '').trim().toLowerCase();
      if (_ndScope === 'vice') {
        // Vice-scope files never route to a Series / Performer folder
        // — they park as no_dir when neither a parent-folder hint nor
        // any scene tag maps to a configured Vice. Saying "no Series
        // folder" here is misleading (the file is already in the
        // Vices tab).
        _ndBits.push('Identified — but no matching Vice');
        _ndBits.push('Add a Vice for this scene’s tags or drop the file into a configured Vice folder');
      } else {
      ...
      statusTip = _ndBits.join(' · ');
    } else if (f.prev_status === 'error') {
      statusTip = f.error || 'Previous run errored';
    } else {
      statusTip = statusKey;
    }
    const statusEl = _renderQueueStatusLed(statusKey, statusTip);
    ...
    const studioLogo = (studio || logoQ)
      ? `<img class="qi-studio-logo" data-logo-fallback="${_queueFileLogoFallback()}" loading="lazy" decoding="async" data-logo-src="/api/studio-logo?name=${encodeURIComponent(studio)}&q=${encodeURIComponent(logoQ)}" alt="${esc(studio || '')}" title="${esc(studio || logoQ)}" onerror="this.onerror=null; this.src=this.dataset.logoFallback">`
      : `<img class="qi-studio-logo" data-logo-fallback="${_queueFileLogoFallback()}" src="${_queueFileLogoFallback()}" alt="Studio">`;
    ...
    const artwork = _queueFilePerformerArtwork(f, perfs);
    const displayPerfs = artwork.names;
    const headshotData = artwork.shots;
    let headshotSlot = '';
    if (headshotData.length) {
      ...
      for (const h of headshotData) {
        ...
        if (url) {
          ...
          imgs.push(`<img class="qi-headshot" loading="lazy" decoding="async" data-name="${safeName}"${ridAttr} src="${url}" alt="${safeName}" title="${safeName}" onerror="this.style.display='none'">`);
        }
        if (imgs.length >= 3) break;
      }
      headshotSlot = `<span class="qi-headshots">${imgs.reverse().join('')}</span>`;
    } else if (displayPerfs.length) {
      // Fallback: server didn't pre-resolve (older payload, edge case).
      // `_hydrateQueueHeadshots` picks this up via `data-perf-names`.
      headshotSlot = `<span class="qi-headshots" data-perf-names="${esc(displayPerfs.join('|'))}"></span>`;
    }
    const profile = headshotData.find(h => h && (h.row_id || h.name)) || {name:displayPerfs[0]};
    const profileButton = profile.name || profile.row_id
      ? `<button type="button" class="btn-icon" data-name="${esc(profile.name || '')}" data-library-row-id="${esc(String(profile.row_id || ''))}" onclick="queuePerformerProfile(event,this)" title="Open performer profile${profile.name ? ': ' + esc(profile.name) : ''}" aria-label="Open performer profile"><i class="fa-solid fa-star"></i></button>` : '';
    const histBits = [];
    ...
    const _titleHtml = (typeof _libraryAndTokenHighlight === 'function')
      ...
      : esc(_titleText);
    return `<div class="queue-item" id="qi-${i}" data-filename="${esc(f.filename)}" tabindex="0" onclick="queueTileSearch(event,this,'openManualSearch')" onkeydown="queueTileSearch(event,this,'openManualSearch')">
      <div class="qi-cell">${statusEl}</div>
      <div class="qi-cell">${studioLogo}${displayPerfs.length ? `<div class="qi-performer-label">${esc(displayPerfs.join(', '))}</div>` : ''}</div>
      <div class="qi-cell-name" title="${esc(f.filename)}">
        ...
        ${metaText && metaClass !== 'qi-meta qi-meta-guess' ? `<div class="${metaClass}">${metaText}</div>` : ''}
      </div>
      <div class="qi-cell qi-cell-headshots">${headshotSlot}</div>
      <div class="qi-cell"><span class="badge-size">${f.size_mb} MB</span></div>
      <div class="qi-cell">${f.has_phash ? `<button type="button" class="badge-cached qi-phash-btn" title="Click to view phash — matches Stash's fingerprint format" data-phash="${esc(f.phash || '')}" data-filename="${esc(f.filename || '')}" onclick="showQueuePhashPopup(event, this)"><i class="fa-solid fa-fingerprint queue-phash-fa"></i></button>` : ''}${f.in_duplicate_group ? `<span class="q-dup-badge" title="Duplicate — ${esc(String(f.duplicate_group_size || 2))} files share the same phash"><i class="fa-solid fa-clone"></i><span>${esc(String(f.duplicate_group_size || 2))}</span></span>` : ''}</div>
      <div class="qi-cell qi-actions">
        ${profileButton}
        <button type="button" class="btn-icon" onclick="_qCall('openQueueFilmstrip', this)" title="Filmstrip"><i class="fa-solid fa-photo-film"></i></button>
        <button type="button" class="btn-icon qi-filing-btn ${_viceMatchesFilename(f.filename) ? 'is-vice-match' : ''}" onclick="_qCall('openManualSearch', this)" title="Search / manual (includes suggested matches + Vice)"><span style="display:inline-block;width:1.1em;height:1.1em;background-color:currentColor;-webkit-mask:url(/static/icons/search.svg) center/contain no-repeat;mask:url(/static/icons/search.svg) center/contain no-repeat;vertical-align:middle"></span></button>
        <button type="button" class="btn-icon qi-delete-btn" onclick="_qCall('confirmDeleteQueueFile', this)" title="Delete"><i class="fa-solid fa-trash"></i></button>
        <button type="button" class="btn-run-file" onclick="_qCall('runQueueFile', this)" title="Run"><i class="fa-solid fa-play"></i></button>
      </div>
    </div>`;
  }
  ...
  function _queueFileLogoFallback() {
    const icon = {jav: 'jav', movies: 'movies', vices: 'vices'}[queueMode] || 'studio';
    return `/static/icons/${icon}.svg?v=20260909-colours`;
  }
  ...
  function _renderMovieQueueItem(f, i) {
    ...
    // Movies rows are flat (no folder tree), so cell 2 — where scene
    // rows show the studio logo — is otherwise empty. Fill it with the
    // Movies dir icon so the row visually reads as a movie release and
    // matches the Vices / JAV tab's folder-tile aesthetic.
    return `<div class="queue-item" id="qi-${i}" data-filename="${esc(f.filename)}" tabindex="0" onclick="queueTileSearch(event,this,'openMovieManualSearch')" onkeydown="queueTileSearch(event,this,'openMovieManualSearch')">
      <div class="qi-cell">${statusEl}</div>
      <div class="qi-cell"><img class="qi-studio-logo" data-logo-fallback="${_queueFileLogoFallback()}" ${f.match_studio ? `data-logo-src="/api/studio-logo?name=${encodeURIComponent(f.match_studio)}"` : `src="${_queueFileLogoFallback()}"`} alt="Studio" onerror="this.onerror=null; this.src=this.dataset.logoFallback"></div>
      <div class="qi-cell-name" title="${esc(f.filename)}">
      ...
      <div class="qi-cell"></div>
      <div class="qi-cell qi-actions">
        <button type="button" class="btn-icon" onclick="_qCall('openMovieManualSearch', this)" title="Search"><i class="fa-solid fa-magnifying-glass"></i></button>
        <button type="button" class="btn-icon qi-delete-btn" onclick="_qCall('confirmDeleteQueueFile', this)" title="Delete"><i class="fa-solid fa-trash"></i></button>
        <button type="button" class="btn-run-file" onclick="_qCall('runQueueFile', this)" title="Auto-match"><i class="fa-solid fa-play"></i></button>
      </div>
    </div>`;
  }
  ...
  function toggleQueueErrorsOnly() {
    ...
    //: filter chip reflect the same state as this toggle. Matches the
    //: Errors tile: error + no_dir combined.
    _queueStatusFilter = _queueErrorsOnly ? 'error|no_dir' : '';
  ...
  //: Parent posix path (or '') for a queue rel-path key. `Jane Smith/scene.mp4`
  //: → `Jane Smith`; `scene.mp4` → ''; `A/B/c.mp4` → `A/B`.
  // File artwork represents the performer folder, not the full scene cast.
  function _queueFilePerformerArtwork(file, names) {
    const shots = (Array.isArray(file.performer_headshots) ? file.performer_headshots : [])
      .filter(h => h && h.name);
    const normal = value => String(value || '').normalize('NFKC').trim().toLowerCase()
      .replace(/[._]+/g, ' ').replace(/\s+/g, ' ');
    const folders = String(file.filename || '').replace(/\\/g, '/').split('/').slice(0, -1).reverse();
    for (const folder of folders) {
      const shot = shots.find(h => normal(h.name) === normal(folder));
      const name = names.find(n => normal(n) === normal(folder));
      if (shot || name) return {names: [shot ? shot.name : name], shots: shot ? [shot] : []};
    }
    // Unsorted files and studio folders still get one consistent portrait.
    const shot = shots[0];
    return {names: shot ? [shot.name] : names.slice(0, 1), shots: shot ? [shot] : []};
  }

  function _queueFileParent(fn) {
    const s = String(fn || '');
    const i = s.lastIndexOf('/');
    return i >= 0 ? s.substring(0, i) : '';
  }

  function _queueTreeModeActive() {
    if (_queueActiveKind !== 'scenes') return false;
    if (_queueStatusFilter) return false;
    if (_queueMatchedOnly) return false;
    if (_queueVicesOnly) return false;
    if (_queueErrorsOnly) return false;
    const term = (document.getElementById('queueFilter')?.value || '').trim();
    if (term) return false;
    return true;
  }

  // Vices tab is scoped inside `<source_dir>/Vices/` — its tree root is
  // one level deeper than Scenes so the user doesn't have to click
  // through a redundant "Vices" tile at root to reach per-vice
  // subfolders. JAV mirrors the pattern with a `JAV/` prefix rooted at
  // the JAV download watch dir. Returns the on-disk prefix to prepend
  // to `_queueCurrentFolder` when computing scope in tree-mode.
  function _queueTreeRootPrefix() {
    if (queueMode === 'vices') return 'Vices';
    if (queueMode === 'jav')   return 'JAV';
    return '';
  }
  ...
  function _recomputeQueueFilter() {
    ...
    // Status filter can be a single status or a `|`-joined set (e.g.
    // 'error|no_dir' for the combined Errors tile).
    const statusSet = statusFilter ? statusFilter.split('|') : null;
    // Virtual studio-alias folder view: `_queueCurrentFolder` uses the
    // `@studio:` prefix to mean "flat list of every file whose backend
    // library-guess resolved to this studio", regardless of the file's
    // real disk parent. Backend attaches `match_studio` + `match_guess`
    // via _library_guess_from_release_name when a filename's alnum
    // prefix hits a studio favourite's folder / canonical / alias slug.
    if (_queueCurrentFolder && _queueCurrentFolder.startsWith('@studio:')) {
      const target = _queueCurrentFolder.substring('@studio:'.length).trim();
      files.forEach((f, i) => {
        if (!f.match_guess) return;
        if (String(f.match_studio || '').trim() !== target) return;
        if (statusSet && !statusSet.includes(f.prev_status || '')) return;
        ...
        _queueFilteredIndexes.push({ kind: 'file', fi: i });
      });
      return;
    }
    const treeMode = _queueTreeModeActive();
    if (!treeMode) {
      // Flat mode — legacy behaviour. Any active filter (status,
      // matched-only, vices-only, errors-only, text search) or the
      // movies tab drops the tree so the user sees a global list.
      files.forEach((f, i) => {
        if (statusSet && !statusSet.includes(f.prev_status || '')) return;
        if (_queueMatchedOnly) {
          const src = (f.match_source || '').trim();
    ...
    // Tree mode: files whose parent path === current folder appear as
    // file rows; anything deeper feeds a subfolder tile that shows the
    // total recursive count + per-status tally. `scope` matches the
    // current folder inclusively (`''` at root matches all paths).
    //
    // `_queueCurrentFolder` is user-facing (relative to the tab's own
    // root); `effectiveScope` prepends the mode's on-disk prefix so
    // path comparisons line up with `f.filename`. On the Vices tab the
    // prefix is `Vices` — root shows per-vice folders instead of one
    // "Vices" tile.
    const rootPrefix = _queueTreeRootPrefix();
    const effectiveScope = rootPrefix
      ? (_queueCurrentFolder ? rootPrefix + '/' + _queueCurrentFolder : rootPrefix)
      : _queueCurrentFolder;
    const effectiveScopePrefix = effectiveScope ? effectiveScope + '/' : '';
    const fileEntries = [];
    const folderMap = new Map();
    // Virtual-studio bucketing runs only on the scenes-root tree view
    // (no on-disk rootPrefix, no navigated folder). Vices / JAV have
    // their own tab-scoped tree roots; nesting virtual tiles inside a
    // real disk folder would double-count files that also carry a
    // guessed studio.
    const virtualStudioBucketing = !rootPrefix && !_queueCurrentFolder;
    files.forEach((f, i) => {
      const fn = f.filename || '';
      if (effectiveScope) {
        if (fn !== effectiveScope && !fn.startsWith(effectiveScopePrefix)) return;
      }
      const parent = _queueFileParent(fn);
      if (parent === effectiveScope) {
        // Alias-guessed studio → virtual @studio: folder tile so
        // e.g. `brcc.foo.mp4` collects under a "Backroom Casting
        // Couch" tile at root even though it's physically loose in
        // source_dir. Only fires when the pipeline's filename-token
        // guesser found the studio (match_guess === true) AND that
        // studio has a folder in the library — otherwise the tile
        // just says "here's a studio you don't file to", which is
        // noise. Real scraped matches leave the queue entirely on
        // filing.
        if (virtualStudioBucketing && f.match_guess) {
          const studioRaw = String(f.match_studio || '').trim();
          const studioKey = (typeof _studioIfInLibrary === 'function')
            ? _studioIfInLibrary(studioRaw)
            : studioRaw;
          if (studioKey) {
            const path = '@studio:' + studioKey;
            let info = folderMap.get(path);
            if (!info) {
              info = { name: studioKey, path, count: 0, statuses: {}, virtual: 'studio' };
              folderMap.set(path, info);
            }
            info.count += 1;
            const st = f.prev_status || 'pending';
            info.statuses[st] = (info.statuses[st] || 0) + 1;
            return;
          }
        }
        fileEntries.push({ kind: 'file', fi: i });
        return;
      }
      // Nested somewhere under effectiveScope — bucket into its top-level
      // subfolder (relative to effectiveScope).
      const rem = effectiveScope ? parent.substring(effectiveScopePrefix.length) : parent;
      const seg = rem.split('/')[0];
      if (!seg) return;
      // Folder path stays user-facing (relative to the tab's own root)
      // so queueEnterFolder writes it into _queueCurrentFolder cleanly.
      const path = _queueCurrentFolder ? _queueCurrentFolder + '/' + seg : seg;
      let info = folderMap.get(path);
      if (!info) {
        info = { name: seg, path, count: 0, statuses: {} };
        folderMap.set(path, info);
      }
      info.count += 1;
      const st = f.prev_status || 'pending';
    ...
    // On the Vices tab root, backfill an empty tile for every
    // configured vice that doesn't yet have any files in its folder.
    // The vice list itself is authoritative — the drop-inboxes exist
    // on disk regardless — so an empty Painal/ folder should still
    // read on the tab so the user knows where to drop new files.
    if (queueMode === 'vices' && !_queueCurrentFolder && Array.isArray(_viceNames)) {
      for (const vname of _viceNames) {
        const path = String(vname || '').trim();
        if (!path) continue;
        if (folderMap.has(path)) continue;
        folderMap.set(path, { name: path, path, count: 0, statuses: {} });
      }
    }
    // Folders first (alphabetical), files second (already sorted by
    // display name in _cmpQueueFiles / _applySceneQueuePayload).
    const folderEntries = [...folderMap.values()]
      .sort((a, b) => a.name.localeCompare(b.name, undefined, { numeric: true, sensitivity: 'base' }))
      .map(info => ({ kind: 'folder', info }));
    _queueFilteredIndexes = folderEntries.concat(fileEntries);
    // Folder we were browsing has been emptied out entirely (all files
    // filed or deleted) — snap back to root so the empty pane doesn't
    // strand the user without any way forward except the breadcrumb.
    if (_queueCurrentFolder && !_queueFilteredIndexes.length) {
      _queueCurrentFolder = '';
      _recomputeQueueFilter();
    }
  }
  ...
  function filterQueueByStatus(status) {
    ...
    //: close, etc). Treat the combined 'error|no_dir' set — the Errors
    //: tile's filter — as the errors-only pressed state.
    const nextSet = next ? next.split('|') : [];
    _queueErrorsOnly = nextSet.includes('error') && (nextSet.length === 1 || (nextSet.length === 2 && nextSet.includes('no_dir')));
    const errBtn = document.getElementById('btnQueueErrorsOnly');
    if (errBtn) {
    ...
    // no_dir only applies to scenes/vices/jav — flip off the movies tab
    // if the filter mentions it and we're currently on movies.
    if (nextSet.includes('no_dir') && _queueActiveKind === 'movies') {
  ...
  function _computeRenderSig(slice, files) {
    ...
    //: handled out-of-band without a rebuild. Tree mode injects the
    //: current folder + each folder tile's count/status mix so
    //: navigating in/out never gets skip-rebuilt against a stale sig.
    const parts = [
      _queueActiveKind, _queuePage, _queuePageSize,
      _queueFilteredIndexes.length, _queueCurrentFolder,
    ];
    for (const item of slice) {
      if (!item) continue;
      if (item.kind === 'folder') {
        const info = item.info || {};
        const statusKey = Object.keys(info.statuses || {}).sort()
          .map(k => k + ':' + info.statuses[k]).join(',');
        parts.push('F', info.path || '', info.count || 0, statusKey);
        continue;
      }
      const f = files[item.fi];
      if (!f) continue;
      parts.push(
  ...
  //: Folder tile. Reuses `.queue-item` chrome so the row heights line
  //: up with file rows on the same page. Whole row is clickable via
  //: the onclick — inner buttons stop propagation.
  function _renderQueueFolderRow(info) {
    const statuses = info.statuses || {};
    //: `filed` never actually gets rendered on a folder tile (filed
    //: rows leave the queue) so the dot was pure noise. `no_dir` is
    //: really just an error variant from the user's POV, so both
    //: fold into the red LED — the tooltip still names the split.
    const errorCount = (statuses.error || 0) + (statuses.no_dir || 0);
    const errorTitle = `error/no_dir: ${errorCount}`;
    const dotOrder = [
      { key: 'error', title: errorTitle, count: errorCount },
      { key: 'unmatched', title: `unmatched: ${statuses.unmatched || 0}`, count: statuses.unmatched || 0 },
      { key: 'pending', title: `pending: ${statuses.pending || 0}`, count: statuses.pending || 0 },
      { key: 'processing', title: `processing: ${statuses.processing || 0}`, count: statuses.processing || 0 },
    ];
    const hidden = new Set(['filed', 'error', 'no_dir', 'unmatched', 'pending', 'processing']);
    const dots = [];
    const _pad = (n) => String(Math.min(n, 999)).padStart(3, '0');
    for (const d of dotOrder) {
      const zero = d.count === 0 ? ' qi-folder-dot-group--zero' : '';
      dots.push(
        `<span class="qi-folder-dot-group${zero}" title="${esc(d.title)}">` +
          `<span class="qi-folder-dot qi-folder-dot-${d.key}"></span>` +
          `<span class="qi-folder-dot-count">${_pad(d.count)}</span>` +
        `</span>`
      );
    }
    for (const k of Object.keys(statuses)) {
      ...
      if (!n) continue;
      dots.push(
        `<span class="qi-folder-dot-group" title="${esc(k)}: ${n}">` +
          `<span class="qi-folder-dot qi-folder-dot-other"></span>` +
          `<span class="qi-folder-dot-count">${_pad(n)}</span>` +
        `</span>`
      );
    }
    const dotsHtml = `<span class="qi-folder-dots">${dots.join('')}</span>`;
    const countText = info.count === 1 ? '1 file' : `${info.count} files`;
    const label = `<span class="qi-folder-meta-label">${countText}</span>`;
    const pathAttr = esc(info.path);
    // Every folder gets two visuals fed off the same name:
    //   1. purpose-built folder SVG in cell 1 — `dir_star.svg` for
    //      performers, `dir_studio.svg` for studios. Rendered as an
    //      <img> (no colour override) so whatever the artwork paints
    //      is what shows. The type resolves async via
    //      `_hydrateQueueFolderKinds` (lookup against
    //      `favourite_entities`), which swaps the placeholder src.
    //   2. big studio logo OR star headshot in the headshot slot
    //      (cell 4) — carries the row's identity graphic where files
    //      would show their own headshots.
    //   Cell 2 is left empty so folder rows still line up with file
    //   rows below (grid template unchanged).
    const folderName = (info.name || '').trim();
    const nameAttr = esc(folderName);
    const enc = encodeURIComponent(folderName);
    // Headshot pile FIRST in DOM order so it wins the slot when the
    // name resolves to a performer; the studio logo sits behind it
    // and only shows when the pile stays empty (async hydrator adds
    // the `qi-folder-hs-resolved` marker after it fills the pile —
    // CSS hides the logo when that marker is present).
    const mainSlot = folderName ? `
        <span class="qi-headshots qi-folder-hs" data-perf-names="${nameAttr}" data-folder-hs="1"></span>
        <img class="qi-studio-logo qi-folder-logo" loading="lazy" decoding="async"
             data-logo-src="/api/studio-logo?name=${enc}"
             alt="${nameAttr}" title="${nameAttr}"
             onerror="this.onerror=null; this.src='/static/icons/studio.svg?v=20260909-colours';">` : '';
    ...
    // JAV tabs know their folders map to a fixed asset — pre-set
    // that SVG and skip the favourite lookup (vice / jav release
    // folders aren't in favourite_entities).
    const isVicesTab = queueMode === 'vices';
    const isJavTab   = queueMode === 'jav';
    let folderSlot;
    if (folderName && isVicesTab) {
      folderSlot = `<img class="qi-folder-svg qi-folder-svg-vice" src="/static/icons/dir_vices.svg" alt="" aria-hidden="true">`;
    } else if (folderName && isJavTab) {
      folderSlot = `<img class="qi-folder-svg qi-folder-svg-jav" src="/static/icons/dir_jav.svg" alt="" aria-hidden="true">`;
    } else if (folderName) {
      folderSlot = `<img class="qi-folder-svg" data-folder-kind-lookup="${nameAttr}" alt="" aria-hidden="true">`;
    } else {
      folderSlot = `<img class="qi-folder-svg qi-folder-svg-neutral" src="/static/icons/dir_studio.svg" alt="" aria-hidden="true">`;
    }
    // DB-link slot: five placeholder <a> tags, one per external DB
    // we might link the performer to. All start hidden; the folder-
    // kind hydrator receives a `links` map from the lookup endpoint
    // and reveals the anchors whose site actually has a stored URL,
    // filling in `href` on the way. Order (StashDB → FansDB → TPDB →
    // JAVStash → TMDB → IAFD → Freeones → Babepedia → Coomer →
    // JAVDatabase) matches Discover Card / performer popup so users
    // see the same left-to-right sequence everywhere.
    const dbLinkSlot = folderName ? `
        <span class="qi-folder-dblinks" data-folder-dblinks="${nameAttr}">
          <a class="qi-folder-dblink" data-db="stashdb"    href="#" target="_blank" rel="noopener" title="StashDB"    onclick="event.stopPropagation()"><img src="/static/logos/stashdb.webp" alt="StashDB"></a>
          <a class="qi-folder-dblink" data-db="fansdb"     href="#" target="_blank" rel="noopener" title="FansDB"     onclick="event.stopPropagation()"><img src="/static/logos/fansdb.webp" alt="FansDB"></a>
          <a class="qi-folder-dblink" data-db="tpdb"       href="#" target="_blank" rel="noopener" title="ThePornDB" onclick="event.stopPropagation()"><img src="/static/logos/tpdb.webp" alt="TPDB"></a>
          <a class="qi-folder-dblink" data-db="javstash"   href="#" target="_blank" rel="noopener" title="JAVStash"   onclick="event.stopPropagation()"><img src="/static/logos/javstash.webp" alt="JAVStash"></a>
          <a class="qi-folder-dblink" data-db="tmdb"       href="#" target="_blank" rel="noopener" title="TMDB"       onclick="event.stopPropagation()"><img src="/static/logos/tmdb.webp" alt="TMDB"></a>
          <a class="qi-folder-dblink" data-db="iafd"       href="#" target="_blank" rel="noopener" title="IAFD"       onclick="event.stopPropagation()"><img src="/static/logos/iafd.webp" alt="IAFD"></a>
          <a class="qi-folder-dblink" data-db="freeones"   href="#" target="_blank" rel="noopener" title="Freeones"   onclick="event.stopPropagation()"><img src="/static/logos/freeones.webp" alt="Freeones"></a>
          <a class="qi-folder-dblink" data-db="babepedia"  href="#" target="_blank" rel="noopener" title="Babepedia"  onclick="event.stopPropagation()"><img src="/static/logos/babepedia.webp" alt="Babepedia"></a>
          <a class="qi-folder-dblink" data-db="coomer"     href="#" target="_blank" rel="noopener" title="Coomer"     onclick="event.stopPropagation()"><img src="/static/logos/coomer.webp" alt="Coomer"></a>
          <a class="qi-folder-dblink" data-db="javdatabase" href="#" target="_blank" rel="noopener" title="JAVDatabase" onclick="event.stopPropagation()"><img src="/static/logos/javdatabase.webp" alt="JAVDatabase"></a>
        </span>` : '';
    const isVirtual = info.virtual === 'studio';
    // Bulk manual sort needs a real on-disk folder to enumerate; a
    // virtual @studio: group is just a filter, so hide that button.
    const bulkBtn = isVirtual ? '' :
      `<button type="button" class="btn-icon" onclick="event.stopPropagation(); openBulkFolderSort('${pathAttr.replace(/'/g, "\\'")}', '${(info.name || '').replace(/'/g, "\\'")}')" title="Bulk manual sort — every file in this folder"><i class="fa-solid fa-list-check"></i></button>`;
    // Small badge on the name so the virtual tile reads clearly as
    // "alias-matched studio", not a real disk folder.
    const virtualBadge = isVirtual
      ? `<span class="qi-folder-virtual-badge" title="Alias-matched — files stay in the queue root until identified"><i class="fa-solid fa-wand-magic-sparkles"></i></span>`
      : '';
    const tileClass = isVirtual ? 'queue-item queue-item-folder queue-item-folder-virtual' : 'queue-item queue-item-folder';
    const tileTitle = isVirtual ? `Show files matched by alias to ${info.name}` : `Open ${pathAttr}`;
    return `<div class="${tileClass}" data-folder-path="${pathAttr}" onclick="queueEnterFolder('${pathAttr.replace(/'/g, "\\'")}')" role="button" tabindex="0" title="${esc(tileTitle)}">
      <div class="qi-cell qi-cell-folder-icon">
        ${folderSlot}
      </div>
      <div class="qi-cell"></div>
      <div class="qi-cell-name qi-cell-folder-name-row">
        <span class="qi-title qi-folder-name">${esc(info.name)}${virtualBadge}</span>
      </div>
      <div class="qi-cell qi-cell-headshots qi-cell-folder-main">${mainSlot}</div>
      <div class="qi-cell qi-cell-folder-status">
        <span class="qi-meta qi-folder-meta">${label}${dotsHtml}</span>
      </div>
      <div class="qi-cell"></div>
      <div class="qi-cell qi-actions qi-actions-folder">
        ${dbLinkSlot}
        ${bulkBtn}
        <button type="button" class="btn-icon" onclick="event.stopPropagation(); queueEnterFolder('${pathAttr.replace(/'/g, "\\'")}')" title="Open"><i class="fa-solid fa-arrow-right"></i></button>
      </div>
    </div>`;
  }

  function _renderQueueBreadcrumb() {
    if (_queueActiveKind !== 'scenes') return '';
    if (!_queueTreeModeActive() && !(_queueCurrentFolder && _queueCurrentFolder.startsWith('@studio:'))) return '';
    const crumbs = [`<button type="button" class="qi-crumb${_queueCurrentFolder ? '' : ' is-active'}" onclick="queueBreadcrumbGoto('')"><i class="fa-solid fa-house"></i> Root</button>`];
    if (_queueCurrentFolder && _queueCurrentFolder.startsWith('@studio:')) {
      // Virtual studio tile — one hop, name shown as the studio label
      // without the internal `@studio:` prefix leaking into the UI.
      const studio = _queueCurrentFolder.substring('@studio:'.length);
      crumbs.push('<span class="qi-crumb-sep">/</span>');
      crumbs.push(`<button type="button" class="qi-crumb is-active" title="Alias-guessed studio group"><i class="fa-solid fa-wand-magic-sparkles" style="margin-right:4px"></i>${esc(studio)}</button>`);
      return `<div class="qi-breadcrumb">${crumbs.join('')}</div>`;
    }
    const segs = _queueCurrentFolder ? _queueCurrentFolder.split('/') : [];
    let acc = '';
    for (let i = 0; i < segs.length; i++) {
      ...
      crumbs.push('<span class="qi-crumb-sep">/</span>');
      crumbs.push(`<button type="button" class="qi-crumb${last ? ' is-active' : ''}" onclick="queueBreadcrumbGoto('${safe}')">${esc(segs[i])}</button>`);
    }
    // Root has no folder to leave, and no folder rows would still be
    // useful to browse — render the crumb bar so the tree is
    // discoverable, but only when there's actually a folder to jump
    // into (either we're already inside one, or the root has subfolders).
    const hasFolders = _queueFilteredIndexes.some(item => item && item.kind === 'folder');
    if (!segs.length && !hasFolders) return '';
    return `<div class="qi-breadcrumb">${crumbs.join('')}</div>`;
  }
  ...
  function _renderQueuePage() {
    ...
    if (!files.length && !_queueFilteredIndexes.length) {
    ...
    if (!skipRebuild) {
      if (list && list.contains(sp)) _undockSearchPanel();
      const crumb = _renderQueueBreadcrumb();
      const rows = slice.map(item => {
        if (item && item.kind === 'folder') return _renderQueueFolderRow(item.info);
        return renderFn(files[item.fi], item.fi);
      }).join('');
      list.innerHTML = crumb + rows;
      if (_queueActiveKind === 'scenes') {
        _hydrateQueueHeadshots().finally(_loadQueueFolderLogos);
        _hydrateQueueFolderKinds();
      } else {
        _loadQueueFolderLogos();
      }
  ...
  function queueEnterFolder(path) {
    const next = String(path || '');
    if (_queueCurrentFolder === next) return;
    _queueCurrentFolder = next;
    _queuePage = 0;
    _recomputeQueueFilter();
    _renderQueuePage();
  }
  window.queueEnterFolder = queueEnterFolder;

  function queueBreadcrumbGoto(path) {
    queueEnterFolder(path || '');
  }
  window.queueBreadcrumbGoto = queueBreadcrumbGoto;
  ...
  async function _refreshViceSlugs() {
    ...
    try {
      ...
      // Vices tab tree backfill uses `_viceNames` — repaint if this
      // fetch resolved after the initial render so empty vice folders
      // land on the tab without waiting for the next poll tick.
      if (queueMode === 'vices' && !_queueCurrentFolder) {
        try { _recomputeQueueFilter(); _renderQueuePage(); } catch(_) {}
      }
  ...
  // Filenames the last scan was fired with — reused verbatim by the
  // alias-retry banner so the user can re-scan the same batch under
  // a different spelling without going back to the picker.
  let _iafdBulkLastFilenames = [];
  ...
  function _iafdBulkScopedFiles() {
    const files = window._queueFiles || [];
    if (_queueCurrentFolder.startsWith('@studio:')) {
      const studio = _queueCurrentFolder.slice('@studio:'.length).trim();
      return files.filter(f => f.match_guess && String(f.match_studio || '').trim() === studio);
    }
    const path = [_queueTreeRootPrefix(), _queueCurrentFolder].filter(Boolean).join('/');
    return path ? files.filter(f => String(f.filename || '').startsWith(path + '/')) : files;
  }

  function _buildIafdPerformerIndex(files = _iafdBulkScopedFiles(), folderPerformer = '') {
    const index = new Map();  // performer_name_lower -> { name, count, filenames:[] }
    for (const f of files) {
      const raw = (folderPerformer || f.performers || '').trim();
      if (!raw) continue;
  ...
  let _iafdPickerGeneration = 0;
  async function openIafdBulkPicker() {
    const generation = ++_iafdPickerGeneration;
    const files = _iafdBulkScopedFiles().slice();
    const folder = _queueCurrentFolder;
    const list = document.getElementById('qIafdPerformerList');
    window._iafdPerfEntries = [];
    list.innerHTML = '<div class="qs-perf-empty">Loading performers…</div>';
    document.getElementById('qIafdPickerModal').classList.add('open');
    let folderPerformer = '';
    if (folder && !folder.startsWith('@studio:')) {
      const names = folder.split('/').filter(Boolean).reverse();
      try {
        const response = await fetch('/api/library/favourite/lookup-batch', {
          method:'POST', credentials:'same-origin',
          headers:{'Content-Type':'application/json'}, body:JSON.stringify({names})
        });
        if (!response.ok) throw new Error('Could not identify the folder performer');
        const data = await response.json();
        folderPerformer = names.find(name => {
          const hit = (data.results || {})[name];
          return hit && (hit.kind === 'performer' || hit.kind === 'star');
        }) || '';
      } catch (_) { /* Existing cast metadata remains usable without a folder match. */ }
    }
    if (generation !== _iafdPickerGeneration) return;
    const entries = _buildIafdPerformerIndex(files, folderPerformer);
    if (!entries.length) {
      list.innerHTML = '<div class="qs-perf-empty" style="padding:18px;text-align:center">No stars detected in this folder or queue. Use a library performer folder or match a performer to these files first.</div>';
    } else {
  ...
  async function launchIafdBulkScan(entryIdx, evt) {
    ...
    // Remember the file list so the alias-retry banner can re-scan
    // the same batch under a different name without re-picking.
    _iafdBulkLastFilenames = (entry.filenames || []).slice();
  ...
  function _renderIafdBulkResults(payload) {
    ...
    if (unmatched.length) {
      baseGroups.push({
        ...
        files:       unmatched.map(fn => ({
          filename:                  fn,
          candidates:                (data.suggestions || {})[fn] || [],
          parsed_title:              '',
          parsed_site:               '',
          parsed_date:               '',
          scene_idx:                 -1,
    ...
    // No-match banner: filmography empty AND perf_url empty means the
    // scan couldn't find this performer on IAFD at all. Surface the
    // alias input so the user can retry under a different spelling
    // without closing + reopening the picker.
    const noMatch = (!_iafdFilmography.length) && !(data.perf_url || '').trim();
    const aliasBanner = document.getElementById('qIafdAliasBanner');
    if (aliasBanner) {
      if (noMatch) {
        aliasBanner.style.display = 'block';
        const nameEl = document.getElementById('qIafdAliasBannerName');
        if (nameEl) nameEl.textContent = _iafdBulkPerformer;
        const input = document.getElementById('qIafdAliasInput');
        if (input) {
          input.value = _iafdBulkPerformer;
          // Delay focus so the modal has time to paint before we
          // grab the caret — otherwise the browser can lose focus
          // to the auto-scroll of the group list.
          setTimeout(() => { try { input.focus(); input.select(); } catch(_) {} }, 60);
        }
      } else {
        aliasBanner.style.display = 'none';
      }
    }
        ...
        const fileRows = (g.files || []).map((f, fi) => {
          ...
          return `<div class="iafd-bulk-file-row" style="display:grid;grid-template-columns:22px 1fr 320px;gap:10px;align-items:center;padding:12px 18px;border-radius:0;background:linear-gradient(145deg, rgba(var(--brand-purple-rgb),0.14) 0%, rgba(var(--panel-hi-rgb),0.5) 55%, rgba(var(--panel-lo-rgb),0.35) 100%);border:none;border-top:1px solid rgba(255,255,255,0.06);box-shadow:inset 0 1px 0 rgba(255,255,255,0.06)">
            ...
            <div style="display:flex;flex-direction:column;align-items:stretch;gap:6px;min-width:0">
              ${filmInput}
              ${f.match_reasons ? `<span id="iafdMatchReason-${gi}-${fi}" style="font-size:11px;color:var(--dim)">${esc(f.match_reasons.join(' · '))}</span>` : ''}
              ${(f.candidates || []).length ? `<label style="font-size:11px;color:var(--dim)">Suggested films — choose to review
                <select class="field-input" style="width:100%;margin-top:4px" onchange="if(this.value)iafdBulkChangeFilm(${gi},${fi},this.value)">
                  <option value="">Select a candidate…</option>
                  ${f.candidates.map(c => `<option value="${esc(c.url)}">${esc(_filmLabel(c))} — ${esc(c.reasons.join(' · '))}</option>`).join('')}
                </select></label>` : ''}
  ...
  async function iafdBulkChangeFilm(gi, fi, value) {
    ...
    if (!file) return;
    const pickVersion = (file._pickVersion || 0) + 1;
    file._pickVersion = pickVersion;
    ...
    const hit = _iafdFilmography.find(f => f.url === labRaw) || _iafdFilmByLabel.get(labRaw.toLowerCase());
    if (!hit) {
    ...
    const reasonEl = document.getElementById(`iafdMatchReason-${gi}-${fi}`);
    if (reasonEl) reasonEl.textContent = 'Film selected manually';
    const bd = await _iafdBulkFetchBreakdown(hit.url);
    if (file._pickVersion !== pickVersion) return;
    ...
    const normalName = value => String(value || '').normalize('NFKD')
      .replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim();
    const matches = file._all_scenes.map((scene, index) => ({scene, index})).filter(({scene}) =>
      fnSceneNum !== null && fnSceneNum !== undefined
        ? String(scene.number) === String(fnSceneNum)
        : (scene.cast || []).some(name => normalName(name) === normalName(_iafdBulkPerformer)));
    const sceneIdx = matches.length === 1 ? matches[0].index : -1;
    _iafdBulkTicked[gi + ':' + fi] = false;
    const checkbox = document.getElementById(`iafdBulkTick-${gi}-${fi}`);
  ...
  // Alias-retry: re-run the bulk scan for the same file list under a
  // different name. Reuses the scan-progress overlay + result renderer
  // so the flow feels identical to the initial pick.
  async function iafdBulkRescanAlias() {
    if (_iafdBulkScanInFlight) return;
    const input = document.getElementById('qIafdAliasInput');
    const newName = ((input && input.value) || '').trim();
    if (!newName) {
      window.toast && window.toast('Enter an alternate name to search', { kind: 'error' });
      if (input) input.focus();
      return;
    }
    if (newName.toLowerCase() === (_iafdBulkPerformer || '').toLowerCase()) {
      window.toast && window.toast('Same name — try a different spelling / alias', { kind: 'error' });
      return;
    }
    const filenames = _iafdBulkLastFilenames.slice();
    if (!filenames.length) {
      window.toast && window.toast('Nothing to rescan — the file list was lost', { kind: 'error' });
      return;
    }
    _iafdBulkScanInFlight = true;
    _iafdBulkPerformer = newName;
    // Hide the results modal, show the scan overlay, kick off the
    // progress poll (same shape as launchIafdBulkScan).
    document.getElementById('qIafdResultsModal').classList.remove('open');
    _iafdSetScanProgress(0, filenames.length, '');
    document.getElementById('qIafdScanModal').classList.add('open');
    _iafdBulkAbortCtl = ('AbortController' in window) ? new AbortController() : null;
    const token = ++_iafdBulkProgressToken;
    _iafdPollScanProgress(token);
    try {
      const r = await fetch('/api/iafd/bulk-scan', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ performer: newName, filenames }),
        signal: _iafdBulkAbortCtl ? _iafdBulkAbortCtl.signal : undefined,
      });
      const d = await r.json();
      _iafdBulkProgressToken += 1;
      ...
      _iafdBulkScanInFlight = false;
      if (d.error) {
        window.toast && window.toast(`Rescan failed: ${d.error}`, { kind: 'error' });
        // Put the results modal back so the user can retry.
        document.getElementById('qIafdResultsModal').classList.add('open');
        return;
      }
      _renderIafdBulkResults(d);
      ...
      _iafdBulkAbortCtl = null;
      _iafdBulkScanInFlight = false;
      if (e && (e.name === 'AbortError' || (e.message || '').toLowerCase().includes('abort'))) {
        document.getElementById('qIafdResultsModal').classList.add('open');
        return;
      }
      window.toast && window.toast(`Rescan failed: ${e.message || e}`, { kind: 'error' });
      document.getElementById('qIafdResultsModal').classList.add('open');
    }
  }
  window.iafdBulkRescanAlias = iafdBulkRescanAlias;
  ...
  function closeIafdBulkResults() {
    ...
    _iafdBulkPerformer = '';
    _iafdBulkLastFilenames = [];
    const aliasBanner = document.getElementById('qIafdAliasBanner');
    if (aliasBanner) aliasBanner.style.display = 'none';
  }

  async function iafdBulkFileSelected() {
    if (_iafdBulkFiling) return;
    //: Flatten the ticked pairs into a single POST to
    //: /api/iafd/bulk-file. The backend fans the batch out onto the
    //: shared _manual_action_queue so each file drains visibly on
    //: the sidebar Sort card as it completes.
    const items = [];
    const indexMap = {};  // filename -> [gi, fi] so we can untick on submit
    const skippedNoFilm = [];
    ...
    progressEl.textContent = `Queueing ${items.length} item(s)…`;
    try {
      ...
      // All queued — close the modal immediately. The sidebar Sort
      // card takes over from here; the queue rows disappear as each
      // item files. Untick every submitted row up-front so if the
      // user re-opens the modal they see a clean slate.
      const queued = Number(kd.queued || items.length);
      for (const fn of Object.keys(indexMap)) {
        _iafdBulkTicked[`${indexMap[fn][0]}:${indexMap[fn][1]}`] = false;
      }
      // closeIafdBulkResults() bails out while _iafdBulkFiling is
      // still set (its "don't close mid-file" guard), so clear the
      // flag before requesting the close — otherwise the modal stays
      // stuck on "Queueing N items…" indefinitely.
      _iafdBulkFiling = false;
      _iafdBulkRefreshFileBtn();
      closeIafdBulkResults();
      if (typeof window.toast === 'function') {
        const missing = Array.isArray(kd.missing) ? kd.missing.length : 0;
        const msg = missing
          ? `${queued} queued (${missing} skipped — no longer in source dir)`
          : `${queued} file(s) queued for filing`;
  ...
  function _applySceneQueuePayload(d, preserveView) {
    const savedScroll = preserveView ? window.scrollY : 0;
    const savedPage = preserveView ? _queuePage : 0;
    // Partition the raw payload by scope so Scenes / Vices / JAV tabs
    // each render only their own files. Absent/'' scope defaults to
    // 'scene' (parity with the backend fallback in
    // _build_queue_snapshot).
    const raw = Array.isArray(d.files) ? d.files : [];
    const _sc = (f) => (f.scope || 'scene');
    const sceneFiles = raw.filter(f => { const s = _sc(f); return s !== 'vice' && s !== 'jav'; });
    const viceFiles  = raw.filter(f => _sc(f) === 'vice');
    const javFiles   = raw.filter(f => _sc(f) === 'jav');
    const activeFiles = (queueMode === 'vices') ? viceFiles
                      : (queueMode === 'jav')   ? javFiles
                      : sceneFiles;
    // Recompute per-scope status counts for the stat cards. `status_counts`
    // on the payload is the whole-list roll-up — split it so each tab's
    // Unmatched card reads only its own bucket.
    const _tally = (arr) => {
      const c = { total: arr.length, unmatched: 0, error: 0, no_dir: 0, pending: 0, untracked: 0, filed: 0 };
      arr.forEach(f => {
        const ps = f.prev_status;
        if (ps === 'unmatched' || ps === 'error' || ps === 'no_dir' || ps === 'pending') c[ps] += 1;
        else if (ps === 'filed') c.filed += 1;
        else c.untracked += 1;
      });
      return c;
    };
    window._sceneQueueStatusCounts = _tally(sceneFiles);
    window._viceQueueStatusCounts  = _tally(viceFiles);
    window._javQueueStatusCounts   = _tally(javFiles);
    const qc = document.getElementById('queueCount');
    if (qc) { qc.textContent = activeFiles.length ? `-${activeFiles.length}-` : ''; qc.style.color = 'var(--text)'; }
    _paintQueueStats(_queueStatsPayload || {});
    _syncHeaderQueuePill(activeFiles.length || 0);
    if (d.error) {
      document.getElementById('queueList').innerHTML = `<div class="empty">${esc(d.error)}</div>`;
      const pager = document.getElementById('queuePager');
      if (pager) pager.style.display = 'none';
    ...
    window._queueFiles = activeFiles.slice().sort(_cmpQueueFiles);
  ...
  async function loadQueue(opts) {
    ...
    // Vices mode consumes the same /api/queue payload as Scenes and
    // filters client-side on scope (see _applySceneQueuePayload). Fall
    // through to the scene path.
  ...
  /** Warm the queue payload that the active tab doesn't itself load, so
   * the per-tab counter tiles (Jav / Movies / Scenes / Vices) and the
   * Total tile can render even before the user switches tabs. Silent
   * on failure — the tile just stays as an em-dash. */
  async function _prefetchInactiveQueue() {
    const needScene = queueMode === 'movies' && !_sceneQueuePayload;
    const needMovie = queueMode !== 'movies' && !_movieQueuePayload;
    const mountGen = _queueLoadGen;
    const tasks = [];
    if (needScene) {
      tasks.push((async () => {
        try {
          ...
          window._sceneQueueStatusCounts = _tally(raw.filter(f => { const s = _sc(f); return s !== 'vice' && s !== 'jav'; }));
          window._viceQueueStatusCounts  = _tally(raw.filter(f => _sc(f) === 'vice'));
          window._javQueueStatusCounts   = _tally(raw.filter(f => _sc(f) === 'jav'));
        } catch (_) { /* silent */ }
      })());
    }
    if (needMovie) {
  ...
  const _backdropDrawers = new WeakMap();
  const _backdropResize = new ResizeObserver(entries => {
    entries.forEach(({target}) => _backdropDrawers.get(target)?.());
  });
  window.addEventListener('resize', () => {
    document.querySelectorAll('#queueList .qi-headshot-backdrop').forEach(target => {
      _backdropDrawers.get(target)?.();
    });
  });
  const _queueLogoCache = new Map();
  const _queueLogoPending = new Map();
  async function _loadQueueFolderLogos() {
    const revision = _headshotRevision;
    const images = Array.from(document.querySelectorAll('#queueList .qi-studio-logo[data-logo-src]'))
      .filter(img => !img.classList.contains('qi-folder-logo') ||
        !img.closest('.qi-cell-folder-main')?.classList.contains('qi-folder-hs-resolved'));
    const keys = [...new Set(images.map(img => img.dataset.logoSrc))];
    const missing = keys.filter(key => {
      const cached = _queueLogoCache.get(key);
      return (!cached || cached.expires <= Date.now()) && !_queueLogoPending.has(key);
    });
      ...
      const request = (async () => {
        try {
          const items = chunk.map(key => {
            ...
            return {key, name: url.searchParams.get('name') || '', q: url.searchParams.get('q') || ''};
          });
          const response = await fetch('/api/studio-logos/lookup', {
            method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({items})
          });
          if (!response.ok) throw new Error('Logo lookup failed');
          const data = await response.json();
          if (revision !== _headshotRevision) return;
          for (const key of chunk) {
            if (!Object.prototype.hasOwnProperty.call(data.logos || {}, key)) continue;
            const url = data.logos[key];
            if (_queueLogoCache.size >= 2048) _queueLogoCache.delete(_queueLogoCache.keys().next().value);
            _queueLogoCache.set(key, {url, expires: Date.now() + (url ? 60000 : 5000)});
          }
        } catch (error) {
          console.warn('Queue logo batch:', error);
        } finally {
          if (revision === _headshotRevision) chunk.forEach(key => _queueLogoPending.delete(key));
        }
      })();
      chunk.forEach(key => _queueLogoPending.set(key, request));
    }
    await Promise.all(keys.map(key => _queueLogoPending.get(key)).filter(Boolean));
    if (revision !== _headshotRevision) return;
    images.forEach(img => {
      if (!img.isConnected) return;
      const key = img.dataset.logoSrc;
      if (!key) return; // Another overlapping batch already applied this image.
      const cached = _queueLogoCache.get(key);
      // Preserve the individual endpoint as a fallback if the batch fails.
      img.src = cached ? (cached.url || img.dataset.logoFallback || '/static/icons/studio.svg?v=20260909-colours') : key;
      delete img.dataset.logoSrc;
    });
  }
  // Decorative copy stays outside the foreground crop and its fade.
  function _syncQueueHeadshotBackdrop(slot) {
    const cell = slot.matches('.qi-headshots') ? slot.parentElement : slot;
    if (!cell) return;
    const source = cell.querySelector('.qi-headshot');
    const sourceUrl = source ? source.getAttribute('src') : '/static/img/missing.webp';
    let backdrop = cell.querySelector('.qi-headshot-backdrop');
    if (!backdrop) {
      backdrop = document.createElement('span');
      backdrop.className = 'qi-headshot-backdrop';
      ...
      const drawMosaic = () => {
        ...
        // Use whole device pixels for both spacing and gutters. Scaling
        // a CSS-resolution canvas at 125% made every fourth gap wider.
        const pitch = Math.max(2, Math.round(10 * ratio));
        const gap = Math.max(1, Math.round(ratio));
        const sample = document.createElement('canvas');
        sample.width = Math.ceil(width / pitch);
        sample.height = Math.ceil(height / pitch);
        const sampleCtx = sample.getContext('2d');
        if (!sampleCtx) return;
        const scale = backdrop.classList.contains('is-missing')
        ...
        sampleCtx.drawImage(img, (width - imageWidth) / (2 * pitch),
          (height - imageHeight) / (2 * pitch), imageWidth / pitch, imageHeight / pitch);
        canvas.width = width;
        ...
        ctx.drawImage(sample, 0, 0, sample.width * pitch, sample.height * pitch);
        for (let x = pitch - gap; x < width; x += pitch) ctx.clearRect(x, 0, gap, height);
        for (let y = pitch - gap; y < height; y += pitch) ctx.clearRect(0, y, width, gap);
        backdrop.classList.add('is-pixelated');
      };
      img.onload = drawMosaic;
      _backdropDrawers.set(backdrop, drawMosaic);
      img.onerror = () => { backdrop.style.display = 'none'; };
      backdrop.append(img, canvas);
      cell.prepend(backdrop);
    }
    _backdropResize.observe(backdrop);
  ...
  async function _hydrateQueueHeadshots() {
    _backdropResize.disconnect();
    document.querySelectorAll('#queueList .qi-cell-headshots').forEach(_syncQueueHeadshotBackdrop);
    const mountGen = _queueLoadGen;
    ...
    slots.forEach(slot => {
      ...
      _syncQueueHeadshotBackdrop(slot);
      // Folder-row headshot slots carry a `data-folder-hs` marker.
      // When the pile resolves to at least one avatar, tag the
      // parent cell with `qi-folder-hs-resolved` so CSS can hide
      // the sibling studio-logo — headshots always win the slot
      // when both would otherwise render.
      if (slot.dataset.folderHs && imgs.length) {
        const cell = slot.closest('.qi-cell-folder-main');
        if (cell) cell.classList.add('qi-folder-hs-resolved');
      }
    });
  }

  //: Classify each queue folder as studio vs performer by hitting
  //: `/api/library/favourite/lookup`, then stamp `.is-studio` /
  //: `.is-star` on the type-badge placeholder and light up the
  //: DB-link anchors in the actions cell for every DB the row is
  //: linked to. Results cache in `_folderKindCache` for the life of
  //: the page — folders won't be re-classified on every repaint.
  const _folderKindCache = new Map();  // name.toLowerCase() → { kind, links }
  function _applyFolderDbLinks(row, links) {
    if (!row) return;
    const container = row.querySelector('.qi-folder-dblinks');
    if (!container) return;
    const anchors = container.querySelectorAll('.qi-folder-dblink[data-db]');
    let shown = 0;
    anchors.forEach(a => {
      const db = a.getAttribute('data-db');
      const url = links && links[db];
      if (url) {
        a.href = url;
        a.style.display = '';
        shown += 1;
    ...
    container.classList.toggle('qi-folder-dblinks-empty', shown === 0);
  }
  async function _hydrateQueueFolderKinds() {
    const mountGen = _queueLoadGen;
    const slots = Array.from(document.querySelectorAll('#queueList .qi-folder-svg[data-folder-kind-lookup]'));
    if (!slots.length) return;
    // Group slots by name; dedupe fetches.
    const byName = new Map();
    slots.forEach(slot => {
      const raw = slot.dataset.folderKindLookup || '';
      const key = raw.trim().toLowerCase();
      if (!key) return;
      if (!byName.has(key)) byName.set(key, { display: raw.trim(), slots: [] });
      byName.get(key).slots.push(slot);
    });
    const apply = (kind, links, slot) => {
      slot.classList.remove('is-studio', 'is-star', 'is-unknown');
      if (kind === 'studio') {
        slot.classList.add('is-studio');
        slot.src = '/static/icons/dir_studio.svg';
      } else if (kind === 'performer' || kind === 'star') {
        slot.classList.add('is-star');
        slot.src = '/static/icons/dir_star.svg';
      } else {
        slot.classList.add('is-unknown');
        // Unknown → fall back to the studio dir SVG so the row still
        // has a folder graphic; there's no generic "unknown" asset.
        slot.src = '/static/icons/dir_studio.svg';
      }
      slot.removeAttribute('data-folder-kind-lookup');
      // Reveal DB-link anchors on the same row for every site the
      // favourites row has an id/url stored for.
      const row = slot.closest('.queue-item-folder');
      _applyFolderDbLinks(row, links || {});
    };
    //: Batch every uncached name into one POST — the per-name endpoint
    //: fired up to 25 concurrent GETs, which saturated Chrome's HTTP/1.1
    //: pool for this origin and stalled navigation away from /unmatched
    //: (measured 5.5 s in a DevTools trace, 2026-08-31). One round-trip
    //: keeps the connection pool free for the browser's next document
    //: request.
    const needFetch = [];
    for (const [key, ent] of byName.entries()) {
      const cached = _folderKindCache.get(key);
      if (cached) {
        ent.slots.forEach(s => apply(cached.kind, cached.links, s));
        continue;
      }
      needFetch.push({ key, ent });
    }
    if (!needFetch.length) return;
    let results = {};
    try {
      const r = await fetch('/api/library/favourite/lookup-batch', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'same-origin',
        body: JSON.stringify({ names: needFetch.map(x => x.ent.display) }),
      });
      if (mountGen !== _queueLoadGen) return;
      if (r.ok) {
        const d = await r.json();
    ...
    } catch (_) { /* fall through — treat as all-unknown below. */ }
    for (const { key, ent } of needFetch) {
      const row = results[ent.display];
      const kind = (row && row.kind === 'studio') ? 'studio'
                 : (row && row.kind === 'performer') ? 'performer'
                 : 'unknown';
      const links = (row && row.links) || {};
      _folderKindCache.set(key, { kind, links });
      ent.slots.forEach(s => apply(kind, links, s));
    }
  }
  ...
  function _paintQueueStats(data) {
    ...
    // Per-tab queue count comes from the tab's live queue payload
    // (status_counts.total). This is the same source the queue list
    // itself renders from, so tile numbers and list length agree.
    const sceneStatus = window._sceneQueueStatusCounts || null;
    const movieStatus = window._movieQueueStatusCounts || null;
    const viceStatus  = window._viceQueueStatusCounts  || null;
    const javStatus   = window._javQueueStatusCounts   || null;
    const sceneTotal = sceneStatus ? (sceneStatus.total || 0) : null;
    const movieTotal = movieStatus ? (movieStatus.total || 0) : null;
    const viceTotal  = viceStatus  ? (viceStatus.total  || 0) : null;
    const javTotal   = javStatus   ? (javStatus.total   || 0) : null;
    setText('statScenes', sceneTotal != null ? sceneTotal : '—');
    setText('statMovies', movieTotal != null ? movieTotal : '—');
    setText('statVices',  viceTotal  != null ? viceTotal  : '—');
    setText('statJav',    javTotal   != null ? javTotal   : '—');
    // Total tile: sum whatever payloads have arrived. Everything null
    // (first paint before any queue endpoint returns) → em-dash.
    const parts = [sceneTotal, movieTotal, viceTotal, javTotal].filter(v => v != null);
    setText('statTotal', parts.length ? parts.reduce((a, b) => a + b, 0) : '—');
    // Errors folded into a single tile for the active tab (error + no_dir).
    // Prefer the live queue payload so the tile stays in lockstep with
    // the filter pill; the /api/stats aggregate is a first-paint fallback.
    const activeStatus = (queueMode === 'movies' ? movieStatus
                        : queueMode === 'vices'  ? viceStatus
                        : queueMode === 'jav'    ? javStatus
                        :                          sceneStatus) || null;
    if (activeStatus) {
      setText('statErrors', (activeStatus.error || 0) + (activeStatus.no_dir || 0));
    } else if (data.scenes || data.movies || data.vices || data.jav) {
      const activeStats = (queueMode === 'movies' ? (data.movies || {})
                         : queueMode === 'vices'  ? (data.vices  || {})
                         : queueMode === 'jav'    ? (data.jav    || {})
                         :                          (data.scenes || {}));
      setText('statErrors', (activeStats.errors || 0) + (activeStats.no_dir || 0));
    }
    // Filed + Suggested come from the /api/stats endpoints. Skip if
    // this call is queue-payload-only (data has no stats keys) so we
    // don't clobber the last good stats-derived paint with zeros.
    if (data.scenes || data.movies || data.vices || data.jav) {
      const activeStats = (queueMode === 'movies' ? (data.movies || {})
                         : queueMode === 'vices'  ? (data.vices  || {})
                         : queueMode === 'jav'    ? (data.jav    || {})
                         :                          (data.scenes || {}));
      setText('statFiled', activeStats.filed || 0);
    }
    if (data.sugg) setText('statSuggested', data.sugg.count || 0);
  }
  ...
  async function loadQueueStats() {
    ...
    const failed = [];
    // Try the aggregate dashboard endpoint first — one round-trip
    // instead of six. Falls back to the per-tile fan-out if it 404s
    // (older backend) or errors.
    try {
      const r = await fetch('/api/queue/dashboard', { credentials: 'same-origin' });
      if (r.ok) {
        const d = await r.json();
        payload.scenes = d.scenes || {};
        payload.movies = d.movies || {};
        payload.vices  = d.vices  || {};
        payload.wanted = d.wanted || { items: [] };
        payload.sugg   = d.sugg   || { count: 0 };
        payload.dup    = d.dup    || { groups: 0, files: 0 };
        paintIfMounted();
        if (mountGen !== _queueLoadGen) return;
        _queueStatsErrorActive = false;
        return;
      }
      // Non-2xx: fall through to per-tile fan-out.
    ...
    await Promise.all([
      ...
      fetchInto('/api/vices/stats', 'vices', {}),
  ...
  function _markQueueStatsStale(failed) {
    ...
    for (const f of failed) {
      if (f.key === 'scenes' && queueMode === 'scenes') {
        dashTiles.add('statFiled'); dashTiles.add('statErrors');
      } else if (f.key === 'movies' && queueMode === 'movies') {
        dashTiles.add('statFiled'); dashTiles.add('statErrors');
      } else if (f.key === 'vices' && queueMode === 'vices') {
        dashTiles.add('statFiled'); dashTiles.add('statErrors');
      } else if (f.key === 'jav' && queueMode === 'jav') {
        dashTiles.add('statFiled'); dashTiles.add('statErrors');
      } else if (f.key === 'sugg') {
  ...
  let _manualPreviewObserver;
  function _fitManualPreview() {
    const body = document.getElementById('qManualExtrasBody');
    const frame = document.getElementById('mThumbFrame');
    if (!body || !frame) return;
    const column = frame.parentElement;
    const fit = () => {
      if (!body.clientHeight || !body.clientWidth) return;
      ...
      const height = Math.max(0, body.clientHeight - parseFloat(style.paddingTop) - parseFloat(style.paddingBottom) - used - 2);
      const size = Math.max(0, Math.min(width, height * 16 / 9));
      frame.style.width = `${size}px`;
      frame.style.height = `${size * 9 / 16}px`;
    };
    if (!_manualPreviewObserver) {
      _manualPreviewObserver = new ResizeObserver(fit);
      _manualPreviewObserver.observe(body);
      for (const child of column.children) if (child !== frame) _manualPreviewObserver.observe(child);
    }
    fit();
  }
  ...
  async function openManualSearch(filename) {
    ...
    // Start the cached filmstrip request before metadata and scene search.
  ...
  function toggleQSearchSource(btn) {
    ...
    _qsSaveSources(_qsCurrentSources().filter(source => source !== 'IAFD'));
  }
  ...
  async function runQueueSearch() {
    ...
    // Consume IAFD opt-in for this request only, even if the request fails.
    document.querySelectorAll('.qs-source-pill[data-source="IAFD"]').forEach(p => {
    ...
    try {
      const r = await fetch('/api/search', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(payload), signal: AbortSignal.timeout(20000) });
      ...
      const d = await r.json();
      if (mySeq !== _qRunSearchSeq) return;
      if (!r.ok || d.error) { resultsEl.innerHTML = `<div class="empty">${esc(d.error || 'Search failed')}</div>`; return; }
      const sourceWarnings = (d.warnings || []).join(' · ');
      if (sourceWarnings && typeof window.toast === 'function') {
        window.toast(sourceWarnings + ' Other selected sources were searched independently.', { kind: 'info' });
      }
      ...
      let combined = Array.isArray(d.results) ? d.results : [];
      ...
      if (!combined.length) { resultsEl.innerHTML = `<div class="empty">${sourceWarnings ? 'No matches returned by the available sources. Some sources were unavailable; see sidebar alerts. Try removing the date or studio filter.' : 'No results found. Try removing the date or studio filter.'}</div>`; return; }
  ...
  function _mRenderMatchBanner(source, externalId, statusMsg) {
    ...
    const linkHtml = url ? `<a href="${esc(url)}" target="_blank" rel="noopener noreferrer" aria-label="Open matched ${esc(label)} record" title="Open matched ${esc(label)} record" onclick="event.stopPropagation()">${logoHtml}</a>` : logoHtml;
    el.innerHTML = `<span class="banner-label">Matched</span>${linkHtml}`;
    el.title = statusMsg || '';
  ...
  async function _submitScenePick(btn, origHtml) {
    ...
    try {
      const d = await _postManualFileWithRetry(
        { filename: _qConfirmFilename, scene },
        (attempt, secondsElapsed) => {
          if (btn) btn.innerHTML = `<i class="fa-solid fa-clock"></i> Waiting for pipeline… ${secondsElapsed}s`;
          if (attempt === 1) window.toast('Pipeline busy — your file will be sorted automatically');
        },
  ...
  // ── Bulk manual sort (folder-scoped) ────────────────────────────
  // Opens against a queue subdirectory. Lists every pending file under
  // that folder with the same 5-frame filmstrip we render in the
  // popup, plus editable Title / Date / Additional performers fields.
  // Directory name (auto-classified server-side as studio or performer)
  // is the routing anchor — no source-DB lookup runs.
  let _bulkFolderState = { folder: '', name: '', rows: [], tokens: [] };

  async function openBulkFolderSort(folderPath, folderName) {
    const modal = document.getElementById('bulkFolderModal');
    if (!modal) return;
    _bulkFolderTeardownPolls();

    const path = (folderPath || '').trim();
    const name = (folderName || path.split('/').pop() || '').trim();

    const files = window._queueFiles || [];
    // Vices tab tree-mode roots at `Vices/` — the folder path passed
    // in is user-facing (e.g. "Cum Haters"), but files carry the full
    // on-disk rel (`Vices/Cum Haters/foo.mp4`). Prepend the mode's
    // root prefix so parent comparisons line up with the raw rel.
    const rootPrefix = (typeof _queueTreeRootPrefix === 'function') ? _queueTreeRootPrefix() : '';
    const effectivePath = rootPrefix
      ? (path ? rootPrefix + '/' + path : rootPrefix)
      : path;
    const scopePrefix = effectivePath ? effectivePath + '/' : effectivePath;
    // Only pending / unmatched / errored / no_dir files — filed rows
    // aren't in the queue any more, so status filter is really "not
    // already gone through the pipeline as a match."
    const items = files.filter(f => {
      const fn = f && f.filename || '';
      if (!fn) return false;
      if (effectivePath && !(fn === effectivePath || fn.startsWith(scopePrefix))) return false;
      const parent = _queueFileParent(fn);
      return parent === effectivePath;
    });

    _bulkFolderState = { folder: path, name, rows: [], tokens: [] };

    document.getElementById('bulkFolderName').textContent = name || path || 'Root';
    const kindBadge = document.getElementById('bulkFolderKindBadge');
    if (kindBadge) kindBadge.textContent = 'Detecting…';
    const statusEl = document.getElementById('bulkFolderStatus');
    if (statusEl) statusEl.textContent = '';

    const listEl = document.getElementById('bulkFolderList');
    const emptyEl = document.getElementById('bulkFolderEmpty');
    if (!items.length) {
    ...
    // Render row skeletons first so the modal paints instantly, then
    // fill in per-row date + filmstrip async without blocking the UI.
    const rowsHtml = items.map((f, i) => {
      const fn = f.filename || '';
      const base = fn.split('/').pop() || fn;
      const guessTitle = base.replace(/\.[^.\\/]+$/, '');
      const safeIdx = 'bf_' + i;
      const stripSlots = Array.from({length: 5}, () =>
        '<div class="qfs-thumb"><div class="qfs-thumb-empty"><i class="ts-icon-scenes" aria-hidden="true"></i></div></div>'
      ).join('');
      return `<div class="bulk-folder-row is-selected" data-row-idx="${i}" data-filename="${esc(fn)}">
        <div class="bulk-folder-check"><input type="checkbox" checked onchange="bulkFolderToggleRow(${i}, this.checked)"></div>
        <div class="bulk-folder-strip-col">
          <div class="bulk-folder-strip" data-strip="${i}">${stripSlots}</div>
          <div class="bulk-folder-filename" title="${esc(fn)}">${esc(base)}</div>
        </div>
        <div class="bulk-folder-fields">
          <div class="field-row"><label class="field-label">Title *</label><input class="field-input" data-field="title" data-idx="${i}" type="text" value="${esc(guessTitle)}" placeholder="Scene title"></div>
          <div class="field-row-split">
            <div class="field-row"><label class="field-label">Additional performers</label><input class="field-input" data-field="performers" data-idx="${i}" type="text" placeholder="Name, Name" autocomplete="off" spellcheck="false"></div>
            <div class="field-row"><label class="field-label">Date *</label><input class="field-input" data-field="date" data-idx="${i}" type="text" placeholder="YYYY-MM-DD"></div>
          </div>
          <div class="bulk-folder-row-status" id="${safeIdx}_status"></div>
        </div>
      </div>`;
    }).join('');
    listEl.innerHTML = rowsHtml;

    _bulkFolderState.rows = items.map(f => ({
      filename: f.filename,
      selected: true,
      status: '',
      done: false,
    ...
    modal.classList.add('open');
    _bulkFolderUpdateStatusLine();
    _bulkFolderRefreshSelectToggle();

    // Classify the folder (studio vs performer) — cheap ping. Cache it
    // via the badge so the user can see how the file will be routed
    // before submitting.
    _bulkFolderClassifyFolder(name || path);

    // Kick off per-file date + filmstrip loads. Bounded parallelism
    // keeps the source-file mtime probes and ffmpeg seeks from
    // saturating the backend when the folder has many entries.
    _bulkFolderPopulateAsync(items);
  }

  async function _bulkFolderClassifyFolder(folderName) {
    const badge = document.getElementById('bulkFolderKindBadge');
    if (!badge) return;
    const n = (folderName || '').trim();
    if (!n) { badge.textContent = 'PERFORMER'; return; }
    // Vice folders win before the favourites lookup — if the folder
    // name matches a configured vice (case-insensitive), the bulk
    // submit is going to file under that vice regardless of what
    // /api/library/favourite/lookup would say.
    try {
      await _refreshViceSlugs();
    } catch(_) {}
    if (Array.isArray(_viceNames) && _viceNames.some(v => (v || '').trim().toLowerCase() === n.toLowerCase())) {
      badge.textContent = 'VICE';
      return;
    }
    // Piggy-back on /api/studio-logo — 200 means we have a studio logo
    // for this name; 404 = fall back to performer. Cheap and cached.
    // (Backend also classifies definitively at submit time.)
    try {
      const r = await fetch('/api/library/favourite/lookup?name=' + encodeURIComponent(n), { credentials: 'same-origin' });
      if (r.ok) {
        const d = await r.json();
        const kind = ((d && d.kind) || '').toLowerCase();
    ...
    badge.textContent = 'PERFORMER';
  }

  async function _bulkFolderPopulateAsync(items) {
    const CONCURRENCY = 4;
    let cursor = 0;
    const worker = async () => {
      while (cursor < items.length) {
        ...
        // Date first — cheap, non-blocking.
        try {
          const r = await fetch('/api/queue/file-mtime?filename=' + encodeURIComponent(f.filename), { credentials: 'same-origin' });
          if (r.ok) {
            const d = await r.json();
            const dateInput = document.querySelector(`.bulk-folder-modal input[data-field="date"][data-idx="${idx}"]`);
            if (dateInput && !dateInput.value) dateInput.value = d.date || '';
          }
        } catch(_) {}
        // Filmstrip — poll the same endpoint the standalone popup
        // uses so cached frames come back instantly.
        _bulkFolderLoadFilmstrip(idx, f.filename);
      }
    };
  ...
  function _bulkFolderTeardownPolls() {
    // Cancel any in-flight filmstrip polls so reopening on a different
    // folder doesn't race an old tokens list into the DOM.
    _bulkFolderState.tokens = (_bulkFolderState.tokens || []).map(() => 0);
  }

  async function _bulkFolderLoadFilmstrip(rowIdx, filename) {
    const stripEl = document.querySelector(`.bulk-folder-modal .bulk-folder-strip[data-strip="${rowIdx}"]`);
    if (!stripEl) return;
    const myToken = Date.now() + Math.random();
    _bulkFolderState.tokens[rowIdx] = myToken;

    const paint = (d) => {
      if (_bulkFolderState.tokens[rowIdx] !== myToken) return;
      const thumbs = Array.isArray(d.thumbs) ? d.thumbs : [];
      const byIdx = new Map(thumbs.map(t => [t.i, t]));
      const slotsHtml = [];
      for (let i = 0; i < 5; i++) {
        const t = byIdx.get(i);
        if (t) {
          slotsHtml.push(`<div class="qfs-thumb is-clickable" onclick="openQueueFilmstrip('${filename.replace(/'/g, "\\'")}')" title="Enlarge in filmstrip popup"><img src="${esc(t.url)}" alt="Frame ${t.i + 1}" loading="lazy"></div>`);
        } else if (d.generating) {
          slotsHtml.push('<div class="qfs-thumb"><div class="qfs-thumb-empty" title="Generating frame…"><span class="loader loader--btn loader--muted" role="status" aria-label="Generating"></span></div></div>');
        } else if (d.backoff) {
          slotsHtml.push('<div class="qfs-thumb"><div class="qfs-thumb-empty" title="ffmpeg gave up"><i class="fa-solid fa-triangle-exclamation"></i></div></div>');
        } else {
          slotsHtml.push('<div class="qfs-thumb"><div class="qfs-thumb-empty"><i class="ts-icon-scenes" aria-hidden="true"></i></div></div>');
        }
      }
      stripEl.innerHTML = slotsHtml.join('');
    };
    ...
    try {
      while (true) {
        ...
        paint(d);
        if (d.ready || d.backoff || !d.generating) return;
        if (Date.now() - startMs > POLL_TIMEOUT_MS) { paint({ ...d, backoff: true, generating: false }); return; }
        await new Promise(res => setTimeout(res, POLL_INTERVAL_MS));
      }
    } catch(_) { /* silent */ }
  }

  function bulkFolderToggleRow(idx, checked) {
    const row = document.querySelector(`.bulk-folder-modal .bulk-folder-row[data-row-idx="${idx}"]`);
    if (row) row.classList.toggle('is-selected', !!checked);
    if (_bulkFolderState.rows[idx]) _bulkFolderState.rows[idx].selected = !!checked;
    _bulkFolderUpdateStatusLine();
    _bulkFolderRefreshSelectToggle();
  }

  function bulkFolderSelectAll(on) {
    const modal = document.getElementById('bulkFolderModal');
    if (!modal) return;
    modal.querySelectorAll('.bulk-folder-check input[type="checkbox"]').forEach(cb => {
      cb.checked = !!on;
      const row = cb.closest('.bulk-folder-row');
      if (row) row.classList.toggle('is-selected', !!on);
    });
    (_bulkFolderState.rows || []).forEach(r => { r.selected = !!on; });
    _bulkFolderUpdateStatusLine();
    _bulkFolderRefreshSelectToggle();
  }

  // Single toggle button flips between select-all and clear-all based
  // on whether any selectable rows are currently ticked. Rows that
  // already queued (is-processed) are out of scope on both sides.
  function bulkFolderToggleSelectAll() {
    const rows = (_bulkFolderState.rows || []).filter(r => !r.done);
    const anySelected = rows.some(r => r.selected);
    bulkFolderSelectAll(!anySelected);
  }

  function _bulkFolderRefreshSelectToggle() {
    const btn = document.getElementById('bulkFolderSelectToggle');
    if (!btn) return;
    const rows = (_bulkFolderState.rows || []).filter(r => !r.done);
    const anySelected = rows.some(r => r.selected);
    if (anySelected) {
      btn.innerHTML = '<i class="fa-solid fa-square"></i><span class="btn-label" style="margin-left:6px">None</span>';
      btn.title = 'Deselect every file';
    } else {
      btn.innerHTML = '<i class="fa-solid fa-check-double"></i><span class="btn-label" style="margin-left:6px">All</span>';
      btn.title = 'Select every file';
    }
  }

  // Auto-name: sequential 4-digit numbers in row order. Only rewrites
  // rows that haven't already been queued (is-processed) — those are
  // out the door, no point relabelling them. Zero-pads to 4 digits
  // (0001…9999) — big enough for any realistic folder, small enough
  // that a two-digit user tweak still reads cleanly.
  function bulkFolderAutoName() {
    const modal = document.getElementById('bulkFolderModal');
    if (!modal) return;
    const rows = Array.from(modal.querySelectorAll('.bulk-folder-row'))
      .filter(r => !r.classList.contains('is-processed'));
    if (!rows.length) return;
    rows.forEach((row, i) => {
      const idx = row.getAttribute('data-row-idx');
      const titleEl = modal.querySelector(`input[data-field="title"][data-idx="${idx}"]`);
      if (!titleEl) return;
      titleEl.value = String(i + 1).padStart(4, '0');
      titleEl.classList.remove('field-invalid');
    });
  }

  function _bulkFolderUpdateStatusLine() {
    const rows = _bulkFolderState.rows || [];
    const sel = rows.filter(r => r.selected).length;
    const el = document.getElementById('bulkFolderStatus');
    if (el) el.textContent = `${sel} of ${rows.length} selected`;
  }

  function closeBulkFolderSort() {
    const modal = document.getElementById('bulkFolderModal');
    if (modal) modal.classList.remove('open');
    _bulkFolderTeardownPolls();
    _bulkFolderState = { folder: '', name: '', rows: [], tokens: [] };
  }

  async function submitBulkFolderSort() {
    const rows = _bulkFolderState.rows || [];
    if (!rows.length) { closeBulkFolderSort(); return; }
    const modal = document.getElementById('bulkFolderModal');
    if (!modal) return;

    // Collect selected rows + validate. Highlight any invalid field
    // in-place so the user sees what to fix without a toast maze.
    const payloadItems = [];
    let firstBad = null;
    modal.querySelectorAll('.field-input.field-invalid').forEach(el => el.classList.remove('field-invalid'));

    rows.forEach((r, idx) => {
      if (!r.selected || r.done) return;
      const titleEl = modal.querySelector(`input[data-field="title"][data-idx="${idx}"]`);
      const dateEl  = modal.querySelector(`input[data-field="date"][data-idx="${idx}"]`);
      const perfEl  = modal.querySelector(`input[data-field="performers"][data-idx="${idx}"]`);
      const title = (titleEl && titleEl.value || '').trim();
      const date  = (dateEl && dateEl.value || '').trim();
      const perfs = (perfEl && perfEl.value || '').trim();
      if (!title) {
        if (titleEl) titleEl.classList.add('field-invalid');
        if (!firstBad) firstBad = titleEl;
        return;
      }
      if (!date || !/^\d{4}-\d{2}-\d{2}$/.test(date)) {
        if (dateEl) dateEl.classList.add('field-invalid');
      ...
      payloadItems.push({
        _row: idx,
        filename: r.filename,
        title, date, performers: perfs,
      });
    });

    if (firstBad) {
      firstBad.focus();
      window.toast && window.toast('Fix the highlighted field(s) before processing', { kind: 'error' });
      return;
    }
    if (!payloadItems.length) {
      window.toast && window.toast('Nothing to process — tick at least one row', { kind: 'error' });
      return;
    }

    const btn = document.getElementById('bulkFolderSubmitBtn');
    const origHtml = btn ? btn.innerHTML : '';
    if (btn) { btn.disabled = true; btn.innerHTML = '<span class="loader loader--btn" role="status" aria-label="Loading"></span> <span class="btn-label">Queueing…</span>'; }

    try {
      const r = await fetch('/api/file/manual/bulk-folder', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'same-origin',
        body: JSON.stringify({
          folder: _bulkFolderState.name || _bulkFolderState.folder,
          items: payloadItems.map(({_row, ...rest}) => rest),
        }),
      });
      let d = {};
      try { d = await r.json(); } catch(_) { d = {}; }
      if (!r.ok || d.error) {
        window.toast && window.toast(`Bulk sort failed: ${d.error || (r.status + ' ' + r.statusText)}`, { kind: 'error' });
        if (btn) { btn.disabled = false; btn.innerHTML = origHtml; }
        return;
      }

      const kind = (d.kind || 'performer').toUpperCase();
      const folderLabel = _bulkFolderState.name || _bulkFolderState.folder;
      const errCount = Array.isArray(d.errors) ? d.errors.length : 0;

      // Everything went into the manual-action queue on the backend —
      // no reason to keep the modal open waiting for the pipeline to
      // drain. Close immediately, surface the queued count, and let
      // the queue rows themselves track per-file progress.
      if (btn) { btn.disabled = false; btn.innerHTML = origHtml; }
      closeBulkFolderSort();
      ...
      if (errCount) {
        ...
        window.toast && window.toast(
          `${d.queued || 0} queued as ${kind} → ${folderLabel} (${errCount} skipped: ${(firstErr && firstErr.error) || 'error'})`,
          { kind: 'warn' }
        );
      } else {
        window.toast && window.toast(
          `${d.queued || 0} file(s) queued as ${kind} → ${folderLabel}`,
          { kind: 'success' }
        );
      }

      // Give the queue a beat to see the new manual-action state, then
      // refresh so the folder row's status dots reflect the new work.
      setTimeout(() => {
        if (typeof loadQueue === 'function') loadQueue({ preserveView: true, force: true });
        if (typeof loadQueueStats === 'function') loadQueueStats();
      }, 600);
      // Kick the sidebar activity banner so its Sort progress card
      // paints with the freshly queued items without waiting on the
      // next tick.
      if (window.TsActivity && window.TsActivity.refresh) window.TsActivity.refresh();
    } catch (e) {
      console.error('bulk-folder submit failed:', e);
      window.toast && window.toast('Bulk sort failed: ' + (e.message || e), { kind: 'error' });
      if (btn) { btn.disabled = false; btn.innerHTML = origHtml; }
    }
  }

  window.openBulkFolderSort   = openBulkFolderSort;
  window.closeBulkFolderSort  = closeBulkFolderSort;
  window.submitBulkFolderSort = submitBulkFolderSort;
  window.bulkFolderSelectAll  = bulkFolderSelectAll;
  window.bulkFolderToggleSelectAll = bulkFolderToggleSelectAll;
  window.bulkFolderToggleRow  = bulkFolderToggleRow;
  window.bulkFolderAutoName   = bulkFolderAutoName;
...
/* Block 2/3: queue duplicates modal. */
  ...
  async function loadSuggestions() {
    ...
    try {
      ...
      const d = await r.json();
      _suggestionsFiles = (d.files || []).slice().sort((a,b) => {
        const score = file => Math.max(0,...(file.candidates || []).map(c => Number(c.score) || 0));
  ...
  function _renderSuggestionsPanel(f) {
    ...
    const sourcePathSafe = esc(f.source_path || f.filename);
    ...
    return `
      <div class="suggestion-file" data-filename="${fnameSafe}">
        ...
        <div style="display:flex;align-items:center;gap:12px;margin-bottom:14px">
          ...
          <div style="flex:1;min-width:0">
            <div style="font-family:var(--secs);font-size:16px;color:var(--text);letter-spacing:0.02em;white-space:normal;overflow-wrap:anywhere" title="${sourcePathSafe}">${sourcePathSafe}</div>
...
(function relocateModalOverlays() {
  ...
  if (window.tsOnLibraryChange) window.tsOnLibraryChange(async () => {
    _headshotRevision += 1;
    _headshotCache.clear();
    _folderKindCache.clear();
    _queueLogoCache.clear();
    _queueLogoPending.clear();
    _queueLoadGen += 1;
    _sceneQueuePayload = null;
    _movieQueuePayload = null;
    if (typeof TS_DL_PAGE !== 'undefined' && TS_DL_PAGE === 'queue') {
      await loadQueue({preserveView:true, force:true});
      await _prefetchInactiveQueue();
    }
    await _hydratePerfPiles(document);
  });
