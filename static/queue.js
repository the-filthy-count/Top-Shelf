 ...
 * Block 1: main queue + Prowlarr search overlay + IAFD picker. */
  ...
  async function runQueueSearch() {
    ...
    try {
      ...
      const sourceWarnings = (d.warnings || []).map(esc).join(' · ');
      ...
      if (!combined.length) { resultsEl.innerHTML = `<div class="empty">${sourceWarnings || 'No results found. Try removing the date or studio filter.'}</div>`; return; }
      window._qSearchResults = combined;
      if (sourceWarnings) queueMicrotask(() => {
        if (mySeq !== _qRunSearchSeq) return;
        const notice = document.createElement('div');
        notice.className = 'empty';
        notice.textContent = (d.warnings || []).join(' · ');
