...
from media_budget import media_slot
...
# Shared across both hash jobs: avoid 50 simultaneous decoders.
_SPRITE_CONCURRENCY = 2
...
def get_video_duration(video_path: Path) -> float:
    ...
    2. ``stream=duration`` — per-stream header duration.
    3. ``packet=pts_time`` — bounded last-resort packet scan.
    ...
    # Header probes are cheap; only scan packets as a bounded last resort.
    strategies = [
        (["-show_entries", "format=duration"], 20),
        (["-select_streams", "v:0", "-show_entries", "stream=duration"], 20),
        (["-select_streams", "v:0", "-show_entries", "packet=pts_time"], 30),
    ]
...
def get_sprite_screenshot(video_path: Path, t: float) -> Image.Image:
    ...
    cmd = ["ffmpeg", "-hide_banner", "-loglevel", "error", "-y",
           "-threads", "1", "-ss", str(t), "-i", input_arg,
           "-threads", "1", "-filter_threads", "1",
           "-frames:v", "1", "-vf", f"scale={SCREENSHOT_SIZE}:-1",
           "-c:v", "bmp", "-f", "rawvideo", "-"]
    try:
        with media_slot("Hash frames"):
...
def build_sprite(video_path: Path, duration: float = None) -> Image.Image:
    ...
    try:
        with ThreadPoolExecutor(max_workers=_SPRITE_CONCURRENCY) as pool:
