...
from app.core.matching import _strip_ts_uid, _release_name_with_folders
...
class ProcessingPipeline:
    ...
    async def process_single(self, video: Path) -> dict:
        ...
        if not matches:
            ...
            # Phash missed — but the filename tokeniser may still
            # recognise a library performer / studio. Reuse the same
            # group-into-subfolder mechanism the ``no_dir`` path uses so
            # an unmatched-but-hinted file lands in
            # ``<source_dir>/<canonical>/<file>`` right on Run instead
            # of waiting for the next snapshot's auto-group sweep.
            group_hint = self.callbacks.get("group_video_into_hint_subfolder")
            if callable(group_hint):
                try:
                    guess = db._library_guess_from_release_name(_release_name_with_folders(filename))
                    perfs = [p for p in (guess.get("performers") or []) if p]
                    studio_hint = (guess.get("studio") or "").strip()
                except Exception:
                    perfs, studio_hint = [], ""
                _, filename = await asyncio.to_thread(
                    group_hint, video, filename, settings, perfs, studio_hint,
                )
    ...
    async def run_pipeline(self, filenames: list = None) -> None:
        ...
        try:
            ...
            vdl_raw = (settings.get("vice_download_watch_dir") or "").strip()
            vdl_root = Path(vdl_raw).expanduser() if vdl_raw else None

            if filenames:
                video_files = [
                    p for f in filenames
                    if (p := db.resolve_source_video_path(source_dir, f, vice_dl_root=vdl_root)) is not None
                ]
            else:
                ...
                # root-level files. Vice download inbox is walked too
                # so vice-scope files stuck in ``<vice_dl>/<Vice>/…``
                # process on Run All alongside everything in source_dir.
                collected: list[Path] = []
                if source_dir.exists() and source_dir.is_dir():
                    for p in source_dir.rglob("*"):
                        if p.is_file() and p.suffix.lower() in VIDEO_EXTENSIONS:
                            collected.append(p)
                if vdl_root and vdl_root.exists() and vdl_root.is_dir():
                    for p in vdl_root.rglob("*"):
                        if p.is_file() and p.suffix.lower() in VIDEO_EXTENSIONS:
